...
4. ShaderDemo 的 frag 用 `{`...`}` 模板字符串表达式传（依赖 page.tsx blockJS:false）。**frag 内部绝不能留空行**——空行会让 prettier-MDX 把模板串当 markdown，乘号 `*` 被改成 `_`/`\*`、缩进丢失，GLSL 编译报 `'_' : syntax error`、Demo 白屏。各逻辑段靠 `//` 注释分隔、连续不留空行（参照 shaders.mdx/colors.mdx）；`prettier --write` 后 `grep -nE "[0-9a-zA-Z.)] _ [0-9a-zA-Z(]|\*"` 自查零损坏。
