...
order: 13
description: >-
  构造并逆转 BWT，以 MTF、RLE0 和统计编码形成 bzip，继而推导 compression boosting 与 FM-index
  backward search。
demo: true
math: true
sourceUrl: >-
...
officialUnitId: pae-14
---

import {
  BlockSortingCompressionCostLab,
  BlockSortingCompressionTraceLab,
  BlockSortingCompressionEvidenceLab,
} from "@/components/mdx/advanced-algorithm-engineering/v2/block-sorting-compression";
import {
  Objectives,
  Callout,
  Exercises,
...
<Stepper>
  ...
  <Step title="2. 状态、传输与正式节点">
    沿“14 Block-Sorting Compression”的官方目录坐标逐步推进，保存旋转/后缀次序、L 列、primary、Occ、MTF/RLE 流与逆变换结果。

    <BlockSortingCompressionTraceLab />

  </Step>
  <Step title="3. 单变量故障与恢复">
    固定输入后注入“丢失 primary index 或对相同字符使用不稳定次序，使 LF 环无法闭合”，定位首个分叉，撤销后以同一输入重放。
...
<Callout type="trap" title="本章首要反例">
  丢失 primary index 或对相同字符使用不稳定次序，使 LF
  环无法闭合。这会破坏“变换携带足够的 primary/sentinel
  信息并能逐字节逆变换”；应先从旋转/后缀次序、L 列、primary、Occ、MTF/RLE
  流与逆变换结果定位，不能用最终输出或一次耗时掩盖。
</Callout>

<Callout type="trap" title="成本模型也有适用域">
  “14 Block-Sorting Compression”中的“LF(i) = C[L[i]] + Occ(L[i],
  i)”只在本页声明的输入、存储与操作单位下成立。若工作集、块大小、字符模型或预处理边界变化，必须重新推导“跟踪
  BWT、MTF、RLE 与熵编码如何逐层改变局部统计”的成本，不能沿用旧数字。
</Callout>

<Callout type="trap" title="一次基准不是工程结论">
  对“14 Block-Sorting
  Compression”只报告最快一次运行，会隐藏预热、随机种子、尾延迟与“丢失 primary
  index 或对相同字符使用不稳定次序，使 LF
  环无法闭合”。交付时必须同时保存旋转/后缀次序、L 列、primary、Occ、MTF/RLE
  流与逆变换结果，并解释预测与实测的残差。
</Callout>
...
<Answer>
  对“14 Block-Sorting Compression”，以 Cambridge
...
<Answer>
  先固定一个可手算输入，记录旋转/后缀次序、L 列、primary、Occ、MTF/RLE
  流与逆变换结果；再只改变一个规模、布局或算法参数。若手算、实现和独立预言机的方向或边界不一致，先拒绝性能结论。
</Answer>

**问题 3：恢复证据。** 怎样证明“丢失 primary index 或对相同字符使用不稳定次序，使 LF 环无法闭合”已经修复？

<Answer>
  为“14 Block-Sorting Compression”保存正常基线，注入“丢失 primary index
  或对相同字符使用不稳定次序，使 LF
  环无法闭合”并标记首个不同状态；撤销后用完全相同的输入、随机种子、机器与参数重放。只有旋转/后缀次序、L
  列、primary、Occ、MTF/RLE 流与逆变换结果重新一致，修复才可交接。
</Answer>
