...
block-sorting compression（）先做一件反直觉的事：它不立即删除 redundancy，而是重排字符，让相似 context 的前驱聚到一起。后续简单的 Move-To-Front、Run-Length Encoding 与 entropy coder 才能吃到这些 runs 和偏斜分布。
...
LF mapping（）定义为：若 L[i] 的第 k 个 occurrence 对应 F[j] 的第 k 个 occurrence，则 LF[i]=j。
...
compression boosting（）解释了 BWT 不只是“看起来 runs 多”。在 sorted rows 中，以同一 context w 开头的 rows 连续；其 L 区段 Lw 恰好收集原串中 w 的 preceding symbols。
...
compressed indexing（）沿用 BWT、C 与 Rank。FM-index 可理解为 compressed suffix array，也可理解为 searchable bzip representation。
