...
} from "@/components/mdx/advanced-algorithm-engineering/v2/dictionary-based-compressors";
import {
  Objectives,
  Callout,
  Exercises,
...
<Stepper>
  ...
  <Step title="2. 状态、传输与正式节点">
    沿“13 Dictionary-Based Compressors”的官方目录坐标逐步推进，保存输入 hash、窗口、匹配、token、字典新增、输出位置与 round-trip。

    <DictionaryBasedCompressorsTraceLab />

  </Step>
  <Step title="3. 单变量故障与恢复">
    固定输入后注入“复制重叠 match 时先缓存源片段，错误地禁止新输出继续成为复制源”，定位首个分叉，撤销后以同一输入重放。
...
<Callout type="trap" title="本章首要反例">
  复制重叠 match 时先缓存源片段，错误地禁止新输出继续成为复制源。这会破坏“token
  流在声明的窗口与字典规则下唯一恢复原始字节序列”；应先从输入
  hash、窗口、匹配、token、字典新增、输出位置与
  round-trip定位，不能用最终输出或一次耗时掩盖。
</Callout>

<Callout type="trap" title="成本模型也有适用域">
  “13 Dictionary-Based Compressors”中的“LZ77 token = (distance, length,
  next-symbol)”只在本页声明的输入、存储与操作单位下成立。若工作集、块大小、字符模型或预处理边界变化，必须重新推导“比较
  LZ77、LZ78 与 LZW 的窗口、短语字典和解码同步”的成本，不能沿用旧数字。
</Callout>

<Callout type="trap" title="一次基准不是工程结论">
  对“13 Dictionary-Based
  Compressors”只报告最快一次运行，会隐藏预热、随机种子、尾延迟与“复制重叠 match
  时先缓存源片段，错误地禁止新输出继续成为复制源”。交付时必须同时保存输入
  hash、窗口、匹配、token、字典新增、输出位置与
...
<Answer>
  对“13 Dictionary-Based Compressors”，以 Cambridge
...
<Answer>
  先固定一个可手算输入，记录输入 hash、窗口、匹配、token、字典新增、输出位置与
...
<Answer>
  为“13 Dictionary-Based Compressors”保存正常基线，注入“复制重叠 match
  时先缓存源片段，错误地禁止新输出继续成为复制源”并标记首个不同状态；撤销后用完全相同的输入、随机种子、机器与参数重放。只有输入
  hash、窗口、匹配、token、字典新增、输出位置与
