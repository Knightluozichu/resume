...
order: 10
description: >-
  从 d-gap 和自定界要求出发，比较 Elias、Rice、PForDelta、Variable-byte、Interpolative 与
  Elias-Fano 的长度、分布假设和随机访问能力。
demo: true
math: true
sourceUrl: >-
...
officialUnitId: pae-11
---

import {
  IntegerCodingCostLab,
  IntegerCodingTraceLab,
  IntegerCodingEvidenceLab,
} from "@/components/mdx/advanced-algorithm-engineering/v2/integer-coding";
import {
  Objectives,
  Callout,
  Exercises,
...
<Stepper>
  ...
  <Step title="2. 状态、传输与正式节点">
    沿“11 Integer Coding”的官方目录坐标逐步推进，保存输入域、参数、逐项码字、位偏移、总位数、解码序列与边界样例。

    <IntegerCodingTraceLab />

  </Step>
  <Step title="3. 单变量故障与恢复">
    固定输入后注入“把只支持正整数的码直接用于零或在差分时溢出，破坏码流边界”，定位首个分叉，撤销后以同一输入重放。
...
<Callout type="trap" title="本章首要反例">
  把只支持正整数的码直接用于零或在差分时溢出，破坏码流边界。这会破坏“编码可唯一解码，整数域与零值约定明确，round-trip
  保持完整序列”；应先从输入域、参数、逐项码字、位偏移、总位数、解码序列与边界样例定位，不能用最终输出或一次耗时掩盖。
</Callout>

<Callout type="trap" title="成本模型也有适用域">
  “11 Integer Coding”中的“Elias-Fano ≤ n ceil(log2(U/n)) + 2n
  bits”只在本页声明的输入、存储与操作单位下成立。若工作集、块大小、字符模型或预处理边界变化，必须重新推导“按整数分布、单调性与查询需求选择自定界和块级编码”的成本，不能沿用旧数字。
</Callout>

<Callout type="trap" title="一次基准不是工程结论">
  对“11 Integer
  Coding”只报告最快一次运行，会隐藏预热、随机种子、尾延迟与“把只支持正整数的码直接用于零或在差分时溢出，破坏码流边界”。交付时必须同时保存输入域、参数、逐项码字、位偏移、总位数、解码序列与边界样例，并解释预测与实测的残差。
</Callout>
...
<Answer>
  对“11 Integer Coding”，以 Cambridge
...
<Answer>
  先固定一个可手算输入，记录输入域、参数、逐项码字、位偏移、总位数、解码序列与边界样例；再只改变一个规模、布局或算法参数。若手算、实现和独立预言机的方向或边界不一致，先拒绝性能结论。
</Answer>
...
<Answer>
  为“11 Integer
  Coding”保存正常基线，注入“把只支持正整数的码直接用于零或在差分时溢出，破坏码流边界”并标记首个不同状态；撤销后用完全相同的输入、随机种子、机器与参数重放。只有输入域、参数、逐项码字、位偏移、总位数、解码序列与边界样例重新一致，修复才可交接。
</Answer>
