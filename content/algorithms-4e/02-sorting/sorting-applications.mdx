...
比较器契约（）不能用“差值很小就相等”破坏transitivity。例如a近似b、b近似c却a明显大于c，会让sort没有一致目标。Integer comparison也不应直接 `return a-b`，因为overflow可反转order；使用 `Integer.compare`。
...
Kendall tau distance（）衡量两个permutations的pairwise disagreement。先构建第二ranking的inverse positions，再把第一ranking映射成position sequence；分歧pairs恰是该sequence inversions。
