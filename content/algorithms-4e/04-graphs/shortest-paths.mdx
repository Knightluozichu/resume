...
section: 图
order: 18
description: 4.4 · Shortest Paths覆盖 6 个作者站正式主题，以章专属状态模型、逐步轨迹、反例恢复和独立预言机验收。
demo: true
math: true
...
officialUnitId: algs4-4.4
---

import {
  ShortestPathsModelLab,
  ShortestPathsTraceLab,
  ShortestPathsCounterexampleLab,
} from "@/components/mdx/algorithms-4e/v2/shortest-paths";
import {
  Objectives,
  Callout,
  Exercises,
...
“4·4 · Shortest Paths”对应 Robert Sedgewick 与 Kevin Wayne 的 _Algorithms, Fourth Edition_（Addison-Wesley Professional，2011）。对这一节，[作者维护的本节页面](https://algs4.cs.princeton.edu/44sp/)提供与教材协同的浓缩正文、Java 实现、图示、习题和部分答案；[全书作者站](https://algs4.cs.princeton.edu/home/)给出 6 章、30 节的完整结构，并明确区分在线资料与纸质教材的学习用途。

“4·4 · Shortest Paths”的作者页公开经授权的在线节选和配套资源，但不是整本教材全文。因此“4·4 · Shortest Paths”采用 **independent-rewrite / authorized-sample**：中文讲解、推导与实验独立组织，不声称逐段翻译；算法名称、API 和示例边界以作者页、[官方代码索引](https://algs4.cs.princeton.edu/code/)及[官方勘误](https://algs4.cs.princeton.edu/errata/)交叉核对。
...
<Stepper>
  ...
  <Step title="2. 逐状态执行">
    按真实控制顺序推进，不跳过中间状态。每一步都要保存图与源点、distTo、edgeTo、松弛次序、优先队列、负环证书与全边校验器，并检查“所有边满足三角不等式，edgeTo 紧边链从可达顶点回到源点且路径权重等于 dist”仍然成立。

    <ShortestPathsTraceLab />

  </Step>
  <Step title="3. 反例、拒绝与恢复">
    固定同一输入，注入“在存在可达负权边时仍把 Dijkstra 出队顶点永久 settle，或只更新 dist 不更新 edgeTo”。先定位首个分叉，再撤销故障并重放；最终结果相同但中间证据不同，仍不能判定恢复完成。
...
<Callout type="trap" title="4.4 · Shortest Paths 的首要反例">
  在存在可达负权边时仍把 Dijkstra 出队顶点永久 settle，或只更新 dist 不更新
  edgeTo。它会破坏“所有边满足三角不等式，edgeTo
  紧边链从可达顶点回到源点且路径权重等于
  dist”；应从图与源点、distTo、edgeTo、松弛次序、优先队列、负环证书与全边校验器中找首个不一致状态，而不是只看最终输出。
</Callout>

<Callout type="trap" title="复杂度结论必须带前提">
  “relax(v→w)：若 dist[w] > dist[v]+weight(v,w)，同时更新 dist[w] 与 edgeTo[w]”只对本节声明的输入表示、操作单位和算法版本成立。若字符表、图权、比较器、随机化、存储层次或预处理边界改变，必须重新推导，不能沿用旧数字。
</Callout>

<Callout type="trap" title="实现互比不等于独立正确">
  对“4.4 · Shortest
  Paths”而言，两个实现可能共享同一边界错误。交付前既要比较作者代码合同，也要用小规模朴素预言机或数学证书独立核对图与源点、distTo、edgeTo、松弛次序、优先队列、负环证书与全边校验器。
</Callout>
...
<Answer>
  先以“4.4 · Shortest Paths”作者页为分母，逐项定位
  最短路径、加权有向图、松弛、Dijkstra算法、无环图最短路径、Bellman-Ford算法与负权环。每项都必须同时出现在解释段、组件模型与本题清单中；缺任一层就不能把该概念记为完整覆盖。
</Answer>

**问题 2：最小推演。** 怎样证明“relax(v→w)：若 dist[w] > dist[v]+weight(v,w)，同时更新 dist[w] 与 edgeTo[w]”不是孤立结论？

<Answer>
  选择“非负权图”的输入：s→a=2、s→b=5、a→b=1。按 初始化源点标签 → 选择待松弛顶点
  → 计算候选距离 → 更新标签与前驱 → 验证路径或负环 保存状态，并用“Dijkstra 先
  settle a，再把 b 从 5 改进为
  3”核对方向；随后只改变一个变量，检查公式的前提和边界是否仍成立。
</Answer>

**问题 3：故障恢复。** 怎样证明“在存在可达负权边时仍把 Dijkstra 出队顶点永久 settle，或只更新 dist 不更新 edgeTo”已经修复？

<Answer>
  保存图与源点、distTo、edgeTo、松弛次序、优先队列、负环证书与全边校验器作为正常基线；注入该故障并标记第一个违反“所有边满足三角不等式，edgeTo
  紧边链从可达顶点回到源点且路径权重等于
  dist”的状态；撤销后用完全相同的输入、版本和随机种子重放。只有全部状态与独立预言机重新一致，才可发布修复结论。
</Answer>
...
<Attribution
  ...
  adaptedUrl="https://algs4.cs.princeton.edu/44sp/"
/>
