...
officialUnitId: aad8-17-memory-optimization
---
...
import { Aad817MemoryOptimizationDecisionLab, Aad817MemoryOptimizationMechanismMap, Aad817MemoryOptimizationFailureDiagram } from "@/components/mdx/android-advanced-decryption/aad8-17-memory-optimization";
...
第17章 内存优化 的在《第17章 内存优化》中，最小证据包包含：出版社目录、Android版本、源码文件和符号、调用/状态图、PID/TID或加载器身份、一条失败注入、日志/追踪/堆证据、最终结果、已知限制、停止、恢复、回退、责任人与复核人。
