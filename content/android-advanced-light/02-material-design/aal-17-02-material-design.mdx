...
import { Aal17PipelineLab, Aal17ExperimentLab, Aal17EvidenceLab } from "@/components/mdx/android-advanced-light/diagrams/aal-17-02-material-design";
...
<Stepper>
  <Step title="1. 建立调用与所有权图">逐个连接17个节点，标出输入、线程、对象、View或源码边界；先预测失败位置。<Aal17PipelineLab /></Step>
  <Step title="2. 控制变量制造反例">切换正常、配置变化、线程竞争、所有者销毁或版本漂移，一次只改变一个条件。<Aal17ExperimentLab /></Step>
  <Step title="3. 通过交付证据门">用层级与间距标注、动效状态、错误输入、无障碍语义、滚动协作与折叠边界截图逐项核对；任何生命周期或版本证据缺失都不得交付。<Aal17EvidenceLab /></Step>
</Stepper>

<Callout type="trap">**核心陷阱。** 把Material Design降为颜色和阴影皮肤，忽略信息层级、运动因果、输入反馈与可访问性。修复先恢复明确输入、所有者、线程和失败合同。</Callout>
...
<Answer>这些节点共同构成“把材质、空间、动画、样式、图标、图像与组件原则落实到Snackbar、TextInputLayout、FAB、TabLayout、NavigationView和CoordinatorLayout”的输入、机制、源码与验收链；删去正式节点会让调用或所有权证据断裂。</Answer>

**问题 2**：最小正确性合同是什么？
<Answer>相同版本、输入与生命周期下，业务结果、线程交付、错误和资源释放必须确定且可重放。</Answer>

**问题 3**：怎样构造最有效的反例？
<Answer>主动让“把Material Design降为颜色和阴影皮肤，忽略信息层级、运动因果、输入反馈与可访问性”成立，再运行取消、配置变化、所有者销毁或版本漂移场景；错位、泄漏或重复即失败。</Answer>
...
**问题 6**：独立交接需要什么？
<Answer>需要源码与版本锁、完整目录映射、层级与间距标注、动效状态、错误输入、无障碍语义、滚动协作与折叠边界截图、失败测试、迁移账本和回退条件。</Answer>

</Exercises>

<Glossary>
  <GlossaryItem term="材质隐喻">以表面、层级和运动帮助用户理解界面结构与操作结果的设计模型。</GlossaryItem>
  <GlossaryItem term="CoordinatorLayout">让子View通过Behavior协同滚动和位置变化的容器。</GlossaryItem>
  <GlossaryItem term="Snackbar">短暂展示操作结果并可提供单一恢复动作的底部反馈组件。</GlossaryItem>
  <GlossaryItem term="TextInputLayout">为文本输入提供浮动标签、错误提示和计数等外层能力的组件。</GlossaryItem>
  <GlossaryItem term="Behavior">声明CoordinatorLayout子项响应依赖与嵌套滚动的协作规则。</GlossaryItem>
</Glossary>
