...
import { AdaeFlowLab, AdaeFailureLab, AdaeEvidenceLab } from "@/components/mdx/android-art-exploration/diagrams/adae15-01-activity-lifecycle-launch-mode";
...
<Steps>
  ...
  <Step title="3. 通过可重放证据门">用生命周期轨迹、任务栈快照、重建状态、启动矩阵和Intent匹配测试核对环境、调用链、失败安全、释放、测试和迁移；缺项时不得把正常运行视为完成。<AdaeEvidenceLab /></Step>
</Steps>

<Callout type="trap">**本章核心陷阱。** 把onDestroy当作必然回调，或把启动模式与Intent Flags混为同一层配置。修复时先恢复所有者、线程/进程、状态与释放合同，再写能推翻实现的故障测试。</Callout>
...
<Answer>覆盖8个节点，从“第1章 Activity的生命周期和启动模式”到“1.3 IntentFilter的匹配规则”；主线是从典型与异常生命周期、LaunchMode、Flags和IntentFilter建立Activity任务栈与状态恢复模型。</Answer>

**问题 2**：怎样建立本章最小可执行切片？
<Answer>固定Android版本、JDK、构建插件、设备、进程和输入，贯通一次用户动作、framework调用、线程/进程边界与可观察结果，并保存生命周期轨迹、任务栈快照、重建状态、启动矩阵和Intent匹配测试。</Answer>

**问题 3**：本章最需要推翻的错误假设是什么？
<Answer>错误假设是“把onDestroy当作必然回调，或把启动模式与Intent Flags混为同一层配置”。保持其他条件不变，注入重建、进程退出、错误线程、队列压力、无效输入或资源未释放；异常即推翻。</Answer>
...
**问题 6**：达到独立交接标准需要哪些证据？
<Answer>需要环境和设备指纹、源码版本、构建产物、输入、线程/进程时序、状态快照、失败测试、资源释放、版本边界与生命周期轨迹、任务栈快照、重建状态、启动矩阵和Intent匹配测试。</Answer>
