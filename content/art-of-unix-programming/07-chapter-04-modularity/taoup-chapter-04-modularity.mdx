...
draft: false
officialUnitId: "taoup-chapter-04-modularity"
---
...
·
<Term
  def={"模块尺寸在第4章 模块性：保持清晰，保持简洁中对应正交检查的可复核状态。"}
>
  模块尺寸
</Term>
·
...
·
<Term
  def={"转换成本在第4章 模块性：保持清晰，保持简洁中对应模块测试的可复核状态。"}
>
  转换成本
</Term>
·
<Term
  def={"改动传播在第4章 模块性：保持清晰，保持简洁中对应封装单元的可复核状态。"}
>
  改动传播
</Term>
...
<Callout type="trap" title="不要把 2003 年工具选择当成永恒排名">
  ...
  模块尺寸 与 改动传播，再决定原则是否仍成立。
</Callout>

<Callout type="trap" title="不要用最终输出掩盖中间越界">
  即使结果看似正确，只要“如果拆分增加了双向依赖或重复规则，应合并边界而非继续细分。”没有保持，或故障“把同一规则复制到多个模块，并用更多适配器掩盖重复知识”未被明确拒绝，本次设计仍不合格。
</Callout>
