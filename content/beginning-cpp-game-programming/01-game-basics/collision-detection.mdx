...
<Term def="封装球的位置、速度、尺寸与可绘制形状，并维护运动和重置不变量的 Pong 游戏对象。">Ball 类</Term>，
...
<Term def="表示每秒位置变化的二维向量；x/y 分量独立决定水平和垂直方向与速率。">速度向量</Term>
不能和每帧位移混用。`reset`
同时恢复位置与速度，避免新发球继承旧轮次的屏外位置或反弹方向。

## 积分与墙壁反射

<Term def="用当前速度和经过时间近似推进位置的数值步骤，本章采用 position += velocity * dt 的显式欧拉形式。">运动积分</Term>
...
<Term def="把物体位置投影回合法边界并反转碰撞法向速度分量的简化弹性响应。">速度反射</Term>
使用 `abs`
固定反弹方向，避免球已朝内却因重复条件又翻回。窗口比球还窄时需要暂停或把最大坐标钳为零，不能构造负边界。

## AABB 检测到底保证什么

<Term def="Axis-Aligned Bounding Box，以不旋转且与坐标轴对齐的矩形近似对象范围。">AABB</Term>
...
<Term def="检测到离散重叠后，把对象移回接触边界以消除交叠并恢复下一帧可分离状态的步骤。">穿透修正</Term>
...
<Term def="把 Ball 放回发球位置并恢复有效速度、清除旧碰撞状态，以开始下一轮。">发球重置</Term>
