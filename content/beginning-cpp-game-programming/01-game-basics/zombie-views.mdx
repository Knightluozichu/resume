...
<Term def="本书的僵尸生存射击项目，世界可扩展到多个屏幕，玩家在竞技场中移动、瞄准并对抗不断生成的僵尸。">Zombie Arena</Term>
...
<Term def="协调窗口、资源、输入、更新、视图和渲染阶段的 Zombie Arena 应用层，不替代 Player 等对象维护自身不变量。">僵尸游戏引擎</Term>
...
<Term def="定义游戏世界合法位置范围的矩形，Player、僵尸与相机约束都使用同一世界坐标契约。">竞技场边界</Term>
例如 3000x3000
世界单位，不等于窗口像素尺寸。把边界作为值传给更新，而不是散落全局常量。

## Player 类：输入意图与世界状态分开

<Term def="封装玩家位置、移动速度、朝向和精灵表示，并在世界边界内维护逻辑与视觉一致的对象。">Player 类</Term>
...
<Term def="把非零方向向量缩放为长度一，使多轴组合只改变方向而不改变移动速率。">方向归一化</Term>
...
<Term def="玩家和游戏对象使用的逻辑位置空间，独立于窗口像素并由 View 映射到可见区域。">世界坐标</Term>
...
<Term def="SFML 定义二维相机中心、尺寸、旋转和归一化视口的对象，用于把世界坐标映射到渲染目标区域。">SFML View</Term>
...
<Term def="以玩家或目标位置为中心、显示可滚动游戏世界的 View；地图和实体都在此视图下绘制。">worldView</Term>
每帧在 Player 更新后跟随。
<Term def="使用固定逻辑屏幕坐标绘制分数、弹药和提示的 View，不随世界相机中心移动。">hudView</Term>
保持固定。
...
<Term def="限制相机中心使整个 View 保持在地图合法范围内，避免显示世界边界外区域。">相机钳制</Term>
前置条件是竞技场尺寸不小于 View；关卡配置阶段先验证，否则 clamp 范围无效。

## 像素到世界的输入映射

<Term def="把窗口像素位置按指定 View 逆变换为逻辑坐标的过程，用于鼠标瞄准、选择和世界命中测试。">坐标映射</Term>
...
<Term def="持续处理窗口事件、采样输入、更新玩家和世界、更新相机并分层绘制的 Zombie Arena 帧循环。">主游戏循环</Term>
