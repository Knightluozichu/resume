...
<Term def="Player 发出的移动投射物，具有方向、速度、伤害、寿命并在首次有效命中后退役。">火球</Term>

先必须是正确的玩法对象，之后才能成为可靠声源。

<Term def="根据声源与 Listener 的相对位置计算左右方向、距离衰减和混音权重的过程。">声音空间化</Term>
...
<Term def="输入层在按键边沿产生的火球施放请求，不保证最终一定生成对象。">CastIntent</Term>
...
<Term def="一次成功施放后，在下一次施放前必须由逻辑时间消耗完的等待值。">火球冷却</Term>

只在 Playing 递减，Paused 冻结；重启恢复初始值。是否在生成失败时回滚冷却要写成明确协议，推荐先验证资源再接受施放。

## Factory 构造完整 Projectile

<Term def="根据施放者、出生点、方向和配置创建完整火球对象的工厂边界。">FireballFactory</Term>
...
<Term def="创建火球时记录的施放者身份，用于避免自伤并归属伤害与得分。">owner id</Term>

是稳定 GameObjectId，不保存 Player 裸指针。

## 高速投射物需要扫掠碰撞

<Term def="检查火球从上一位置到候选位置经过的连续路径，而不只检查终点重叠的碰撞方法。">扫掠碰撞</Term>
...
<Term def="在本帧运动区间中最先发生接触的归一化时间和目标，用于确定唯一首次命中。">最早命中</Term>
...
<Term def="火球从生成起允许存活的逻辑秒数，耗尽后即使未命中也请求销毁。">projectile TTL</Term>

同时设置世界边界，防止飞出关卡的对象永久存在。

<Term def="确保命中、超时和越界多个条件同帧发生时只提交一次销毁的状态。">退役标记</Term>

由 ProjectileUpdate 私有保存；标记后不再产生伤害或音效。世界提交器对重复 DestroyCommand 也应幂等。

## SFML 音频的 Listener 与声源

<Term def="SFML 音频场景中的听者位置和朝向，本项目通常跟随已提交 Player 世界位置。">Listener</Term>
...
<Term def="把像素世界距离转换成一致音频距离的比例，使衰减参数具有可调语义。">音频坐标比例</Term>
...
<Term def="声源在该距离以内保持近似完整音量、避免无限接近 Listener 时爆音的阈值。">minimum distance</Term>

通过 `setMinDistance` 设置；

<Term def="控制超过 minimum distance 后音量随距离下降速度的参数。">attenuation</Term>
...
<Term def="包含火球施放、命中或爆炸世界坐标的已提交事件，可直接成为空间声源输入。">SpatialSoundEvent</Term>

SoundEngine 从 FireballSpawned 播放施放声，从 FireballHit 播放命中声。事件位置来自规则提交点，不读取已销毁火球。

<Term def="在 voice 池全忙时根据重要性、距离和年龄决定保留或替换声音的策略。">空间 voice 优先级</Term>
...
<Term def="读取 PlayerSnapshot 并显示生命、火球冷却和状态提示，不拥有真实玩法值的界面组件。">HUD class</Term>
...
<Term def="Player 在一帧提交后的生命、火球冷却与可施放状态集合，供 HUD 只读。">PlayerSnapshot</Term>
...
<Term def="输入、火球运动碰撞、世界提交、Listener 更新、音频消费和 HUD 投影的固定顺序。">火球帧协议</Term>
