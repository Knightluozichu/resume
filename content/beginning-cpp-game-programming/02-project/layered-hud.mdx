...
<Term def="按世界、世界叠加、固定 HUD 和全屏面板的顺序使用不同 View 绘制同一帧的技术。">分层视图</Term>。
...
<Term def="在 worldView 下绘制地图和游戏实体的阶段，所有对象使用世界坐标并随相机滚动。">世界图层</Term>
...
<Term def="在固定逻辑屏幕坐标中绘制分数、弹药、生命和提示的阶段，不受世界相机中心影响。">HUD 图层</Term>
...
<Term def="显示生命、弹药、分数、波次和提示的一组 Text/Shape 对象，由游戏状态投影生成但不拥有规则。">HUD 对象</Term>
不应保存“真正分数”并让 Game 再读取。业务层产生只读
<Term def="某一帧用于界面显示的生命、弹药、分数、波次和阶段值集合。">GameSnapshot</Term>，
HUD 比较前后值并更新视图。
...
<Term def="把数值状态转换成可显示字符串、几何宽度和颜色，并写入对应 HUD 对象的过程。">HUD 更新</Term>
...
<Term def="以屏幕边缘、中心或另一控件为稳定参考放置 UI，使文本长度变化后仍保持预期对齐。">锚点布局</Term>
...
<Term def="仅在对应状态值变化时重新格式化或布局 HUD 元素的更新策略。">脏标记</Term>
可减少工作，但状态比较必须完整；漏标记比每帧更新更糟。

## Home 屏幕是状态而非背景图

<Term def="游戏启动或返回菜单时显示标题与开始入口、停止战斗更新并接管输入的阶段。">Home 屏幕</Term>
...
<Term def="达到升级条件后暂停战斗、显示可选能力，并在一次合法选择后应用效果并恢复游戏的阶段。">LevelUp 屏幕</Term>
...
<Term def="玩家在升级面板选定并经合法性检查后，对 Player 状态执行一次的能力修改。">升级提交</Term>
...
<Term def="Player 死亡后冻结战斗、显示最终分数并只接受重启或返回 Home 的终局阶段。">GameOver 屏幕</Term>
...
<Term def="在窗口比例与逻辑比例不同时用空白边保留等比内容区域的 viewport 策略。">letterbox</Term>
