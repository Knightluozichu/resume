...
section: Run! 项目
order: 18
description: 第 18 章：平台、Player 控制与 Animator：保留第三版项目代码讲解，并以源码—状态—输出切片、确定性轨迹和故障重放完成验收。
demo: true
math: false
sourceUrl: >-
...
} from "@/components/mdx/beginning-cpp-game-programming/v2/platforms-player-animation-controls";
...
- [Packt：Beginning C++ Game Programming, Third Edition](https://www.packtpub.com/en-us/product/beginning-c-game-programming-9781835088258)：在“第 18 章：平台、Player 控制与 Animator”中，核对 2024 年第三版、C++20、SFML、四个项目、21 个正式教学章节及章节次序；不把商品页当作正文全文。
- [PacktPublishing：第三版官方代码仓库](https://github.com/PacktPublishing/Beginning-C-Game-Programming-Third-Edition)：在“第 18 章：平台、Player 控制与 Animator”中，核对 Timber、Pong、ZombieShooter、Run 四个项目的公开代码与资源组织；代码许可证不等于原书正文授权。
- [SFML 2.6 官方教程](https://www.sfml-dev.org/tutorials/2.6/)：在“第 18 章：平台、Player 控制与 Animator”中，核对本书使用的 SFML 2.6 系列 Window、Graphics、View、VertexArray、Shader 与 Audio API 语义。
...
<Callout type="trap" title="能显示不等于状态正确">
  偶尔出现正确画面、声音或分数，不能证明所有权、时间步和阶段提交正确。必须用输入意图、物理速度、接地标志、动画状态、累计时间、帧索引和碰撞体复算中间状态。
</Callout>

<Callout type="trap" title="API 版本不能混用">
  “第 18 章：平台、Player 控制与 Animator”以 SFML 2.6.1 时代接口为基线。直接粘贴
  SFML 3
  示例可能改变本页依赖的事件、时长、角度或构造合同；迁移必须围绕“控制输入先形成意图，物理决定实际运动，Animator
  根据已提交状态选帧；动画不能反向决定碰撞位置”单独记录和验证。
</Callout>

<Callout type="trap" title="重置界面不等于恢复程序">
  重置按钮必须恢复源码/资源假设、初始状态和输入序列。若只清空可见提示，却没有重新满足“控制输入先形成意图，物理决定实际运动，Animator
  根据已提交状态选帧；动画不能反向决定碰撞位置”，故障仍然存在。
</Callout>
...
<Term def="承载玩家、参与从上方落地碰撞并随进程持续生成回收的地形对象。">
  平台地形
</Term>

不能只随机 x、y。若间隙超过 Player 的水平可达距离，程序仍能运行但关卡必死。生成器必须把物理参数转成约束，再在约束内加入变化。

<Term def="在给定起跳速度、重力、水平速度和高度差下，Player 能安全落到下一平台的空间范围。">
  可达区域
</Term>
...
<Term def="规划下一平台起点、宽度和高度，并把合法候选交给 Factory 的服务。">
  PlatformGenerator
</Term>
...
<Term def="小于理论极限的倍率，为帧离散、反应时间和边缘碰撞误差保留可玩余量。">
  可达安全系数
</Term>
...
<Term def="描述平台类型、位置、尺寸和资源键，经过验证后在安全点生成对象的命令。">
  平台生成命令
</Term>
...
<Term def="位于主相机后方一定距离的世界边界，越过后平台可请求销毁。">
  回收线
</Term>
...
<Term def="把键盘或手柄输入映射为水平轴和跳跃边沿，再交给 Player 规则的适配层。">
  玩家控制
</Term>
...
<Term def="从未按到按下只持续一个逻辑帧的输入变化，用于跳跃等一次动作。">
  输入边沿
</Term>
...
<Term def="根据速度、重力和受限时间步计算 Player 候选位置的过程。">
  运动积分
</Term>
...
<Term def="碰撞解算确认 Player 由空中转为站在平台上的提交状态。">
  grounded 状态
</Term>
...
<Term def="保存当前动画片段、帧索引和累计时间，并按 dt 采样动画帧的组件。">
  Animator 类
</Term>

Animator 不读取键盘，也不修改 grounded。它接收 Player 已提交的动画状态，输出 texture rect。

<Term def="一组有序帧矩形、各帧时长以及循环策略，描述一个完整视觉动作。">
  AnimationClip
</Term>
...
<Term def="精灵表中某一时刻使用的纹理矩形及持续时间，是动画片段的最小采样单元。">
  动画帧
</Term>
...
<Term def="非循环片段首次到达最后一帧时产生一次、用于切换后续表现的事实。">
  动画完成事件
</Term>
...
<Term def="根据已提交速度、grounded 和规则事件选择 Idle、Run、Jump、Fall、Land 片段的表现状态机。">
  玩家动画状态机
</Term>

玩家动画由玩法状态派生：grounded 且水平速度明显为零选 Idle；grounded 且移动选 Run；空中向上选 Jump；空中向下选 Fall；PlayerLanded 可触发一次 Land。

<Term def="从一个 AnimationClip 切到另一个时，规定是否重置帧索引与累计时间的策略。">
  片段切换策略
</Term>
...
<Term def="平台生成、Player 运动碰撞和动画选择在同一帧中的固定先后协议。">
  行动帧协议
</Term>
...
<Term def="用大量随机种子验证每个连续平台组合都满足可达约束的自动化测试。">
  生成属性测试
</Term>
