...
order: 14
description: 对齐官方第 3 版第 14 章：可靠保存和加载最高分，按规则事件播放音效，完成升级、新波次、重启与 Zombie Arena 收尾。
demo: false
math: false
...
import {
  BcgpHighScoreFileMap,
  BcgpSoundEventMap,
  BcgpWaveRestartMap,
} from "@/components/mdx/beginning-cpp-game-programming/diagrams/sound-file-io";

<Objectives>

- 能实现完整解析、范围验证、临时写入、关闭检查和发布替换组成的最高分文件事务
- 能设计由 Shot/Hit/Pickup/Death 规则事件驱动、Buffer 生命周期稳定且 voice 数量受限的音效层
- 能实现波次清空、升级选择、新群体提交和完整重启，并验证所有子系统恢复一致
...
<Term def="应用在当前用户权限下可写、用于保存配置和进度的平台目录。">用户数据目录</Term>
...
<Term def="从存档读取、解析并验证得到的历史最大分数，缺文件时使用明确默认值。">最高分</Term>
...
<Term def="要求所有预期字段成功转换、范围合法且除允许空白外没有尾随未知内容的解析策略。">完整解析</Term>
...
<Term def="先把完整新内容写入同目录临时文件，成功关闭后再替换正式文件的保存协议。">临时文件发布</Term>
...
<Term def="由射击、命中、拾取、死亡和升级等一次性规则转换产生、供音频与视觉反馈消费的值。">游戏事件</Term>
...
<Term def="按稳定声音标识查找预加载 SoundBuffer、取得可用 Sound voice 并开始一次播放的组件。">音效层</Term>
...
<Term def="预创建并复用一组 sf::Sound 播放实例，使同类短音效可有限叠加而不在热路径分配。">voice 池</Term>
...
<Term def="当前已发布僵尸群从至少一只存活转为零只存活的状态边沿。">波次清空</Term>
...
<Term def="一波敌人全部消灭后冻结战斗、允许玩家选择能力并规划下一波的阶段。">升级阶段</Term>
只触发一次音效和面板。空初始群体属于构建错误，不应被误判为完成一波。

## 难度规划与新波次

<Term def="根据波次号计算敌人数量、速度、生命和生成约束，并有明确上限的配置。">波次计划</Term>
...
<Term def="依据波次计划在局部候选容器完整生成敌人，成功后替换当前群体的提交过程。">新波次生成</Term>
...
<Term def="在多个对象均准备成功后一次替换游戏阶段、Player 属性和新群体，避免半升级状态可见。">波次提交</Term>
...
<Term def="从 GameOver 或 Home 创建一局全新 Zombie Arena，恢复所有一局范围状态而保留应用级资源缓存和最高分。">游戏重启</Term>
