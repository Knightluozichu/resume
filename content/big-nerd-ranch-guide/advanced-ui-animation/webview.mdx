...
section: 高级UI与动画
order: 29
description: 网页浏览：保留第四版正文机制，以所有者—状态—结果合同、事件轨迹和章专属故障完成可重放验收。
demo: true
math: false
...
import {
  ...
  WebviewLifecycleLab,
  WebviewFaultLab,
} from "@/components/mdx/big-nerd-ranch-guide/v2/webview";
...
<Stepper>
  ...
  <Step title="② loadUrl 加载网页">
    拿到 WebView 实例后调用 `webView.loadUrl("https://...")`
    即可加载远程页面；也可以用 `loadDataWithBaseURL()` 直接加载 HTML
    字符串来显示富文本、隐私协议等本地内容。`WebSettings` 需显式开启
    `javaScriptEnabled`。
  </Step>
  <Step title="③ addJavascriptInterface 暴露原生方法">
    在 Kotlin 端定义一个带 `@JavascriptInterface` 注解的类，通过
    `webView.addJavascriptInterface(instance, "Android")` 将其注入 JS
    全局作用域。之后 Web 页面里的 JavaScript 就能通过 `Android.方法名()`
    直接调用原生代码。
  </Step>
  <Step title="④ evaluateJavascript 调用 JS">
    Android 到 JS 的桥梁是 `webView.evaluateJavascript("js代码")`——它在 WebView
    的渲染线程异步执行 JS 字符串，结果通过回调以 JSON 格式返回。注意必须在
    `onPageFinished` 之后调用，否则 JS 环境尚未就绪。
  </Step>
  <Step title="⑤ WebChromeClient 处理 alert/console">
    `WebChromeClient` 负责跟"浏览器外观"相关的回调：`onProgressChanged`
    获取加载进度、`onReceivedTitle` 获取网页标题、`onJsAlert` 拦截 JS 的
    `alert()` 弹窗并替换为原生 Dialog——让 WebView 的行为更像一个真正的浏览器。
  </Step>
</Stepper>
...
<Term def="拦截 URL 加载、处理页面请求和 SSL 错误的回调类">WebViewClient</Term>
负责页面导航相关的事件。最常用的三个回调：
...
<Callout type="trap">
  **关键安全红线的提醒**：`onReceivedSslError` 中的 `handler.proceed()`
  会忽略证书错误继续加载——这在开发环境测试自签名证书时方便，但**生产环境绝对不能随意
  `proceed()`**。否则等于把用户暴露在中间人攻击之下。正确做法：开发阶段仅在
  BuildConfig.DEBUG 时 proceed，生产环境提示用户或拒绝加载。
</Callout>

### WebChromeClient——处理 UI 相关回调

<Term def="处理 WebView 中与浏览器 UI 相关的回调（进度条、标题、JS 对话框、文件选择等）">
  WebChromeClient
</Term>
负责进度、标题、JS 对话框等"浏览器 UI"相关的事件：
...
<Callout type="warn">
  **安全警告**：`@JavascriptInterface` 标记的方法会在 WebView
  的线程池中执行（不在主线程），且只有 API 17+ 才需要该注解。API 16
  及以下，**所有 public 方法**都能被 JS 调用——这是严重的安全漏洞。必须把
  `targetSdkVersion` 设为 17 以上，且只暴露真正需要的方法。
</Callout>
...
<Callout type="tip">
  性能关键：如果页面复杂，务必在 `AndroidManifest.xml` 中为 WebView 所在的
  Activity
  开启**硬件加速**：`android:hardwareAccelerated="true"`。大部分设备默认开启，但
  Android 4.0 以下需要显式设置。没有硬件加速，大量 CSS 动画和 Canvas
  绘制会卡到不能看。
</Callout>
...
<Callout type="trap">
  **现象**：WebView 加载 HTTPS 页面时一片空白，也没有错误提示。**原因**：Android
  5.0+ 的 WebView 默认阻止混合内容（HTTPS 页面里嵌 HTTP
  资源）。**修法**：`webView.settings.mixedContentMode =
  WebSettings.MIXED_CONTENT_COMPATIBILITY_MODE`（仅非生产环境），或让后端把所有资源改成
  HTTPS。
</Callout>

<Callout type="trap">
  **现象**：内存泄漏——退出 WebView Activity 后内存不释放。**原因**：WebView 持有
  Activity 的 Context 引用，且内部有独立的渲染线程和 JS 引擎。**修法**：①在
  `onDestroy()` 中调用 `webView.destroy()`；②不要直接在布局 XML 中声明
  WebView——用代码动态创建并传入 `applicationContext`（或用独立的进程）。
</Callout>

<Callout type="trap">
  **现象**：`evaluateJavascript` 调用后无回调，或 JS 报 "Android is not
  defined"。**原因**：在 `onPageFinished` 之前调了 `evaluateJavascript`——此时 JS
  环境未就绪。或 `addJavascriptInterface` 在 `loadUrl`
  之后调用。**修法**：`addJavascriptInterface` 必须在 `loadUrl`
  之前调用；`evaluateJavascript` 必须在 `onPageFinished` 之后调用。
</Callout>
...
<Answer>
  覆盖10个节点，从“29. Browsing the Web and WebView”到“Challenge: Using the Back
  Button for Browser History”。主线是在隐式浏览器、WebView、自定义UI和Custom
  Tabs间按信任边界选择，并处理旋转与历史，交付物是URL策略、WebViewClient/WebChromeClient边界、旋转状态、后退历史和注入风险测试。
</Answer>

**问题 2**：怎样建立本页最小可执行实验？

<Answer>
  在“网页浏览”的验证中，固定JDK、SDK、Gradle、设备API、语言、权限和输入，完成一个从用户动作到可观察结果的垂直切片；保存干净构建、设备操作、原始日志、状态快照与断言。
</Answer>

**问题 3**：为什么只在正常点击路径运行不能证明完成？

<Answer>
  它没有覆盖组件重建、进程死亡、异步取消、权限拒绝、外部输入失败、资源释放和版本变化。应逐项注入故障，并用URL策略、WebViewClient/WebChromeClient边界、旋转状态、后退历史和注入风险测试核对。
</Answer>

**问题 4**：怎样设计能推翻当前实现的反例？

<Answer>
  在“网页浏览”的验证中，保持其他变量不变，分别注入旋转、后台回收、进程死亡、拒权、断网、无效输入与重复操作；任何状态丢失、越权、崩溃、泄漏或重复副作用都推翻完成结论。
</Answer>
...
<Answer>
  需要版本与设备指纹、源码提交、输入、构建命令、生命周期轨迹、原始日志、失败测试、安全边界与发布产物，让他人凭URL策略、WebViewClient/WebChromeClient边界、旋转状态、后退历史和注入风险测试重放。
</Answer>
...
<GlossaryItem term="WebViewClient">
  WebView 的导航管家。拦截 URL
  加载（`shouldOverrideUrlLoading`）、处理页面开始/结束事件、处理 SSL
  证书错误等。类比：浏览器地址栏的"逻辑层"。
</GlossaryItem>

<GlossaryItem term="WebChromeClient">
  WebView 的 UI 管家。负责进度条更新、标题获取、JavaScript alert/prompt
  弹窗、文件选择器等跟"浏览器外观"相关的回调。类比：浏览器的标签页和弹窗控制。
</GlossaryItem>

<GlossaryItem term="evaluateJavascript">
  在 WebView 的 JS 引擎里异步执行一段 JS 代码字符串，结果通过回调返回（JSON
  格式）。必须在 `onPageFinished` 之后调用，否则 JS 环境未就绪。
</GlossaryItem>

<GlossaryItem term="addJavascriptInterface">
  把一个 Kotlin/Java 对象绑定到 WebView 的 JS 全局作用域。JS
  端通过绑定的名字（如 "Android"）直接调 Kotlin 方法。方法必须标记
  `@JavascriptInterface`（API 17+ 的安全要求）。
</GlossaryItem>

<GlossaryItem term="混合内容 (Mixed Content)">
  一个 HTTPS 页面内部加载了 HTTP 协议的资源（图片、CSS、JS）。Android 5.0+
  默认阻止混合内容，因为 HTTP 资源可能被篡改，破坏 HTTPS 的安全性。
</GlossaryItem>
...
<Callout type="trap">
  在“网页浏览”中，正常点击成功只能证明一个样本。若重建、取消、无响应者、离线、旧schema或目标SDK变化会产生不同结果，就必须把该边界写进状态机与测试。
</Callout>

<Callout type="trap">
  “网页浏览”若只留下最终截图，就无法判断旧所有者是否收到回调、后台任务是否重复、状态是否来自错误层或复位是否真正清空实验。
</Callout>
