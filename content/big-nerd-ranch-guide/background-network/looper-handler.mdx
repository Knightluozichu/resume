...
section: 后台与网络
order: 25
description: Looper、Handler和HandlerThread：保留第四版正文机制，以所有者—状态—结果合同、事件轨迹和章专属故障完成可重放验收。
demo: true
math: false
...
import {
  LooperHandlerContractLab,
  LooperHandlerLifecycleLab,
  LooperHandlerFaultLab,
} from "@/components/mdx/big-nerd-ranch-guide/v2/looper-handler";

import { Objectives } from "@/components/mdx/objectives";
import { MessageLoopDiagram } from "@/components/mdx/diagrams/message-loop-diagram";
...
- <Term def="MessageQueue 是每个有 Looper 的线程内部的消息队列数据结构。它是一个按时间戳排序的优先队列——先到的不一定先处理，核心看时间戳。只负责存和取，不负责执行。">
    **MessageQueue（消息队列）**
  </Term>
  ：一个按时间戳排序的优先队列。只有放消息和取消息两个操作，不负责执行。每个有
  Looper 的线程自动带一个。

- <Term def="Looper 是一个线程的消息循环引擎。它调用 Looper.prepare() 初始化自己和 MessageQueue，然后调用 Looper.loop() 进入无限循环——不停地从 MessageQueue 取消息，分发给目标 Handler 的 handleMessage()。没有 Looper 的线程不能处理消息。">
    **Looper（消息泵）**
  </Term>
  ：线程的消息循环引擎。`Looper.prepare()` 初始化消息队列，`Looper.loop()`
  进入无限循环——不停地从消息队列里取消息，分发给目标 Handler。

- <Term def="Handler 是消息发送和处理的门面。它挂载在某个线程的 Looper 上——默认是创建它的那个线程的 Looper。通过它可以往关联线程的消息队列投递 Message 或 Runnable，消息处理回调 handleMessage() 也在关联线程上执行。">
    **Handler（消息收发器）**
  </Term>
...
<Callout type="trap">
  ...
  **原因**：HandlerThread 的 `Looper.loop()` 是无限循环。Thread 的 `run()`
  方法永远不会返回，即使 Activity 已经
  `onDestroy()`。这个线程会一直占据内存直到进程被系统杀掉。 **修法**：在
  `onDestroy()` 或合适的生命周期回调中调用
  `handlerThread.quitSafely()`。`quitSafely()` 跟 `quit()`
  的区别：前者会等待队列中已存在的消息全部处理完再退出，后者直接丢弃所有待处理消息立即退出。
</Callout>

<Callout type="trap">
  **现象**：在 `handleMessage()` 里做了耗时操作，发现后续消息都被卡住了。
  **原因**：Looper 是单线程循环——同一时刻只处理一条消息。如果你在
  `handleMessage()` 里睡 5 秒，那这 5
  秒内队列里其他消息全在排队等待，包括主线程的 UI 绘制消息——界面直接卡死（老版
  ANR ）。 **修法**：`handleMessage()` 必须快速返回（&lt;16ms
  最佳）。耗时任务应该投递到自己内部再起子线程处理，或使用 `WorkManager`（第 27
  章）。不要把 Handler 的"线程切换"和"任务并行" 搞混了。
</Callout>
...
<Answer>
  覆盖13个节点，从“25. Loopers, Handlers, and HandlerThread”到“Challenge:
  Preloading and
  Caching”。主线是用MessageQueue、Looper、Handler和HandlerThread解释跨线程调度，并把下载器绑定到View生命周期，交付物是缩略图下载状态机、消息轨迹、取消与缓存测试、StrictMode报告。
</Answer>

**问题 2**：怎样建立本页最小可执行实验？

<Answer>
  在“Looper、Handler和HandlerThread”的验证中，固定JDK、SDK、Gradle、设备API、语言、权限和输入，完成一个从用户动作到可观察结果的垂直切片；保存干净构建、设备操作、原始日志、状态快照与断言。
</Answer>

**问题 3**：为什么只在正常点击路径运行不能证明完成？

<Answer>
  它没有覆盖组件重建、进程死亡、异步取消、权限拒绝、外部输入失败、资源释放和版本变化。应逐项注入故障，并用缩略图下载状态机、消息轨迹、取消与缓存测试、StrictMode报告核对。
</Answer>

**问题 4**：怎样设计能推翻当前实现的反例？

<Answer>
  在“Looper、Handler和HandlerThread”的验证中，保持其他变量不变，分别注入旋转、后台回收、进程死亡、拒权、断网、无效输入与重复操作；任何状态丢失、越权、崩溃、泄漏或重复副作用都推翻完成结论。
</Answer>
...
<Answer>
  需要版本与设备指纹、源码提交、输入、构建命令、生命周期轨迹、原始日志、失败测试、安全边界与发布产物，让他人凭缩略图下载状态机、消息轨迹、取消与缓存测试、StrictMode报告重放。
</Answer>
...
<Glossary>
  <GlossaryItem term="MessageQueue（消息队列）">
    每个有 Looper
    的线程内部的消息存储结构。它是一个按时间戳排序的优先队列——一条消息什么时候该被执行，取决于它的
    `when` 字段。只负责存和取，不负责执行。详见本章"消息队列三件套"一节。
  </GlossaryItem>
  <GlossaryItem term="Looper（消息泵）">
    线程的消息循环引擎。`Looper.prepare()` 创建 MessageQueue
    并绑定当前线程，`Looper.loop()` 进入无限循环——不停从队列取消息，交给目标
    Handler 处理。主线程的 Looper
    由系统创建且永不退出。详见本章"消息队列三件套"一节。
  </GlossaryItem>
  <GlossaryItem term="Handler（消息收发器）">
    挂载在某个线程 Looper 上的消息发送和处理器。`sendMessage()`
    往关联线程的消息队列投 Message，`handleMessage()`
    在关联线程上执行。一个线程可以有多个 Handler。详见本章"消息队列三件套"一节。
  </GlossaryItem>
  <GlossaryItem term="HandlerThread">
    Android 封装好的自带 Looper + MessageQueue 的后台线程。不用手动
    `Looper.prepare()` / `Looper.loop()`——`start()`
    即进入消息循环。适合需要串行排队执行的后台任务。详见本章"HandlerThread"一节。
  </GlossaryItem>
</Glossary>
...
<Callout type="trap">
  在“Looper、Handler和HandlerThread”中，正常点击成功只能证明一个样本。若重建、取消、无响应者、离线、旧schema或目标SDK变化会产生不同结果，就必须把该边界写进状态机与测试。
</Callout>

<Callout type="trap">
  “Looper、Handler和HandlerThread”若只留下最终截图，就无法判断旧所有者是否收到回调、后台任务是否重复、状态是否来自错误层或复位是否真正清空实验。
</Callout>
