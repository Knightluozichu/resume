...
“音频播放与单元测试”把“掌握 MediaPlayer 音频播放和 JUnit 单元测试——用 Mockito 隔离依赖，写出可测试的代码。在“音频播放与单元测试”的验证中，”拆成三个可观察层次：先选择目录节点并确认责任边界，再改变一个生命周期或外部条件，最后回到构建指纹、用户操作、状态快照、原始日志和行为断言。点击后的文字、状态与轨迹必须同步变化，重置后回到相同基线。
