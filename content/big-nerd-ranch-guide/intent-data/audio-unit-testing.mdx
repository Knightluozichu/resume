...
section: Intent与数据
order: 20
description: 音频播放与单元测试：保留第四版正文机制，以所有者—状态—结果合同、事件轨迹和章专属故障完成可重放验收。
demo: true
math: false
...
import {
  AudioUnitTestingContractLab,
  AudioUnitTestingLifecycleLab,
  AudioUnitTestingFaultLab,
} from "@/components/mdx/big-nerd-ranch-guide/v2/audio-unit-testing";

import { Objectives } from "@/components/mdx/objectives";
import { Step, Stepper } from "@/components/mdx/stepper";
import { TestPyramidDiagram } from "@/components/mdx/diagrams/test-pyramid-diagram";
...
<Stepper>
  ...
  <Step title="② setDataSource + prepare">
    如果不用 `create()`，则手动 `setDataSource(path)`
    指定音频来源（本地文件/网络 URI），然后调用
    `prepare()`（同步，阻塞当前线程）或
    `prepareAsync()`（异步，推荐）。异步准备完毕后通过 `OnPreparedListener`
    回调通知。
  </Step>
  <Step title="③ start">
    调用 `start()` 开始播放。播放完成后触发
    `OnCompletionListener`——在此回调中决定是循环播放（`seekTo(0)` +
    `start()`）还是释放资源（`release()`）。
  </Step>
  <Step title="④ pause / stop">
    `pause()` 暂停播放，之后可以 `start()` 从暂停位置恢复。`stop()` 停止并重置到
    Idle 状态——之后若想再播必须重新 `prepare()`。用 `isPlaying()`
    检查当前到底在播还是暂停。
  </Step>
  <Step title="⑤ release">
    调用 `release()` 彻底释放 MediaPlayer 持有的系统音频资源。**活动
    onStop/onDestroy 中必须调用**——不释放会导致 AudioFlinger 资源泄漏，其他 App
    可能无声音。每次 `create()` 新实例前也应先 `release()` 旧实例。
  </Step>
</Stepper>
...
<Callout type="trap">
  **现象**：MediaPlayer 播完一首歌再播下一首——无声或崩溃。**原因**：没有先
  `release()` 旧的 MediaPlayer 就创建新的——系统音频资源耗尽。**修法**：每次 play
  前先 `release()` 确保旧实例被清理。
</Callout>

<Callout type="trap">
  **现象**：测试跑到 Room DAO 那一层要 5
  秒——因为真的在写数据库。**原因**：单元测试应该隔离数据库依赖。**修法**：Room
  DAO 的测试用 `Room.inMemoryDatabaseBuilder`（内存数据库），或直接 mock DAO
  接口。
</Callout>

<Callout type="trap">
  **现象**：MediaPlayer 在 Activity destroy 后仍占音频焦点。**原因**：未在
  onStop/onDestroy 调用 release()。**修法**：生命周期绑定点释放，或改用
  ExoPlayer/Media3 更好管理。
</Callout>
...
<Answer>
  覆盖15个节点，从“20. Unit Testing and Audio Playback”到“Challenge: Play Sound
  Across
  Rotation”。主线是以SoundPool加载/播放/释放生命周期为被测边界，用测试替身核对对象交互和旋转行为，交付物是音频资源清单、SoundPool状态机、单元测试、mock交互与资源释放证据。
</Answer>

**问题 2**：怎样建立本页最小可执行实验？

<Answer>
  在“音频播放与单元测试”的验证中，固定JDK、SDK、Gradle、设备API、语言、权限和输入，完成一个从用户动作到可观察结果的垂直切片；保存干净构建、设备操作、原始日志、状态快照与断言。
</Answer>

**问题 3**：为什么只在正常点击路径运行不能证明完成？

<Answer>
  它没有覆盖组件重建、进程死亡、异步取消、权限拒绝、外部输入失败、资源释放和版本变化。应逐项注入故障，并用音频资源清单、SoundPool状态机、单元测试、mock交互与资源释放证据核对。
</Answer>

**问题 4**：怎样设计能推翻当前实现的反例？

<Answer>
  在“音频播放与单元测试”的验证中，保持其他变量不变，分别注入旋转、后台回收、进程死亡、拒权、断网、无效输入与重复操作；任何状态丢失、越权、崩溃、泄漏或重复副作用都推翻完成结论。
</Answer>
...
<Answer>
  需要版本与设备指纹、源码提交、输入、构建命令、生命周期轨迹、原始日志、失败测试、安全边界与发布产物，让他人凭音频资源清单、SoundPool状态机、单元测试、mock交互与资源释放证据重放。
</Answer>
...
<GlossaryItem term="SoundPool">
  短音效播放器——预加载音频到内存，低延迟播放（&lt;10ms）。适合按钮点击声、通知铃声、游戏音效。不适合长音频（音乐用
  MediaPlayer）。
</GlossaryItem>

<GlossaryItem term="Mockito">
  Java/Kotlin 的 mock 测试框架。`mock()` 创建假对象，`when().thenReturn()`
  控制行为，`verify()` 验证方法是否被调用。隔离依赖、专注测试被测代码。
</GlossaryItem>
...
<Callout type="trap">
  在“音频播放与单元测试”中，正常点击成功只能证明一个样本。若重建、取消、无响应者、离线、旧schema或目标SDK变化会产生不同结果，就必须把该边界写进状态机与测试。
</Callout>

<Callout type="trap">
  “音频播放与单元测试”若只留下最终截图，就无法判断旧所有者是否收到回调、后台任务是否重复、状态是否来自错误层或复位是否真正清空实验。
</Callout>
