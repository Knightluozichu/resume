...
section: Intent与数据
order: 22
description: XML drawable：保留第四版正文机制，以所有者—状态—结果合同、事件轨迹和章专属故障完成可重放验收。
demo: true
math: false
...
import {
  XmlDrawablesContractLab,
  XmlDrawablesLifecycleLab,
  XmlDrawablesFaultLab,
} from "@/components/mdx/big-nerd-ranch-guide/v2/xml-drawables";
...
<Callout type="trap">
  **现象**：shape 设了 corners radius，但 View 上看到的是直角。**原因**：View
  本身有背景色覆盖了 drawable。**修法**：确保
  `android:background="@drawable/rounded_bg"` 后的代码没有重新设 background 或
  backgroundTint。
</Callout>
...
<Callout type="trap">
  **现象**：vector 在 API 21 以下崩溃。**原因**：旧系统需 AppCompat 矢量兼容或
  VectorDrawableCompat。**修法**：Gradle 开启 vectorDrawables.useSupportLibrary
  或提供 png fallback。
</Callout>
...
<Answer>
  覆盖9个节点，从“22. XML Drawables”到“Challenge: Button
  Themes”。主线是组合shape、selector和layer-list表达状态化视觉，并比较mipmap与9-patch的用途，交付物是统一按钮、状态矩阵、图层顺序、密度与拉伸测试。
</Answer>
...
<Answer>
  在“XML
  drawable”的验证中，固定JDK、SDK、Gradle、设备API、语言、权限和输入，完成一个从用户动作到可观察结果的垂直切片；保存干净构建、设备操作、原始日志、状态快照与断言。
</Answer>

**问题 3**：为什么只在正常点击路径运行不能证明完成？

<Answer>
  它没有覆盖组件重建、进程死亡、异步取消、权限拒绝、外部输入失败、资源释放和版本变化。应逐项注入故障，并用统一按钮、状态矩阵、图层顺序、密度与拉伸测试核对。
</Answer>
...
<Answer>
  在“XML
  drawable”的验证中，保持其他变量不变，分别注入旋转、后台回收、进程死亡、拒权、断网、无效输入与重复操作；任何状态丢失、越权、崩溃、泄漏或重复副作用都推翻完成结论。
</Answer>
...
<Answer>
  需要版本与设备指纹、源码提交、输入、构建命令、生命周期轨迹、原始日志、失败测试、安全边界与发布产物，让他人凭统一按钮、状态矩阵、图层顺序、密度与拉伸测试重放。
</Answer>
...
<GlossaryItem term="Shape Drawable">
  用 XML
  定义的几何图形（矩形/椭圆/线/环）。支持纯色(solid)、圆角(corners)、描边(stroke)、渐变(gradient)等属性。替代
  PNG 背景图。
</GlossaryItem>

<GlossaryItem term="Selector（State-List）">
  根据 View 状态（按下/禁用/选中/聚焦）自动切换 drawable 的 XML
  定义。创建按钮、列表项等交互反馈的标准方式。
</GlossaryItem>

<GlossaryItem term="Vector Drawable">
  用 XML
  路径（pathData）描述的矢量图。与分辨率无关——任意缩放不失真，一个文件适用所有屏幕密度。
</GlossaryItem>
...
<Callout type="trap">
  在“XML
  drawable”中，正常点击成功只能证明一个样本。若重建、取消、无响应者、离线、旧schema或目标SDK变化会产生不同结果，就必须把该边界写进状态机与测试。
</Callout>

<Callout type="trap">
  “XML
  drawable”若只留下最终截图，就无法判断旧所有者是否收到回调、后台任务是否重复、状态是否来自错误层或复位是否真正清空实验。
</Callout>
