...
section: UI与Fragment
order: 9
description: 使用RecyclerView显示列表：保留第四版正文机制，以所有者—状态—结果合同、事件轨迹和章专属故障完成可重放验收。
demo: true
math: false
...
import {
  RecyclerviewContractLab,
  RecyclerviewLifecycleLab,
  RecyclerviewFaultLab,
} from "@/components/mdx/big-nerd-ranch-guide/v2/recyclerview";
...
- <Term def="RecyclerView.Adapter 负责两件事：在需要时创建 ViewHolder（工厂），在数据变化时给 ViewHolder 塞新数据（填表员）。它是 RecyclerView 和数据源之间的桥梁。">
    Adapter
  </Term>
  —— 数据源 → View
  的桥梁。它知道有多少条数据，知道每条数据长什么样，负责"按需生产"ViewHolder
- <Term def="RecyclerView.ViewHolder 是一个「座位」，里面放了一个条目的所有子 View 引用（findViewById 的结果），避免反复 findViewById。">
    ViewHolder
  </Term>
  —— 条目的"座位"。它缓存一个条目布局里各个子 View 的引用，让你不用每次都
  `findViewById`
- <Term def="RecyclerView.LayoutManager 决定列表怎么摆——是纵向线性列表还是网格，还是瀑布流。它只管「位置和排列」，不管数据。">
    LayoutManager
  </Term>
  —— 条目怎么排列。LinearLayoutManager 排成一列，GridLayoutManager
  排成网格，StaggeredGridLayoutManager 排成瀑布流
...
<Callout type="tip">
  想象工厂流水线：`onCreateViewHolder`
  是造一个新的"空壳座位"（这个不常调——只有在回收池没货时才造新座）；`onBindViewHolder`
  是让数据"对号入座"（这个每条都调）。
</Callout>
...
<Callout type="trap">
  **现象 →** RecyclerView 什么都不显示，一片空白。**原因 →** 忘了设
  LayoutManager。没有 LayoutManager，RecyclerView
  不知道条目该怎么摆，就等于没布局。**修法 →** 加上 `recyclerView.layoutManager
  = LinearLayoutManager(this)`。
</Callout>

<Callout type="trap">
  **现象 →** 点击条目时 `position` 对不上——点第三条，跳转到第二条的数据。**原因
  →** 在 `onBindViewHolder` 里用 `holder.adapterPosition` 获取位置，但列表更新后
  adapterPosition 可能变了。**修法 →** 点击时调 `holder.bindingAdapterPosition`
  或直接在 bind 时把数据对象传给 lambda。
</Callout>

<Callout type="trap">
  **现象 →** `submitList()` 后列表整屏闪烁/重建，动画也没了。**原因 →**
  `DiffUtil` 的 `areContentsTheSame()` 返回了 `false`（因为每次都是新 `Crime`
  对象）或直接没实现 DiffUtil。**修法 →** 确保数据类是 `data class`（自动生成
  `equals`），DiffUtil 的两个回调正确实现——`areItemsTheSame` 比
  ID，`areContentsTheSame` 用 `==` 比较整个对象。
</Callout>
...
<Answer>
  覆盖11个节点，从“9. Displaying Lists with RecyclerView”到“Challenge:
  RecyclerView ViewTypes”。主线是把数据、Adapter、ViewHolder、item
  view和回收过程连接为可测列表流水线，交付物是列表绑定轨迹、回收验证、点击合同与多ViewType挑战。
</Answer>

**问题 2**：怎样建立本页最小可执行实验？

<Answer>
  在“使用RecyclerView显示列表”的验证中，固定JDK、SDK、Gradle、设备API、语言、权限和输入，完成一个从用户动作到可观察结果的垂直切片；保存干净构建、设备操作、原始日志、状态快照与断言。
</Answer>

**问题 3**：为什么只在正常点击路径运行不能证明完成？

<Answer>
  它没有覆盖组件重建、进程死亡、异步取消、权限拒绝、外部输入失败、资源释放和版本变化。应逐项注入故障，并用列表绑定轨迹、回收验证、点击合同与多ViewType挑战核对。
</Answer>

**问题 4**：怎样设计能推翻当前实现的反例？

<Answer>
  在“使用RecyclerView显示列表”的验证中，保持其他变量不变，分别注入旋转、后台回收、进程死亡、拒权、断网、无效输入与重复操作；任何状态丢失、越权、崩溃、泄漏或重复副作用都推翻完成结论。
</Answer>
...
<Answer>
  需要版本与设备指纹、源码提交、输入、构建命令、生命周期轨迹、原始日志、失败测试、安全边界与发布产物，让他人凭列表绑定轨迹、回收验证、点击合同与多ViewType挑战重放。
</Answer>
...
<Glossary>
  <GlossaryItem term="Adapter">
    数据到 View 的桥梁。它告诉 RecyclerView 有多少条数据、每条数据对应的 View
    长什么样。类比餐厅：Adapter
    是服务员——厨房（数据源）做好菜，服务员端到桌上（屏幕）。
  </GlossaryItem>
  <GlossaryItem term="ViewHolder">
    一个条目的 View 引用仓库。它把条目布局里的各个子 View
    缓存起来，避免每次都要全屏
    `findViewById`。类比高铁座椅——人换了（数据变了），但座椅本身（ViewHolder）可以复用。
  </GlossaryItem>
  <GlossaryItem term="LayoutManager">
    决定列表条目怎么排列的角色。LinearLayoutManager
    是竖排或横排线性列表，GridLayoutManager 是网格，StaggeredGridLayoutManager
    是不等高的瀑布流。三个可互换，不影响 Adapter 的代码。
  </GlossaryItem>
  <GlossaryItem term="DiffUtil">
    比较新旧两个列表的"差异计算器"。算出哪些条目是新增的、哪些被删了、哪些内容变了，让
    RecyclerView 只刷新真正变化的条目而不是整屏重建。
  </GlossaryItem>
</Glossary>
...
<Callout type="trap">
  在“使用RecyclerView显示列表”中，正常点击成功只能证明一个样本。若重建、取消、无响应者、离线、旧schema或目标SDK变化会产生不同结果，就必须把该边界写进状态机与测试。
</Callout>

<Callout type="trap">
  “使用RecyclerView显示列表”若只留下最终截图，就无法判断旧所有者是否收到回调、后台任务是否重复、状态是否来自错误层或复位是否真正清空实验。
</Callout>
