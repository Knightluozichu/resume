...
section: UI与Fragment
order: 8
description: UI fragment与fragment管理器：保留第四版正文机制，以所有者—状态—结果合同、事件轨迹和章专属故障完成可重放验收。
demo: true
math: false
...
import {
  UiFragmentsContractLab,
  UiFragmentsLifecycleLab,
  UiFragmentsFaultLab,
} from "@/components/mdx/big-nerd-ranch-guide/v2/ui-fragments";
...
<Stepper>
  ...
  <Step title="② FragmentManager.beginTransaction 开启事务">
    拿到 `supportFragmentManager` 后调用 `beginTransaction()`，开启一个 Fragment
    事务——后续所有 add/replace/remove
    操作都在这个事务里排队，最后一次性提交。事务保证操作的原子性：要么全做，要么全不做。
  </Step>
  <Step title="③ add 或 replace 放入容器">
    `add(R.id.fragment_container, fragment)` 把 Fragment 加到 Activity
    布局预留的 `FrameLayout` 插槽中——首次添加用 add；`replace()`
    则是先移除容器内已有 Fragment，再放入新 Fragment，相当于"换一块积木"。
  </Step>
  <Step title="④ commit 提交事务">
    调用 `commit()` 将事务提交给主线程排队执行。如果需要支持返回键回退，在
    commit 前调用 `addToBackStack(null)`——把它压入回退栈，按返回键时
    FragmentManager 自动逆向操作回到上一个 Fragment。
  </Step>
</Stepper>
...
<Callout type="warn">
  注意 `inflate()` 第三个参数必须传 `false`——意思是"不要自动把 inflate 出来的
  View 加到 container 里"。FragmentManager 会自己决定什么时候加，你代劳的话会报
...
<Callout type="trap">
  **现象 →** 旋转屏幕后 Fragment 上的数据显示了旧内容。**原因 →** Activity
  重建时默认重建 Fragment，你没处理 Arguments 或 ViewModel 的保留。**修法 →** 用
  ViewModel 跨重建保留数据；Arguments Bundle 会自动保留，标准参数用 Arguments
  传即可。
</Callout>

<Callout type="trap">
  **现象 →** `FragmentTransaction.commit()` 后界面没变，还报了
  `IllegalStateException`。**原因 →** 你在 `onCreate()` 或 `onStart()`
  里提交事务，但此时 Activity 还没恢复完，状态不能丢。**修法 →** 改为
  `commitAllowingStateLoss()`（风险——丢失状态），或者把事务推迟到
  `onResumeFragments()` 里执行。
</Callout>

<Callout type="trap">
  **现象 →** `findViewById()` 在 Fragment 里返回 null。**原因 →** 你在
  `onCreateView()` 里调了 `findViewById()`，但 View 还没创建完毕。**修法 →** 把
  View 操作移到 `onViewCreated()` 里——那时 `view` 已经是个可用的 View 对象了。
</Callout>
...
<Answer>
  覆盖8个节点，从“8. UI Fragments and the Fragment Manager”到“Application
  Architecture with
  Fragments”。主线是建立Activity宿主、FragmentManager、事务及Fragment视图生命周期的所有权图，交付物是CriminalIntent骨架、Fragment事务轨迹、重建与泄漏测试。
</Answer>
...
<Answer>
  在“UI
  fragment与fragment管理器”的验证中，固定JDK、SDK、Gradle、设备API、语言、权限和输入，完成一个从用户动作到可观察结果的垂直切片；保存干净构建、设备操作、原始日志、状态快照与断言。
</Answer>

**问题 3**：为什么只在正常点击路径运行不能证明完成？

<Answer>
  它没有覆盖组件重建、进程死亡、异步取消、权限拒绝、外部输入失败、资源释放和版本变化。应逐项注入故障，并用CriminalIntent骨架、Fragment事务轨迹、重建与泄漏测试核对。
</Answer>
...
<Answer>
  在“UI
  fragment与fragment管理器”的验证中，保持其他变量不变，分别注入旋转、后台回收、进程死亡、拒权、断网、无效输入与重复操作；任何状态丢失、越权、崩溃、泄漏或重复副作用都推翻完成结论。
</Answer>
...
<Answer>
  需要版本与设备指纹、源码提交、输入、构建命令、生命周期轨迹、原始日志、失败测试、安全边界与发布产物，让他人凭CriminalIntent骨架、Fragment事务轨迹、重建与泄漏测试重放。
</Answer>
...
<Glossary>
  <GlossaryItem term="Activity">
    Android 里代表一个全屏页面的组件。像一个舞台，Fragment
    是上面的演员——舞台提供场地，演员负责表演。
  </GlossaryItem>
  <GlossaryItem term="Fragment">
    寄生在 Activity 里的可复用 UI
    模块。有自己的布局、生命周期和逻辑，但不能独立显示——必须有一个宿主
    Activity。
  </GlossaryItem>
  <GlossaryItem term="FragmentManager">
    Fragment 的管家。负责把 Fragment 往 Activity 里加、换、删，管理回退栈。通过
    `supportFragmentManager` 获取。
  </GlossaryItem>
  <GlossaryItem term="FragmentTransaction">
    FragmentManager 执行操作时的一个"打包事务"。把要做的 add/replace/remove
    等操作写到事务里，最后 commit 一次性提交。像银行转账——要么全做，要么全不做。
  </GlossaryItem>
  <GlossaryItem term="Bundle">
    Android 里的"键值对背包"，用于在组件间传递数据。Fragment 用
    Bundle（Arguments）接收参数，因为构造器不能带参数。
  </GlossaryItem>
</Glossary>
...
<Callout type="trap">
  在“UI
  fragment与fragment管理器”中，正常点击成功只能证明一个样本。若重建、取消、无响应者、离线、旧schema或目标SDK变化会产生不同结果，就必须把该边界写进状态机与测试。
</Callout>

<Callout type="trap">
  “UI
  fragment与fragment管理器”若只留下最终截图，就无法判断旧所有者是否收到回调、后台任务是否重复、状态是否来自错误层或复位是否真正清空实验。
</Callout>
