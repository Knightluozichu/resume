...
**现象 →** 把185个目录层级当成"读完了"，却无法从任一条目定位到公式、代码或失败样本。
**原因 →** 只记标题不记证据，目录沦为清单而非可执行坐标。
**修法 →** 每个正式标题至少绑定一次手算、一段可运行代码和一个单故障样本。

**现象 →** 用今天模型的输出反向改写原书示例，结论与2024年坐标冲突。
**原因 →** 模型服务端漂移，未区分原书示例与本站重放。
**修法 →** 复刻保留历史语境，运行实验另记当前版本，二者分开存档。

**现象 →** 同一请求重跑两次结果不同，却无法定位是模型、提示还是种子。
**原因 →** 没有冻结随机参数和依赖版本，差异无法归因。
**修法 →** 你可以亲自验证：固定seed跑两次，再放开seed跑两次，对比差异来源。
...
**问题 1：概念理解。** 学习地图为什么要先冻结应用合同，而非直接演示框架？

<Answer>

框架演示不等于应用完成。冻结合同把需求、输入、模型、提示、数据、工具、输出验证、监控与回退连成闭环，使每一步都可审计、可回退，避免把"跑通Demo"误判为"应用完成"。

</Answer>

**问题 2：实践应用。** 给定一次模型请求，应冻结哪些字段才能复现？

<Answer>

至少保存来源、任务版本、输入哈希、模型与端点、提示、检索快照、工具权限、随机种子、依赖锁、输出、逐样本判断、延迟和成本。只看最终文本无法区分模型漂移、提示注入或检索缺失。

</Answer>

**问题 3（独立实现）：** 独立实现一个简化版 release_gate：输入一组记录，检查每条是否含 input、config、output、judgment、limits 五个字段，返回缺失索引列表。
