...
**现象 →** 全书都“看懂了”，动手却建不出最小应用。
**原因 →** 只记模型名与框架，没重建任务、数据与调用图。
**修法 →** 从冻结任务开始手写最小调用链，再逐能力叠加。

**现象 →** 复习覆盖了185个目录项，仍无法定位故障。
**原因 →** 没把章节连成失败到根因的链路。
**修法 →** 用索引从症状反查章节，记录首个分叉对象。

**现象 →** 应用能跑，但无法证明可发布。
**原因 →** 没设质量、安全、延迟、成本、复现五道门禁。
**修法 →** 发布前重放冻结回归集，五项全过才晋级。
...
**问题 1：概念理解。** 全书总复习的“通过标准”是什么？

<Answer>

能独立重建最小LLM应用，并证明质量、安全、性能、成本与复现五道门禁全部通过后再发布，而非记住模型或框架名。

</Answer>

**问题 2：实践应用。** 为什么复习要贯通185个目录项而不是挑重点？

<Answer>

应用调用链横跨模型、提示、编排、对话、检索、SQL、代码、多模态、微调与治理；任一环节缺失都会让故障无法定位，需把目录连成调用图。

</Answer>

**问题 3（独立实现）：** 独立实现一个简化版发布门禁：给定五项检查结果，返回是否可发布及未通过项。

<Answer>

```python
def release_gate_review(checks):
    gates = ["quality", "safety", "latency", "cost", "repro"]
    failed = [g for g in gates if not checks.get(g, False)]
