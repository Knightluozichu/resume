...
## 方法一：<Term term="构造函数副作用">构造函数</Term>与静态成员
...
- **现象**：以为构造函数方案只能靠"数构造次数"，忽略数组创建与作用域。
- **原因**：静态成员累计依赖"每个元素构造一次"的副作用，作用域外不保证顺序。
- **修法**：明确 `new A[n]` 恰好触发 n 次构造；静态成员函数返回累计值。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：虚函数方案在递归终止分支忘记返回，导致无限递归或未定义行为。
- **原因**：基类指针调用的虚函数必须能区分"终止"与"递推"两个分支。
- **修法**：终止分支（n=0）返回 0，递推分支调虚函数继续——两个分支由派生类覆盖实现。

</Callout>

## 方法二：<Term term="虚函数">虚函数</Term>动态分派终止递归
...
## 方法三：<Term term="函数指针表">函数指针表</Term>终止递归
...
## 方法四：<Term term="模板特化">模板特化</Term>在编译期终止
...
共同递推是 f(n)=f(n-1)+n。构造函数用"n 次构造的副作用"累计；虚函数用基类/派生类分派区分终止与递推；函数指针表用索引选择终止或递推函数；模板特化在编译期展开。
...
`new A[n]` 触发 n 次构造函数调用，每次把当前计数加一并累加；静态成员属于类而非实例，跨所有对象共享，最终返回 1+2+…+n。
...
基类 `Sum` 的虚函数在 n=0 时返回 0（终止），派生类 `SumDerived` 覆盖为 n+f(n-1)（递推）。用基类指针调用时，n=0 命中终止分支，停止递归。
...
<Glossary>
  <GlossaryItem term="构造函数副作用">利用"每个对象构造时执行一次"的特性累计状态，是本题核心替代手段。</GlossaryItem>
  <GlossaryItem term="虚函数">C++ 运行时多态机制，派生类覆盖基类实现，用于区分终止与递推分支。</GlossaryItem>
  <GlossaryItem term="函数指针表">用数组下标选择函数指针，替代条件判断完成分支。</GlossaryItem>
  <GlossaryItem term="模板特化">编译期为特定模板参数生成专门实现，递归在编译期展开。</GlossaryItem>
</Glossary>
