...
officialUnitId: coi-66
---
import { ConstuctArrayPrefixSuffixDiagram } from "@/components/mdx/coding-interviews/diagrams/constuct-array";
...
- **现象**：直接用总积除以每个元素，含零时结果错误。
- **原因**：零位置无法通过除法恢复，且题目禁止除法。
- **修法**：用前缀积×后缀积，不用除法。
...
- **现象**：只扫描一次，同时维护前缀和后缀，逻辑混乱。
- **原因**：两个方向需要独立扫描。
- **修法**：第一遍从左到右写前缀积，第二遍从右到左乘后缀积。

</Callout>

## 把排除自身拆成<Term term="前缀积">前缀积</Term>与<Term term="后缀积">后缀积</Term>
...
## 第二遍把<Term term="后缀积">后缀积</Term>乘回结果
...
## <Term term="上下三角">下三角与上三角</Term>的正确性
...
<Answer>含零时总积为 0，零位置无法恢复。且题目禁止除法。</Answer>

**问题 2：** 前缀积和后缀积如何计算？

<Answer>前缀积 B[i]=A[0]×...×A[i-1]；后缀积 temp 从右向左累乘 A[i+1]×...×A[n-1]。</Answer>

**问题 3：** 长度为 1 的数组返回什么？

<Answer>B[0] = 1（除自身外所有元素乘积为空积）。</Answer>
...
<Glossary>
  <GlossaryItem term="构建乘积数组">每个元素等于除自身外所有元素乘积。</GlossaryItem>
  <GlossaryItem term="前缀积">当前元素左侧所有元素的乘积。</GlossaryItem>
  <GlossaryItem term="后缀积">当前元素右侧所有元素的乘积。</GlossaryItem>
  <GlossaryItem term="零处理">含零时无需特殊分支，前缀×后缀自动处理。</GlossaryItem>
  <GlossaryItem term="上下三角">前缀积对应下三角矩阵，后缀积对应上三角矩阵。</GlossaryItem>
</Glossary>
