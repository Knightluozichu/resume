...
作者不枚举每个起点和终点，而是用**small和big两个指针**维护一个<Term term="滑动窗口">滑动窗口</Term>。初始small为1、big为2，最小合法窗口正好包含两个数。

<ContinuousSequenceWindowDiagram />

窗口内所有值之和称为。它只需在边界移动时加一个新big或减一个旧small，无需每轮重新求和。

## <Term term="等差序列">等差序列</Term>公式锁定问题结构
...
- **现象**：窗口和小于 s 时移动 big、大于 s 时移动 small，但忘记每次移动后更新窗口和。
- **原因**：窗口和是增量维护的，移动指针后必须同步加减。
- **修法**：扩张时 `cur += big`，收缩时 `cur -= small`，保持 cur 恒等于当前窗口和。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：窗口上界取 s 而非 (s+1)/2，多算或漏算边界。
- **原因**：连续序列至少两个数，small 超过 s/2 后不可能再有合法窗口。
- **修法**：small 循环条件为 `small <= (s+1)/2`，与终止中点分析一致。
...
窗口和小于 s 时只扩 big（增和），大于 s 时只缩 small（减和）。正数保证两个方向单调：任何解序列都会被某个窗口状态命中，不会跳过。
...
连续序列至少包含两个数，small ≥ big 时窗口非法；small 超过 s/2 后，最小两数之和已超 s，不可能再有解。
...
1+2+3+4+5、4+5+6、7+8 三组。它们分别对应窗口 [1,5]、[4,6]、[7,8]。
...
<Glossary>
  <GlossaryItem term="滑动窗口">由 small 和 big 维护的连续正整数区间，通过扩张/收缩保持和为 s。</GlossaryItem>
  <GlossaryItem term="等差序列">相邻项差恒定的数列，连续正整数序列是公差为 1 的等差序列。</GlossaryItem>
  <GlossaryItem term="单调性">窗口和随扩张增大、随收缩减小，保证移动方向唯一、不漏解。</GlossaryItem>
</Glossary>
