...
officialUnitId: coi-51
---<Objectives>

- 能递归拆解逆序对为左右内部与跨段三类，归并时从后向前批量计数
- 能解释"一次累计<Term term="批量计数">批量</Term>右段全部剩余元素"的计数方法
- 能处理 <Term term="计数">64 位计数</Term>与零长度输入边界
...
对数组A，<Term term="逆序对">逆序对</Term>定义为：
...
- **现象**：用 int 计数导致大数组溢出。
- **原因**：逆序对数量可能 O(n²)，int 不够。
- **修法**：用 long long 或 64 位整数计数。

</Callout>

## <Term term="分治">分治</Term>拆成三类
...
<Step title="64 位计数">
逆序对数量可能 O(n²)，必须用 64 位整数；零长度输入需特判。
...
## 为什么作者从后向前<Term term="归并">合并</Term>
...
前面元素 > 后面元素即构成逆序对。相等元素（≥）不构成逆序，必须严格大于。
...
两段已排序，从后向前比较：<Term term="左段">左段</Term>末尾 > 右段末尾时，左段该元素与右段全部剩余元素都构成逆序，一次累计右段剩余长度。
...
n 个元素逆序对最多 n(n-1)/2，n=10^5 时约 5×10^9，超 32 位 int 范围。
...
<Glossary>
  <GlossaryItem term="逆序对">数组中前面元素严格大于后面元素的一对下标。</GlossaryItem>
  <GlossaryItem term="分治">把问题拆成左右两半分别求解再合并。</GlossaryItem>
  <GlossaryItem term="归并">合并两个有序段的过程，本题从后向前合并并计数。</GlossaryItem>
  <GlossaryItem term="递归">函数调用自身，分治的典型实现方式。</GlossaryItem>
  <GlossaryItem term="批量计数">一次统计多个逆序对，从后向前归并时的计数技巧。</GlossaryItem>
  <GlossaryItem term="左段">归并时左半段已排序的子数组。</GlossaryItem>
</Glossary>
