...
import { InversePairDecompositionDiagram, InversePairReverseMergeLab, InversePairBufferSwapMap, InversePairContractDiagram, InversePairsOfficialCaseLab } from "@/components/mdx/coding-interviews/diagrams/inverse-pairs";
...
- 能解释逆序对定义中的下标顺序、严格大于和相等值边界
- 能计算两个有序半段从尾部归并时每轮新增的跨段对数
- 能实现作者data与copy交替的归并计数及64位工程版本
- 能分析非空零长度、输入改写、递归边界和int计数溢出
- 能设计覆盖作者8组用例与暴力参考对拍的测试
...
对数组A，<Term def="满足下标i小于j且A[i]严格大于A[j]的有序下标对。">逆序对</Term>定义为：
...
把区间从中点分成左半L和右半R。任意逆序对只可能属于三类：两个下标都在L；两个都在R；前下标在L而后下标在R。前两类分别称为<Term def="两个下标都落在同一个递归半段中的逆序对，由子调用负责统计。">内部逆序</Term>，第三类称为<Term def="前一个元素来自左半段、后一个元素来自右半段的逆序对。">跨段逆序</Term>。
...
这种借助有序连续区间、一次加上整段大小的动作称为<Term def="利用两个半段已有顺序，用一次比较同时确定一个元素与另一段多个元素的配对结果。">批量计数</Term>。它把跨段统计与归并放在同一线性扫描中。
...
这种<Term def="递归每深入一层就交换源数组与目标数组角色，使父层直接读取子层已排序结果。">缓冲区换位</Term>省掉每层显式整段回拷，但阅读难度更高。关键不变式是：Core返回时，start到end的有序结果位于它的copy参数中。
...
<Callout type="trap" title="看到入口另建copy就断言输入只读">
递归参数交替后，子层会把结果写入原data。若调用方要求输入保持不变，应先把输入复制到独立工作数组，再让算法只修改工作副本。
</Callout>
...
<Callout type="trap" title="以为Test8已经覆盖所有空数组表示">
Test8只覆盖nullptr加0；非空哨兵地址加0长度走的是另一条路径。测试输入语义要按指针与长度组合检查，不能只看“长度都是0”。
</Callout>
...
<Stepper>
  <Step title="先按定义枚举小样本">
    <InversePairDecompositionDiagram />
    把总数拆为左内部、右内部和跨段三部分。
  </Step>
  <Step title="从两个有序半段尾部比较">
    <InversePairReverseMergeLab />
    左值更大时一次加入右段全部剩余数量。
  </Step>
  <Step title="跟踪data与copy角色">
    <InversePairBufferSwapMap />
    子调用交换源目标缓冲区，父层直接读取有序半段。
  </Step>
  <Step title="核对接口与数值边界">
    <InversePairContractDiagram />
    检查零长度非空指针、输入改写、相等值和int溢出。
  </Step>
  <Step title="执行8组作者测试">
    <InversePairsOfficialCaseLab />
    覆盖升降序、单双元素、重复值与空指针。
...
**问题 1：左段1、4、7与右段2、3、6归并时，为什么7胜过6要一次加3？**

<Answer>右段升序且j指向最大剩余值6；7大于6就必然也大于右段剩余2和3，因此形成7与2、3、6三对。</Answer>

**问题 2：相等元素为什么走不计数分支？**

<Answer>逆序定义使用严格大于。左值等于右值不构成逆序对，作者反向归并时把右值先写入输出，不增加count。</Answer>

**问题 3：作者建立copy后，为什么原data仍可能被修改？**

<Answer>递归调用交换data与copy参数；顶层子调用把自己的有序结果写入其copy参数，而该参数正是顶层原data，所以原数组的左右半段会被排序。</Answer>

**问题 4：长度0为什么nullptr安全而非空指针不安全？**

<Answer>入口条件先检查data是否为空，nullptr直接返回；非空指针且length为0不满足length小于0，会调用区间0到负1，而Core只有start等于end基例，无法处理空区间。</Answer>
...
<Glossary>
  <GlossaryItem term="逆序对">前下标更小但前值严格大于后值的一对元素。</GlossaryItem>
  <GlossaryItem term="内部逆序">两个下标都在同一递归半段内的逆序对。</GlossaryItem>
  <GlossaryItem term="跨段逆序">前元素在左半段、后元素在右半段的逆序对。</GlossaryItem>
  <GlossaryItem term="批量计数">利用半段有序性，用一次比较确定一整段配对数量。</GlossaryItem>
  <GlossaryItem term="缓冲区换位">递归层间交换源数组与目标数组角色的实现技巧。</GlossaryItem>
</Glossary>

<Attribution
  name="《剑指Offer：名企面试官精讲典型编程题（第2版）》面试题51"
  href="https://github.com/zhedahht/CodingInterviewChinese2/blob/2d6eb6d9127d1ad728ba60bb25bc5a5e91e519ac/51_InversePairs/InversePairs.cpp"
  author="何海涛"
