...
officialUnitId: coi-40
---
import { KLeastPartitionDiagram, KLeastTradeoffDiagram, KLeastOutputContractDiagram } from "@/components/mdx/coding-interviews/diagrams/k-least-numbers";
...
作者给出两条适用于不同约束的路线。解法一复用上一题的<Term term="Partition">Partition</Term>，在完整可写数组上做；解法二把当前最小的k项保存在降序multiset里，可逐个读取数据且不修改输入。
...
- **现象**：用降序 multiset 时，新元素比堆顶大却直接跳过，或比堆顶小却没先弹出再插入。
- **原因**：堆维护的是"当前 k 个最小"，需要与堆顶（最大值）比较后决定是否替换。
- **修法**：新元素 < 堆顶时先 `pop` 堆顶再 `insert`；否则保持原样。堆顶始终是候选集合中最大者。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：把解法一的结果当成有序序列，或假设它不修改原数组。
- **原因**：Partition 只保证第 k 位左边都不更大，内部无序；且原地操作会重排输入。
- **修法**：明确结果契约（无序集合 vs 有序输出），如需保留原数组则用解法二。

</Callout>

## 解法二：<Term term="最大堆">降序multiset</Term>维护最大候选
...
## <Term term="时间复杂度">两案不是只看大O符号</Term>
...
题目只关心分界线：第 k 小位置确定后，它左边恰有 k-1 个不更大的元素，前 k 个位置即答案集合。Partition 单轮 O(n) 平均，不必排完整数组。
...
数据逐个到达时，每次与新元素比较并维护"当前 k 个最小"只需 O(log k) 插入/删除，不修改输入数组，也不需要一次读完全部数据。
...
解法一（Partition）原地重排输入、结果内部无序；解法二（堆）不修改输入、结果同样无序。若要求有序输出，两方案都要最后排序。
...
<Glossary>
  <GlossaryItem term="Partition">快速排序的划分操作，把数组按基准分成左边不更大、右边不小于的两段，返回基准最终位置。</GlossaryItem>
  <GlossaryItem term="最大堆">堆顶为最大值的堆结构，本题用降序 multiset 实现，堆顶即候选集合中最大者。</GlossaryItem>
  <GlossaryItem term="时间复杂度">衡量算法随输入规模增长的运行时间，Partition 期望 O(n)、堆方案 O(n log k)。</GlossaryItem>
</Glossary>
