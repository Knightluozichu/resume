...
import { GiftGridDPDiagram, GiftOptimalPathMap, GiftRollingArrayLab, GiftContractDiagram, MaxValueOfGiftsOfficialCaseLab } from "@/components/mdx/coding-interviews/diagrams/max-value-of-gifts";
...
- 能解释礼物的最大价值为什么具有无后效性的动态规划结构
- 能计算作者4乘4样例的完整累计表并回溯一条最优路径
- 能实现作者二维表与一维数组压缩空间的两种解法
- 能分析正值前提、负值反例、空输入、单行单列和整数溢出
- 能设计覆盖作者六组用例及路径恢复扩展的测试
...
把从起点到位置行i、列j的最大累计价值称为<Term def="从左上起点沿合法移动到达指定格时能够取得的最大累计价值。">到格最优</Term>。若当前礼物价值是value，来自上方的最优值是up，来自左方的是left，那么状态转移为：
...
对任意格子，它的<Term def="在合法移动约束下，能够一步进入当前格的上方格或左方格。">前驱格</Term>只有上方和左方。任意完整路径进入当前格时必经其中一个，因此没有第三类路径被遗漏。
...
作者输入不是二维容器，而是一段连续int内存。第i行第j列通过values[i乘cols加j]读取，这种按行连续存放的布局称为<Term def="二维矩阵先依次存完一整行，再存下一行的连续内存布局。">行主序</Term>。
...
更新列j之前，maxValues[j]仍保存上一行同列，也就是up；maxValues[j减1]已经在本轮更新，保存当前行左方，也就是left。写回后，maxValues[j]变成当前格到格最优。这一组随扫描位置改变解释、但始终维持上述关系的数组称为<Term def="按行扫描时，同列元素在更新前代表上方状态、更新后代表当前格状态的一维列数组。">滚动列状态</Term>。
...
从终点依据累计表反向寻找最优前驱的过程称为<Term def="从终点沿着能保持最优累计值的前驱反向移动，再反转坐标序列以恢复路线。">路径回溯</Term>。若既要O(cols)工作空间又要输出路径，可使用分治式路径恢复，或另存每格方向位；后一种仍需要O(rows乘cols)位空间，只是常数比完整整数表小。
...
<Callout type="trap" title="把缺失前驱永远设为0">
作者代码正确依赖“每格价值大于0”的题设。若允许负值，只有起点可直接取自身；第一行只能继承左方，第一列只能继承上方，内部格才比较两者，或把缺失方向设为可靠的负无穷。
</Callout>
...
<Callout type="trap" title="只检查rows和cols，不检查实际缓冲区长度">
作者的裸指针接口由调用者保证内存至少包含rows乘cols个int。现代接口若能获得span长度，应验证尺寸乘积，否则错误维度会导致越界读取，而不是普通的动态规划错误。
</Callout>
...
<MaxValueOfGiftsOfficialCaseLab />
...
<Stepper>
  <Step title="定义到格最优">
    <GiftGridDPDiagram />
    每格只从上方和左方的累计最优转移，填出完整4乘4表。
  </Step>
  <Step title="从累计表恢复路线">
    <GiftOptimalPathMap />
    从右下比较合法前驱，反向取得一条总和53的路线。
  </Step>
  <Step title="压缩为一维列数组">
    <GiftRollingArrayLab />
    更新前同列代表上方，更新后同列代表当前格，左邻已是当前行。
  </Step>
  <Step title="核对题设与工程边界">
    <GiftContractDiagram />
    正值允许缺失前驱写0；负值、缓冲区长度和溢出需要额外契约。
  </Step>
  <Step title="执行而不只是定义测试">
    <MaxValueOfGiftsOfficialCaseLab />
    前五组由main调用，第六组空输入虽已定义却被遗漏。
  </Step>
</Stepper>
...
**问题 1：4乘4样例第二行第三列为什么是24？**

<Answer>该格礼物价值是9，上方到格最优为14，左方到格最优为15；所有合法路径只能来自这两个前驱，所以取15再加9得到24。</Answer>

**问题 2：一维数组更新第i行第j列之前，maxValues中哪两个位置分别代表什么？**

<Answer>maxValues[j]尚未覆盖，代表上一行同列的up；maxValues[j减1]已在当前行更新，代表当前行左侧的left。写回maxValues[j]后，它代表当前格。</Answer>

**问题 3：为什么礼物允许负值后不能继续把不存在的前驱设为0？**

<Answer>0会大于合法路径的负累计值，使转移仿佛从棋盘外中途进入。例如单行负5、负2会被错误算成负2，而合法路径必须收集两格，总和负7。</Answer>

**问题 4：如何同时返回最大价值和一条最优路径？**

<Answer>保留完整二维累计表，从右下开始比较上方和左方累计值，选择能产生当前最优值的前驱，直到左上，再反转坐标序列。并列时按固定规则任选即可。</Answer>
...
<Glossary>
  <GlossaryItem term="到格最优">从左上沿合法移动到达指定格时的最大累计礼物价值。</GlossaryItem>
  <GlossaryItem term="前驱格">能够一步进入当前格的上方格或左方格。</GlossaryItem>
  <GlossaryItem term="行主序">二维矩阵逐行连续存放在一维内存中的布局。</GlossaryItem>
  <GlossaryItem term="滚动列状态">更新前保存上方、更新后保存当前格的一维列累计数组。</GlossaryItem>
  <GlossaryItem term="路径回溯">从终点按累计最优反向选择前驱并恢复完整路线的过程。</GlossaryItem>
</Glossary>

<Attribution
  name="《剑指Offer：名企面试官精讲典型编程题（第2版）》面试题47"
  href="https://github.com/zhedahht/CodingInterviewChinese2/blob/2d6eb6d9127d1ad728ba60bb25bc5a5e91e519ac/47_MaxValueOfGifts/MaxValueOfGifts.cpp"
  author="何海涛"
