...
作者给出两种方案。第一种用随机<Term term="Partition">Partition</Term>反复缩小区间，把正确元素放到；第二种不改变数组，只维护候选和票数，把不同数字成对抵消。二者都只能产生候选，不能省略最后的计数。

<MajorityPartitionDiagram />

## 解法一：基于Partition的第k小

解法一就是<Term term="第k小">基于Partition的第k小</Term>：作者计算 `middle = length >> 1`，在整个数组上调用工具函数 `Partition`。该函数随机选主元，把小于主元的元素移到左边，并返回主元最终下标。这里不需要把数组完全排序，只需让某次返回下标等于middle。
...
- **现象**：用次数抵消找到候选后直接返回，没做完整计数验证。
- **原因**：抵消法只保证"若存在多数元素则它是候选"，不保证候选真的超过一半。
- **修法**：候选产生后必须再扫一遍数组统计真实次数，少于一半返回无效标志。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：次数抵消时，遇到不同数字把票数减到 0 后忘记重置候选。
- **原因**：票数为 0 意味着当前候选被完全抵消，应换成当前数字并重置票数为 1。
- **修法**：`count == 0` 时令 `candidate = 当前值; count = 1`，否则按相同/不同增减。

</Callout>

## 解法二：<Term term="次数抵消">次数抵消</Term>
...
出现次数超过一半意味着相同元素超过 n/2 个，排序后它们占据的连续区间长度 > n/2，必然覆盖中位位置 ⌊n/2⌋。
...
候选与票数记录"当前最可能的多数元素"。票数为 0 时重置候选；相同票数 +1、不同 -1。若存在多数元素，抵消后它必然留下作为最终候选。
...
Partition 只把中位候选放到位，抵消法只在线压缩候选——两者都不能证明候选真的超过一半。必须再扫一遍统计真实次数，少于一半返回无效。
...
<Glossary>
  <GlossaryItem term="Partition">快速排序划分操作，本题用它反复缩小区间定位中位候选。</GlossaryItem>
  <GlossaryItem term="次数抵消">用候选与票数成对抵消不同数字的在线算法，不修改数组、O(1) 空间。</GlossaryItem>
  <GlossaryItem term="验证">候选产生后完整统计真实次数，确认是否真的超过一半。</GlossaryItem>
  <GlossaryItem term="第k小">数组排序后位于第 k 位的元素，Partition 可 O(n) 期望定位。</GlossaryItem>
</Glossary>
