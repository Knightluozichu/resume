...
officialUnitId: coi-56-01
---

import { NumberAppearingOnceDiagram } from "@/components/mdx/coding-interviews/diagrams/number-appearing-once";
...
- **现象**：大输入时超时或内存溢出。
- **原因**：算法<Term term="复杂度">复杂度</Term>或空间使用不当。
- **修法**：优化时间或空间复杂度。
...
## 从“<Term term="异或">异或</Term>为什么突然失效”开始

先预测：1、1、1、2、2、2、3中唯一值是3。若像上一小题那样把所有值直接异或，三个1异或后仍是1，三个2异或后仍是2，最终结果是1异或2异或3，也就是0，并非答案3。

原因是相同<Term term="数字">数字</Term>出现两次才会异或归零；出现三次等价于留下一个副本。本题明确要求找出**<Term term="数组">数组</Term>中唯一只出现<Term term="一次">一次</Term>的数字**，而**其他数字都出现三次**，必须换一种能让三份贡献消失的聚合方式。

对每个<Term term="二进制">二进制</Term><Term term="位">位</Term>置独立<Term term="统计">统计</Term>1的数量，这称为。任意重复值出现三次时，它在每一位要么贡献0，要么贡献3；对3取余都会归零。
...
因此该位总数除以3的余数，恰好等于唯一值在这一位的<Term term="比特">比特</Term>。这种让三份相同贡献在模3下归零称为。

每个位置得到的0或1称为。把所有余数按原位放回，就恢复完整整数。


这不是数值总和对3取余。整数加法会产生进位，不同位相互影响；位<Term term="计数">计数</Term>让32个位置独立处理，才不会丢失位模式。
...
现代实现应明确使用int32_t与uint32_t，把<Term term="位运算">位运算</Term>放在无符号类型上，再用bit_cast按相同位模式解释回有符号值。
...
## 作者（<Term term="边界条件">边界条件</Term>）异常写法的边界
...
数组中只出现一次的两个数字，是所有数字中唯一不成对的。所有数字异或，得到两个目标数字的异或结果。按异或结果中某一位分组，两个数字分别落入不同组，每组再各自异或即得答案。
