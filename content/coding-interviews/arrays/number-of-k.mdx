...
import { NumberOfKSortedRunDiagram, FirstKBoundaryLab, LastKBoundaryLab, NumberOfKContractDiagram, NumberOfKOfficialCaseLab } from "@/components/mdx/coding-interviews/diagrams/number-of-k";
...
- 能解释排序如何让所有目标k聚成一个连续区间
- 能计算第一个k和最后一个k两次递归二分的区间变化
- 能实现作者边界搜索及标准库equal_range等价版本
- 能分析开头结尾、目标缺失、空输入、中点溢出与无序输入
- 能设计覆盖作者11组用例并与线性计数对拍的测试
...
非递减排序保证相同值不会分散在多个位置：所有k构成一个<Term def="排序数组中所有值等于k的元素形成的单一连续下标区间。">连续重复段</Term>。若该段首下标first为2、尾下标last为5，出现次数就是5减2加1等于4。
...
普通二分命中data[middle]等于k时不能立刻返回。只有middle等于0，或左邻data[middle减1]不等于k，middle才是<Term def="连续重复段中最小的下标；它等于k且左侧不存在另一个k。">左边界</Term>。
...
最后一个k称为<Term def="连续重复段中最大的下标；它等于k且右侧不存在另一个k。">右边界</Term>。命中时只有middle等于length减1，或右邻不等于k，才能返回。
...
last减first加1是<Term def="同时包含左右端点的下标区间元素个数，计算式为last减first加1。">闭区间计数</Term>。漏掉加1会让单元素命中返回0，也会把四个3算成3。
...
这类修改过命中分支、专门寻找最左或最右满足位置的搜索称为<Term def="命中目标值后仍按边界方向继续收缩，寻找最左或最右合法位置的二分查找。">边界二分</Term>。
...
<Callout type="trap" title="把边界二分用于无序数组">
相等值若分散在多处，舍弃半区的推理不成立，first到last之间也可能夹杂其他值。无序输入应先排序、使用哈希计数，或直接线性扫描。
</Callout>
...
<Callout type="trap" title="找到任意一个k后向两侧线性扩展">
这种做法最坏会扫描整个重复段；数组全是k时退化为O(n)。两次边界二分才能保证重复再多仍是O(logn)。
</Callout>
...
<Stepper>
  <Step title="先把相等值看成连续区间">
    <NumberOfKSortedRunDiagram />
    排序把所有k聚在first到last的闭区间中。
  </Step>
  <Step title="二分查找第一个k">
    <FirstKBoundaryLab />
    命中后检查左邻；仍为k就继续向左收缩。
  </Step>
  <Step title="二分查找最后一个k">
    <LastKBoundaryLab />
    镜像检查右邻；仍为k就继续向右收缩。
  </Step>
  <Step title="核对排序与边界契约">
    <NumberOfKContractDiagram />
    处理空输入、不存在、数组端点、中点溢出和标准库等价。
  </Step>
  <Step title="执行11组作者用例">
    <NumberOfKOfficialCaseLab />
    覆盖重复段位置、范围外目标、全同、单元素和空指针。
  </Step>
</Stepper>
...
**问题 1：第一次二分命中下标3的3，为什么不能立即返回？**

<Answer>下标2仍是3，说明下标3不是左边界；要把end移到2继续找第一个3。查次数需要左右两个边界，不是任意命中点。</Answer>

**问题 2：为什么出现次数要加1？**

<Answer>first和last都属于重复段，是闭区间。下标2到5包含2、3、4、5四个位置，数量为5减2加1。</Answer>

**问题 3：数组全为3时两次二分仍是对数复杂度吗？**

<Answer>是。左边界搜索每次向左折半，右边界搜索每次向右折半，不会线性遍历全部3；最终分别得到0和length减1。</Answer>

**问题 4：旧页的n除k高频算法为何不能用于本题？**

<Answer>本题k是具体目标值且依赖排序连续段，只返回一个次数；高频算法的k是阈值参数、输入可无序并返回多个候选，问题定义不同。</Answer>
...
<Glossary>
  <GlossaryItem term="连续重复段">排序数组中所有等于目标值的连续区间。</GlossaryItem>
  <GlossaryItem term="左边界">重复段中第一个目标值的下标。</GlossaryItem>
  <GlossaryItem term="右边界">重复段中最后一个目标值的下标。</GlossaryItem>
  <GlossaryItem term="边界二分">命中后继续朝指定方向收缩以寻找极端位置的二分。</GlossaryItem>
  <GlossaryItem term="闭区间计数">包含两个端点的区间长度last减first加1。</GlossaryItem>
</Glossary>

<Attribution
  name="《剑指Offer：名企面试官精讲典型编程题（第2版）》面试题53（一）"
  href="https://github.com/zhedahht/CodingInterviewChinese2/blob/2d6eb6d9127d1ad728ba60bb25bc5a5e91e519ac/53_01_NumberOfK/NumberOfK.cpp"
  author="何海涛"
