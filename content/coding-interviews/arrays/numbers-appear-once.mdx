...
import { NumbersAppearOnceDiagram } from "@/components/mdx/coding-interviews/diagrams/numbers-appear-once";
...
先预测：数组2、4、3、6、3、2、5、5中，除了4和6，其他数字都出现两次。若建立哈希表当然能<Term term="计数">计数</Term>，但会使用O(n)额外空间；题目要求O(n)时间和O(1)空间。

<Term term="异或">异或</Term>满足同一个数与自身异或为0、0与任意数异或仍为该数，并且交换顺序不影响结果。因此2与2、3与3、5与5都能消失。这种偶数次同值归零称为。
...
然后**按异或结果中某一位<Term term="分组">分组</Term>**。a和b会被分开；任意一对相同数字每一位都相同，所以必定一起进入同一组，绝不会被拆散。最终**两个数字分别落入不同组**，每组其余配对值继续抵消。
...
## 作者（<Term term="边界条件">边界条件</Term>）契约未被代码完整验证


合法题目保证恰有两个不同目标，所以总体异或必定非0。若调用方传入所有值都<Term term="成对">成对</Term>、两个目标其实相同、目标数不是两个，结果可能为0。
