...
import { NumbersAppearOnceXorDiagram, NumbersAppearOnceSplitBitLab, NumbersAppearOnceGroupMap, NumbersAppearOnceContractDiagram, NumbersAppearOnceOfficialCaseLab } from "@/components/mdx/coding-interviews/diagrams/numbers-appear-once";
...
- 能解释全部元素异或后为何只剩两个目标的异或值
- 能计算作者从右侧寻找首个置位及两组的最终结果
- 能实现作者辅助函数和无符号可移植分组版本
- 能分析非法契约、负数右移、输出指针、顺序与偶数次扩展
- 能设计覆盖作者3组用例并用频次参考实现对拍的测试
...
异或满足同一个数与自身异或为0、0与任意数异或仍为该数，并且交换顺序不影响结果。因此2与2、3与3、5与5都能消失。这种偶数次同值归零称为<Term def="相同位模式出现偶数次时，连续异或的结果为0。">异或抵消</Term>。

<NumbersAppearOnceXorDiagram />

把**所有数字异或**后，示例只剩4异或6，结果为2。这个聚合值称为<Term def="输入数组全部元素逐项异或后的值，合法输入下等于两个唯一目标的异或。">总体异或</Term>。

## 一个总体异或为何还不够

设两个只出现一次的数字为a和b。总体异或等于a异或b，但仅凭这个值通常不能唯一还原a与b；许多数字对可能得到同一个异或结果。

关键是a与b不同，所以a异或b必定非0，二进制中至少有一位为1。该位为1意味着a与b在这一位不同：一个是1，另一个是0。选择这样的位作为<Term def="总体异或结果中任意一个为1的二进制位，用来把输入划分成两组。">分组位</Term>。

然后**按异或结果中某一位分组**。a和b会被分开；任意一对相同数字每一位都相同，所以必定一起进入同一组，绝不会被拆散。最终**两个数字分别落入不同组**，每组其余配对值继续抵消。

<NumbersAppearOnceSplitBitLab />
...
<NumbersAppearOnceGroupMap />

相同数字完整留在同一组的性质称为<Term def="一对相同元素依据同一位分类时必定进入同组，因此仍可在组内异或归零。">同组配对</Term>。它是分组后抵消仍成立的核心，不是“随机拆成两半”。
...
<Callout type="trap" title="总体异或为0后仍继续分组">
0没有任何置位，不能把两个不同目标分开。工程接口应在选位前检查结果是否为0并返回错误；这仍无法证明所有频次都合法，只能拒绝一个明显矛盾。
</Callout>
...
<Callout type="trap" title="把任意置位误判成只能选最低位">
作者选最低置位只是确定性实现。总体异或中最高位或其他任意1位都能分开两个目标；真正错误的是选择值为0的位，因为两个目标在该位相同。
</Callout>
...
作者测试采用集合语义：expected1和expected2可与result1、result2同序或交换。这种不承诺先后称为<Term def="两个正确答案作为无序集合返回，调用方不得依赖哪个写入第一个输出。">输出无序性</Term>。
...
<Stepper>
  <Step title="让所有重复对先抵消">
    <NumbersAppearOnceXorDiagram />
    全体异或只留下两个目标的异或值。
  </Step>
  <Step title="从差异中选择分组位">
    <NumbersAppearOnceSplitBitLab />
    总体异或的置位保证两个目标在该位不同。
  </Step>
  <Step title="保持重复元素同组">
  ...
  <Step title="核对指针与位操作契约">
    <NumbersAppearOnceContractDiagram />
    非零总体异或和有效输出地址都由调用方保证。
  </Step>
  <Step title="执行作者3组测试">
    <NumbersAppearOnceOfficialCaseLab />
    最小输入与四次重复扩展不能省略。
...
**问题 1：为什么不能直接把总体异或当成其中一个答案？**

<Answer>总体异或等于两个目标a异或b，通常既不等于a也不等于b，而且同一结果可由许多数字对产生。必须用它的差异位把目标分开后分别异或。</Answer>

**问题 2：选择总体异或中的最高置位是否错误？**

<Answer>不错误。任何为1的位都表示两个目标在该位不同，都能把它们分开；作者选最低置位只是方便且确定。选择为0的位才无法保证分离。</Answer>

**问题 3：为何数字1出现四次仍会从Test3中消失？**

<Answer>1异或1为0，两对1分别归零；更一般地，同一位模式出现任意偶数次都抵消。题面要求两次，但算法性质更宽。</Answer>

**问题 4：为何只检查xorAll非0还不能完整验证输入？**

<Answer>三个或更多奇数频次值的总体异或也可能非0，分组后得到的是各组聚合值，不保证是两个原值。完整验证需要频次统计或其他额外约束检查。</Answer>
...
<Glossary>
  <GlossaryItem term="异或抵消">相同位模式出现偶数次后异或归零。</GlossaryItem>
  <GlossaryItem term="总体异或">数组全部元素的异或聚合值。</GlossaryItem>
  <GlossaryItem term="分组位">总体异或中用于区分两个目标的任意置位。</GlossaryItem>
  <GlossaryItem term="同组配对">相同元素依据同一位分类后必定进入同组。</GlossaryItem>
  <GlossaryItem term="输出无序性">两个答案正确但先后次序不属于接口承诺。</GlossaryItem>
</Glossary>

<Attribution
  name="《剑指Offer：名企面试官精讲典型编程题（第2版）》面试题56（一）"
  href="https://github.com/zhedahht/CodingInterviewChinese2/blob/2d6eb6d9127d1ad728ba60bb25bc5a5e91e519ac/56_01_NumbersAppearOnce/NumbersAppearOnce.cpp"
  author="何海涛"
