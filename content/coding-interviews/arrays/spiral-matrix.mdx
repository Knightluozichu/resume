...
officialUnitId: coi-29
---
import { SpiralRingPathDiagram, SpiralEdgeConditionMap, SpiralStartInvariantDiagram } from "@/components/mdx/coding-interviews/diagrams/spiral-matrix";
...
作者解决“顺时针打印矩阵”时，不维护会在圈内变化的上下左右四个变量，而是让第k圈的<Term term="环起点">环起点</Term>固定为start。由它计算右端列endX与下端行endY，再按上、右、下、左依次处理。每完成一圈才增加start。
...
## 四条边的条件为何逐步变严（<Term term="边界条件">边界条件</Term>）
...
- **现象**：把只有一行的矩阵仍按"上、右、下、左"四段打印，导致重复打印。
- **原因**：退化成线段的一圈被误当矩形边框。
- **修法**：右边只在 `start < endY` 时打印，下边只在 `start < endY && start < endX` 时打印，左边同理逐步变严。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：用 start 判断环是否继续时，写成 `start < rows && start < cols` 但忘记考虑行或列可能为 0。
- **原因**：空矩阵或零行零列需要提前返回。
- **修法**：入口检查行、列是否为 0，返回空序列；再按 start 与行列比较循环。
...
## 与<Term term="收缩法">四边界收缩法</Term>的关系
...
上边打印全部 5 个，右边无新行，下边又从右向左打印 5 个——重复。修复：下边仅在 `start < endY`（至少两行）时打印，左边同理需要两行两列。
...
第 k 圈左上角为 (start,start)，其 2 倍必须小于行/列数，否则没有新的一圈可打印。这保证每圈至少占一行和一列。
...
从 (0,0) 起：上边 1,2,3,4；右边 8,12；下边 11,10,9；左边 5；内圈 6,7 再 11（实际按完整四边规则输出 6,7,11,10 后结束）。完整序列以作者实现为准。
...
<Glossary>
  <GlossaryItem term="环起点">每圈的左上角坐标 (start,start)，由它计算右端列与下端行。</GlossaryItem>
  <GlossaryItem term="边界条件">四条边各自是否启用的判定，随行/列退化逐步变严。</GlossaryItem>
  <GlossaryItem term="收缩法">另一种维护上下左右四边界逐步收缩的等价实现。</GlossaryItem>
</Glossary>
