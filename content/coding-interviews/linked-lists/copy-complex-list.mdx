...
import { ComplexListInterleaveDiagram, ComplexSiblingMappingMap, ComplexListSplitDiagram, CopyComplexListOfficialCaseLab } from "@/components/mdx/coding-interviews/diagrams/copy-complex-list";
...
- 能解释交织结构怎样以next替代原到克隆的哈希映射
- 能计算跨节点、自指和成环sibling的克隆目标地址
- 能实现作者CloneNodes、ConnectSiblingNodes与ReconnectNodes
- 能分析临时修改、异常失败、链外引用、并发与深拷贝边界
- 能设计覆盖作者五组普通、自指、环、单点与空链测试
...
“**复杂链表的复制**”中每个节点有m_pNext和m_pSibling，后者也常叫random。值和next只能确定主链，随机指针可能向前、向后、指自己、为空或形成环。核心任务是建立<Term def="每个原节点到其唯一新节点的对应关系，供next与sibling重建使用。">身份映射</Term>。

哈希表可以显式保存原地址到克隆地址；作者则把映射编码进临时链表结构，让<Term def="每个原节点后紧跟自身克隆节点的临时next序列。">交织链</Term>满足原节点.next就是它的克隆。
...
“**复制节点插入原节点后面**”带来<Term def="无需外部容器、通过原节点next在常数时间得到对应克隆节点的关系。">局部映射</Term>：clone(x)等于x.next。阶段二和阶段三都依赖它，不能提前恢复next。
...
<Term def="把交织序列中的原节点与克隆节点分别重新连接成两条独立主链的过程。">拆链</Term>不是只移动返回指针。如果忘记恢复原next，调用方原链仍夹着克隆；如果忘记连接克隆next，返回链只有头或仍穿过原节点。
...
<Callout type="trap" title="设置克隆sibling时直接复制原地址">
A′.sibling=A.sibling会让克隆链指向原节点，原链销毁后出现悬空引用。正确目标是A.sibling对应的克隆，即交织状态下A.sibling.next。
</Callout>

## 深拷贝应验证什么

<Term def="新结构拥有独立节点地址，但值、next拓扑和sibling拓扑与原结构同构的复制。">深拷贝</Term>不只是打印值相同。对第i个原节点和克隆节点，应满足：地址不同、值相同；next为空状态相同且克隆next对应原next；sibling为空状态相同且克隆sibling对应原sibling。
...
<Callout type="trap" title="宣称交织法始终不修改输入">
最终拆分会恢复原next，但三个阶段之间输入确实被改变。观察时序、并发安全和异常失败都能看见这段临时变异；“最终恢复”不等于“从未修改”。
</Callout>
...
<CopyComplexListOfficialCaseLab />
...
<Stepper>
  <Step title="把克隆插到原节点后"><ComplexListInterleaveDiagram />原到克隆的身份映射变成一次next读取，不需要哈希表。</Step>
  <Step title="按目标next复制sibling"><ComplexSiblingMappingMap />前后引用、自指与sibling环都通过同一局部公式重建。</Step>
  <Step title="恢复原链并拆出克隆链"><ComplexListSplitDiagram />每轮同时修复两个next，不能提前破坏未处理映射。</Step>
  <Step title="跑完五组官方结构"><CopyComplexListOfficialCaseLab />跨节点、自指、环、单点和空链覆盖随机关系边界。</Step>
</Stepper>
...
**问题 1：原A.sibling指向原C时，克隆A′应怎样定位C′？**

<Answer>交织后原C.next就是克隆C′，所以令A′.sibling等于A.sibling.next。不能直接复制A.sibling，否则会指回原C。</Answer>

**问题 2：为什么sibling形成2与4互指不会让算法死循环？**

<Answer>阶段二只沿next主链遍历，每个sibling仅被读取一次并通过目标next映射；不会沿sibling继续走，因此其环不影响终止。</Answer>

**问题 3：拆分时必须恢复哪些指针？**

<Answer>原节点next要跳过克隆恢复到下一原节点；克隆节点next要跳过下一原节点连接到下一克隆。两条更新同步进行，才能同时恢复输入和形成独立输出。</Answer>

**问题 4：O(1)辅助空间为何不表示只分配常数内存？**

<Answer>结果本身必须新建n个克隆节点，这属于必要输出。O(1)只指除输出外不再使用随n增长的哈希表或数组；总新节点分配仍是O(n)。</Answer>
...
<Glossary>
  <GlossaryItem term="身份映射">从每个原节点到其唯一克隆节点的对应关系。</GlossaryItem>
  <GlossaryItem term="交织链">原节点与对应克隆节点交替连接的临时主链。</GlossaryItem>
  <GlossaryItem term="局部映射">通过原节点next直接取得其克隆的常数时间关系。</GlossaryItem>
  <GlossaryItem term="拆链">恢复原next并将克隆next连接为独立链的过程。</GlossaryItem>
  <GlossaryItem term="深拷贝">节点地址独立而值与引用拓扑同构的复制结果。</GlossaryItem>
</Glossary>

<Attribution
  name="《剑指Offer：名企面试官精讲典型编程题（第2版）》面试题35"
  href="https://github.com/zhedahht/CodingInterviewChinese2/blob/2d6eb6d9127d1ad728ba60bb25bc5a5e91e519ac/35_CopyComplexList/CopyComplexList.cpp"
  author="何海涛"
