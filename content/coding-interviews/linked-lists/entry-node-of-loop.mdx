...
import { LoopMeetingDiagram, LoopLengthCountingMap, LoopGapEntryDiagram, LoopEntryOfficialCaseLab } from "@/components/mdx/coding-interviews/diagrams/entry-node-of-loop";
...
- 能解释快慢指针为何能区分无环链表与有环链表
- 能计算从任意相遇点绕一圈得到的环中节点数
- 能实现作者判断、计数、先行环长和同步找入口的三阶段方案
- 能分析作者方案与相遇后重置头指针方案的等价关系
- 能设计覆盖单节点自环、头中尾入口、无环和空链的身份测试
...
无环单链表的快指针最终会到空，或停在没有后继的尾节点。有环时，快慢指针进入有限环后，快指针每轮相对慢指针多前进一步；这个每轮净追近量称为<Term def="两个移动位置每轮步数之差，决定环上相对位置怎样变化。">相对速度</Term>。相对位置只有有限种，差值按环长取模逐格变化，最终必回到0而相遇。这种“**快慢指针判断有环**”只需常数个指针。

把第一次相等的环内节点称为<Term def="一倍速和两倍速指针在环内第一次指向同一物理节点的位置。">相遇节点</Term>。它为后续计数提供一个已知在环内的安全起点，却不直接等于<Term def="从链表头沿next首次进入循环部分时到达的节点。">环入口</Term>。
...
<Callout type="trap" title="只检查fast不检查fast的下一节点">
两倍速移动需要连续读取两条next边。无环奇数长度链表中，fast可能停在尾节点；若直接执行fast->next->next，会先对空后继解引用。作者拆成两次前进并在中间检查。
</Callout>

## 从相遇点绕一圈统计环长

取得相遇节点后，作者把 nodesInLoop 初始化为1，让一个指针沿 next 前进，直到它的下一节点重新是 meeting。每经过一个新环内节点就加1。这个值就是<Term def="从任一环内节点沿next回到自身所经过的不同节点数量。">环长</Term>，也就是“**统计环中节点数**”。
...
把<Term def="一指针从头先行L步，随后两指针同步移动并持续相差一整圈的关系。">固定环长间隔</Term>建立起来后，定位过程与上一题“倒数第k个节点”同构：先制造距离，再同步直到满足终点条件。这里终点不是尾部，而是两个地址相同。
...
<Callout type="trap" title="用节点值判断相遇或回到起点">
链表允许重复值，两个不同节点可以都保存3。判环、计数和入口定位都必须比较地址身份；值相等既不能证明相遇，也不能证明绕完一圈。
</Callout>
...
<Stepper>
  <Step title="先让快慢指针相遇"><LoopMeetingDiagram />fast每轮两步、slow每轮一步；无环到空，有环在环内相遇。</Step>
  <Step title="从相遇点数完整一圈"><LoopLengthCountingMap />只移动一个指针，首次回到meeting时计数就是环长L。</Step>
  <Step title="让两头指针相隔L步"><LoopGapEntryDiagram />p1先行整圈长度，再与p2同步，首次相等处就是入口。</Step>
  <Step title="用七组入口位置封口"><LoopEntryOfficialCaseLab />单节点、中部、头部、尾部、无环和空链覆盖三阶段边界。</Step>
</Stepper>
...
<Answer>两者进入长度L的环后，fast每轮比slow多走一步，相对位置每轮增加1模L。有限的L种相对位置中，至多L轮就会回到差值0，因此指向同一节点。</Answer>

**问题 2：为什么从任意相遇点都能数出正确环长？**

<Answer>相遇点已在唯一环内。从它沿next前进，首次回到自身之前会按环方向恰好访问每个环内节点一次；经过的节点数与选择哪个环内起点无关，都是L。</Answer>

**问题 3：两个指针相隔L步为何在入口相遇？**

<Answer>后指针从头走a步到入口，前指针同期总共走a+L步。进入环后多走L步等于多绕完整一圈，位置不变，所以前指针也在入口。</Answer>

**问题 4：单节点自环为何能通过作者检测？**

<Answer>slow初始化为head->next，等于head；fast初始化为slow->next，也等于head。循环第一次比较就相等，环长计数初值1，两个头指针相隔一步后仍都在head。</Answer>
...
<Glossary>
  <GlossaryItem term="相遇节点">一倍速与两倍速指针在环内首次指向同一物理节点的位置。</GlossaryItem>
  <GlossaryItem term="环入口">从链表头沿next第一次进入循环部分的节点。</GlossaryItem>
  <GlossaryItem term="环长">从任一环内节点沿next回到自身经过的不同节点数。</GlossaryItem>
  <GlossaryItem term="相对速度">fast每轮比slow多前进的步数，本题为一步。</GlossaryItem>
  <GlossaryItem term="固定环长间隔">一个指针先行L步后，两指针同步并始终相差一整圈的关系。</GlossaryItem>
</Glossary>

<Attribution
  name="《剑指Offer：名企面试官精讲典型编程题（第2版）》面试题23"
  href="https://github.com/zhedahht/CodingInterviewChinese2/blob/2d6eb6d9127d1ad728ba60bb25bc5a5e91e519ac/23_EntryNodeInListLoop/EntryNodeInListLoop.cpp"
  author="何海涛"
