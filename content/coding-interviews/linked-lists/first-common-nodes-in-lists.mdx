...
先预测：两条链表各自包含一个值为6的节点，并不代表它们相交。题目所说的**两个链表的第一个公共节点**，是两个next指针最终指向同一个内存对象；比较的是<Term term="节点地址">节点地址</Term>。

一条单链表节点只有一个next。两条无环链一旦指向同一个节点，后续每一步都会沿这个节点唯一的next继续，因此从交点到尾部全部共享。这段公共部分称为<Term term="共享后缀">共享后缀</Term>。
...
- **现象**：两个节点值相等就判定为公共节点。
- **原因**：公共节点要求两个指针指向同一个对象（地址相等），不是值相等。
- **修法**：用节点地址（指针）比较，不是值比较。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：用 int 计算长度差，未考虑无符号类型转换陷阱。
- **原因**：size_t 无符号相减可能大数减小数得大正数。
- **修法**：用 int 或 long long 存长度差，或先比较再用较大减较小。
...
地址相等。两个节点值相同但内存地址不同，不是同一个对象，不构成公共节点。
...
两个头到尾部的距离不同。让长链表先走多出的长度，两指针到尾距离相等，同步前进即可在交点相遇。
...
size_t 无符号相减，大数减小数得大正数而非负数。应先用 int 或比较后确定大小再减。
...
<Glossary>
  <GlossaryItem term="节点地址">节点在内存中的指针值，唯一标识同一个对象。</GlossaryItem>
  <GlossaryItem term="共享后缀">两链表从公共节点到尾部共享的节点序列。</GlossaryItem>
  <GlossaryItem term="长度差">两链表长度差值，用于对齐尾距。</GlossaryItem>
</Glossary>
