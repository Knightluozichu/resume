...
import { KthGapDiagram, KthPointerInvariantMap, KthBoundaryDiagram, KthOfficialCaseLab } from "@/components/mdx/coding-interviews/diagrams/kth-node-from-end";
...
- 能解释为什么快指针领先k-1条边时慢指针会停在倒数第k个节点
- 能计算k为1、等于长度和大于长度时两指针的位置
- 能实现作者单遍扫描版本及等价的先行k步版本
- 能分析空链表、k为0、负数转换、环和并发修改边界
- 能设计覆盖作者六组输入与节点身份的自动断言

</Objectives>

## 从“倒数第1个是谁”开始

先预测：链表 1→2→3→4→5 中，倒数第1个是5，倒数第2个是4，倒数第5个是1；倒数第0个没有意义。作者明确从1开始计数，这个<Term def="从尾节点记为1并向前递增的位置编号规则。">倒数序号</Term>决定了快指针应领先多少步。

若先遍历得到长度 n，目标正数位置是 n-k+1，再从头走一遍即可。但题目希望只扫描一次。单向链表不能从尾部向前走，所以需要在第一次经过节点时保留一个与尾部相对距离有关的位置。

让 ahead 与 behind 都从头开始，ahead 先走 k-1 条 next 边。之后两者同步前进；ahead 到达尾节点时，behind 就是链表中倒数第k个节点。两个速度相同但保持固定距离的位置称为<Term def="在同一链表上保持确定领先距离并同步前进的两个遍历位置。">快慢指针</Term>。

<KthGapDiagram />

这里“快”不是每轮走两步，而是先启动 k-1 步；同步阶段两者速度相同。快慢只是相对起点不同。把它和判圈算法中一倍、两倍速度的快慢指针混为一谈，容易写错间隔。

## 为什么必须保持k-1条边

长度为 n 的链表中，倒数第k个节点是正数第 n-k+1 个。behind 从第1个节点走到这里需要 n-k 步。ahead 先到第k个节点，随后再走 n-k 步，恰好到第n个尾节点。

因此两个指针的<Term def="ahead相对behind始终领先k-1条next边，覆盖连续k个节点的关系。">固定间隔</Term>是 k-1 条边，或等价地说两端之间覆盖 k 个节点。原书要点“**保持k-1个节点间隔**”在这里指两个位置相差k-1次next移动。k=1 时领先0条边，两指针重合并最终一起到尾；k=n 时 ahead 先行到尾，behind不再移动并返回头。
...
<Callout type="trap" title="把同步条件改为ahead不为空却不改其他步骤">
作者版本让ahead领先k-1步并停在尾节点。若循环一直走到ahead为空，behind也会多走一步，返回倒数第k-1个。可以采用走到空的版本，但必须改为ahead先行k步。
</Callout>
...
这两个版本都正确，但不能拿第一个版本的先行 k-1 步搭配第二个版本的走到空，或反过来。面试时先写清“间隔是几条边、终点是尾节点还是空”，再落循环条件，可以消除 off-by-one。

返回 const ListNode* 能表达函数只观察链表，不修改节点。若调用者需要可变节点，可提供重载或返回 ListNode*；不应为了方便在只读接口中去掉 const。

## 三类防御检查对应三种失败

空链表没有任何节点，立即返回空；k=0违反从1开始的计数规则，立即返回空；k大于长度则在先行阶段发现后继不足。也就是说，“**k非法与链表不足k个节点**”是两种不同失败：前者在入口拒绝，后者在先行过程中识别。把这些入口和过程检查称为<Term def="在执行危险解引用或算术前主动拒绝非法输入和不可满足条件。">防御式检查</Term>。
...
算法中 ahead 与 behind 各自访问一部分链表，但没有先完整计数再重新从头扫描。每条 next 边被访问常数次，总工作量不超过约两倍长度，仍属于<Term def="不先完成一次全链统计再从头重启，而是在同一前向过程中完成定位。">单遍扫描</Term>和 O(n) 时间。
...
<Callout type="trap" title="只检查返回节点的value">
存在重复值时，错误节点也可能拥有同样数值。题目返回的是物理节点，应比较地址或稳定ID；同时明确返回值不拥有节点，不能由调用者单独释放。
</Callout>
...
<Stepper>
  <Step title="先建立k-1条边间隔"><KthGapDiagram />ahead先行，两个指针覆盖连续k个节点。</Step>
  <Step title="同步保持固定间隔"><KthPointerInvariantMap />两者每轮各走一步，ahead到尾时behind落在正数第n-k+1个。</Step>
  <Step title="逐类拒绝非法输入"><KthBoundaryDiagram />空头、k为0和长度不足分别在入口或先行阶段返回空。</Step>
  <Step title="用六组官方位置封口"><KthOfficialCaseLab />中间、尾、头、空链、超长k和零k覆盖所有分支。</Step>
</Stepper>
...
**问题 1：为什么k=1时快指针不需要先走？**

<Answer>作者版本需要保持k-1条边间隔，k=1时为0，两指针始终重合。同步到ahead位于尾节点时，behind也位于尾节点，正好是倒数第1个。</Answer>

**问题 2：先行k步版本为什么要让ahead走到空？**

<Answer>先行k步后ahead与behind相差k条边。同步直到ahead越过尾部为空时，behind距离空位置仍有k条边，也就是从behind起共有k个节点，因此它是倒数第k个。</Answer>

**问题 3：k大于长度时在哪一步发现？**

<Answer>作者版本尝试让ahead走满k-1步。若当前ahead已经是尾节点却仍需前进，ahead->next为空，立即返回空，不会进入同步循环或解引用空指针。</Answer>

**问题 4：若链表可能有环，为什么不能直接运行本算法？**

<Answer>有环结构没有尾节点，倒数位置无定义，ahead的next也永远不为空，循环不会终止。应先由容器保证无环，或先独立判圈并拒绝有环输入。</Answer>
...
<Glossary>
  <GlossaryItem term="倒数序号">以尾节点为1并向链表头方向递增的位置编号。</GlossaryItem>
  <GlossaryItem term="快慢指针">在同一结构上按不同起点或速度维护关系的两个遍历位置。</GlossaryItem>
  <GlossaryItem term="固定间隔">ahead相对behind始终领先k-1条next边的关系。</GlossaryItem>
  <GlossaryItem term="防御式检查">在危险解引用或算术前主动拒绝非法输入的检查。</GlossaryItem>
  <GlossaryItem term="单遍扫描">不先完整统计再重启，而在同一前向过程中完成定位。</GlossaryItem>
</Glossary>

<Attribution
  name="《剑指Offer：名企面试官精讲典型编程题（第2版）》面试题22"
  href="https://github.com/zhedahht/CodingInterviewChinese2/blob/2d6eb6d9127d1ad728ba60bb25bc5a5e91e519ac/22_KthNodeFromEnd/KthNodeFromEnd.cpp"
  author="何海涛"
