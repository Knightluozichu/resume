...
officialUnitId: coi-25
---
import { MergeHeadChoiceDiagram, MergeRecursionInvariantMap, MergeTiePolicyDiagram } from "@/components/mdx/coding-interviews/diagrams/merge-sorted-lists";
...
这就是合并两个排序的链表的局部决策依据。选择较小头后，该节点成为<Term term="归并节点">归并节点</Term>；未选链保持不动，选中链前进一格，剩余问题仍是两条排序链的合并。
...
- **现象**：合并时创建新节点复制值，而不是重连原节点。
- **原因**：题目通常要求重连原节点（所有权不变），复制会破坏契约。
- **修法**：直接修改较小头节点的 next 指向递归/迭代结果，复用原节点。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：递归版忘写空链基准，或两条链同时为空时未处理。
- **原因**：递归必须有一个链表为空时直接返回另一条。
- **修法**：`if (!head1) return head2; if (!head2) return head1;` 作基准。
...
每条链自身升序，头节点是该链最小值；合并结果的最小剩余节点必是两链头中较小者，无需扫描全部节点。
...
任一链表为空时直接返回另一条链（空链合并结果即非空链）。两链均非空时取较小头，其 next 指向剩余合并结果。
...
严格小于版本取 list1 优先；也可用小于等于保持稳定性。两种策略都合法，需在接口契约中明确。
...
<Glossary>
  <GlossaryItem term="归并节点">每次合并被选中加入结果链的较小头节点。</GlossaryItem>
  <GlossaryItem term="相等策略">两链头值相等时的取舍规则（取左/取右/稳定）。</GlossaryItem>
  <GlossaryItem term="迭代">用循环维护当前指针，与递归等价但 O(1) 辅助空间。</GlossaryItem>
</Glossary>
