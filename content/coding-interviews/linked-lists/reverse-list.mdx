...
import { ReversePointerOrderDiagram, ReverseInvariantMap, ReverseHeadTailDiagram, ReverseOfficialCaseLab } from "@/components/mdx/coding-interviews/diagrams/reverse-list";
...
- 能解释previous、current、next分别保存已反转段、当前节点和剩余入口
- 能计算每轮改链后新头、当前位置与未处理后缀的变化
- 能实现作者记录原尾为新头的迭代反转版本
- 能分析更新顺序、递归栈、所有权、环和并发修改风险
- 能设计空链、单节点、多节点与双重反转的身份测试
...
把 previous 指向的部分称为<Term def="已经完成next反向、以previous为入口并最终以空结束的链表前缀。">已反转前缀</Term>，把 current 及其后面仍维持原方向的部分称为<Term def="尚未改写next、仍由current进入的原方向链表后缀。">未处理后缀</Term>。算法每轮从后缀头取一个节点，放到反转前缀头。
...
第一步叫<Term def="在覆盖current的next之前，把原后继地址保存到独立变量以维持剩余链表可达。">后继暂存</Term>。若省略，剩余节点并非立即释放，而是失去所有可达路径，最终造成逻辑丢失和内存泄漏。若先推进 current 再改链，又会失去需要修改的旧当前节点。

<ReverseInvariantMap />

每轮结束时，previous前缀已反转且尾部指向空，current仍是未处理后缀入口。两个部分覆盖原链表全部节点且互不重叠。current最终为空时，未处理后缀为空，previous就覆盖全部节点。

<Callout type="trap" title="先重写current->next再读取后继">
单向节点只有一条前向链接。覆盖后再从current读取next得到的是previous，而不是原后继；循环可能在已反转前缀中打转，原后缀永久丢失。必须先保存下一个节点。
</Callout>

## 忠实实现作者的迭代版本

作者维护 pReversedHead、pNode、pPrev。每轮先取得 pNext；当 pNext 为空时，当前节点是原尾，作者把它记录为<Term def="反转完成后用于进入整条链表的节点，等于原链表尾节点。">新头</Term>。随后再反向链接并推进。
...
这种操作称为<Term def="复用原节点存储、仅修改链接完成变换，不为节点序列分配同规模辅助空间。">原地反转</Term>。所有节点地址和value保持不变，但由head可达的顺序改变。外部持有某节点指针仍指向同一对象，不过它看到的next已经不同。
...
<Callout type="trap" title="忽略返回值继续使用旧head">
反转后旧head已是尾节点，next为空。函数内部结构完全正确，但调用者若不保存新头，会只看到一个节点并可能泄漏其余节点。接口调用必须写成head=reverseList(head)。
</Callout>
...
<Stepper>
  <Step title="先保存后继再改链接"><ReversePointerOrderDiagram />next保住剩余链表入口，current才能安全指向previous。</Step>
  <Step title="维护前缀与后缀不变量"><ReverseInvariantMap />previous段已反转，current段保持原方向；每轮只迁移一个节点。</Step>
  <Step title="核对新头和新尾身份"><ReverseHeadTailDiagram />原尾next为空时被记录为新头，原头最先指向空成为新尾。</Step>
  <Step title="用三类输入与双反转封口"><ReverseOfficialCaseLab />多节点、单节点、空链和身份恢复共同检查完整性。</Step>
</Stepper>
...
**问题 1：为什么必须先保存current的next？**

<Answer>current->next是未处理后缀的唯一入口。重写为previous后，若没有独立next变量，就无法找到原后继，后面所有节点会失去可达路径；先保存才能在改链后继续遍历。</Answer>

**问题 2：循环结束为什么previous就是新头？**

<Answer>每轮把current接到previous前面，再让previous指向current。最后一个处理的是原尾，它成为整个已反转前缀最前端；current随后为空，所以previous覆盖全部节点并指向新头。</Answer>

**问题 3：递归版本为什么必须把head->next设为空？**

<Answer>回溯时先建立原后继指回head的反向边，原来的head指向后继仍存在；若不切断，两节点互相指向形成环。设为空也让最外层原头成为新尾。</Answer>

**问题 4：怎样测试实现没有只交换value？**

<Answer>构造时保存每个节点地址，反转后按预期逆序逐个比较地址，并确认value仍附着在原对象。只比较值序列无法区分反向链接和交换节点内容。</Answer>
...
<Glossary>
  <GlossaryItem term="已反转前缀">已经完成next反向、以previous为入口并以空结束的节点段。</GlossaryItem>
  <GlossaryItem term="未处理后缀">仍保持原next方向、由current进入的剩余节点段。</GlossaryItem>
  <GlossaryItem term="后继暂存">覆盖current的next前先保存原后继地址的操作。</GlossaryItem>
  <GlossaryItem term="新头">反转后整条链表的入口节点，等于原链表尾节点。</GlossaryItem>
  <GlossaryItem term="原地反转">不创建同规模节点副本，只修改原节点链接完成逆序。</GlossaryItem>
</Glossary>

<Attribution
  name="《剑指Offer：名企面试官精讲典型编程题（第2版）》面试题24"
  href="https://github.com/zhedahht/CodingInterviewChinese2/blob/2d6eb6d9127d1ad728ba60bb25bc5a5e91e519ac/24_ReverseList/ReverseList.cpp"
  author="何海涛"
