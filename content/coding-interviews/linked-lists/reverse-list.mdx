...
所以反转链表不是把箭头“一起翻面”，而是逐节点迁移。每轮都要先保存下一个节点，再把当前next反向，最后推进处理位置。原书核心短语“<Term term="三指针">前驱、当前与后继指针</Term>”对应三个职责，顺序不能交换。
...
- **现象**：先执行 `current->next = previous`，没保存 next 就丢失整个后缀。
- **原因**：改变 next 前必须暂存原后继，否则后半段链表不可达。
- **修法**：严格顺序：`next = current->next` → `current->next = previous` → `previous = current` → `current = next`。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：反转完成后返回原来的头节点，而不是新的头（原尾节点）。
- **原因**：循环结束后 previous 指向新头，current 指向 null。
- **修法**：返回 `previous`；空链表与单节点时 previous 分别返回 null 与原节点。
...
## 正确性来自前缀与后缀<Term term="不变量">不变量</Term>
...
`current->next = previous` 会覆盖指向后继的唯一链接。若未先保存 next，2 及之后的节点全部不可达，链表被截断。
...
保存 next → 反向链接 → 推进 previous → 推进 current。交换任何一步都会导致丢失后缀或指针错位，破坏"前驱、当前、后继"的职责。
...
返回 previous（原尾节点，反转后的新头）。空链表 previous 为 null 返回 null；单节点 previous 为原节点返回原节点。
...
<Glossary>
  <GlossaryItem term="三指针">前驱、当前、后继三个指针，各司保存/链接/推进之职，顺序不可交换。</GlossaryItem>
  <GlossaryItem term="不变量">算法执行中保持的性质：已处理前缀已反转、未处理后缀保持原序。</GlossaryItem>
  <GlossaryItem term="递归">用递归栈保存反转上下文，与迭代版功能等价但可能栈溢出。</GlossaryItem>
</Glossary>
