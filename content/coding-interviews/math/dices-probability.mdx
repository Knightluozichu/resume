...
题目默认每颗骰子公平、彼此独立。n颗骰子的<Term term="样本空间">样本空间</Term>共有6的n次方个结果；点数和s最小为n，最大为6n，因此只会访问5n+1个和状态。
...
- **现象**：把点数和的结果数当成等概率，认为 2~12 共 11 种等概率。
- **原因**：每种和对应的底层有序结果数不同。
- **修法**：按 6^n 个等可能结果计数，统计每个和的出现次数再除以总数。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：递归枚举时未剪枝，n=6 时 6^6=46656 条路径仍可枚举，但 n 再大就爆炸。
- **原因**：递归无剪枝时指数增长。
- **修法**：n 较大时用 DP 交替数组，O(n²) 时间。

</Callout>

## 方法一：<Term term="递归枚举">递归枚举</Term>每条投掷路径
...
## 方法二：把上一轮分布<Term term="卷积">卷积</Term>一次
...
和 7 有 6 种有序结果 (1,6)~(6,1)，和 2 只有 1 种 (1,1)。概率按底层等可能结果计数，不是按和均分。
...
递归枚举适合 n 较小（≤6），代码直观；DP 交替数组适合 n 较大，O(n²) 线性增长。
...
概率和为 1、分布对称（均值处最高）、点数范围 n~6n 共 5n+1 个值。
...
<Glossary>
  <GlossaryItem term="样本空间">所有可能结果的集合，n 颗骰子共 6^n 种。</GlossaryItem>
  <GlossaryItem term="递归枚举">遍历所有投掷序列，统计每个和出现的次数。</GlossaryItem>
  <GlossaryItem term="卷积">把上一轮分布与当前骰子 1-6 均匀分布叠加。</GlossaryItem>
</Glossary>
