...
officialUnitId: coi-43
---
import { NumberOf1RecursiveDecompositionDiagram, NumberOf1HighestDigitDiagram, NumberOf1ComplexityDiagram } from "@/components/mdx/coding-interviews/diagrams/number-of-1";
...
题目“**1到n整数中1出现的次数**”统计十进制表示里的字符1，不是二进制置位数。作者先给出逐数基线，再把n写成字符串，做<Term term="递归分解">递归分解</Term>。
...
- **现象**：把数字 1 的个数和二进制 1 的个数混淆。
- **原因**：题目要求十进制字符 '1' 的计数，不是二进制置位数。
- **修法**：明确按十进制位统计，每位独立计算 1 出现次数。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：其余位统计时，最高位不参与循环，漏算高位贡献。
- **原因**：三个部分必须分别统计再求和。
- **修法**：最高位贡献 + 其余位均匀分布 + 后缀递归，三部分不重不漏。
...
## <Term term="最高位">最高位</Term>贡献的三种情况
...
## <Term term="均匀分布">其余位置</Term>为何均匀出现
...
最高位贡献（最高位 >1 时全额计算，=1 时计后缀+1）、其余位均匀分布（固定位置数字为 1 的次数）、后缀递归（去掉最高位后的子问题）。
...
最高位确定后，其余位在完整区间内每个数字出现的次数相同，与最高位值无关，可直接公式计算。
...
cur=0 时高位贡献 high×位权；cur=1 时加 low+1；cur>1 时加 (high+1)×位权。三种情况统一了递归分解的数学表达式。
...
<Glossary>
  <GlossaryItem term="递归分解">把 n 按十进制最高位拆成三部分递归统计。</GlossaryItem>
  <GlossaryItem term="最高位">十进制表示中最左边的数字，决定区间划分。</GlossaryItem>
  <GlossaryItem term="均匀分布">其余位在完整区间内各数字出现的对称性。</GlossaryItem>
</Glossary>
