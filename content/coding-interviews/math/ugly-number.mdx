...
题目表述为“<Term term="丑数">只包含因子2、3和5</Term>”，准确含义是除1外没有其他<Term term="质因子">质因子</Term>，而不是要求2、3、5必须全部出现。2、9、25都合法；7、14、21含其他质因子，不合法。习惯上把1作为第一个<Term term="丑数">丑数</Term>。
...
反过来，已有丑数乘2、3或5不会引入其他质因子，结果仍是丑数。于是所有待生成值来自三条<Term term="候选流">递增</Term>：
...
## <Term term="候选流">三个指针</Term>各自指向什么
...
- **现象**：判断丑数时只检查能否被 2、3、5 中任一整除，把 14 误判为丑数。
- **原因**：要求是"除 1 外无其他质因子"，不是"含有 2、3、5 中某因子"。
- **修法**：反复除以 2、3、5 直到除尽，看最终是否剩 1。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：算法 2 中推进指针时只推进最小的那个，并列最小值时只推一个。
- **原因**：并列候选会产生重复丑数，破坏"不重复"性质。
- **修法**：所有产生当前最小值的指针必须同时推进。
...
## 为什么必须推进所有<Term term="并列指针">并列指针</Term>
...
<Step title="三条候选流合并">
已有丑数乘 2、3、5 仍是丑数，三流各有一指针取最小合并，线性生成。
...
<Step title="复杂度对比">
算法 1 每数 O(log n) 判断，算法 2 线性生成；按目标规模选择。
...
14 能被 2 整除但除完剩 7，含其他质因子。丑数要求除 1 外无其他质因子；反复除以 2、3、5 后剩 1 才是丑数。
...
三个指针分别指向"下一个乘 2/3/5 的丑数"候选流。并列最小值时若只推进一个，同一个丑数会被重复加入数组，破坏不重复性质，也影响后续有序性。
...
每个丑数恰好被三个指针各扫描一次（O(1) 次操作），n 个丑数总时间 O(n)，空间 O(n)（存储数组）。
...
<Glossary>
  <GlossaryItem term="丑数">除 1 外只含 2、3、5 质因子的正整数，如 2、9、25。</GlossaryItem>
  <GlossaryItem term="候选流">算法 2 中由"已生成丑数 × 因子"构成的有序候选序列。</GlossaryItem>
  <GlossaryItem term="并列指针">产生同一最小候选值的多个指针，必须同时推进以避免重复。</GlossaryItem>
  <GlossaryItem term="质因子">一个数的质数因子，丑数的质因子只能是 2、3、5。</GlossaryItem>
  <GlossaryItem term="候选流">由"已生成丑数 × 因子"构成的递增候选序列，三流取最小合并。</GlossaryItem>
</Glossary>
