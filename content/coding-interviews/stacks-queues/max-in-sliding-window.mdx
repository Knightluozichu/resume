...
import { MaxSlidingWindowCandidateDiagram, MaxSlidingWindowDequeLab, MaxSlidingWindowExpiryMap, MaxSlidingWindowContractDiagram, MaxSlidingWindowOfficialCaseLab } from "@/components/mdx/coding-interviews/diagrams/max-in-sliding-window";
...
- 能解释较小且更早的元素为何可从候选集合永久删除
- 能计算作者示例中每一步队列下标、过期边界和输出
- 能实现作者首窗特化版与统一单循环版
- 能分析相等值、窗口边界、索引类型和真实队列操作成本
- 能设计覆盖作者9组用例并与朴素窗口扫描对拍的测试
...
窗口右移时，大部分元素仍然保留。真正需要跟踪的不是全部窗口成员，而是仍有机会成为当前或未来最大值的<Term def="保存在双端队列中、仍可能成为某个窗口最大值的数组位置。">候选下标</Term>。

<MaxSlidingWindowCandidateDiagram />

作者用**双端队列**保存这些下标。下标从队首到队尾递增，对应数组值则<Term def="双端队列从队首到队尾对应的数组值严格下降，队首值最大。">值单调递减</Term>，所以队首下标对应当前**滑动窗口的最大值**。
...
旧队尾此后不可能优先成为最大值，可以永久弹出。持续删除直到队尾值严格大于新值，再把i压入。这称为<Term def="新值大于等于队尾候选时，持续从队尾删除被支配下标。">队尾淘汰</Term>。

过期是另一回事。新窗口左边界为i减size加1，任何下标不大于i减size都已离窗。若这样的下标位于队首，必须从前端删除，称为<Term def="队首候选下标已经小于新窗口左边界时，从前端移除。">队首过期</Term>。
...
<Callout type="trap" title="队列只保存值">
仅凭值可以维护递减顺序，却不知道队首对应元素何时离开窗口。重复值出现时更无法区分新旧副本。保存下标后既能查num[index]比较值，也能用index判断过期。
</Callout>
...
<Callout type="trap" title="在作者循环中先加入新值再输出上一窗口">
新值属于下一个窗口，若提前参与候选会污染上一窗口结果。作者先输出旧窗；统一模板则先更新新窗并只在新窗完成后输出，两种顺序各自闭合。
</Callout>
...
这种把偶尔昂贵操作分摊到所有元素上的证明称为<Term def="统计每个下标全程至多入队一次、出队一次，从总操作数推出线性复杂度。">摊还分析</Term>。总时间O(n)，队列空间O(size)。
...
<MaxSlidingWindowOfficialCaseLab />
...
<Stepper>
  <Step title="只保留可能接任最大值的下标">
    <MaxSlidingWindowCandidateDiagram />
    队首是当前最大值，后续候选按值递减。
  </Step>
  <Step title="区分队尾淘汰与队首过期">
    <MaxSlidingWindowExpiryMap />
    一个处理值支配，一个处理窗口时间边界。
  </Step>
  <Step title="复盘书中8个输入的队列">
    <MaxSlidingWindowDequeLab />
    每个新下标都可能淘汰队尾或促使队首过期。
  </Step>
  <Step title="核对窗口与复杂度契约">
    <MaxSlidingWindowContractDiagram />
    合法size、下标类型和摊还证明共同保证结果。
  </Step>
  <Step title="执行作者9组测试">
    <MaxSlidingWindowOfficialCaseLab />
    综合、单双调、边界窗口与无效输入全部覆盖。
  </Step>
</Stepper>
...
**问题 1：新值等于队尾值时为何作者删除旧下标？**

<Answer>两者值相同，新下标出现更晚、过期也更晚，在共同窗口中不比旧值差；旧下标永远不会优先成为最大值，因此可安全淘汰。</Answer>

**问题 2：为什么队列只保存值会出错？**

<Answer>值无法说明对应元素是否已离开窗口，重复值也无法区分新旧副本。下标同时支持按值比较与按位置过期。</Answer>

**问题 3：一次新值可能弹出很多元素，为何总时间仍是O(n)？**

<Answer>每个下标只入队一次，弹出后不再返回。全程所有队尾淘汰和队首过期总数至多n，偶尔昂贵的一轮被分摊到所有下标。</Answer>

**问题 4：作者为何在主循环前先输出队首？**

<Answer>循环下标i代表即将进入下一个窗口的新元素，当前队列仍对应上一个完整窗口，必须先输出。若先加入i，上一窗口会被不属于它的新值污染。</Answer>
...
<Glossary>
  <GlossaryItem term="候选下标">仍可能成为当前或未来窗口最大值的数组位置。</GlossaryItem>
  <GlossaryItem term="值单调递减">队首到队尾对应数组值严格下降的不变式。</GlossaryItem>
  <GlossaryItem term="队尾淘汰">新值支配旧候选时从后端持续删除。</GlossaryItem>
  <GlossaryItem term="队首过期">最早候选离开窗口时从前端删除。</GlossaryItem>
  <GlossaryItem term="摊还分析">按每个下标全生命周期统计总队列操作次数。</GlossaryItem>
</Glossary>

<Attribution
  name="《剑指Offer：名企面试官精讲典型编程题（第2版）》面试题59（一）"
  href="https://github.com/zhedahht/CodingInterviewChinese2/blob/2d6eb6d9127d1ad728ba60bb25bc5a5e91e519ac/59_01_MaxInSlidingWindow/MaxInSlidingWindow.cpp"
  author="何海涛"
