...
officialUnitId: coi-59-02
---

import { QueueWithMaxDiagram } from "@/components/mdx/coding-interviews/diagrams/queue-with-max";
...
## 从“队首不是<Term term="最大值">最大值</Term>”开始

普通<Term term="队列">队列</Term>只能O(1)读取队首，无法直接知道所有剩余元素中的最大值。每次max扫描整队需要O(n)；每次入队后重新排序又破坏FIFO或增加维护成本。
...
源码注释误复制了上一题“<Term term="滑动窗口最大值">滑动窗口最大值</Term>”的题面，但实际类名、操作与测试都是“队列的最大值”。以下按可执行类还原。
...
这个不变式与上一题固定窗口<Term term="单调队列">单调队列</Term>相同，只是“过期”不再由下标和窗口大小计算，而由显式pop_front操作触发。
...
## 作者（<Term term="边界条件">边界条件</Term>）14次最大值检查
