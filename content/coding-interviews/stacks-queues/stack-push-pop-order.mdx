...
officialUnitId: coi-31
---
import { StackSequenceSimulationDiagram, StackTargetDecisionMap, StackSequenceCounterexampleDiagram } from "@/components/mdx/coding-interviews/diagrams/stack-push-pop-order";
...
“<Term term="栈序列">栈的压入、弹出序列</Term>”问题考查的不是集合成员，而是后进先出约束。给定互不相同的压入数字后，判断第二个序列能否由某组合法push/pop交错产生。作者不枚举所有交错方式，而是使用逐个满足目标弹出值。
...
- **现象**：只检查元素集合相同就判定序列合法。
- **原因**：栈的 LIFO 约束还会限制弹出顺序。
- **修法**：必须模拟 push/pop 操作，不能只比较集合。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：输入耗尽后辅助栈还有元素，返回 true。
- **原因**：序列不合法时辅助栈最终非空。
- **修法**：循环结束后检查辅助栈是否为空，非空则返回 false。

</Callout>

## 目标驱动的<Term term="辅助栈">辅助栈</Term>模拟
...
## <Term term="合法序列">合法序列</Term>怎样被逐步消费
...
## 为什么匹配时立即<Term term="弹出">弹出</Term>是安全的
...
## <Term term="容器版本">长度安全的现代容器版本</Term>
...
## <Term term="输入验证">输入验证</Term>、重复值与工程扩展
...
集合相同只保证数字相同，不保证 LIFO 顺序约束。必须模拟每个 push/pop 操作。
...
遍历弹出序列，辅助栈顶匹配时弹出，不匹配时继续压入直到匹配或耗尽；最终辅助栈为空才合法。
...
当前栈顶就是目标值，留在栈中只会阻塞后续元素。立即弹出不会影响后续弹出序列的可行性。
...
<Glossary>
  <GlossaryItem term="栈序列">给定 push 顺序后，合法 pop 序列的判断问题。</GlossaryItem>
  <GlossaryItem term="辅助栈">用于模拟压入弹出过程的栈结构。</GlossaryItem>
  <GlossaryItem term="弹出">栈顶元素被移除的操作。</GlossaryItem>
  <GlossaryItem term="合法序列">满足 LIFO 约束的栈弹出序列。</GlossaryItem>
  <GlossaryItem term="容器版本">用标准容器实现的现代 C++ 版本。</GlossaryItem>
  <GlossaryItem term="输入验证">检查数组长度、空指针等前置条件。</GlossaryItem>
</Glossary>
