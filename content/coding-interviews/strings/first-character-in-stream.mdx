...
officialUnitId: coi-50-02
---
import { OccurrenceSentinelDiagram, FirstPositionScanMap, StaticAndStreamUniqueDiagram } from "@/components/mdx/coding-interviews/diagrams/first-character-in-stream";
...
题目要求随时回答“**字符流中第一个只出现一次的字符**”。与完整静态串不同，流每到一个字符就可能让旧答案永久失效，也可能增加新候选。算法需要持久保存每个字符的<Term term="唯一性">唯一性</Term>和<Term term="到达顺序">到达顺序</Term>。
...
- **现象**：重复字符出现后仍被当作候选，忘记标记为重复。
- **原因**：首次出现后再次出现时，该字符不再是唯一候选。
- **修法**：第二次出现时把位置标记为 -1（重复），不再参与扫描。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：查询时扫描整个 256 数组，误以为 O(1) 但实际 O(256)。
- **原因**：256 是常数，O(256) = O(1) 在理论正确，但可优化。
- **修法**：维护一个最小位置候选队列，查询时直接取队首。
...
## <Term term="三态">三态</Term>为何足以处理只增不删的流
...
## 作者查询：扫描 256 槽找<Term term="最小位置">最小位置</Term>
...
-1（未出现）、≥0（首次出现位置）、-2（重复）。三态足以跟踪每个字符的唯一性状态。
...
扫描 256 槽，找出所有 ≥0 的位置的最小值，对应字符即答案；若全部小于 0 则返回无解。
...
流式无法回头，每个字符到达后旧答案可能永久失效；静态串可扫两遍。流式需持久保存每个字符的状态与到达顺序。
...
<Glossary>
  <GlossaryItem term="唯一性">字符在流中只出现一次的性质。</GlossaryItem>
  <GlossaryItem term="到达顺序">字符首次出现的流位置序号。</GlossaryItem>
  <GlossaryItem term="三态">未出现、首次位置、重复三种状态。</GlossaryItem>
  <GlossaryItem term="最小位置">唯一字符中最早到达的流位置。</GlossaryItem>
</Glossary>
