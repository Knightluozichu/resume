...
officialUnitId: coi-58-02
---
import { LeftRotatePartitionDiagram, LeftRotateContractMap, LeftRotateProofDiagram } from "@/components/mdx/coding-interviews/diagrams/left-rotate-string";
...
把前n个字符记作<Term term="分段">A</Term>，其余字符记作B，原串就是AB，目标就是BA。问题因此从“旋转每个字符”转化为“交换两个连续分段”，而作者复用了上一小题的Reverse函数，只用三次翻转完成。
...
- **现象**：直接移动每个字符到新位置，忘记段内顺序会被破坏。
- **原因**：把 AB 变 BA 若逐个搬移，A 段内部顺序会翻转。
- **修法**：先翻转 A、再翻转 B、最后整体翻转——三次翻转恰好恢复段内顺序。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：旋转位数 n 大于串长时直接越界或结果错误。
- **原因**：左旋 n 位与左旋 n mod len 位等价。
- **修法**：入口把 n 对 len 取模（len>0），或按严格合同拒绝 n 越界。

</Callout>

## <Term term="三次翻转">三次翻转</Term>的作者实现
...
## 现代严格版与<Term term="循环归一化">循环归一化</Term>版
...
AB = "ab"+"cdefg"：翻转 A 得 "ba"，翻转 B 得 "gfedc"，整体翻转 "bagfedc" 得 "cdefgab"。三次翻转把 AB 变 BA 且恢复段内顺序。
...
前两次翻转把 A、B 段内顺序反转，整体翻转把整串反转——两次反转复合后段内顺序恢复，同时两段位置互换。
...
左旋 n 位等价于左旋 n mod len 位（len>0）。可先取模再执行，或按合同拒绝 n 越界的输入。
...
<Glossary>
  <GlossaryItem term="分段">把字符串切成前 n 个字符 A 与剩余字符 B，问题转化为交换 AB 为 BA。</GlossaryItem>
  <GlossaryItem term="三次翻转">分别翻转 A、B 再整体翻转，O(1) 额外空间的原地换位方法。</GlossaryItem>
  <GlossaryItem term="循环归一化">左旋 n 位等价于左旋 n mod len 位，避免越界。</GlossaryItem>
</Glossary>
