...
officialUnitId: coi-48
---
import { DuplicateDecisionDiagram, BruteForceSearchMap, LowercaseAlphabetContractDiagram } from "@/components/mdx/coding-interviews/diagrams/longest-substring-without-dup";
...
扫描到某个位置i时，先不追问全局答案，只问“以i结尾的最长无重复连续后缀有多长”。这个量称为<Term term="最长无重复后缀">最长无重复后缀</Term>。它能由前一个状态和当前字符的历史位置在线更新。
...
- **现象**：把"子字符串"当成"子序列"，挑选不连续的不同字符。
- **原因**：子字符串必须连续，子序列允许跳过。
- **修法**：明确题目要求连续区间；`acfr` 是子字符串、`abcfr` 是子序列。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：用 256 数组记录所有字符，忽略题目约定的字符集（如仅小写字母）。
- **原因**：字符集契约决定数组大小与初始化方式。
- **修法**：先确认字符集；数组按约定大小初始化，避免访问越界。
...
## 作者第二解：以结束位置做<Term term="动态规划">动态规划</Term>
...
## <Term term="滑动窗口">动态规划或滑动窗口</Term>其实是同一状态
...
子字符串必须是原串连续区间，子序列可跳过中间字符。题目要求连续区间，因为"不含重复字符"在连续场景下才有滑动窗口/DP 的递推结构。
...
用字符上次出现位置记录历史。新字符未在窗口内出现时，长度 +1；出现在窗口内时，窗口收缩到上次出现位置之后，长度更新为当前间距。
...
两者都维护"当前无重复窗口"：DP 显式记录以 i 结尾的后缀长度，滑动窗口隐式用左右指针表达同一区间。状态更新规则相同，复杂度同为 O(n)。
...
<Glossary>
  <GlossaryItem term="最长无重复后缀">以当前位置结尾、不含重复字符的最长连续子串长度。</GlossaryItem>
  <GlossaryItem term="动态规划">用前一状态与历史位置递推当前状态，O(n) 求解。</GlossaryItem>
  <GlossaryItem term="滑动窗口">用左右指针维护无重复窗口，与 DP 状态等价。</GlossaryItem>
</Glossary>
