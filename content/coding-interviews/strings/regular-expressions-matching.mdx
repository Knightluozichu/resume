...
本题的“正则表达式匹配”不是在字符串里寻找一个可匹配子串，而是要求模式覆盖字符串的**全部字符**。模式 .a 只能消费两个字符，字符串还剩一个 a，因此失败。只有字符串和模式同时到达末尾才成功，这种契约称为<Term term="完整匹配">完整匹配</Term>。
...
- **现象**：用"子串匹配"代替"完整匹配"，认为模式 .a 能匹配 aaa。
- **原因**：题目要求模式覆盖全部字符。
- **修法**：字符串和模式必须同时到达末尾才返回 true。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：星号只考虑匹配多次，忽略匹配零次的分支。
- **原因**：星号允许前一字符出现零次或多次。
- **修法**：递归两个分支——匹配零次（跳过模式两位）和匹配多次（消费一个字符）。

</Callout>

## <Term term="星号">星号</Term>把一个状态拆成三种选择
...
## 两个<Term term="终止条件">终止条件</Term>决定完整匹配
...
完整匹配要求模式覆盖字符串的全部字符，两者同时到达末尾。子串匹配只要求模式出现在字符串中任意位置。
...
匹配零次：跳过模式中 x* 两个字符，字符串不动；匹配多次：消费字符串一个字符（如果匹配），模式不变。
...
使用备忘录（memoization）记录 (strIdx, patIdx) 的结果，避免重复递归。
...
<Glossary>
  <GlossaryItem term="完整匹配">模式覆盖字符串全部字符，两者同时到达末尾。</GlossaryItem>
  <GlossaryItem term="星号">前一字符出现零次或多次的通配符。</GlossaryItem>
  <GlossaryItem term="终止条件">字符串和模式为空或星号的分支结束条件。</GlossaryItem>
</Glossary>
