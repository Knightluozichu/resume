...
题目给出英文句子，要求“<Term term="翻转单词顺序">翻转单词顺序</Term>”，但每个单词内部字符顺序保持不变。输入“I am a student.”应变成“student. a am I”，<Term term="标点">标点</Term>按普通字母处理，仍跟着student这个单词。
...
先预测三个<Term term="边界">边界</Term>：“Wonderful”只有一个单词，会翻转两次回到原样；三个空格没有单词，仍是三个空格；“I  am”含两个连续空格，输出是“am  I”，作者不会压缩分隔符。

<Callout type="trap" title="常见误区">

- **现象**：只整体翻转句子，单词内部字符也被写反。
- **原因**：整段首尾交换会同时反转单词内字符。
- **修法**：整体翻转后再对每个单词单独翻转一次，恢复单词内部顺序。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：多个连续空格或两端空格时，单词边界判定出错。
- **原因**：用"空格切换"找单词会误判空单词。
- **修法**：只对非空格区间做局部翻转，连续空格原样保留。

</Callout>

## <Term term="整体翻转">整体翻转</Term>与局部翻转的组合
...
第一步整体翻转整个句子（单词位置反转、单词内字符也反转）；第二步对每个空格分隔的单词再翻转一次（单词内字符恢复、位置不变）。两次作用域不同的反转恰好抵消。
...
"student. a am I"。标点按普通字母处理，仍跟随 student 这个单词。
...
<Glossary>
  <GlossaryItem term="翻转单词顺序">保持单词内字符不变，只反转单词的排列顺序。</GlossaryItem>
  <GlossaryItem term="整体翻转">把整段字符首尾交换，改变单词位置。</GlossaryItem>
  <GlossaryItem term="单词">空格分隔的连续非空格字符段。</GlossaryItem>
  <GlossaryItem term="标点">句子中的标点符号，本题按普通字母随单词处理。</GlossaryItem>
  <GlossaryItem term="边界">连续空格、两端空格、单单词、空串等特殊输入。</GlossaryItem>
  <GlossaryItem term="抵消">整体翻转与局部翻转作用域不同，两次反转相互抵消。</GlossaryItem>
</Glossary>
