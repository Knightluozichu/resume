...
import { TranslationMappingDiagram, TranslationCountLab, TranslationPathsDiagram, TranslationContractDiagram, TranslateNumbersOfficialCaseLab } from "@/components/mdx/coding-interviews/diagrams/translate-numbers-to-strings";
...
- 能解释本题0到25映射与常见1到26解码的差异
- 能计算12258从右向左每个后缀的翻译方法数
- 能实现作者数组动态规划和常数空间等价版本
- 能分析0、01、10、25、26、负数与空字符串边界
- 能设计覆盖作者九组用例和枚举路径参考的测试
...
题目“**把数字翻译成字符串**”规定“**0到25对应字母**”。每个单独数字字符0到9都能做<Term def="消费当前位置一个数字字符，并按0到9映射为a到j。">单字符翻译</Term>；相邻两个字符只有组成10到25时，才能做<Term def="同时消费相邻两个字符，且其数值必须位于10到25闭区间。">双字符翻译</Term>。

<TranslationMappingDiagram />
...
<TranslationContractDiagram />

<Callout type="trap" title="看到字符0就把当前状态清零">
本题0本身对应a，单字符路径始终合法。清零会让0、100和101全部少算；需要拒绝的是00、01等小于10的双字符组合。
</Callout>
...
令counts[i]表示从下标i开始的后缀有多少种翻译。先取一位时，后续选择数就是counts[i+1]；若两位数合法，再加counts[i+2]。这就是<Term def="从某个下标开始直到末尾的全部合法翻译路径数量。">后缀方法数</Term>。

作者从右向左计算，因为当前位置依赖右侧一个或两个状态。最后一个字符只能单独翻译，所以counts[last]为1。倒数第二位若两字符有效，在单字符路径之外再加一条直接消费完全部字符的路径。

概念上可在字符串末尾放一个值为1的<Term def="所有输入字符都已消费时的一种完成方式，用作两位转移越过末尾的基例。">虚拟空后缀</Term>，统一写成：
...
这五个计数对应五条真实的<Term def="把整个数字串切分为若干1位或合法2位编码，并逐段映射为字母的完整方案。">翻译路径</Term>，不是抽象数字：

<TranslationPathsDiagram />
...
<Callout type="trap" title="把上界写成26">
作者从0开始编号字母，z对应25，不是26。把26纳入会让126和426各多一条，直接违反Test4与Test5。
</Callout>
...
<TranslateNumbersOfficialCaseLab />
...
<Stepper>
  <Step title="先固定0到25映射"><TranslationMappingDiagram />单个0合法，两位编码只允许10到25。</Step>
  <Step title="定义每个后缀的方法数"><TranslationCountLab />从右向左把取一位和合法取两位的计数相加。</Step>
  <Step title="把计数对应到真实路径"><TranslationPathsDiagram />12258恰有bccfi、bwfi、bczi、mcfi、mzi五种。</Step>
  <Step title="逐项检查零与两位边界"><TranslationContractDiagram />区分0、01、10、25、26、负数和空辅助输入。</Step>
  <Step title="跑完九组作者测试"><TranslateNumbersOfficialCaseLab />覆盖重叠组合、连续零、复杂样例与无效负数。</Step>
</Stepper>
...
**问题 1：数字0为什么有一种而不是零种翻译？**

<Answer>作者映射从0开始，0对应a；整数0转成字符串"0"，最后一个字符的单字符路径计数为1。</Answer>

**问题 2：100为何有两种翻译？**

<Answer>可以切成1|0|0，每位单独翻译；也可切成10|0。00不在10到25之间，不能作为第三条双字符路径。</Answer>

**问题 3：12258在下标1的后缀2258为何有3种？**

<Answer>先取单个2后接258的2种；或取合法22后接58的1种，两类首段互斥，合计3。</Answer>

**问题 4：为什么506只有一种？**

<Answer>50超过25，06小于10，两个相邻组合都不能整体翻译；只能按5|0|6逐位翻译，且0本身合法。</Answer>
...
<Glossary>
  <GlossaryItem term="单字符翻译">消费一个数字字符并按0到9映射字母的路径。</GlossaryItem>
  <GlossaryItem term="双字符翻译">消费两个字符且组合值在10到25的路径。</GlossaryItem>
  <GlossaryItem term="后缀方法数">从某下标开始到末尾的合法翻译总数。</GlossaryItem>
  <GlossaryItem term="虚拟空后缀">字符全部消费完后的一种完成方式，用作递推基例。</GlossaryItem>
  <GlossaryItem term="翻译路径">覆盖整个数字串的一组合法一位或两位切分及其字母结果。</GlossaryItem>
</Glossary>

<Attribution
  name="《剑指Offer：名企面试官精讲典型编程题（第2版）》面试题46"
  href="https://github.com/zhedahht/CodingInterviewChinese2/blob/2d6eb6d9127d1ad728ba60bb25bc5a5e91e519ac/46_TranslateNumbersToStrings/TranslateNumbersToStrings.cpp"
  author="何海涛"
