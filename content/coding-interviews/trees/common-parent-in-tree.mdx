...
import { CommonParentDecisionMap, CommonParentTreeDiagram, CommonParentConventionDiagram } from "@/components/mdx/coding-interviews/diagrams/common-parent-in-tree";
...
这些答案会改变可用信息和最优路径。直接写常见的二叉树递归LCA，只覆盖了其中一种定义，也完全没有还原作者源码中的普通多叉树与路径列表。原章的核心面试能力是<Term term="澄清结构">澄清结构</Term>。
...
- **现象**：不问树的结构就写二叉树递归 LCA。
- **原因**：普通多叉树与 BST 的最优算法不同。
- **修法**：先澄清结构（BST/父指针/普通树），再选算法。
...
- **现象**：节点不在树中时，仍返回伪祖先。
- **原因**：未先验证目标节点存在性。
- **修法**：路径回溯前先验证节点在树中，不存在则返回空。

</Callout>

## 三种结构，三条解题路线

### <Term term="二叉搜索树">二叉搜索树</Term>利用大小关系
...
## <Term term="路径回溯">路径回溯</Term>如何只保留成功分支
...
树的结构（BST/父指针/普通树）、目标节点是否保证在树中、"祖先"是否允许自身。
...
保存根到两个节点的路径，再同步扫描两条路径，最后一个相同节点即 LCA。
...
路径回溯前先验证节点存在性；不存在则返回空或无效标志，避免返回伪祖先。
...
<Glossary>
  <GlossaryItem term="澄清结构">编写 LCA 算法前先确认树的具体类型与约束。</GlossaryItem>
  <GlossaryItem term="二叉搜索树">左子树值 < 根值 < 右子树值，可利用大小关系找 LCA。</GlossaryItem>
  <GlossaryItem term="路径回溯">保存根到节点的路径，逐层回溯找公共祖先。</GlossaryItem>
</Glossary>
