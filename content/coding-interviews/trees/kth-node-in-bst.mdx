...
import { KthNodeBstOrderDiagram, KthNodeTraversalLab, KthNodeCounterFlowDiagram, KthNodeContractMap, KthNodeOfficialCaseLab } from "@/components/mdx/coding-interviews/diagrams/kth-node-in-bst";
...
- 能解释二叉搜索树中序访问为何形成升序排名
- 能计算引用参数k在递归返回过程中的每次递减
- 能实现作者递归版、迭代版与真正第k大版本
- 能分析题面语义冲突、非法k、越界、树高和重复键约定
- 能设计覆盖作者5种树形28个断言并检查提前停止的测试
...
原因在于源码使用左子树、根、右子树的<Term def="先访问左子树，再访问根节点，最后访问右子树的深度优先遍历次序。">中序遍历</Term>。对二叉搜索树，这个次序得到5、6、7、8、9、10、11，作者从最小端开始计数。
...
二叉搜索树满足左子树键小于当前键、右子树键大于当前键。递归访问左子树会先输出所有较小键，随后输出当前键，再由右子树输出所有较大键。由归纳可知整棵树的中序结果是<Term def="二叉搜索树按中序访问得到的非降或严格升序节点排列。">有序节点序列</Term>。
...
这里k采用从1开始的<Term def="规定k=1代表序列第一个节点、k=2代表第二个节点的接口含义。">排名语义</Term>。k等于0不是第一个，而是非法输入。

## 为什么k必须按引用传递

每个递归调用都属于同一次中序序列扫描。左子树访问了多少节点，根和右子树必须接着使用剩余排名，而不是重新从原始k开始。

作者让核心函数接收unsigned int引用。这个跨层共享并持续递减的变量称为<Term def="由所有递归层共同读写、每访问一个有效节点就减少1的剩余排名状态。">共享计数器</Term>。

如果k按值传递，左子树内部的递减不会反馈给父调用。父节点仍以旧k判断，多个子树会各自从头计数，结果错误。另一种正确设计是让递归函数同时返回“是否命中”和“已访问数量”，但结构更复杂。

<KthNodeCounterFlowDiagram />

target也承担停止信号。左子树已经找到目标时，父层不再处理当前节点；当前节点命中后，不再进入右子树。这种<Term def="目标节点一旦找到，就沿调用栈直接回传并跳过其余尚未访问分支。">提前终止</Term>避免遍历剩余较大节点。
...
<Callout type="trap" title="把未找到误写成已经找到">
提前退出判断中的极性不能凭感觉写。answer为空表示还要继续搜索，answer非空才表示目标已经找到；应至少用一棵单节点树验证第一次调用不会直接退出。
</Callout>
...
<Callout type="trap" title="只看题面文字忽略测试方向">
若按“第k大”实现右、根、左，k=1会返回11，与作者TestA1期望5冲突。复刻仓库时应按左、根、右；若产品需求明确要第k大，则应改函数名、遍历方向和全部期望值。
</Callout>
...
<Stepper>
  <Step title="把BST展开成升序排名">
    <KthNodeBstOrderDiagram />
    左、根、右得到有序节点序列，k从1开始。
  </Step>
  <Step title="共享并递减剩余排名">
    <KthNodeCounterFlowDiagram />
    左子树消费的节点数必须通过引用反馈给父层。
  </Step>
  <Step title="手算k等于4的访问过程">
    <KthNodeTraversalLab />
    访问到8时命中，并停止9、10、11的遍历。
  </Step>
  <Step title="区分第k大和源码第k小">
    <KthNodeContractMap />
    作者断言按升序；真正第k大需要反向中序。
  </Step>
  <Step title="执行5种树形28个断言">
    <KthNodeOfficialCaseLab />
    平衡、双向单链、单节点与空树共同覆盖边界。
  </Step>
</Stepper>
...
**问题 1：示例树中KthNode(root, 4)为何返回8？**

<Answer>中序序列为5、6、7、8、9、10、11。k从1起算，第4个节点是8；共享计数器依次从4减到1，并在访问8时命中。</Answer>

**问题 2：为什么k必须按引用传给左右子树？**

<Answer>左子树已经访问的节点要从剩余排名中扣除，根和右子树必须继续使用同一状态。按值传递会让每层各自从原k计数，破坏全树统一排名。</Answer>

**问题 3：作者函数在k等于1时返回5还是11？若产品真要第1大呢？**

<Answer>作者函数按左、根、右返回最小值5。产品若要求第1大，应改用右、根、左并返回11，同时更新函数名、说明和测试期望。</Answer>

**问题 4：为什么复杂度不能笼统写成O(log n)？**

<Answer>普通节点没有子树大小，算法要实际访问前k个中序节点，时间O(h加k)；斜树高度和k都可能接近n。只有平衡且带排名元数据的顺序统计树才能把任意排名查询稳定到O(log n)。</Answer>
...
<Glossary>
  <GlossaryItem term="中序遍历">按左子树、根、右子树访问二叉树的次序。</GlossaryItem>
  <GlossaryItem term="有序节点序列">BST中序产生的按键值非降排列节点序列。</GlossaryItem>
  <GlossaryItem term="共享计数器">所有递归层共同递减的剩余排名k。</GlossaryItem>
  <GlossaryItem term="提前终止">命中目标后跳过尚未访问的节点并沿调用栈回传。</GlossaryItem>
  <GlossaryItem term="排名语义">k从1还是0起算，以及按节点还是不同键计数的接口约定。</GlossaryItem>
</Glossary>

<Attribution
  name="《剑指Offer：名企面试官精讲典型编程题（第2版）》面试题54"
  href="https://github.com/zhedahht/CodingInterviewChinese2/blob/2d6eb6d9127d1ad728ba60bb25bc5a5e91e519ac/54_KthNodeInBST/KthNodeInBST.cpp"
  author="何海涛"
