...
“**从上到下打印二叉树**”要求深度小的节点先输出；同一层还要从左到右。它就是<Term term="层序遍历">层序遍历</Term>。递归调用栈天然偏向深度优先，而能保存当前层尚未访问节点和下一层刚发现的节点。
...
- **现象**：用递归前序/中序/后序试图实现层序。
- **原因**：递归天然深度优先，无法保持层序。
- **修法**：用队列尾入头出，每层处理完再处理下一层。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：队列中同时混入多个层次，左右顺序混乱。
- **原因**：入队时未按左-右顺序。
- **修法**：每次出队一个节点，先入队其左孩子，再入队右孩子。

</Callout>

## <Term term="先进先出">先进先出</Term>怎样扩展局部左右顺序
...
## 忠实还原作者<Term term="双端队列">deque</Term>实现
...
递归调用栈天然深度优先，先深入左子树再回溯。层序需要广度优先，用队列保持每层节点顺序。
...
出队一个节点，先入队左孩子，再入队右孩子。保证同层从左到右、层间从上到下。
...
<Glossary>
  <GlossaryItem term="层序遍历">从上到下、从左到右的二叉树遍历。</GlossaryItem>
  <GlossaryItem term="先进先出">队列的特性，先入队先出队。</GlossaryItem>
  <GlossaryItem term="双端队列">两端都可插入删除的队列，用于层序。</GlossaryItem>
</Glossary>
