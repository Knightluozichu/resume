...
作者用“<Term term="双栈">两个栈</Term>”。一个负责本层弹出，另一个接收孩子。栈的后进先出会反转压入顺序，因此当前层必须根据下一层期望方向决定先压哪个孩子。
...
- **现象**：用单个栈或队列，然后事后反转整行结果。
- **原因**：队列无法自然产生反向输出。
- **修法**：用两个栈轮换，栈的后进先出自动反转压入顺序，相邻层自然交替方向。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：奇数层和偶数层孩子压栈顺序弄反，导致方向错乱。
- **原因**：奇数层从左到右打印时，孩子应先右后左入栈；偶数层反之。
- **修法**：当前层为奇数：先左后右入栈；当前层为偶数：先右后左入栈。

</Callout>

## <Term term="奇偶层">奇偶层</Term>的孩子压栈顺序
...
## 为什么<Term term="双栈">双栈</Term>能保持跨父节点顺序
...
栈的后进先出自然反转压入顺序，相邻层交替弹出方向。一个队列无法产生反向输出，需事后反转整行。
...
奇数层（从左到右打印）：先压左孩子再压右孩子；偶数层（从右到左打印）：先压右孩子再压左孩子。
...
一个栈负责当前层弹出，另一栈接收下一层孩子。当前层处理完时交换角色，两个栈交替使用。
...
<Glossary>
  <GlossaryItem term="双栈">两个栈轮换负责当前层弹出与下一层压入。</GlossaryItem>
  <GlossaryItem term="奇偶层">奇数层从左到右，偶数层从右到左的交替方向。</GlossaryItem>
  <GlossaryItem term="双栈">两个栈在层间角色互换的机制。</GlossaryItem>
</Glossary>
