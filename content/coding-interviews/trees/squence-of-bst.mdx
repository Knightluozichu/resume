...
这里的<Term term="递归验证子序列">递归验证子序列</Term>，是指每次确定当前根和左右边界后，把两个更短的连续片段交给同一规则验证；只有左右片段都通过，当前序列才算合法，不能只检查顶层分割。
...
<Glossary>
  ...
  <GlossaryItem term="递归验证子序列">对左右连续片段重复检查根位置与取值范围，直到每个片段都满足后序规则。</GlossaryItem>
