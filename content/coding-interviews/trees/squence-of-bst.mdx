...
import { BstPostorderPartitionDiagram, BstPostorderRecursionMap, BstPostorderFailureDiagram, BstPostorderOfficialCaseLab } from "@/components/mdx/coding-interviews/diagrams/squence-of-bst";
...
- 能解释后序末尾根如何把连续序列分成左右子树
- 能计算合法与非法序列的分割点和首个冲突位置
- 能实现作者递归扫描及线性单调栈验证版本
- 能分析空序列、互异值前提、重复策略和最坏复杂度
- 能设计覆盖作者八组完整树、单链、单点、反例与空输入测试
...
二叉树后序顺序是左子树、右子树、根，因此任意非空连续子序列的<Term def="后序子序列最后一个值，对应当前子树的根节点。">后序根</Term>都在末尾。二叉搜索树又要求左侧所有值小于根，右侧所有值大于根。于是“**最后一个数字是根节点**”同时给出了结构位置和值域分割依据。
...
令root等于末尾值。从序列开头扫描，首个大于root的位置i是<Term def="左段结束、右段开始的位置；可等于0或根下标，表示一侧为空。">分割点</Term>。i之前的值都未触发大于条件，在互异前提下全部小于root，构成候选左子树。

从i继续扫描到根前，任何值小于root都形成<Term def="候选右子树连续区域中出现小于当前根的值，证明本层无法构成BST。">右段冲突</Term>，立即返回false。若没有冲突，顶层满足“**左子树小于根，右子树大于根**”，但还不能返回true：左右段内部也必须分别能形成二叉搜索树。
...
<Callout type="trap" title="直接递归长度0并沿用作者的false基例">
叶节点的左右子树都是空段，若无条件递归并让空段返回false，任何叶子都会使整棵树失败。要么像作者一样跳过空段，要么把内部递归的空区间定义为true，并单独处理外部空输入政策。
</Callout>
...
把<Term def="对分割后的左右连续片段重复执行末尾取根、大小分区和合法性判断。">递归验证子序列</Term>看作构造性证明：算法虽不分配节点，却隐含地给出每棵子树的根与左右范围。
...
逆向读取后序得到根、右、左顺序。维护一个递减关系的节点栈和<Term def="从右子树退回某个祖先左子树后，后续值必须严格小于的祖先根界限。">值域上界</Term>upper。读到更小值时不断弹出，表示从右链回退到祖先左侧，并把最后弹出的祖先设为新上界；若后续值大于上界，就跨回了已关闭的右侧区域。
...
<Callout type="trap" title="把单调递增或递减序列当成明显非法">
后序递增可对应每个节点只有左孩子的BST，递减可对应每个节点只有右孩子的BST。它们不仅合法，还是递归版最坏时间与最深调用栈的关键测试。
</Callout>
...
<Stepper>
  <Step title="取子序列最后一个数为根"><BstPostorderPartitionDiagram />根前连续区域只能先是小值左段，再是大值右段。</Step>
  <Step title="检查右段是否回落到根以下"><BstPostorderFailureDiagram />一旦进入右段又发现小值，当前层立即false。</Step>
  <Step title="递归验证左右连续子段"><BstPostorderRecursionMap />顶层分区正确仍不够，每个子段都要重复相同规则。</Step>
  <Step title="跑完八组官方序列"><BstPostorderOfficialCaseLab />完整树、双单链、单点、跨分区反例和空输入锁定契约。</Step>
</Stepper>
...
**问题 1：4、8、6、12、16、14、10如何完成第一次分割？**

<Answer>根是10，首个大于10的是12。左段4、8、6全部小于10，右段12、16、14全部大于10；再分别以6和14为根递归验证。</Answer>

**问题 2：7、4、6、5在哪一步失败？**

<Answer>根5，首个元素7已启动右段；继续扫描时4小于5，违反右子树所有值大于根的条件，顶层即可返回false。</Answer>

**问题 3：为什么递增1、2、3、4、5是合法后序？**

<Answer>根5之前所有值都小于5，右子树为空；递归后根4之前又都小于4。它对应5到1的全左链，每层后序都是子树后根，因此合法。</Answer>

**问题 4：空序列在递归内部与外部为何可有不同语义？**

<Answer>空子树是合法结构组成，所以内部空区间应视为true或直接跳过；作者外部API把“没有给出序列”视为非法输入并返回false。入口政策与归纳基例应分层定义。</Answer>
...
<Glossary>
  <GlossaryItem term="后序根">当前连续后序子序列最后一个、代表子树根的值。</GlossaryItem>
  <GlossaryItem term="分割点">第一个大于当前根的元素位置，即右段起点。</GlossaryItem>
  <GlossaryItem term="递归验证子序列">对左右连续片段重复后序根和大小分区判断。</GlossaryItem>
  <GlossaryItem term="值域上界">单调栈从右侧回退到左子树后不可越过的祖先根界限。</GlossaryItem>
  <GlossaryItem term="右段冲突">候选右子树区域中出现小于当前根的值。</GlossaryItem>
</Glossary>

<Attribution
  name="《剑指Offer：名企面试官精讲典型编程题（第2版）》面试题33"
  href="https://github.com/zhedahht/CodingInterviewChinese2/blob/2d6eb6d9127d1ad728ba60bb25bc5a5e91e519ac/33_SquenceOfBST/SquenceOfBST.cpp"
  author="何海涛"
