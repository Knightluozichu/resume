...
因此判断树的子结构需要两层工作：**先定位相同根节点**，再从该位置**再比较左右子树**。值相同只说明它是<Term term="候选根">候选根</Term>，并不能证明形状和值都覆盖B。
...
- **现象**：根值相同就直接判定匹配成功，没比较左右子树。
- **原因**：值相同只是候选根，形状和值都覆盖 B 才成立。
- **修法**：根值相同后必须递归比较左右子树，全部覆盖才算子结构。

</Callout>

<Callout type="trap" title="易错点">

- **现象**：把子结构当成完整子树，要求 B 的叶子必须也是 A 的叶子。
- **原因**：子结构允许在 B 到达空节点时提前结束匹配。
- **修法**：内层匹配中 B 为空即返回真（已覆盖），A 为空且 B 非空才失败。
...
## <Term term="浮点容差">浮点容差</Term>的原书边界与工程升级
...
根值相同只说明 A 的某节点是候选根，还需递归比较左右子树，确认 B 的每个非空节点都被 A 对应位置覆盖且值相等。
...
完整子树要求 B 的叶子在 A 中也是叶子；子结构允许 B 到空节点即结束匹配，可在中途截断。
...
A 为空且 B 非空返回失败（无法覆盖）；B 为空返回成功（已覆盖完毕）；两者都非空继续比较左右。
...
<Glossary>
  <GlossaryItem term="候选根">主树中值与模板树根相等的节点，需进一步比较子树才能确认。</GlossaryItem>
  <GlossaryItem term="子结构">模板树可被主树某子树覆盖（允许 B 中途截断）的包含关系。</GlossaryItem>
  <GlossaryItem term="浮点容差">比较浮点值时允许的误差范围，避免直接等号误判。</GlossaryItem>
</Glossary>
