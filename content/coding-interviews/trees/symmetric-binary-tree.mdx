...
officialUnitId: coi-28
---
import { SymmetricTraversalDiagram, MirrorPairBoundaryMap, SymmetricShapeCounterexampleDiagram } from "@/components/mdx/coding-interviews/diagrams/symmetric-binary-tree";
...
对称的二叉树要求沿根轴折叠后，结构和节点值都重合。每次比较的不是同一父节点下两个随意孩子，而是<Term term="对称位置">对称位置</Term>。
...
- **现象**：只比较左右孩子值相等，不管空位置对齐。
- **原因**：对称要求结构镜像，空节点也必须对称。
- **修法**：同步遍历时同时比较值和非空状态，任一不匹配即 false。
...
- **现象**：全同值树（如全 1）误判为对称。
- **原因**：值全等但结构不对齐。
- **修法**：同步遍历核对的是"对称位置"的对，不是值的集合相等。

</Callout>

## <Term term="对称前序">对称前序</Term>不是简单倒序

的递归方向与普通前序相反。对一对节点p、q，下一层必须**同时比较左-右与右-左**：
...
## <Term term="空节点">空节点</Term>是结构的一部分
...
沿根轴折叠后，结构和节点值都重合。每次比较对称位置的一对节点。
...
值全等但结构不对齐，空节点位置不对称，镜像位置值虽相等但结构不匹配。
...
<Glossary>
  <GlossaryItem term="对称位置">镜像折叠后对应的节点对。</GlossaryItem>
  <GlossaryItem term="对称前序">根右左的遍历顺序，与正常前序根左右镜像对称。</GlossaryItem>
  <GlossaryItem term="空节点">没有子节点的位置，对称遍历中必须参与比较。</GlossaryItem>
</Glossary>
