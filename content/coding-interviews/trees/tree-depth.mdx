...
import { TreeDepthPathDiagram, TreeDepthRecurrenceLab, TreeDepthCallStackDiagram, TreeDepthContractMap, TreeDepthOfficialCaseLab } from "@/components/mdx/coding-interviews/diagrams/tree-depth";
...
- 能解释作者按节点数定义二叉树深度及空树基例
- 能计算一棵分叉树自叶到根的每个递归返回值
- 能实现递归、层序遍历与显式栈三种求深度方式
- 能分析节点数与边数、树高、栈溢出、环和共享子树边界
- 能设计覆盖作者5种树形并与迭代实现对拍的测试
...
题目把从根到<Term def="没有左右孩子的终止节点；根到它的路径构成一条完整向下路径。">叶节点</Term>依次经过的节点组成一条路径，并把最长路径上的节点数量定义为**二叉树的深度**。因此这条路径深度为4，不是3。

<TreeDepthPathDiagram />

这种以节点数为单位的约定称为<Term def="以根到叶路径所含节点数量衡量深度，空树为0、单节点树为1。">节点计数深度</Term>。若另一套接口按边数定义高度，同一条路径会记为3；两种约定都可用，但不能在基例和测试中混用。

## 从根问题拆成两个子问题

非空树的最长路径必定从根开始，然后进入左子树或右子树，不能同时走两侧。于是先求左右两个<Term def="与原问题结构相同但输入规模更小的左右子树深度计算。">子问题</Term>，再选更大的结果并为当前根加1。

这正是作者概念“**左右子树深度的较大值加1**”。空指针没有节点，深度为0；非空节点的结果是max(leftDepth, rightDepth)加1。

若两个子树同深，选哪一侧都得到相同数值。若只需要深度，不需要返回具体路径，就无需记录父指针或节点列表。

<TreeDepthRecurrenceLab />
...
源码先完整计算左深度，再计算右深度，最后取较大值。这种先取得孩子结果、再计算父结果的模式称为<Term def="先完成左右子树递归，再在当前节点组合返回值的自底向上处理方式。">后序汇总</Term>。它与“用一个全局最大值在遍历时更新”是不同实现。
...
<Callout type="trap" title="把路径边数当成作者的节点数">
作者Test4的单节点树期望1，Test5空树期望0。若把叶节点返回0、空树也返回0，就无法区分二者，并会让所有非空树结果少1。
</Callout>
...
空间不是O(1)。未返回的函数调用保存在<Term def="递归尚未完成时保存参数、返回位置与局部变量的调用栈空间。">递归栈</Term>中，峰值等于树高h，因此辅助空间O(h)。
...
<Callout type="trap" title="用访问集合掩盖树结构损坏">
在明确的二叉树接口中，出现重复节点或环通常是上游所有权错误。遍历层加集合可以防死循环，但也会改变共享节点的路径语义；应先决定要容错还是要尽早拒绝坏结构。
</Callout>
...
<Stepper>
  <Step title="确认最长根到叶路径">
    <TreeDepthPathDiagram />
    作者按路径节点数计深度，Test1答案为4。
  </Step>
  <Step title="自底向上计算返回值">
    <TreeDepthRecurrenceLab />
    空树给0，非空节点取左右较大值再加1。
  </Step>
  <Step title="理解递归栈中的汇总">
    <TreeDepthCallStackDiagram />
    每层只返回子树深度，不依赖全局最大值。
  </Step>
  <Step title="核对复杂度与结构前提">
    <TreeDepthContractMap />
    时间O(n)，空间O(h)，输入必须有限无环。
  </Step>
  <Step title="执行作者5组测试">
    <TreeDepthOfficialCaseLab />
    普通树、左右单链、单节点和空树覆盖主要边界。
  </Step>
</Stepper>
...
**问题 1：为什么作者Test1的深度是4而不是3？**

<Answer>作者按节点数计路径长度。最长路径1、2、5、7包含4个节点；3只是这4个节点之间的边数。</Answer>

**问题 2：为什么不能把左右子树深度相加再加1？**

<Answer>根到叶路径离开根后只能进入一棵子树。相加会计算一条从左叶经过根到右叶的路径，那是树的直径候选，不是根到叶深度。</Answer>

**问题 3：递归版时间O(n)，为何空间不是O(n)固定结论？**

<Answer>同时存活的调用只沿一条根到当前节点路径，峰值由树高h决定，所以是O(h)。平衡树为O(log n)，斜树才退化为O(n)。</Answer>

**问题 4：递归、BFS和显式DFS应如何选择？**

<Answer>树高受控且只求深度时递归最简；极深树用显式DFS避免调用栈溢出；还需层级数据或宽度时用BFS。三者都应遵守空树0和节点计数定义。</Answer>
...
<Glossary>
  <GlossaryItem term="叶节点">左右孩子都为空的路径终止节点。</GlossaryItem>
  <GlossaryItem term="节点计数深度">最长根到叶路径包含的节点数量。</GlossaryItem>
  <GlossaryItem term="子问题">对左右子树求深度的同构小规模问题。</GlossaryItem>
  <GlossaryItem term="后序汇总">先取得孩子结果，再在父节点组合返回值。</GlossaryItem>
  <GlossaryItem term="递归栈">尚未返回的函数调用占用的栈空间，峰值由树高决定。</GlossaryItem>
</Glossary>

<Attribution
  name="《剑指Offer：名企面试官精讲典型编程题（第2版）》面试题55（一）"
  href="https://github.com/zhedahht/CodingInterviewChinese2/blob/2d6eb6d9127d1ad728ba60bb25bc5a5e91e519ac/55_01_TreeDepth/TreeDepth.cpp"
  author="何海涛"
