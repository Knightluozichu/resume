---
title: "Overview of Graphics Systems"
type: B
section: "计算机图形学与渲染"
order: 2
description: "光栅与矢量系统、GPU架构、显示技术"
demo: true
math: false
...
- 能用自己的话解释Overview of Graphics Systems的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：光栅与矢量系统与GPU架构之间的关系是什么？

</Objectives>

## 为什么需要Overview of Graphics Systems

Computer Graphics with OpenGL, 4th Edition (Hearn & Baker)将Overview of Graphics Systems作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
光栅与矢量系统、GPU架构、显示技术正是这一哲学在特定领域的具体体现。
...
Vulkan 把Overview of Graphics Systems从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Overview of Graphics Systems涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Overview of Graphics Systems相关功能前，必须完成：
...
// 录制与Overview of Graphics Systems相关的命令
// ... 具体的 vkCmd* 调用 ...
...
启用验证层后，Overview of Graphics Systems相关的常见错误会被捕获：
...
1. Overview of Graphics Systems是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Overview of Graphics Systems涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Computer Graphics with OpenGL, 4th Edition (Hearn & Baker)》（https://www.pearson.com/en-gb/subject-catalog/p/computer-graphics-with-open-gl-pearson-new-international-edition/P200000004424/9781292037196）第 2 章整理重写，
代码示例为教学简化版本。
</Attribution>
