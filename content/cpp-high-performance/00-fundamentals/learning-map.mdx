...
import { LearningMapDecisionLab, LearningMapMechanismMap, LearningMapFailureDiagram } from "@/components/mdx/cpp-high-performance/learning-map";
...
<Term def="由出版社目录核对出的11个正式章节及其概念顺序，是本书内容范围与学习进度的权威基线。">官方章节路线</Term>
...
<Term def="接口对value、borrow、ownership、error、thread safety和复杂度所做的可依赖承诺。">成本契约</Term>
...
<Term def="数据在内存中的排列、字段分组、索引关系与遍历顺序，它决定cache line利用率、pointer dependency和可向量化程度。">数据形状</Term>
...
<Term def="对象从raw storage、construction、ownership到destruction/deallocation的完整时间边界。">生命周期边界</Term>
...
<Term def="将work分配到threads、SIMD lanes或GPU kernels，并定义共享状态、结果合并、异常与backpressure的规则。">执行协议</Term>
...
<Term def="从可证伪假设出发，建立基线并定位原因，修改一个变量后复测和重新剖析的循环。">性能闭环</Term>
...
<Term def="要求correctness、fidelity、measurement与regression证据同时成立后，优化或章节才可通过的验收规则。">证据门禁</Term>
