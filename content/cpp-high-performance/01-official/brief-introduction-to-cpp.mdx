...
<Term def="没有使用某种抽象时不为它支付运行时成本；使用时难以用手写低层代码普遍实现得更好。">
  零成本抽象
</Term>
...
<Term def="抽象在未使用时不产生额外运行时工作，使用时的成本与该能力本身必要成本相符的原则。">
...
<Term def="同一源代码在多个 conforming platforms 上保持规定语义，同时把平台差异隔离在明确边界。">
  可移植性
</Term>
...
<Term def="对象拥有独立状态，可复制或移动为另一个值；lifetime 和变化不依赖隐藏共享身份。">
  值语义
</Term>
...
<Term def="用 const 限定接口，把可修改与只读访问在编译期区分并传播的设计纪律。">
  const correctness
</Term>

## 对象所有权与引用

<Term def="由谁结束对象 lifetime、释放资源，以及 borrower 在何种期限内可使用对象的显式契约。">
  对象所有权
</Term>
...
<Term def="对象所有公开操作前后都必须满足的状态条件；失败路径也不能留下半有效对象。">
  类不变量
</Term>
...
<Term def="把 resource lifetime 绑定到 object lifetime，构造成功即拥有，析构无条件释放的资源管理方式。">
  RAII
</Term>
