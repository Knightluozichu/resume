...
这一节把 class interfaces and exceptions、error handling 与 resource acquisition 放在同一条契约上：接口先验证输入并维护不变量，失败通过 exception 或 error result 表达，资源在获取后立即交给 RAII owner，任何返回路径都不泄漏。
