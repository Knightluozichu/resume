...
import { ModernCppConceptsDecisionLab, ModernCppConceptsMechanismMap, ModernCppConceptsFailureDiagram } from "@/components/mdx/cpp-high-performance/modern-cpp-concepts";
...
<ModernCppConceptsMechanismMap />

<ModernCppConceptsDecisionLab />

<ModernCppConceptsFailureDiagram />
...
<Term def="根据 initializer、占位声明形式和引用折叠规则，在编译期确定具体类型的过程。">自动类型推导</Term>
...
<Term def="在类型推导上下文中形如 T&& 或 auto&&，可根据实参值类别折叠为左值或右值引用的参数。">forwarding reference</Term>
...
<Term def="lambda表达式生成的匿名对象类型，其数据成员保存captures，调用运算符承载body。">closure object</Term>
...
<Term def="通过右值引用和移动操作把资源所有权从源对象转给目标对象，并让源对象保持可析构可赋值状态。">move semantics</Term>
...
<Term def="把资源封装进已有RAII成员，让class无需自行声明copy/move/destructor即可获得正确生命周期语义。">rule of zero</Term>
...
<Term def="在对象内部直接管理一个可能存在的T，engaged状态决定T的lifetime，避免用magic value或nullable owner表达缺失。">std::optional</Term>
...
<Term def="可持有满足要求的任意单个值，并在any_cast时进行runtime type检查的类型擦除容器。">std::any</Term>
