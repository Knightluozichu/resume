...
<Term def="用户可观察指标偏离既定基线，并能追溯到代码、配置或环境变化的性能退化事件。">
  性能回归
</Term>
...
<Term def="把value、borrow、ownership、capture与error路径写成可验证状态关系的接口审计表。">
  契约台账
</Term>
...
<Term def="明确metric、workload、控制变量、样本与决策规则的可重复性能实验描述。">
  测量协议
</Term>
...
<Term def="算法为得到目标结果实际执行的比较、移动、分配、遍历与同步总量。">
  有效工作量
</Term>
...
<Term def="工作从一个阶段移动到另一个阶段后，新阶段承担的时间、内存、异常、同步或维护成本。">
  代价迁移
</Term>
...
<Term def="随着workers增加，吞吐、延迟或完成时间相对单worker的变化关系，以及停止增长的资源原因。">
  扩展曲线
</Term>
...
<Term def="从host准备、数据传输、执行、同步到结果消费的完整用户可见时间，而非只取其中最快的kernel段。">
  端到端成本
</Term>
...
<Term def="由原始测量、profile、correctness tests、代码差异与复测结果组成，可让他人重现决策的材料集合。">
  证据包
</Term>
