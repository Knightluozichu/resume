...
<Term def="多个活动在重叠时间内推进，允许scheduler交错执行，即使任一时刻只有一个core真正运行。">
  concurrency
</Term>

<Term def="多个计算在不同core或execution units上同时执行，以增加throughput或降低完成时间。">
  parallelism
</Term>
...
<Term def="两个线程对同一memory location发生冲突访问、至少一个写且没有happens-before关系，并且访问不是合规atomic同步。">
  data race
</Term>
...
<Term def="一组线程永久等待彼此持有的resource，形成无法自行打破的等待环。">
  deadlock
</Term>
...
<Term def="把等待线程挂起，并在其他线程通知后重新竞争mutex和检查predicate的同步原语。">
  condition variable
</Term>
...
<Term def="由语言定义的线程间可见性与排序规则，包括happens-before、atomic modification order和允许的memory orders。">
  C++ memory model
</Term>
...
<Term def="不依赖互斥锁并保证系统整体持续取得进展的并发算法；单个线程仍可能重试或饥饿。">
  lock-free
</Term>
...
<Term def="多个执行者竞争同一同步点、cache line或有限硬件资源，导致等待、重试或吞吐不再随线程数扩展。">
  contention
</Term>
