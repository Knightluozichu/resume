...
<Term def="把输入递归或分块成较小独立问题，并行求解后再组合partial results的算法设计。">
  divide and conquer
</Term>
...
<Term def="每个parallel task处理的最小工作块大小，用来平衡调度开销、负载均衡和cache locality。">
  grain size
</Term>
...
<Term def="把多个元素或分块的partial values按组合operation归并成一个结果的计算模式。">
  reduction
</Term>
...
<Term def="多个并行worker通过mutex或atomic协调唯一输出索引，避免写入同一slot的共享位置。">
...
<Term def="传给标准算法的策略对象，用来声明实现可采用sequenced、parallel或parallel-unsequenced执行。">
  execution policy
</Term>
...
<Term def="允许重新分组并可并行合并elements与partial results的归约算法，operation必须支持这些组合。">
  reduce
</Term>
...
<Term def="跨CPU、GPU等设备执行kernel的开放并行计算接口，显式管理context、command queue和device memory。">
  OpenCL
</Term>
