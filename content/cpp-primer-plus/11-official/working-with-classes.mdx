...
import { WorkingWithClassesDecisionLab, WorkingWithClassesMechanismMap, WorkingWithClassesFailureDiagram } from "@/components/mdx/cpp-primer-plus/working-with-classes";
...
<WorkingWithClassesMechanismMap />

<WorkingWithClassesDecisionLab />
...
<Term def="为至少含一个用户定义类型操作数的既有 C++ 运算符提供函数实现，使表达式参与普通候选解析。">运算符重载</Term>
...
<Term def="以左操作数为隐式 this、只能让其余显式操作数参与参数转换的重载形式。">成员运算符</Term>

<Term def="把所有操作数写成显式参数，可为左右两侧提供更对称转换机会的命名空间级重载形式。">非成员运算符</Term>
...
<Term def="由类显式授予私有成员访问权、但不属于类成员且没有 this 指针的函数。">友元函数</Term>
...
<Term def="允许从源类型创建类对象、可在隐式转换序列中被使用的单参数或具有默认尾参的构造函数。">转换构造函数</Term>

<Term def="禁止构造函数或转换函数参与多数隐式转换，要求调用点显式写出目标类型意图的说明符。">explicit</Term>
...
<Term def="以 operator TargetType 形式定义从类对象到目标类型的转换成员函数。">转换运算符</Term>
