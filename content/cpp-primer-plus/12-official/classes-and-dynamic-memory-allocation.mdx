...
<Term def="由对象负责最终释放、在该对象有效期内必须保持唯一或共享协议一致的动态资源责任。">
  所有权
</Term>

<Term def="运行时从自由存储区取得、必须由明确 owner 或 RAII 对象结束生命周期的存储。">
  动态内存
</Term>
...
<Term def="对象有效期结束时释放其拥有资源、并保证每项责任只完成一次的特殊成员函数。">
  析构函数
</Term>
...
<Term def="以已有同类型对象初始化一个新对象，并定义新旧对象资源关系的特殊构造函数。">
  复制构造函数
</Term>
...
<Term def="把已存在左对象替换为右对象值，并保持资源与自赋值安全的 operator= 成员函数。">
  赋值运算符
</Term>
...
<Term def="保存类对象地址、通过箭头运算符访问成员，但不自动说明 owner、数量或目标有效期的指针。">
  对象指针
</Term>
...
<Term def="按先进先出规则插入队尾、移除队首，并明确容量与空满失败语义的抽象数据类型。">
  队列
</Term>
