...
<Term def="把另一个类型的对象作为成员，表达 has-a 关系并由外层类选择暴露能力的复用方式。">
  包含
</Term>
...
<Term def="把基类公有和保护能力收进派生类私有实现、阻止外部 is-a 转换的继承方式。">
  私有继承
</Term>
...
<Term def="把基类公有和保护成员转为派生保护成员，使后续派生实现可用但外部不可用的继承方式。">
  保护继承
</Term>
...
<Term def="以类型或常量参数描述一族类定义，由具体模板实参生成具体类类型的代码模式。">
  类模板
</Term>

<Term def="把模板参数替换为具体实参，并对实际使用成员生成和检查具体代码的过程。">
  模板实例化
</Term>
...
<Term def="一个派生类直接继承两个或更多基类，从而组合多个基类子对象与接口的机制。">
  多重继承
</Term>
...
<Term def="在多重继承菱形中由完整派生对象共享、并由最派生类负责初始化的基类子对象。">
  虚基类
</Term>
