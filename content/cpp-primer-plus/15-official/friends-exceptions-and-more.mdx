...
import { FriendsExceptionsAndMoreDecisionLab, FriendsExceptionsAndMoreMechanismMap, FriendsExceptionsAndMoreFailureDiagram } from "@/components/mdx/cpp-primer-plus/friends-exceptions-and-more";
import {
  EppExceptionUnwindFlow,
  EppFriendAccessMap} from "@/components/mdx/cpp-primer-plus/diagrams/friends-exceptions-and-more";
...
<Term def="由一个类明确授权其全部成员访问自身 private/protected 成员的另一个具名类。">友元类</Term>
...
<Term def="被另一个类单独授予 private/protected 访问权的某一个具名成员函数。">友元成员</Term>
...
<Term def="在另一个类作用域中声明、名字受外层访问控制但对象不自动携带外层 this 的独立类。">嵌套类</Term>
...
<Term def="由 throw 建立并沿调用栈传播，直到类型匹配的 catch handler 接管的异常对象与控制流。">异常</Term>
...
<Term def="异常寻找匹配 handler 时逐层退出调用帧，并逆序销毁已构造自动对象的过程。">栈展开</Term>
...
<Term def="在运行时查询多态对象动态类型或执行受检向下/横向转换的语言机制。">运行时类型识别</Term>
