...
import { InputOutputAndFilesDecisionLab, InputOutputAndFilesMechanismMap, InputOutputAndFilesFailureDiagram } from "@/components/mdx/cpp-primer-plus/input-output-and-files";
...
<InputOutputAndFilesMechanismMap />

<InputOutputAndFilesDecisionLab />
...
<Term def="根据流的进制、精度、宽度、填充和浮点策略，把类型值转换为字符序列的输出过程。">格式化输出</Term>
...
<Term def="插入或提取时修改流格式状态、或触发特定动作的函数/对象，例如 hex、setw、ws。">流操纵符</Term>
...
<Term def="按目标类型和流格式规则从字符序列解析值，并根据成功或失败更新流状态的输入过程。">格式化输入</Term>
...
<Term def="由 goodbit、eofbit、failbit、badbit 组成，记录最近 I/O 成功、末尾、格式失败或底层损坏的状态集合。">流状态</Term>
...
<Term def="通过文件流把字符或字节序列与持久文件连接，并由打开模式和流状态约束读写的过程。">文件输入输出</Term>
...
<Term def="以精确字节序列读写、不进行文本格式转换，并要求显式定义布局、字节序和版本的文件。">二进制文件</Term>
...
<Term def="通过显式移动文件 get/put position 直接读写指定偏移，而非从头顺序遍历的访问方式。">随机访问</Term>
