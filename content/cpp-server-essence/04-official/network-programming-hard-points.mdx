...
import { NetworkStateMachineLab, NetworkStateMachineMechanismMap, NetworkStateMachineFailureDiagram } from "@/components/mdx/cpp-server-essence/network-programming-hard-points";
...
跨平台网络通信库应把 native socket type、关闭入口、错误码域和 readiness 事件隔离在适配层：上层只消费统一的连接状态与 progress，不直接依赖 POSIX 或 Winsock 的整数布局。
