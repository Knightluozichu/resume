...
后端服务中的定时器应由 owner EventLoop 统一调度：每个 deadline 绑定 connection generation，取消、重排和回调执行都回到同一线程，避免关闭连接后仍触发旧任务。
...
经典服务器结构把 listenfd 非阻塞、acceptor、I/O loop、Connection/Buffer/Codec/Session 和有界业务池串成明确的数据流；服务器性能瓶颈应以 event-loop lag、queue wait、CPU profile 和 tail latency 定位，而不是凭线程数量猜测。
