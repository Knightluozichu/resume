...
<Term def="把源代码按照选定的 C++ 标准翻译为目标文件，并报告词法、语法和类型问题的工具实现。">
  C++ 编译器
</Term>
...
<Term def="用目标、源文件、编译选项和链接依赖组成的有向关系，决定哪些输入变化需要重建哪些产物。">
  CMake 构建图
</Term>
...
<Term def="CMake 随附的测试运行层，用统一命令枚举测试、执行用例、汇总结果并以退出码报告整体成败。">
  CTest
</Term>
...
<Term def="Google 提供的 C++ 单元测试框架，负责 TEST 注册、夹具生命周期、断言和结构化失败报告。">
  GoogleTest
</Term>
...
<Term def="测试框架把源码中的测试定义枚举为运行器可见用例的过程；发现数量是判断测试是否漏跑的重要信号。">
  测试发现
</Term>
...
<Term def="GoogleTest 配套的模拟工具，用期望调用、参数匹配和返回行为描述对象之间的可观察交互。">
  GoogleMock
</Term>
...
<Term def="轻量的 C/C++ 单元测试框架，提供测试组、检查宏、内存泄漏检测与 mock 支持，适合受限或兼容 C 的工程。">
  CppUTest
</Term>
...
<Term def="由工程之外维护、通过包管理器、源码子树或下载步骤进入构建图的库及其版本契约。">
  第三方依赖
</Term>
...
<Term def="从空构建目录出发，给定相同源码、工具链与依赖身份时能得到相同目标图和测试结论的环境。">
  可重复测试环境
</Term>
