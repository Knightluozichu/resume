...
import {
  CtrRedGreenRefactorEvidence,
  CtrSoundexBoundaryLab,
  CtrSoundexRuleLadder,
} from "@/components/mdx/cpp-testing-recipes/diagrams/tdd-first-example";
...
事后测试可以发现回归，也有价值；但它通常围绕已经存在的函数和分支书写，较难反向改善接口。测试驱动在实现前先问“调用者希望怎样表达行为”，因此 `encode(word)`、固定长度结果和错误契约会在代码结构冻结前得到反馈。
