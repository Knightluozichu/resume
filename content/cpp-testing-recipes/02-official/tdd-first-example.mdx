...
<Term def="接收姓名字符串并返回固定长度音码的教学类；本章通过逐条示例驱动它的接口、规则和内部职责。">
  Soundex 类
</Term>
...
<Term def="通过一连串可验证的小行为扩展系统，每一步都保留之前证据，而不是一次实现全部需求。">
  增量开发
</Term>
...
<Term def="先写一个因缺少目标行为而失败的测试，再写使其通过的最小实现，用测试顺序持续塑造接口和设计。">
  测试驱动
</Term>
...
<Term def="先看见目标测试以正确原因失败，再用最小实现让全部测试通过，最后在全绿保护下改善结构的三阶段循环。">
  红绿重构
</Term>
...
<Term def="为一组测试提供共同对象和生命周期钩子的测试结构；GoogleTest 会为每个 TEST_F 创建独立实例。">
  测试夹具
</Term>

<Term def="在每个测试执行前建立所需初始状态的步骤；它应保持轻量、确定并且不隐藏本测试关键输入。">
  setup
</Term>
...
<Term def="只为一个最小行为选择能区分正确与错误实现的例子，在该行为通过前不同时加入其他规则。">
  一次一件事
</Term>
...
<Term def="把固定输出长度、空输入、元音跳过和重复编码等极端位置写成示例，用来约束循环停止与 API 前置条件。">
  边界规则
</Term>
...
<Term def="用失败测试决定下一条行为和设计压力的开发方式，与实现完成后只验证既有结构的事后测试形成对比。">
  测试驱动与事后测试
</Term>
...
<Term def="一个模块或函数只有一个主要变化原因，使编码映射、输出完成判断和补零策略能够独立理解与修改。">
  单一职责原则
</Term>
...
<Term def="当前实现具有路径或输入契约，但测试集合尚未提供可区分证据的行为缺口。">
  遗漏测试
</Term>
