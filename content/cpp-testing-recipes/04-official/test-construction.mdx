...
<Term def="按领域行为、共同 setup、运行速度与资源边界安排测试文件、套件和执行入口的结构。">
  测试组织
</Term>
...
<Term def="不依赖进程外文件、数据库、网络或真实时钟，能在内存中快速确定执行的测试。">
  快速测试
</Term>

<Term def="需要进程外资源、真实 I/O、长时间计算或多组件启动，执行与失败定位成本较高的测试。">
  慢速测试
</Term>
...
<Term def="按测试名、标签或模式选择测试子集的运行条件，用于定位和短反馈，但不能替代最终全套回归。">
  测试过滤器
</Term>

<Term def="围绕共同领域行为或夹具组织的一组测试，使名称、setup 和运行入口形成可读边界。">
  测试套件
</Term>
...
<Term def="把可观察实际值与行为契约中的期望进行比较，并在不满足时产生结构化诊断的测试语句。">
  断言
</Term>
...
<Term def="类为维护不变量而隐藏的表示与辅助动作，调用者不应依赖其具体结构。">
  私有实现
</Term>
...
<Term def="用同一测试体运行多组命名输入与期望数据，使同一成熟行为规则得到独立案例报告。">
  参数化测试
</Term>
...
<Term def="用失败例逐步塑造新行为，与规则形成后增加代表数据验证范围的两种不同活动。">
  测试驱动与测试
</Term>
