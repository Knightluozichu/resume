...
<Term def="把具体风险映射到最窄有效测试层、可观察结果、失败责任和持续门禁的全书综合方法。">
  风险—证据映射
</Term>
...
<Term def="测试因目标行为尚未满足而按预测失败，且测试被发现、旧集合此前全绿、诊断与预期一致。">
  可信红灯
</Term>
...
<Term def="测试只依赖领域可观察结果和必要交互，使内部容器、helper、缓存或调用顺序可在行为不变时重构。">
  重构稳定性
</Term>
...
<Term def="为查询控制输入、为命令观察结果或为复杂状态提供简化实现，并只承担当前测试所需角色的最小替代对象。">
  最小测试替身
</Term>
...
<Term def="在全部行为护栏通过时只改善内部结构，新增或改变需求则由独立失败测试明确驱动。">
  行为—结构分离
</Term>
...
<Term def="通过 latch/barrier、条件谓词、任务计数、唯一执行检查、压力重复和 sanitizer 验证并发协议的组合。">
  并发证据组合
</Term>
...
<Term def="以固定环境、代表负载、统计分布和预算验证延迟、吞吐、分配或资源消耗的可复查测量。">
  性能基准证据
</Term>
...
<Term def="主干红、随机测试、快层时长、覆盖变化和性能退化都有可见指标、明确 owner、处理动作与时限的团队运行状态。">
  反馈系统健康
</Term>
