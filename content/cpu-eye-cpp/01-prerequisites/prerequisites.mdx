...
<Term def="CPU 可直接取指和执行的二进制编码；assembly mnemonic 是其可读表示。">
  CPU 指令
</Term>
...
<Term def="在线编译与源码—汇编关联工具，用于比较 compiler、version、flags 和 target 对 code generation 的影响。">
  Compiler Explorer
</Term>
...
<Term def="函数参数、返回值、callee-saved registers 与 stack alignment 在特定平台上的二进制调用约定。">
...
<Term def="包含可加载代码、数据和映射元数据，由 operating system 创建进程映像的二进制文件。">
  executable image
</Term>
...
<Term def="每个进程看到的连续地址抽象，由 page tables/MMU 映射到 physical memory 或 backing storage。">
  虚拟内存
</Term>

<Term def="按 executable metadata 创建 virtual mappings、初始化 process/thread state 并转交入口点的系统组件。">
  program loader
</Term>
...
<Term def="executable 中记录零初始化 static storage 大小、load 时由系统提供零页而无需保存全部零字节的区域。">
  BSS
</Term>
...
<Term def="使线程暂停后可恢复的 registers、instruction pointer、stack pointer 与系统调度状态集合。">
  线程上下文
</Term>
...
<Term def="由 base register、index、scale 与 displacement 计算地址，再读写该地址内容的访问方式。">
...
<Term def="一次 function invocation 在 stack 上用于 return address、saved registers、locals 与 spill slots 的逻辑区域。">
  stack frame
</Term>
...
<Term def="register 不足、debug-friendly code 或 calling convention 使 temporary/local 被写入 stack memory 的行为。">
  register spill
</Term>
...
<Term def="记录配置、预测、assembly/binary/runtime evidence 与结论适用边界的可复现实验模板。">
  CPU 视角证据链
</Term>
