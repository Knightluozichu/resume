...
<Term def="源码经过编译后形成的基本块与跳转边，描述每个条件可能选择的执行路径。">
  控制流图
</Term>
...
<Term def="在进入 main 前准备运行库、参数和退出路径的启动代码；具体符号与步骤取决于平台。">
  runtime startup
</Term>
...
<Term def="一段具有类型、存储期和值语义的对象；实现可把其值放在内存、寄存器或直接消除。">
  变量对象
</Term>
...
<Term def="只有一个入口、内部顺序执行并从末尾分支出去的一段指令，是控制流分析的基本节点。">
  basic block
</Term>
...
<Term def="保存某个对象或函数地址表示的变量；类型决定允许的地址运算和解引用解释。">
  指针变量
</Term>
...
<Term def="对象生命周期结束后仍保留其旧地址的指针；即使地址数值未变，也不能合法解引用。">
  悬空指针
</Term>
...
<Term def="由基地址、索引乘比例因子和偏移量组合得到，供一次 load/store 使用的地址。">
  有效地址
</Term>
...
<Term def="访问数组对象有效范围之外的位置；C/C++ 通常不自动检查，行为未定义。">
  数组越界
</Term>
...
<Term def="绑定到已有对象的语言级别别名；必须遵守被引用对象的生命周期，具体实现不承诺一定占用地址槽。">
  引用
</Term>
...
<Term def="后置递增为了产生修改前结果而形成的概念值；优化器可在该结果不可观察时消除它。">
  旧值临时量
</Term>
...
<Term def="把一个类型的值映射为另一类型；可能改变位宽、符号解释、精度、数值范围或对象访问规则。">
  类型转换
</Term>
