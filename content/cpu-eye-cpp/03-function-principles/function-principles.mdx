...
<Term def="函数从进入到返回的一次动态执行实例；递归时同一函数可同时存在多个实例。">
  函数调用实例
</Term>
...
<Term def="调用者与被调者共同遵守的参数分类、寄存器用途、栈对齐、返回值与寄存器保存规则。">
  调用约定
</Term>
...
<Term def="名字可见范围与自动对象生命周期边界；离开路径包括正常返回、分支和异常展开。">
  函数作用域
</Term>
...
<Term def="被调函数进入时为保存返回地址、寄存器和局部状态建立的逻辑区域；优化后可缩小或不存在。">
  栈帧
</Term>
...
<Term def="保存兼容函数入口地址表示的值，可在运行时选择 indirect call target。">
  函数指针
</Term>
...
<Term def="调用方提供目标存储地址、被调方直接在其中构造大返回对象的 ABI 机制，常称 sret。">
  隐藏返回地址
</Term>
...
<Term def="允许或要求直接在最终目标位置构造返回对象，省略中间 copy/move 的语言与优化机制。">
  copy elision
</Term>
...
<Term def="线程执行上下文中的调用存储区域，常由 stack pointer 管理，承载返回状态、spill 与部分自动对象。">
  调用栈
</Term>
...
<Term def="从当前程序计数器出发，逐帧恢复 caller 状态并把代码地址解析为函数和源码位置的过程。">
  backtrace
</Term>
