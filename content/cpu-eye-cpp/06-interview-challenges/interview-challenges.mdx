...
<Term def="先写出语言与平台条件，再给出预测、实验和适用边界的回答结构。">
  证据型回答
</Term>
...
<Term def="名字在源程序哪些区域可见的规则，不等同于对象 lifetime。">
  作用域
</Term>

<Term def="不同 translation units 中的 declarations 是否表示同一 entity 的规则。">
  链接属性
</Term>
...
<Term def="对象在维度、连续性、步长和元素数量上的内存形状；pointer type 只编码其中一部分。">
  数据形状
</Term>
...
<Term def="负责结束对象 lifetime 并释放对应资源的实体；borrower 只能在 owner 保证的期限内访问。">
  所有权
</Term>
...
<Term def="编译器依据 initializer 和占位声明形式在编译期确定 concrete type 的规则。">
  auto 类型推导
</Term>
...
<Term def="每个线程拥有独立对象实例、从该线程使用前初始化并在线程结束阶段销毁的 storage duration。">
...
<Term def="把源码保证、ABI规则、具体汇编与运行观测分层陈述，并主动说明适用条件的方法。">
  分层论证
</Term>
