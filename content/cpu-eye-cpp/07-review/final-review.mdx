...
<Term def="从源码约束到运行观测逐层可追溯，任何结论都能指出所依赖证据与适用条件。">端到端证据链</Term>
...
<Term def="编译器用于表示数据依赖和控制流、再逐步优化并降低为目标机器码的中间形式。">编译器中间表示</Term>
...
<Term def="值只在编译器数据流中存在而没有独立内存位置，常由SSA和寄存器分配表达。">无地址值</Term>
...
<Term def="对象 lifetime 已结束但仍被 pointer、reference、span 或 iterator 访问的错误。">悬空访问</Term>
...
<Term def="一个线程的操作按语言内存模型保证对另一个线程可见和有序的关系。">happens-before</Term>
...
<Term def="把一个现象拆成互斥或可检验的语言、ABI、机器与系统原因，并为每个原因指定证据。">分层假设</Term>
