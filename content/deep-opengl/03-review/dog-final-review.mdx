...
order: 9
description: >-
  深入理解 OpenGL WebGL OpenGL ES 总复习：区分上下文状态、对象存储、绑定点与VAO捕获关系；用GLSL
...
officialUnitIds:
  ...
  - dog-08
---
import { DogFinalReviewStateLab, DogFinalReviewFaultLab, DogFinalReviewEvidenceLab } from "@/components/mdx/deep-opengl/v2/dog-final-review";
import {
  Objectives,
  Callout,
  Glossary,
...
“深入理解 OpenGL WebGL OpenGL ES 总复习”不是某本同名原书的翻译，也不把四套API规范拼成虚构目录。OpenGL状态与对象语义以[OpenGL 4.6 Core规范](https://registry.khronos.org/OpenGL/specs/gl/glspec46.core.pdf)核对，着色语言以[GLSL 4.60规范](https://registry.khronos.org/OpenGL/specs/gl/GLSLangSpec.4.60.pdf)核对，移动端与浏览器边界分别以[OpenGL ES 3.2规范](https://registry.khronos.org/OpenGL/specs/es/3.2/es_spec_3.2.pdf)和[WebGL 2规范](https://registry.khronos.org/webgl/specs/latest/2.0/)核对。八个正式单元是独立课程分类。

## 六个检查词

在“深入理解 OpenGL WebGL OpenGL ES 总复习”的六词核对中，<Term def="context在“深入理解 OpenGL WebGL OpenGL ES 总复习”的“OpenGL Context, Objects, and State”检查中用于区分上下文状态、对象存储、绑定点与VAO捕获关系；应由当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照确认。">context</Term>、<Term def="state machine在“深入理解 OpenGL WebGL OpenGL ES 总复习”的“OpenGL Context, Objects, and State”检查中用于区分上下文状态、对象存储、绑定点与VAO捕获关系；应由当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照确认。">state machine</Term>、<Term def="vao在“深入理解 OpenGL WebGL OpenGL ES 总复习”的“OpenGL Context, Objects, and State”检查中用于区分上下文状态、对象存储、绑定点与VAO捕获关系；应由当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照确认。">vao</Term>、<Term def="vbo在“深入理解 OpenGL WebGL OpenGL ES 总复习”的“OpenGL Context, Objects, and State”检查中用于区分上下文状态、对象存储、绑定点与VAO捕获关系；应由当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照确认。">vbo</Term>、<Term def="glsl es在“深入理解 OpenGL WebGL OpenGL ES 总复习”的“GLSL ES Shaders”检查中用于用GLSL ES阶段接口、精度与uniform合同连接顶点和片元；应由编译日志、链接日志、接口位置、uniform值和参考像素确认。">glsl es</Term>、<Term def="uniform在“深入理解 OpenGL WebGL OpenGL ES 总复习”的“GLSL ES Shaders”检查中用于用GLSL ES阶段接口、精度与uniform合同连接顶点和片元；应由编译日志、链接日志、接口位置、uniform值和参考像素确认。">uniform</Term>。术语只有进入公式、交互状态、代码和可重放输出，才算真正覆盖。
...
<Stepper>
  <Step title="1. 对象与执行路径">
    ...
    <DogFinalReviewStateLab />

  </Step>
  <Step title="2. 单变量故障">
    保持其余条件不变，只切换一个绑定、能力、访问或同步错误，预测首个差异。

    <DogFinalReviewFaultLab />
...
<Callout type="trap" title="本页核心误区">
  把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询。它会破坏“深入理解 OpenGL WebGL OpenGL ES 总复习的输入、状态、输出与恢复结果可用同一证据包重放”，应先由当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳定位。
</Callout>

<Callout type="trap" title="不要跨API套生命周期模板">
  “深入理解 OpenGL WebGL OpenGL ES 总复习”中的OpenGL/WebGL状态机只在自己的规范语义内解释本页。不能把另一套API的Buffer、Fence、bind或隐式同步模型换上标题后当作正文。
</Callout>
...
**问题 1：公式边界。** 在“深入理解 OpenGL WebGL OpenGL ES 总复习”中，如何用一个可手算样本验证 $Draw=F(Ctx,Program,VAO,FBO,State)$，并标明每个量属于哪一对象、stage、空间或时间点？

<Answer>
  针对“深入理解 OpenGL WebGL OpenGL ES 总复习”固定最小输入，逐项记录当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳；只扰动一个量并比较变化方向。若公式、验证消息和输出不一致，先拒绝实现再调查隐藏状态。
</Answer>
...
<Answer>
  1. **context**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  2. **state machine**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  3. **vao**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  4. **vbo**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  5. **glsl es**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  6. **uniform**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  7. **in/out**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  8. **precision**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  9. **webgl context**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  10. **webgl1**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  11. **webgl2**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  12. **context lost**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  13. **opengl es**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  14. **egl**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  15. **precision qualifier**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  16. **tile**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  17. **framebuffer**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  18. **color attachment**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  19. **depth attachment**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  20. **post-processing**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  21. **draw call**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  22. **instancing**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  23. **state sorting**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  24. **bandwidth**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  25. **feature detection**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  26. **extension**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  27. **fallback**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  28. **compatibility**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  29. **glgeterror**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  30. **khr_debug**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  31. **renderdoc**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
  32. **profil**：在“深入理解 OpenGL WebGL OpenGL ES 总复习”章专属实验中保存当前context、program、VAO属性槽、EBO、FBO与逐片元状态快照、编译日志、链接日志、接口位置、uniform值和参考像素、上下文版本、扩展集合、lost/restored事件和重建对象计数、EGL配置、ES版本、附件load/store、外部带宽与shader精度范围、完整性状态、附件尺寸/样本数、draw buffers和逐pass参考像素、CPU/GPU时间戳、draw数、状态切换、带宽和图像差异、版本、扩展、限制、选择路径和降级输出差异、debug message、对象标签、捕获事件、资源快照与GPU时间戳，注入“把ARRAY_BUFFER当前绑定误当成VAO整体状态，或依赖上一个pass残留开关；顶点输出与片元输入位置或类型不一致，或片元高光计算使用过低精度；假设WebGL2或扩展必定存在，或context restored后继续复用旧句柄；把桌面扩展当作ES核心能力，或每个pass都强制保存不会再读取的附件；FBO不完整仍绘制，或把当前颜色附件同时绑定为采样输入；只减少draw call却增加过度绘制或带宽，或用glFinish制造虚假稳定计时；用厂商字符串猜能力，或扩展存在却未检查对应限制与入口；只在帧尾调用glGetError，或用CPU墙钟时间替代GPU查询”，恢复后以相同输入重放。
</Answer>
...
[← 渲染优化策略](/learn/deep-opengl/02-advanced/dog-rendering-optimization)
