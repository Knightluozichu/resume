...
import {
  EcppItem01FederationLab,
  EcppLanguageFederationMap,
} from "@/components/mdx/effective-cpp/diagrams/item-01-language-federation-lab";
...
<Term def="由多种具有不同抽象、接口与成本模型的次语言共同组成，并通过明确边界协作的整体。">语言联邦</Term>。

写 C 风格缓冲区时，指针算术和布局是核心；写对象体系时，不变量与动态绑定主导；写模板时，接口由表达式隐式形成；写 STL 时，容器、迭代器、算法和函数对象构成另一套约定。

<Term def="在某段代码中决定主要正确性规则、扩展方式与成本模型的编程范式。">主导次语言</Term>
...
<Term def="C++ 中保留的 C 风格子集，以基本类型、数组、指针、函数和预处理器为主要工具。">C 次语言</Term>
...
<Term def="二进制接口对名称、调用约定、对象布局和数据表示的稳定约束。">ABI 边界</Term>

要求类型与生命周期协议可见。不能把 `std::vector` 的内部布局当跨编译器 ABI，也不能因 C API 接受裸指针就推断它取得所有权。

<Term def="由文档规定创建、使用和释放顺序，但语言类型本身没有自动执行的约定。">显式资源协议</Term>

最容易在异常或提前返回时被破坏，因此跨出边界后应尽快包装。

## Object-Oriented C++：不变量与运行期替换

<Term def="以 class、封装、继承、virtual 和运行期多态组织对象及其协作的次语言。">Object-Oriented C++</Term>
...
<Term def="对象在构造完成后必须始终成立，并由所有公开操作维持的状态约束。">类不变量</Term>

与

<Term def="通过基类指针或引用在运行时选择派生实现，同时要求派生满足基类语义。">运行期多态</Term>
...
<Callout type="trap" title="陷阱：把所有代码都归入同一种现代 C++ 风格">
  C、Object-Oriented C++、Template C++ 与 STL 的接口和成本模型不同；先识别主导次语言，
  再写边界适配和验证证据，避免为统一风格牺牲 ownership 或诊断质量。
</Callout>

## Template C++：表达式形成隐式接口

<Term def="以 function/class templates、实例化和编译期多态表达泛化算法与类型策略的次语言。">Template C++</Term>
...
<Term def="由模板体中对类型参数执行的全部有效表达式共同定义、实例化时检查的接口。">隐式接口</Term>
...
<Term def="以 containers、iterators、algorithms 和 function objects 协作的数据处理次语言。">STL 次语言</Term>
...
<Term def="由起始与终止迭代器或 range 表达的一段元素序列，是算法操作边界。">迭代器范围</Term>

值语义和拷贝成本变得重要；容器重分配会使部分迭代器失效；算法通常比手写循环更直接表达意图。这些不是 C 指针代码或虚函数体系的同一套问题。

<Term def="对象在复制、移动、比较和销毁时表现得像独立值，并由操作维护等价关系的语义。">值语义</Term>
...
<Term def="为当前问题选择最能直接表达约束的指针、类、多态、模板或算法模型。">抽象选择</Term>
...
<Term def="把一种次语言的所有权、错误、范围和类型约定转换成另一种模型的适配层。">联邦边界适配</Term>
...
<Term def="任意成功、失败、移动和销毁路径中，一个资源都由唯一责任方执行一次释放。">单次释放不变量</Term>
...
<Term def="某种抽象主要消耗的运行时间、空间、编译时间、代码体积和维护复杂度组合。">成本模型</Term>
...
<Term def="固定输入与失败点，验证 C 句柄、RAII 对象、模板策略和 STL 容器协作结果的测试。">联邦边界测试</Term>
...
<Exercises>
  1. **问题 1：federation of languages 与 c。** 分析内存映射文件、可替换压缩器、固定类型序列化器和记录排序分别由哪个模型主导。
...
2. **问题 2：object-oriented c++ 与 template c++。** C API 返回 BufferHandle，要求放入 vector 并用泛型算法处理，写出所有权和失效协议。
...
3. **问题 3：template c++ 与 stl。** 对运行期多态和模板策略实现同一过滤器，列出接口、扩展、诊断和成本差异。
