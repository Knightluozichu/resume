...
<Term def="最派生对象内部代表某个基类或数据成员的独立对象部分。">
  subobject lifetime
</Term>
...
<Term def="对象所有层级成员完成初始化并建立最终不变量之后的正常运行阶段。">
...
<Term def="构造函数调用普通 helper，helper 内再调用 virtual，从而隐藏真实动态调用点的路径。">
...
<Term def="构造尚未完成时把 this 指针发布给外部代码，使外部可能调用 virtual 或读取未初始化状态。">
  this escape
</Term>
...
<Term def="public non-virtual 函数负责前后条件，并调用 private/protected virtual hook 的模式。">
  NVI idiom
</Term>
...
<Term def="由派生类 override、供基类算法在正常运行期定制某一步的 virtual 函数。">
  virtual hook
</Term>
...
<Term def="最派生析构开始后，派生成员逐步销毁，virtual dispatch 随之退回当前基类层。">
...
<Term def="派生资源已经释放后，基类析构仍尝试依赖派生 override 完成清理的错误。">
...
<Term def="由派生类在进入基类构造前计算，并作为普通值传给基类建立状态的数据。">
  derived construction data
</Term>

static helper 没有 this，不能误读未初始化成员。Base 获得完整值后使用非虚逻辑。

<Term def="基类仅依赖构造参数和已完成 base/member 的普通函数，不要求派生 override 的初始化。">
...
<Term def="对象完整构造后由 factory 或 owner 调用、可安全 virtual dispatch 的启动阶段。">
  post-construction start
</Term>

start 失败时 candidate unique_ptr 自动销毁完整对象，不发布半启动实例。

<Term def="对象只在全部构造和启动成功后才移动给外部 owner 的发布规则。">
  publish-after-start
</Term>
...
<Term def="对象从 Constructed、Starting、Started 到 Failed/Stopped 的显式生命周期状态机。">
...
<Term def="start 中部分动作成功后后续失败，系统必须撤销已提交副作用的责任。">
  startup rollback
</Term>
...
<Term def="把对象回调或 this 注册到事件源，使事件源未来可调用该对象的操作。">
...
<Term def="回调可能执行前，对象状态与订阅句柄都已提交且对线程可见的顺序保证。">
...
<Term def="从 constructor/destructor 出发追踪 helper、callback、NVI 和注册调用，查找 virtual dispatch 的过程。">
