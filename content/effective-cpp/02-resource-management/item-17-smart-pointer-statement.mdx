...
<Term def="函数调用中为每个实参计算值并执行其子表达式的过程。">
  function argument evaluation
</Term>

<Term def="new 成功到 smart pointer constructor 完成接管之间，资源暂时没有 owner 的区间。">
  ownership gap
</Term>
...
<Term def="异常传播时只销毁已经完成构造的自动对象，尚未构造的 smart pointer 不参与清理。">
  constructed-owner requirement
</Term>

RAII 只有 owner 真正存在后才生效。

<Term def="resource allocation 成功后因后续实参抛异常而永远丢失 raw pointer 的泄漏。">
...
<Term def="一个完整表达式结束点，所有 value computation 与 side effects 必须完成后才进入下一语句。">
...
<Term def="在单独完整语句中完成 raw resource 到 smart pointer owner 的交接。">
...
<Term def="分配构造对象并直接返回 unique_ptr、不给调用方暴露 raw new 的标准 factory。">
  make_unique
</Term>
...
<Term def="函数签名和调用表达式明确区分临时借用与取得 unique ownership。">
...
<Term def="分配构造对象并返回 shared_ptr，通常把对象和 control block 放入一次 allocation。">
  make_shared
</Term>
...
<Term def="shared_ptr/weak_ptr 保存引用计数和 deleter 等共享状态的结构。">
  control block
</Term>
...
<Term def="对象生命周期已结束，但包含 control block 的合并存储因 weak observers 仍存在而延迟归还。">
  combined-allocation retention
</Term>

大对象且 weak_ptr 长寿命时，分离 `shared_ptr(new T)` 可能更早释放对象存储，代价是两次分配。

<Term def="strong owner 归零时对象销毁，但 weak control state 继续存在直到所有 weak observer 消失。">
...
<Term def="shared/unique owner 析构时用于替代普通 delete 的释放策略对象。">
  custom deleter
</Term>
...
<Term def="显式 smart pointer constructor 将 raw resource 与特定 deleter 一次绑定的操作。">
...
<Term def="内部完成资源创建与 owner 绑定，只向调用方返回 smart pointer 的函数。">
  owning factory
</Term>
...
<Term def="factory 内 raw resource 从创建到 owner 绑定之间不执行任何可抛业务操作的局部规则。">
...
<Term def="C++17 起函数各实参求值彼此 indeterminately sequenced，不再交错子表达式，但先后顺序仍未指定。">
...
<Term def="两个求值必有一个先完整发生，但标准不指定哪一个先的关系。">
...
<Term def="一个函数调用需要多个新资源，每个资源都必须在调用前先进入独立 owner 的场景。">
...
<Term def="依次建立每个 owner，前一步完成后再进行下一个可能失败操作的资源准备顺序。">
...
<Term def="同一 raw pointer 被分别交给两个 shared_ptr constructor，形成两个独立 control block 的错误。">
...
<Term def="对象从现有 control block 获取 shared owner，而不是从 this 新建 control block 的机制。">
...
<Term def="记录 raw acquire、owner construction、其他实参和 release 的顺序与失败点的测试。">
