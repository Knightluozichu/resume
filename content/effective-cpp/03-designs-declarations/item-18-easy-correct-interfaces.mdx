...
import {
  EcppItem18InterfaceContractMap,
  EcppItem18OwnershipBoundaryLab,
...
<Term def="调用形式看似合理、能够编译，却表达了错误语义或违反领域规则的用法。">
  interface misuse
</Term>

参数名只对阅读声明的人有帮助，调用点只剩三个整数。注释、文档和 code review 可以降低概率，却不能封闭错误路径。

<Term def="调用者为了完成常见正确操作所需记忆、检查和手工配对的总负担。">
  caller burden
</Term>
...
<Stepper>
  <Step title="第一步：把不同语义拆成不同类型">
    <EcppItem18InterfaceContractMap />
  </Step>
  <Step title="第二步：在 factory 边界建立不变量">
    <EcppItem18ValidationBoundaryDiagram />
  </Step>
  <Step title="第三步：让 owner 携带跨模块释放责任">
    <EcppItem18OwnershipBoundaryLab />
  </Step>
</Stepper>
...
<Term def="为领域概念建立独立类型，使语义不同但底层表示相同的值不能被意外互换。">
  strong type
</Term>
...
<Term def="由编译器根据参数和返回类型强制检查、调用者无需靠记忆维持的契约。">
...
<Term def="对象一旦成功构造，在整个可观察生命周期内都必须保持成立的领域条件。">
  class invariant
</Term>
...
<Term def="只有验证成功才发布领域对象，把任意原始输入留在系统边缘的构造入口。">
  validating factory
</Term>

日期还存在“二月没有 30 日”这种跨字段约束。可以让 `Date::create` 集中验证，而不是让每个业务函数猜测对象是否合法。

<Term def="只有在所有组成字段联合满足规则后才允许产生对象的构造策略。">
...
<Term def="无需额外标志、特殊顺序或调用者知识即可完成常见安全操作的主入口。">
  safe default path
</Term>

高级逃生口可以存在，但应在名称和类型上明确，例如 `unsafeFromUncheckedBytes`，并限制到基础设施层。不要让普通调用和绕过验证共享同名重载。

<Term def="为少数底层场景显式暴露、名称清楚且作用域受控的非默认低层入口。">
...
<Term def="同类操作在命名、参数顺序、返回值、错误处理和生命周期规则上遵循相同约定。">
...
<Term def="调用者可把已有库和项目经验迁移到新接口，而无需重新学习局部规则。">
  convention transfer
</Term>
...
<Term def="通过返回类型直接编码 ownership、可空性、错误和释放方式的接口设计。">
...
<Term def="创建函数在成功返回前已经把资源绑定到正确 owner 和释放策略。">
...
<Term def="资源的创建函数与唯一正确释放函数组成的不可拆分协议。">
...
<Term def="资源必须由创建它的动态模块使用同一 allocator 和 ABI 配套释放的边界规则。">
...
<Term def="携带创建模块提供的 destroy 回调，使 owner 析构时回到原模块释放的删除策略。">
...
<Term def="调用方看不到对象布局，只通过稳定函数表或 C API 操作的跨模块句柄。">
  opaque handle
</Term>
...
<Term def="同一抽象层对可恢复失败、编程错误和不可恢复故障采用稳定一致的表达方式。">
...
<Term def="构造或操作失败时不向调用者暴露只完成部分初始化、责任不明确的对象。">
...
<Term def="通过适配、弃用诊断、调用点清单和删除期限，把用户从危险入口迁往安全入口。">
...
<Term def="刻意编写应当无法编译、无法构造或必须返回错误的调用，用来验证误用防线。">
...
<Term def="记录资源创建模块、owner 转移、借用和最终释放模块的可核查事件序列。">
