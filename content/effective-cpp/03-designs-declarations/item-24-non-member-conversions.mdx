...
<Term def="只需一个参数即可调用、且未标记 explicit，因此可用于用户定义隐式转换的 constructor。">
  converting constructor
</Term>

数学乘法的 operands 地位相同，但 member function 把左 operand 隐藏为 `this`。这改变了候选函数建立与参数转换的规则。

<Term def="表达式中编译器收集所有可能调用函数并判断参数能否绑定的过程。">
...
<Term def="编译器在函数参数绑定中插入一次用户定义 constructor 或 conversion function。">
...
<Term def="函数调用中除隐式对象参数外、显式写在参数列表中的参数。">
...
<Term def="member function 调用中由 this 表示、决定在哪个 class scope 查找成员的左侧对象。">
  implicit object parameter
</Term>

编译器不会为了发现一个尚不存在的 member candidate，先猜测把 receiver 转成任意用户类型。显式写 `Rational(2) * oneHalf` 才能成立。

<Term def="左右领域地位相同，却因一个 operand 被编码为 this 而获得不同转换规则。">
...
<Term def="根据实参类型关联 namespace/class 补充非成员候选的查找机制。">
...
<Term def="由实参类型确定、供 ADL 搜索函数声明的 namespace 与 class 集合。">
...
<Term def="非成员函数仅使用 public contract，不获得读取私有表示的额外权限。">
...
<Term def="在无法通过稳定 public API 正确高效实现时，向特定 non-member 授予的最小私有权限。">
...
<Term def="不丢失信息、没有单位歧义且保持领域不变量的自动类型转换。">
...
<Term def="必须在调用点写明 constructor 或 named factory，避免编译器静默插入的转换。">
...
<Term def="修改 receiver 状态并保持其 identity 的 compound assignment operation。">
...
<Term def="用按值 lhs 副本调用核心 compound assignment，再按值返回结果的二元运算实现模式。">
...
<Term def="为特定 operand 组合提供显式函数声明，不依赖通用隐式构造。">
  targeted mixed-type overload
</Term>

overload 可做范围检查并定义返回类型。代价是组合数量增加，需要避免 ambiguity 和重复实现。

<Term def="多个 overload 对同一表达式具有同等 conversion rank，编译器无法选择唯一候选。">
  overload ambiguity
</Term>
...
<Term def="在 class definition 中定义、不是 member、通常只通过 ADL 参与候选的 friend function。">
  hidden friend
</Term>
...
<Term def="class 与获权 operator 必须共同审查 representation 变化的维护范围。">
...
<Term def="从调用实参直接推断模板参数的阶段，该阶段通常不执行用户定义隐式转换来完成推导。">
...
<Term def="在推导、conversion 和 overload resolution 三阶段分别验证候选可行性的泛型接口设计。">
...
<Term def="按 lhs/rhs 类型组合记录候选、转换、结果与预期编译状态的测试表。">
