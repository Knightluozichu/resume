...
<Term def="reference、pointer 或 iterator 等能够间接访问另一个对象的值。">
  object handle
</Term>

Item 28 的原则是 Avoid returning handles to object internals（避免返回指向对象内部的句柄）。

<Term def="public API 把 private subobject 的地址、别名或迭代位置交给调用者。">
...
<Term def="const member function 中 this 所提供的只读访问限制，不会传播到任意显式返回别名。">
  const this access path
</Term>

返回 `Point&` 是重新授予写权限。调用者可绕过 Rectangle 对角点顺序、边界和同步的规则。

<Term def="通过 owner public operation 之外的别名修改内部状态，跳过验证与原子更新。">
...
<Term def="const object 在用户可观察语义上保持值不变，而非只要求字段 bitwise 不写。">
...
<Term def="保存容器内部位置、可解引用访问 element 的遍历句柄。">
  iterator handle
</Term>
...
<Term def="public handle 迫使调用者和实现共同依赖内部容器、布局与失效规则。">
...
<Term def="不能修改 pointee、但仍借用 owner 内部 storage 的返回别名。">
  read-only internal borrow
</Term>

若 owner 销毁、替换 data\_ 或 copy-on-write detach，引用可能失效。

<Term def="使先前返回的 handle 不再指向原合法对象的 owner 操作或生命周期事件。">
...
<Term def="所借用 subobject 已结束生命周期，但 reference/pointer/iterator 仍保存其地址的句柄。">
  dangling handle
</Term>

返回 borrow 不延长完整对象生命周期。shared pimpl 也不能自动救场：若 handle 只是 Point reference，没有携带 shared owner，RectData 仍可随临时 Rectangle 释放。

<Term def="完整表达式结束时销毁未被合法 lifetime extension 保留的临时对象。">
...
<Term def="根据调用对象是 lvalue 还是 rvalue 选择或禁止成员函数的限定。">
...
<Term def="从 owner 内部值复制或移动出的独立对象，不受 owner 后续生命周期和失效影响。">
...
<Term def="容器更换 backing storage 后，原 element 地址、reference 与 iterator 不再有效。">
  reallocation invalidation
</Term>

reserve 能降低概率但不是永久 contract；insert/erase/sort 也有各自失效或 identity 变化规则。

<Term def="某类 handle 在哪些修改后仍有效、哪些操作后必须废弃的接口约定。">
...
<Term def="返回 handle 时保护锁已释放，后续解引用与 owner mutation 并发发生的漏洞。">
  escaped-lock handle
</Term>
...
<Term def="数据访问必须与保护锁或版本验证处于同一不可分割作用域的要求。">
...
<Term def="由 owner 验证并原子更新 internal state 的领域操作，而不是发布 mutable alias。">
  owner-mediated command
</Term>

owner 可以加锁、维护 cache、记录事件并拒绝非法状态。

<Term def="所有 internal mutations 都经过 owner 明确 operation，从而可枚举和验证。">
...
<Term def="不直接包含内部地址、每次使用由 owner 验证并解析的逻辑标识。">
...
<Term def="ID 中用于识别 slot 已被删除并复用、阻止旧 handle 命中新对象的版本值。">
  generation counter
</Term>
...
<Term def="按 handle 类型列出 owner 事件、有效性、写权限、同步与替代契约的表。">
