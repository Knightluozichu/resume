...
<Term def="derived object 在任何要求 base contract 的位置都能合法替换 base object 的公开继承关系。">
  public inheritance is-a
</Term>

Item 32 的原则是 Make sure public inheritance models is-a（确定 public 继承塑模 is-a）。这里 is-a 不是日常语言的“看起来属于”，而是程序行为的严格承诺。

<Term def="客户程序只依赖 base contract 时，将对象替换为 derived 后仍保持正确。">
...
<Term def="base type 对合法输入、结果、状态、错误和复杂度给客户的可依赖说明。">
...
<Term def="derived operation 不能要求比 base operation 更多的输入或对象状态条件。">
  precondition contravariance rule
</Term>

<Term def="derived operation 的结果保证不能少于 base，必要时可以更具体或更强。">
...
<Term def="operation 成功返回后，客户可以断言成立的结果与未变化状态。">
...
<Term def="只使用 base public API，并依赖其公开前置/后置条件的程序。">
  valid base client
</Term>
...
<Term def="derived 对 base 合法操作以异常、错误码或 no-op 表示“不支持”的设计气味。">
...
<Term def="所有继承到 derived public interface 的 operations 对其合法状态都必须有一致语义。">
...
<Term def="现实世界按属性或名称建立的分类，不保证程序操作集合可替换。">
  domain taxonomy
</Term>
...
<Term def="抽象类型定义的可执行能力集合，而不仅是实体标签。">
...
<Term def="只声明一种可选行为，让实现者明确选择是否满足该契约的小接口。">
  capability interface
</Term>

调用 fly 的客户依赖 Flyable，而非 Bird；类型系统拒绝 Penguin。

<Term def="客户端只依赖完成任务所需最小操作集合的设计。">
...
<Term def="为了获得已有字段或函数实现而建立关系，但没有行为替换语义。">
...
<Term def="类型拥有另一个对象并通过其 public API 复用能力的 has-a 结构。">
  composition has-a
</Term>
...
<Term def="base 指针/引用管理 derived 时，销毁、复制、移动和借用仍安全的要求。">
...
<Term def="derived 引入资源或缓存后仍维持 base 声明的并发与 ownership 行为。">
...
<Term def="只依据 base public contract 编写，并对所有 derived factories 重复执行的测试集合。">
  base contract suite
</Term>

<Term def="同一套客户行为测试在每个 subtype 上成立的自动化 substitutability 证据。">
...
<Term def="按 base operation 记录各 derived 前置、后置、不变量、失败与测试证据的表。">
