...
<Term def="客户直接调用、derived 通过 override 替换实现的公开虚函数。">
...
<Term def="在 inheritance override 之外表达可替换行为的 interface、callable 或组合机制。">
...
<Term def="public non-virtual function 固定流程，并调用 private/protected virtual hook 的模式。">
  NVI idiom
</Term>

<Term def="只允许 derived override、由 base wrapper 控制调用时机的虚函数。">
...
<Term def="跨所有 implementations 都必须一致执行的 validation、同步、审计与观测逻辑。">
  cross-cutting contract logic
</Term>

derived 只返回 raw health，不会忘记 clamp 或 audit。

<Term def="base 在 hook 前后执行并对全部 derived 生效的条件检查。">
  invariant envelope
</Term>
...
<Term def="访问权限控制调用表达式，virtual 机制独立决定 dispatch target 的关系。">
...
<Term def="base 独占 hook orchestration、derived 只提供 body 的 customization boundary。">
...
<Term def="保存普通函数地址并以固定 signature 间接调用的可替换算法机制。">
  function pointer strategy
</Term>

同一 class 的两个 objects 可选不同 calculator，无需创建 derived types。

<Term def="行为在 object construction/configuration 时选择，而不是由 dynamic class type 决定。">
...
<Term def="以统一 signature 保存并调用不同 concrete callable types 的类型擦除 wrapper。">
...
<Term def="保存环境值并参与函数调用的 lambda/functor 状态。">
...
<Term def="type-erased callable 在内部小缓冲可容纳时避免动态分配的优化。">
...
<Term def="只包含算法所需、与 GameCharacter private representation 解耦的输入值。">
  strategy input context
</Term>

GameCharacter 在锁内生成 context，再在适当同步策略下调用 calculator。

<Term def="算法只依赖公开稳定 context，而不直接读取 owner private fields。">
...
<Term def="把可替换算法封装为独立对象，通过统一 interface 被 context/owner 组合。">
  strategy pattern
</Term>

<Term def="算法自身拥有参数、缓存、资源与生命周期，而非散落在 character subtype。">
...
<Term def="GameCharacter 与算法对象之间的独占、共享或借用生命周期契约。">
...
<Term def="多个 owners 共享同一不可变策略，从而安全复用配置和实现。">
...
<Term def="对象存活期间以新算法替换当前 strategy 的 operation。">
...
<Term def="准备完整新策略后以不可失败 owner swap 发布，失败保留旧策略。">
...
<Term def="为测试注入可观察、可控结果的 callable/strategy，而不创建无业务意义 derived class。">
  algorithm test double
</Term>

Strategy object 也可跑统一 contract suite，验证返回范围、determinism 和 failure behavior。

<Term def="对所有 calculator implementations 重复执行的输入、输出与错误行为测试。">
...
<Term def="通过 template parameter 在编译期组合算法，允许内联且不需要 virtual/type erasure。">
  compile-time policy strategy
</Term>

代价是每个 policy 产生不同 static type、可能 code bloat、实现需 header 可见，不能轻易 runtime replace。

<Term def="编译期 specialization 用更多类型和实例化换取静态分派优化的权衡。">
...
<Term def="按变化轴、contract envelope、state、ownership、dispatch 和测试选择机制的表。">
...
<Term def="决定算法变化是在 class、object、runtime configuration 还是 template instantiation 发生的维度。">
...
<Term def="记录每次 health 调用经过 wrapper、context、callable/strategy、归一化与返回的事件序列。">
