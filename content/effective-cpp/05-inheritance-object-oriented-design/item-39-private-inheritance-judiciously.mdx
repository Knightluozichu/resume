...
<Term def="base 的 public 与 protected 接口在 derived 中变为私有实现，外部不能进行公开向上转换的继承方式。">
  private inheritance
</Term>

Item 39 的原则是 Use private inheritance judiciously（审慎使用 private 继承）。

<Term def="借用另一个类型的实现能力，却不向客户承诺可替换关系。">
...
<Term def="derived 到 base 的标准转换因继承访问级别而对普通客户不可用。">
...
<Term def="derived 成员与 friends 可以访问 private base 的 public/protected 部分的实现权限。">
...
<Term def="owner 仅通过成员的公开接口复用能力，不依赖其 protected 或继承布局的关系。">
...
<Term def="derived 直接依赖 base protected 成员、virtual hooks 和对象布局而形成的紧密实现关系。">
...
<Term def="只有 base、derived 与其 friends 可访问的实现扩展点或状态。">
  protected implementation access
</Term>

<Term def="derived 为 base virtual hook 提供运行期行为的覆盖能力。">
...
<Term def="由私有辅助类型承担 base 协议，再把事件翻译给 owner 的组合结构。">
  nested inheritance adapter
</Term>

<Term def="把继承依赖限制在最小辅助角色，而不让主要领域类型承担 base 身份。">
...
<Term def="adapter 保存 owner 引用时，owner 必须覆盖 adapter callback 全部执行期的约束。">
...
<Term def="没有非静态数据成员、virtual functions 或 virtual bases 的轻量策略类型。">
  empty class
</Term>
...
<Term def="实现允许空 base subobject 不占额外空间，从而避免 empty member 地址和 padding 开销的对象布局优化。">
...
<Term def="用 sizeof、alignment、编译目标和性能测试验证布局假设的证据。">
  layout measurement
</Term>
...
<Term def="允许 potentially-overlapping empty member 与其他成员共享地址空间的 C++20 属性。">
...
<Term def="从替换语义、组合可行性、扩展协议和布局证据逐层缩小 private inheritance 使用范围的方法。">
