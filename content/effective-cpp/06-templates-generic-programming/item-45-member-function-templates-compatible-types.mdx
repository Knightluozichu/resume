...
<Term def="编译器允许从源类型安全形成目标类型、无需显式 cast 的类型关系。">
  compatible type conversion
</Term>

<Term def="类模板内部自身也是 template、可为不同调用参数独立推导类型的成员函数。">
...
<Term def="source 和 destination 拥有相同 class template arguments 的复制。">
  same-specialization copy
</Term>

若为每一对层级手写 overload，类型数量增长后转换边会组合爆炸，也无法覆盖用户未来定义的 derived types。

<Term def="为每一组源目标类型人工声明转换 overload 导致的组合增长。">
...
<Term def="以另一个 specialization 为参数、可在不同 element types 间构造的成员模板。">
  generalized copy constructor
</Term>
...
<Term def="通过 U pointer 初始化 T pointer，让语言内建转换规则决定跨 specialization 是否成立。">
...
<Term def="从 derived element wrapper 转为 base element wrapper 的安全跨类型构造。">
  smart pointer upcast
</Term>

<Term def="通过 wrapper 转换移除底层对象 const 限定、必须拒绝的行为。">
...
<Term def="在 overload candidate 边界声明源 pointer 必须可隐式转为目标 pointer 的约束。">
...
<Term def="用编译期 traits 验证哪些 source/destination specialization 可以构造的测试。">
...
<Term def="允许同一 class template 的不同 specializations 访问彼此 private representation 的友元声明。">
...
<Term def="不同 element-type wrappers 指向同一资源时必须共享同一控制记录与销毁次数。">
...
<Term def="跨类型复制时增加同一 control block 的引用计数，而不是重新接管 raw pointer。">
...
<Term def="参数为本类引用的非模板特殊成员，控制同类型对象复制。">
  ordinary copy constructor
</Term>

<Term def="constructor template 不抑制 compiler 隐式声明普通 copy constructor 的规则。">
...
<Term def="在兼容 element types 的 smart pointer specializations 间进行赋值的成员模板。">
...
<Term def="候选质量相同时，普通函数通常优先于函数模板的 overload selection 规则。">
...
<Term def="wrapper 转换是否允许由 compiler 自动插入的 API 决策。">
...
<Term def="覆盖同类型、继承、cv、访问与 ownership 的正反跨类型检查表。">
