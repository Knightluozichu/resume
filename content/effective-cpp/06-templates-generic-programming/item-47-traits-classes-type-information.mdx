...
<Term def="与某个类型相关、供泛型算法在编译期查询的属性集合。">
  type information
</Term>

<Term def="通过 class template 及 specializations 把类型属性映射为 nested types 或 constants 的结构。">
  traits class
</Term>
...
<Term def="以空类型表示 iterator 能力类别，用于 overload resolution 而非保存数据。">
  iterator category tag
</Term>

<Term def="类型在自身定义中公开、描述其关联类型或属性的名称。">
  nested trait member
</Term>
...
<Term def="无法修改或没有 class body 来提供 nested traits 的既有类型，如 raw pointer。">
...
<Term def="处理常规类型、通常把查询转发到类型自身 nested members 的 traits 定义。">
...
<Term def="为一族仍含自由参数的模板实参形状提供更具体定义，如所有 T pointers。">
  partial specialization
</Term>

<Term def="通过 traits specialization 为外部或内建类型提供类型信息，而无需修改目标类型。">
...
<Term def="把类型属性实例化为空 tag 参数，利用 overload resolution 选择算法实现。">
  tag dispatch
</Term>

<Term def="针对特定 capability 提供操作和复杂度更合适的内部 overload。">
...
<Term def="公开统一接口查询 trait，再把具体 category 传给内部 overload set 的薄函数。">
...
<Term def="更强 category tag 可隐式转换为较弱 tag，从而复用基础实现的继承结构。">
  capability tag hierarchy
</Term>

output iterator 与 input 链表达不同写入语义，不应为了方便强行纳入同一可读层级。

<Term def="类型声明的 tag 必须真实反映其 operations、multi-pass 与复杂度保证。">
...
<Term def="traits 中用 static constexpr 暴露、可参与编译期选择的值属性。">
  trait value member
</Term>
...
<Term def="谁有权为哪些类型提供 traits specialization 的 API 与标准规则。">
...
<Term def="根据 compile-time condition 丢弃未选择 branch，使无效 expressions 不被实例化的语句。">
...
<Term def="通过所需 expressions 和语义要求直接描述类型能力的命名约束。">
  capability concept
</Term>
...
<Term def="从信息命名、主模板、特化、消费到契约测试的 traits 建模步骤。">
