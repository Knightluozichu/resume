...
<Term def="利用 templates、specializations 和实例化规则在编译期执行计算或生成类型与代码的技术。">
  template metaprogramming
</Term>

<Term def="程序运行前由 compiler 根据静态输入求出值、类型或实现选择。">
...
<Term def="template definition 为解决当前问题继续请求更小参数 specialization 的展开。">
  recursive instantiation
</Term>

<Term def="为特定 arguments 提供独立定义，在 TMP 中常承担递归终止或条件分支。">
...
<Term def="compiler 把由模板常量表达式组成的结果归约为最终 compile-time value。">
...
<Term def="根据 compile-time boolean 在两个候选 types 之间选择结果的元函数。">
  type selection
</Term>

<Term def="接收 types/constants 并通过 nested type 或 value 返回编译期结果的模板结构。">
  metafunction
</Term>
...
<Term def="根据静态类型属性生成仅包含适用路径的具体实现。">
  custom code generation
</Term>

<Term def="把原本运行期重复执行的类型判断或组合选择转为编译期工作。">
...
<Term def="由 template arguments 选择锁、检查、分配等正交行为的编译期配置。">
  policy choices
</Term>

<Term def="compiler 为一组 policy arguments 形成只包含对应行为的具体 class。">
...
<Term def="把物理量维度编码到 type arguments，使不兼容单位无法参与错误运算。">
...
<Term def="通过类型系统在程序运行前拒绝违反领域规则的表达式。">
...
<Term def="用嵌套模板类型记录尚未执行的运算结构，在最终消费点统一求值。">
  expression template
</Term>

<Term def="不立即创建中间结果，而把计算推迟到目标上下文的策略。">
...
<Term def="元程序递归展开的层数，过深会触发 compiler limit 或高资源消耗。">
  template instantiation depth
</Term>

<Term def="同一构建中形成的不同 template specializations 数量。">
...
<Term def="可在常量求值中使用普通变量、循环和分支的函数。">
  constexpr computation
</Term>

<Term def="要求每次调用都在 compile time 求值的函数声明。">
  consteval boundary
</Term>
...
<Term def="用命名 constraints 把模板合法输入边界前移并缩短诊断。">
...
<Term def="从静态输入、收益、替代方案、构建预算和诊断共同判断元编程必要性。">
