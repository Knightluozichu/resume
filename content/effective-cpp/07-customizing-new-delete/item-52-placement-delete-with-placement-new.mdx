...
<Term def="除 size 外还接收一个或多个额外参数的 operator new allocation function。">
  placement new
</Term>

<Term def="参数列表与某个 placement new 额外参数对应、供构造失败回收 storage 的 operator delete。">
  placement delete
</Term>
...
<Term def="allocation 已成功但 object construction 未完成并抛出异常的路径。">
  constructor exception
</Term>

<Term def="构造失败后没有匹配 deallocation function，已取得 raw storage 无法自动归还。">
...
<Term def="placement new 的额外参数序列在 placement delete 中按对应类型重现的规则。">
...
<Term def="对象成功构造后由 delete expression 选择、不接收原 placement 额外参数的释放路径。">
...
<Term def="placement delete 仅在对应 new expression 构造失败时由语言自动考虑的作用域。">
...
<Term def="接收 void pointer 并直接在调用者提供 storage 上构造对象的标准 placement form。">
  standard placement form
</Term>

<Term def="placement construction 不取得 buffer ownership，失败时只销毁已构造成员而不释放外部 storage。">
...
<Term def="同一 class 提供的 normal、placement、nothrow、sized 与 aligned allocation/deallocation 函数集合。">
...
<Term def="class scope 中任一同名 operator new/delete declaration 阻止外层或 base overloads 自动参与查找。">
...
<Term def="在 class 中转发 global standard overloads，同时添加自定义 placement pair 的方案。">
...
<Term def="用 using declaration 把 base allocation overload set 重新引入 derived class scope。">
...
<Term def="placement extra arguments 在 allocation、construction 和 failure cleanup 全程保持有效的要求。">
  placement argument lifetime
</Term>

logging delete 自己必须 noexcept，且错误日志路径最好不动态分配，否则原 constructor exception unwind 中又触发失败。

<Term def="placement delete 在处理原构造异常时不得再抛出第二个异常的约束。">
...
<Term def="placement allocation 是否取得并负责归还 storage 的明确策略。">
...
<Term def="覆盖构造成功、各阶段失败、名称查找和 storage ownership 的 placement 测试表。">
