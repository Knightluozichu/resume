...
import { Ejv3e01IntroductionMapLab, Ejv3e01IntroductionExperimentLab, Ejv3e01IntroductionEvidenceLab } from "@/components/mdx/effective-java/diagrams/ejv-3e-01-introduction";
...
- 能解释“第1章 导言”在第3版12章、90条Item与105个正式节点中的位置
...
<Stepper>
  <Step title="先画情境、方案与决策链"><Ejv3e01IntroductionMapLab /></Step>
  <Step title="再改变公开表面积、可变性与调用方"><Ejv3e01IntroductionExperimentLab /></Step>
  <Step title="最后比较原方案、采用Item与反例"><Ejv3e01IntroductionEvidenceLab /></Step>
</Stepper>
...
<Callout type="trap">**成功样本替代边界裁决。** 只背结论，不记录适用前提、替代方案、代价和推翻结论的反例。至少加入错误输入、可变别名、版本升级、并发交错和调用方迁移中的适用项。</Callout>
...
**问题 1**：如何为“第1章 导言”建立第一张Item决策记录？

<Answer>选择最大风险，声明调用方、Java版本和长期合同，列出原方案、建议方案与替代方案；先预测，再运行客户端、边界反例和回滚测试。结果进入Item决策记录模板、Java 9基线与Java 25复核清单。</Answer>

**问题 2**：为什么“遵守Item”仍不足以证明设计有效？

<Answer>建议带有语境和代价，可能与其他Item、现代Java特性或项目约束冲突。需要公开收益承担者、成本承担者、失败语义和反例，而不是机械同意。</Answer>

**问题 3**：遇到“只背结论，不记录适用前提、替代方案、代价和推翻结论的反例”时怎样返工？
