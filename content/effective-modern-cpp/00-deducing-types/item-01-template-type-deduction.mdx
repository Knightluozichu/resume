...
<Term def="从函数实参 expression 与参数模式反推出 template parameter T 的过程。">
  template type deduction
</Term>

<Term def="函数模板声明中包含 T 的完整参数类型模式，如 T reference、const T reference 或 T。">
  ParamType pattern
</Term>
...
<Term def="推导普通 reference 参数时，先忽略 expression type 自带的 reference，再按模式匹配剩余类型。">
  reference stripping for matching
</Term>

rx 的 declared type 是 const int reference，但 expression 参与推导时先去掉 reference 层；const 仍保留在 T。

<Term def="直接修饰对象本身、对按值复制后的新对象不再生效的 const/volatile。">
  top-level cv
</Term>

<Term def="修饰 pointer 指向对象或成员所指对象、属于复合类型内部的 const/volatile。">
  low-level cv
</Term>
...
<Term def="T 由调用实参推导且参数恰为 T double ampersand，可同时绑定 lvalue 和 rvalue 的引用。">
  universal reference
</Term>
...
<Term def="universal reference 接收 lvalue 时，template parameter T 自身被推导为 lvalue reference 的规则。">
  lvalue-reference deduction
</Term>

<Term def="多层 reference 组合时，只要任一层是 lvalue reference，最终结果即 lvalue reference。">
...
<Term def="参数是独立复制对象而非 reference/pointer 模式的推导情形。">
  by-value deduction
</Term>
...
<Term def="按值推导时移除 expression 最外层 const/volatile，而保留复合类型内部限定。">
...
<Term def="pointer 自身是否可改指向与其 pointee 是否可修改的两层独立限定。">
  pointer cv layers
</Term>
...
<Term def="array expression 传给 by-value parameter 时转换为首元素 pointer。">
...
<Term def="function name 传给 by-value parameter 时转换为 function pointer。">
...
<Term def="先从参数模式形成 specialization，再对已确定函数参数执行允许转换的顺序。">
...
<Term def="根据复制、identity、cv、value category 和 lifetime 需求选择参数模式的接口决策。">
...
<Term def="把 expression facts、参数模式、T、最终 ParamType 和 decay 分步记录的表。">
  deduction worksheet
</Term>
