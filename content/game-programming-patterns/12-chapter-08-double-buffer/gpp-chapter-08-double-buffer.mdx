...
- <Term term="后台缓冲">**后台缓冲（back buffer）**</Term>：正在被写入的缓冲，读者看不到——写方的私有工作区。
- <Term term="显示缓冲">**显示缓冲（front buffer）**</Term>：正在被读取显示的缓冲，内容总是完整的。
- <Term term="交换">**交换（swap）**</Term>：把后台与显示缓冲的角色一次性互换——读者瞬间看到新帧。
- <Term term="撕裂">**撕裂（tearing）**</Term>：读写同一块缓冲导致的画面错位——双缓冲要消灭的故障。
...
双缓冲误区：用内容拷贝实现 swap——O(N) 拷贝吃掉性能，应交换指针 O(1)。

</Callout>

<Callout type="trap" title="易错点">

交换期间读者写入的坑：swap 不是原子——读者在交换中写缓冲。

</Callout>

<Callout type="trap" title="易错点">

缓冲区固定分配但对象需动态大小的误区：越界——按最大值分配。
...
<Glossary>
  ...
  <GlossaryItem term="显示缓冲" />
  <GlossaryItem term="交换" />
</Glossary>
