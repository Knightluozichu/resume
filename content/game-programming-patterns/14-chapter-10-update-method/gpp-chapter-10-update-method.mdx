...
- <Term term="更新方法">**更新方法（update method）**</Term>：实体上的 `update(dt)` 方法——每帧被调用一次，推进该实体一段行为。
- <Term term="时间步">**时间步（dt）**</Term>：更新方法的参数，表示本次更新推进的模拟时间量——保证行为与帧率无关。
- <Term term="游戏世界">**游戏世界（game world）**</Term>：持有实体列表、每帧遍历并逐个调用 update 的容器。
- <Term term="逐帧片段">**逐帧片段（frame slice）**</Term>：把"巡逻 3 秒"这类长行为切成每帧一小段——update 里推进一小步，跨帧累积。
...
Update Method 的坑：遍历实体列表时直接增删——迭代器失效踩空，应延迟修改。

</Callout>

<Callout type="trap" title="易错点">

实体在 update 中 delete 自己的坑：栈上 this 悬空——延迟销毁。

</Callout>

<Callout type="trap" title="易错点">

休眠实体仍在列表的误区：遍历开销白付——长休眠要移出。
