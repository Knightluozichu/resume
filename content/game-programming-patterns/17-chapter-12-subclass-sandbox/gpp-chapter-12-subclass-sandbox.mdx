...
- **沙箱基类（sandbox base class）**：提供一组受保护操作（playSound/spawnParticle/damage）的基类——子类唯一能碰引擎的通道。
- **子类（subclass）**：继承基类、实现具体行为（activate()）的能力类——只调用基类提供的操作。
- **受保护操作（protected operation）**：基类封装引擎调用的方法——子类调用它们，但不直接碰引擎。
- **组合（composition）**：子类把多个沙箱操作组合成完整效果——行为 = 沙箱操作的编排。
...
| 练习 | 操作 | 验证判据 |
|---|---|---|
| 波及面对比 | 模拟引擎音频 API 改名，统计基线与沙箱方案各要改几个文件 | 沙箱方案只改基类 1 个文件 → 解耦成立 |
| 能力边界 | 在动画沙箱中"新增"一个"传送"能力，只用现有操作能否表达 | 能组合则边界合理；不能则需向基类加操作 |
| 注入 vs 全局 | 分别用构造注入与 Service Locator 实现基类拿引擎，各写一个测试 | 注入版可换替身 → 注入更可测 |
