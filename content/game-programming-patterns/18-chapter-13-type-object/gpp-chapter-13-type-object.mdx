...
- **类型对象（type object）**：用运行时对象表示一种"种类"——GoblinType 存血量/速度/技能，是共享定义。
- **实例（instance）**：具体怪物，只存自身状态（位置/当前血量），引用一个类型对象。
- **共享（sharing）**：实例从类型对象读取属性——改类型对象 = 改所有该种实例。
- **数据驱动（data-driven）**：类型对象由数据定义（文件/配置）——新种类 = 新数据，不写新类。
...
| 练习 | 操作 | 验证判据 |
|---|---|---|
| 新增种类 | 在动画类型池中"新增"一个"食人魔"类型（只改数据） | 引擎零改动即可生成食人魔 → 数据驱动成立 |
| 共享验证 | 修改 GoblinType.hp 为 80，观察所有哥布林实例 | 所有实例最大血量同步变化 → 共享定义生效 |
| 类型可变实验 | 给一个实例调用 changeBreed(DragonType)，观察属性变化 | 变身生效且受控 → 类型可变受控实现 |
| 继承实验 | 让 GoblinType 继承 MonsterType 基属性，改基属性观察亚种 | 亚种属性同步更新 → 单继承有效 |
