...
- **组件（component）**：封装一个领域能力的类（输入/物理/渲染/音频），有独立的 update/render 接口。
- **实体/容器（entity/container）**：持有组件列表的对象——本身不做行为，只管理组件生命周期。
- **组合（composition）**：实体按需装配组件——能力 = 组件集，新实体 = 新组合。
- **消息转发（message forwarding）**：组件间通信经容器转发，组件互不直接引用——解耦关键。
...
| 练习 | 操作 | 验证判据 |
|---|---|---|
| 拆分类对比 | 在动画中把"会走的机器人"装配成组件集，对比单体类的代码结构 | 组件版每类不到 100 行、单体超过 500 行 → 拆分有效 |
| 组合复用 | 用同一套组件装配"玩家"与"敌人"两个实体 | 零复制、只改组件清单 → 组合复用成立 |
| 通信解耦 | 让物理组件改位置后，渲染组件经容器获知并更新 | 物理组件不认识渲染组件 → 消息转发解耦 |
| 数据配置 | 把实体组件清单写进数据文件，运行时按数据装配 | 改数据即可换实体能力 → 数据驱动装配成立 |
