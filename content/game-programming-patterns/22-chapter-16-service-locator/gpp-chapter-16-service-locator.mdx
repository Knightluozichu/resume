...
- **服务（service）**：被多个系统使用的全局能力（音频、渲染、网络），有明确的接口定义。
- **服务提供者（service provider）**：服务的具体实现——同一服务可有多套实现（真实版、null 版、日志版）。
- **定位器（locator）**：维护"服务名 → 服务实例"映射的中介——客户端通过它获取服务。
- **解耦（decoupling）**：客户端只认识服务接口与定位器，不认识具体实现——换实现只改注册。
...
| 练习 | 操作 | 验证判据 |
|---|---|---|
| 替换实验 | 在动画中把 Audio 从 ConsoleAudio 换成 LoggingAudio（只改注册） | 客户端零改动、行为切换 → 定位器解耦成立 |
| null 兜底 | 不注册任何服务直接 getAudio() 并调用 | 不崩溃、无效果 → null 服务兜底生效 |
| 测试替身 | 测试中注册一个"记录调用"的假音频，断言调用序列 | 断言通过 → 测试可注入替身 |
| 作用域对比 | 对比全局单例与每场景注册两种 scope 的实现复杂度 | 简单场景全局够用 → 别过早分层 |
