...
- **对象池（object pool）**：预分配固定数量的对象槽位，运行时只做"取-还"——不碰堆。
- **acquire/release**：从池中取空闲对象（acquire）与归还复位（release）——替代 new/delete。
- **空闲列表（free list）**：池内"空闲槽"的链表/栈——O(1) 取还的关键结构。
- **碎片化（fragmentation）**：高频 new/delete 导致堆内存零散——池化消灭它。
...
| 练习 | 操作 | 验证判据 |
|---|---|---|
| 分配对比 | 对比 new/delete 与对象池各发射一万颗子弹的耗时 | 池化版本零堆操作 → 分配开销消失 |
| 容量上限 | 让并发子弹数超过池容量，观察 acquire 失败行为 | 按策略拒绝而非崩溃 → 上限处理正确 |
| 复位验证 | release 后立即 acquire，检查对象状态是否残留旧值 | 状态干净 → 复位契约成立 |
| 空闲列表 | 实现"遍历找空闲"与"自由列表"两种 acquire，对比复杂度 | 自由列表 O(1) 显著更快 → 自由列表生效 |
