...
section: "计算机图形学与渲染"
order: 2
description: "辐射通量、辐照度、辐射亮度、BRDF"
demo: true
math: false
...
- 能用自己的话解释Radiometry的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：辐射通量与辐照度之间的关系是什么？

</Objectives>

## 为什么需要Radiometry

Global Illumination Compendium将Radiometry作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
辐射通量、辐照度、辐射亮度、BRDF正是这一哲学在特定领域的具体体现。
...
Vulkan 把Radiometry从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Radiometry涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Radiometry相关功能前，必须完成：
...
1. Radiometry是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Radiometry涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Global Illumination Compendium》（https://books.google.com/books?id=KodRAAAAMAAJ）第 2 章整理重写，
代码示例为教学简化版本。
</Attribution>
