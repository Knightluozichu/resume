...
- 能用自己的话解释Path Tracing的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：无偏路径追踪与俄罗斯轮盘之间的关系是什么？

</Objectives>

## 为什么需要Path Tracing

Global Illumination Compendium将Path Tracing作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
无偏路径追踪、俄罗斯轮盘、NEE正是这一哲学在特定领域的具体体现。
...
Vulkan 把Path Tracing从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Path Tracing涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Path Tracing相关功能前，必须完成：
...
启用验证层后，Path Tracing相关的常见错误会被捕获：
...
1. Path Tracing是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Path Tracing涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Global Illumination Compendium》（https://books.google.com/books?id=KodRAAAAMAAJ）第 8 章整理重写，
代码示例为教学简化版本。
</Attribution>
