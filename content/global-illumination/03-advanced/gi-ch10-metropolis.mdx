...
- 能用自己的话解释Metropolis Light Transport的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：MCMC与突变策略之间的关系是什么？

</Objectives>

## 为什么需要Metropolis Light Transport

Global Illumination Compendium将Metropolis Light Transport作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
Vulkan 把Metropolis Light Transport从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Metropolis Light Transport涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Metropolis Light Transport相关功能前，必须完成：
...
// 录制与Metropolis Light Transport相关的命令
// ... 具体的 vkCmd* 调用 ...
...
启用验证层后，Metropolis Light Transport相关的常见错误会被捕获：
...
1. Metropolis Light Transport是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Metropolis Light Transport涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Global Illumination Compendium》（https://books.google.com/books?id=KodRAAAAMAAJ）第 10 章整理重写，
代码示例为教学简化版本。
</Attribution>
