...
- 能用自己的话解释Precomputed Radiance Transfer的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：球谐函数与PRT之间的关系是什么？

</Objectives>

## 为什么需要Precomputed Radiance Transfer

Global Illumination Compendium将Precomputed Radiance Transfer作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
Vulkan 把Precomputed Radiance Transfer从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Precomputed Radiance Transfer涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Precomputed Radiance Transfer相关功能前，必须完成：
...
// 录制与Precomputed Radiance Transfer相关的命令
// ... 具体的 vkCmd* 调用 ...
...
启用验证层后，Precomputed Radiance Transfer相关的常见错误会被捕获：
...
1. Precomputed Radiance Transfer是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Precomputed Radiance Transfer涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Global Illumination Compendium》（https://books.google.com/books?id=KodRAAAAMAAJ）第 11 章整理重写，
代码示例为教学简化版本。
</Attribution>
