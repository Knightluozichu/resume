...
（word）是本书的基本对象。先固定$w$，再讨论mask、complement、rotate与overflow；没有width，NOT之后有多少high bits为1都没有答案。
...
与storage pattern一一对应。Arithmetic在unsigned word上按modulo $2^w$封闭：
...
等价于unsigned floor division by $2^n$。
