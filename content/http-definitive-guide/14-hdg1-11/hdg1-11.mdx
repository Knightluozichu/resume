...
description: 比较HTTP首部、IP、登录、胖URL和Cookie等客户端识别与会话机制
type: A
...
import { Hdg111MessageLab, Hdg111FlowLab, Hdg111EvidenceLab } from "@/components/mdx/http-definitive-guide/v2/hdg1-11";
...
<Term def="服务器通过响应设置并由客户端按作用域随请求返回的小型状态数据">Cookie</Term>、<Term def="把多个无状态HTTP事务关联到同一交互上下文的机制">会话跟踪</Term>、<Term def="限制Cookie可发送域范围的属性">Domain属性</Term>、<Term def="限制Cookie可发送请求路径范围的属性">Path属性</Term>、<Term def="把用户状态编码进每个页面链接URL的识别技术">胖URL</Term>
...
<Answer>
  另一位学习者无需口头补充即可定位全部18个正式节点，复现正常事务和单变量故障，并依据“状态标识的来源、作用域、持久时间、跨站边界、缓存影响与隐私风险都可追踪”得到相同结论。
</Answer>
