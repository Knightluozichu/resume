...
<Term def="服务器通过响应设置并由客户端按作用域随请求返回的小型状态数据">
  Cookie
</Term>
、<Term def="把多个无状态HTTP事务关联到同一交互上下文的机制">会话跟踪</Term>、
<Term def="限制Cookie可发送域范围的属性">Domain属性</Term>、
<Term def="限制Cookie可发送请求路径范围的属性">Path属性</Term>、
<Term def="把用户状态编码进每个页面链接URL的识别技术">胖URL</Term>
