...
<Term def="以HTTP扩展方法和首部支持远程协作创作与资源管理的协议">WebDAV</Term>、
<Term def="为资源创建写锁并返回锁令牌的WebDAV方法">LOCK</Term>、
<Term def="查询资源属性和集合成员信息的WebDAV方法">PROPFIND</Term>、
<Term def="创建集合资源的WebDAV方法">MKCOL</Term>、
<Term def="FrontPage服务器扩展使用的历史远程发布协议">FrontPage RPC</Term>
