...
<Term def="当程序需要某个特殊成员函数且用户未提供时，由编译器隐式声明并在需要时形成定义的过程。">
...
<Term def="不接收参数或所有参数都有默认值、可在无实参时构造对象的构造函数；隐式版本是否可用和是否平凡取决于类组成。">
...
<Term def="按 base 与 nonstatic data member 的语言顺序，对每个 subobject 执行其默认初始化要求。">
...
<Term def="常见 ABI 中由对象持有、指向其当前 class virtual table 的实现指针；其具体位置与数量不是语言标准固定布局。">
  vptr initialization
</Term>
...
<Term def="在虚继承图中由 most-derived object 只保存一份、被多条继承路径共同引用的基类子对象。">
...
<Term def="用同类型对象初始化新对象的特殊成员函数；隐式版本通常按 base 和 member 逐个复制。">
...
<Term def="隐式拷贝按 base subobject 与 nonstatic member 的声明结构逐个应用各自复制语义。">
  default memberwise initialization
</Term>

### Bitwise Copy Semantics

<Term def="对象表示可按字节复制而不破坏目标对象语义的情形；它要求没有必须执行的非平凡 subobject 或动态表示修复。">
  bitwise copy semantics
</Term>
...
<Term def="编译器把源码中的对象初始化、传参与返回转换为显式构造目标、隐藏参数或直接结果存储，同时保持语言可观察行为。">
...
<Term def="编译器把具名局部返回对象直接构造在调用方结果存储中的允许优化。">
...
<Term def="在 constructor body 之前直接选择 base 与 member constructor 的语法；执行顺序由 base/member 声明决定。">
