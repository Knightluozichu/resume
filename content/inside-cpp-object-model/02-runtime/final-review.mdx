...
import { FinalReviewDecisionLab, FinalReviewMechanismMap, FinalReviewFailureDiagram } from "@/components/mdx/inside-cpp-object-model/final-review";
...
<Term def="把 object layout、call lowering、lifetime state 与 template/exception/RTTI ABI identity 放到同一因果链诊断的方法。">seven-chapter incident analysis</Term>
...
<Term def="跨进程或跨不兼容 module 保存了 vptr、type_info address、allocator-owned pointer 等只在原执行环境有效的对象表示。">foreign object representation</Term>
...
<Term def="most-derived object 内的 base subobjects、nonstatic members、padding、vptr 与 virtual-base metadata 的 ABI-specific bytes。">native object layout</Term>
...
<Term def="construction/destruction 中当前已建立到哪一个 class subobject，并据此限制 virtual behavior 与可访问 invariant 的阶段。">active construction phase</Term>
...
<Term def="从 secondary base view 调 virtual function 时，将 incoming base pointer 调整成 final overrider 所需 this 的 compiler adapter。">secondary-base adjustment thunk</Term>
...
<Term def="把 aligned storage、object lifetime start/end、generation 与匹配 destroy function 作为一个不可分割状态机管理的池槽。">phase-safe object pool</Term>
...
<Term def="host 与 plugin 共同使用的 version、calling convention、data layout、ownership、error 与 function-table contract。">versioned module ABI</Term>
...
<Term def="不暴露 C++ class layout 的未解释 token，只能交回定义它的 versioned function table 操作和销毁。">opaque module handle</Term>
...
<Term def="只包含版本化 value fields、stable IDs 与 relative offsets，不包含 process-local code/data addresses 的共享或线格式。">stable data boundary</Term>
...
<Term def="对 representation、construction/call、copy/lifetime 与 late-bound ABI 四层分别给出可重复 correctness 和 failure-path 证据。">object-model acceptance gate</Term>
