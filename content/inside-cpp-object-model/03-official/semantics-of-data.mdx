...
<Term def="在 class scope、声明位置与语言查找规则下，把一个 member name 解析为特定声明的过程。">
...
<Term def="compiler/ABI 在满足声明顺序约束、alignment、base 与实现元数据要求后，为 nonstatic data 分配对象内位置。">
  data member layout
</Term>
...
<Term def="为满足成员 alignment 或 object array stride 而存在、但不直接承载该成员值的对象表示字节。">
  padding
</Term>
...
<Term def="在 class 外只有一份 storage、可通过 class name 或 object expression 命名但不需要 this offset 的成员。">
  static data member
</Term>
...
<Term def="属于每个 object/subobject 的字段，访问时需要以 this 或 object address 加上布局定位信息。">
...
<Term def="只组合 base subobject 与 derived members、不要求 virtual dispatch 或 virtual base 定位的继承表示。">
  inheritance without polymorphism
</Term>
...
<Term def="对象中支持运行时 virtual dispatch 的实现元数据，通常表现为一个或多个 vptr 及 class-level vtable。">
...
<Term def="让多条继承路径在 most-derived object 中共享一份 base subobject，并通过实现元数据定位它的继承形式。">
  virtual inheritance
</Term>
...
<Term def="在保持相同算法和数据表示时，比较直接对象、封装访问、普通继承、多重继承与虚继承的实际访问成本。">
...
<Term def="类型为 T C::* 的 class-relative member locator；它未绑定具体 object，必须用 dot-star 或 arrow-star 运算符应用。">
...
<Term def="把 pointer-to-member 与具体 object/base view 组合，执行必要 class adjustment 后得到成员 lvalue 的过程。">
