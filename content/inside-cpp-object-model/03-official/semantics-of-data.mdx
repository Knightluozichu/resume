...
import { SemanticsOfDataDecisionLab, SemanticsOfDataMechanismMap, SemanticsOfDataFailureDiagram } from "@/components/mdx/inside-cpp-object-model/semantics-of-data";
...
<SemanticsOfDataMechanismMap />

<SemanticsOfDataDecisionLab />

<SemanticsOfDataFailureDiagram />
...
### access of a data member：机制、边界与证据

在《第3章：数据语义》的官方单元 ico-03 中，access of a data member连接本章第 3 组知识约束。学习时要同时说明它接受什么输入、改变什么状态、在何种边界失效；再以本章示例的编译诊断、固定输入输出或失败用例复核结论，不能只记术语名称。
