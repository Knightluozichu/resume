...
<Term def="member function 调用根据 static type、是否 static/virtual、object view 与 ABI 规则转换成可执行调用的不同形式。">
...
<Term def="需要一个隐式 object parameter 才能访问 nonstatic state，但 function code 由同一 class 的实例共享。">
...
<Term def="把 namespace、class、function name、parameter types 与 cv/ref qualifiers 等签名信息编码进链接符号的 ABI 机制。">
  name mangling
</Term>
...
<Term def="通过 base pointer/reference 调用时，可根据 dynamic type 选择 final overrider 的成员函数。">
...
<Term def="位于 class scope 但没有 hidden this、不能直接访问 nonstatic member、也不参与 virtual dispatch 的函数。">
...
<Term def="在多重继承的某个 base view 上查找 virtual slot，并把 incoming base pointer 调整为 overrider 所需 this 的调用机制。">
  virtual functions under multiple inheritance
</Term>
...
<Term def="由 compiler 生成的小型适配代码，负责 this 或返回指针调整后再进入真正 function body。">
  adjustment thunk
</Term>
...
<Term def="通过 shared virtual-base view 进行 virtual dispatch，并借助 runtime layout metadata 调整到 final overrider object view 的机制。">
  virtual functions under virtual inheritance
</Term>
...
<Term def="在同一语义 workload 下比较 direct、nonstatic、virtual、adjusted virtual 与 inline 调用的指令、分支和代码尺寸成本。">
  function efficiency
</Term>
...
<Term def="类型为 R (C::*)(Args...) 的 class-relative callable representation；应用时必须结合兼容 object，并可能携带 this adjustment 或 virtual slot 信息。">
...
<Term def="member-function pointer 表示 virtual slot，应用到 object 后再进行动态查表而不是固定绑定某个 overrider 的实现方式。">
...
<Term def="允许 compiler 在 call site 展开 function body 的声明/优化属性；它不保证展开，也不改变函数语言语义。">
  inline function
</Term>
...
<Term def="inline expansion 时把 actual arguments 映射为只按调用语义求值的内部值，并为每个 call site 保持 local variable 独立语义。">
