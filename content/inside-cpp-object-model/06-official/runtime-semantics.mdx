...
<Term def="对象在运行期取得 storage、开始 lifetime、执行使用、结束 lifetime 并释放或复用 storage 的语言与实现规则。">
  runtime semantics
</Term>
...
<Term def="依据 storage duration 与控制流，在规定时点开始对象 lifetime，并在正常退出、异常或程序终止时执行匹配 teardown。">
  object construction and destruction
</Term>

### Global Objects

<Term def="具有 static storage duration、在 namespace scope 或等价位置定义，其初始化可能分为静态初始化与动态初始化的对象。">
  global objects
</Term>
...
<Term def="在 block scope 声明、具有 static storage duration，但首次控制经过 declaration 时才初始化的对象。">
  local static objects
</Term>
...
<Term def="连续 storage 中按 index 从前向后构造、在 teardown 或部分失败时逆序析构的一组同类型对象。">
  arrays of objects
</Term>
...
<Term def="在没有逐元素 initializer 时，为对象数组的每个 element 调用可用 default constructor 的要求。">
...
<Term def="可替换或重载的 raw storage allocation/deallocation functions；new/delete expressions 在其外还负责 construction/destruction。">
...
<Term def="array new 分配足够 storage 和可能的实现元数据，逐元素正序构造；delete-array 逐元素逆序析构再释放。">
...
<Term def="在 caller 提供且满足 size/alignment 的 storage 中开始对象 lifetime，不由标准 placement form 分配或释放该 storage。">
...
<Term def="为表达式求值、转换、返回或 reference binding 而物化、通常在 full-expression 结束时销毁的无名 class object。">
  temporary objects
</Term>
...
<Term def="把 prvalue 转换为有 identity、可绑定 reference 且具有明确 destruction point 的 temporary object。">
...
<Term def="认为每个按值返回、转换或 operator expression 都必然产生昂贵可见临时对象，或相反认为 optimizer 会消除所有临时量的错误心智模型。">
  temporary myth
</Term>
