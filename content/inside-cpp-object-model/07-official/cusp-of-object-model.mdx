...
<Term def="位于基本 class layout 之外、通过 compile-time instantiation 或 runtime metadata 扩展泛型、异常与动态类型能力的机制集合。">
  cusp of the object model
</Term>

## 7.1 Templates

<Term def="以 type/value/template parameters 描述一族 declarations，并在使用上下文中形成具体 specialization 的语言机制。">
  templates
</Term>
...
<Term def="用实际 template arguments 形成 specialization，并在需要 definition 时生成相应 class/member/function 语义与代码的过程。">
  template instantiation
</Term>
...
<Term def="definition-time 错误立即诊断，dependent operation 的错误延迟到 substitution/instantiation context，并携带 specialization trace。">
...
<Term def="non-dependent names 在 template definition context 解析，dependent names 在实例化点结合 actual types 与关联查找解析的两阶段模型。">
  name resolution within a template
</Term>
...
<Term def="class template 的 member definition 通常在该 member 被需要时才实例化，未使用 member 不因 class specialization 存在就全部生成。">
  member function instantiation
</Term>
...
<Term def="throw 创建 exception object，runtime 搜索匹配 handler、展开中间 frames，并以 RAII 销毁已构造 automatic objects 的控制流机制。">
...
<Term def="runtime 根据当前 instruction/call-site metadata 与 active frames，判断 throw point 被哪些 try regions 覆盖并寻找 handler。">
...
<Term def="按 exception dynamic type、public base conversions、pointer 或 exact categories 等规则，依 handler 顺序选择第一个可匹配 catch parameter。">
  catch type matching
</Term>
...
<Term def="throw operand 用来初始化独立 exception object；该对象跨 stack unwinding 存活，直到最后一个 handler/rethrow chain 结束。">
...
<Term def="通过 typeid 与 checked dynamic cast，在运行期查询 polymorphic object 的 dynamic type 或验证 hierarchy conversion。">
...
<Term def="以 runtime hierarchy metadata 检查 base-to-derived 或 cross-cast 关系，成功时返回调整后目标 view 的转换。">
...
<Term def="返回 std::type_info identity；对 polymorphic glvalue 表达式报告 dynamic type，对其他适用表达式主要报告 static type。">
  typeid operator
</Term>
...
<Term def="C++ 常见对象模型用预先固定的 layout、virtual slots 与静态 ABI 换取快速访问和分派，但不支持运行期任意改变 class shape。">
...
<Term def="在独立编译/装载模块间共享 C++ class、RTTI、exception 与 allocator contract 时必须一致的 ABI 边界。">
  dynamic shared libraries
</Term>
...
<Term def="多个进程映射同一 bytes，但各自 virtual address、code image、vtable/type_info 与 allocator state 可能不同的 storage boundary。">
  shared memory
</Term>
