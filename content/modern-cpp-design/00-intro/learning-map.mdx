...
<Term def="第一版第1-4章组成的技术部分，建立 Policy、模板检测、类型列表、类生成与 allocator 分层。">
  Part I Techniques
</Term>
...
<Term def="第一版第5-11章组成的组件部分，利用前四章技术构建 callable、lifetime、ownership、creation 与 dispatch 组件。">
  Part II Components
</Term>
...
<Term def="一个设计选择在构建类型时固定、能与其他选择独立组合且由 Host 协议约束的变化维度。">
...
<Term def="把类型、常量和类型关系作为编译期数据，经过 specialization、overload 与元函数产生新的类型或选择。">
...
<Term def="把一个复杂 runtime state machine 拆成 owner 明确、协议窄且可独立验证的层级。">
...
<Term def="从 callback target、global service 到 resource handle 都明确 owner、copy/move、销毁、late access 与 thread domain 的连续契约。">
...
<Term def="以 runtime key 选择单一 creator，或一次选择完整相容产品族的两级 creation boundary。">
...
<Term def="根据一个或多个对象的动态类型选择 operation，并由 cyclic/acyclic/registry/table 决定开放性与成本。">
...
<Term def="用标准设施替换历史机制，同时保留原组件对选择时机、类型契约、所有权和扩展方向的设计审计。">
