...
import { PolicyBasedClassDesignDecisionLab, PolicyBasedClassDesignMechanismMap, PolicyBasedClassDesignFailureDiagram } from "@/components/mdx/modern-cpp-design/policy-based-class-design";
...
<Term def="一个组件同时拥有多个可独立选择的设计维度，每个维度又有多个合理策略，从而形成组合空间。">design multiplicity</Term>
...
<Term def="接收一组策略类型或策略模板、规定公共协议并把策略能力编排成最终组件的主类。">Host</Term>
...
<Term def="封装单一设计决定、由 Host 在编译期选择并调用的类或类模板。">Policy</Term>
...
<Term def="除单一静态操作外，还携带状态、嵌套类型、常量或完整生命周期协议的策略。">enriched Policy</Term>
...
<Term def="模板成员按使用点实例化，使只满足部分协议的策略仍可支持 Host 的其余功能；调用缺失功能时才产生编译错误。">incomplete instantiation</Term>
...
<Term def="若若干策略共同满足 Host 协议且互不破坏对方不变量，它们兼容；否则应在编译期拒绝或重新划分责任。">Policy compatibility</Term>
