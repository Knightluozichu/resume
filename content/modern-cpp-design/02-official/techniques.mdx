...
<Term def="当编译期布尔条件为假时使程序形成清晰诊断，从而在模板实例化点阻止违反不变量的类型或常量。">
...
<Term def="在不指定全部模板实参的情况下，为满足某种类型或非类型参数形状的一族实例提供更具体实现。">
...
<Term def="定义在函数块作用域内、仅在该作用域可见的类型，用于封装一次性适配状态或行为。">
  local class
</Term>
...
<Term def="把编译期整数 N 包装为唯一类型 Int2Type&lt;N&gt;，以便 overload resolution 或 specialization 按值选择实现。">
  Int2Type
</Term>
...
<Term def="把类型 T 包装成可传给函数的空标签 Type2Type&lt;T&gt;，让 overload resolution 显式选择目标类型。">
  Type2Type
</Term>
...
<Term def="依据编译期布尔条件从两个候选中产生一个 Result 类型的元函数。">
  type selection
</Term>
...
<Term def="在不执行转换的情况下，通过 substitution、overload resolution 或 requires-expression 判断 From 是否可形成 To，并由此推断部分继承关系。">
...
<Term def="把不可复制的 std::type_info 引用包装成可复制、可比较、可作为容器键的值对象。">
  TypeInfo wrapper
</Term>
...
<Term def="表示类型级递归结束或不存在类型的哨兵，不承担普通策略行为。">
  NullType
</Term>

<Term def="可作为空基类、占位成员或默认策略单元的无状态类型；它存在但不提供业务状态。">
  EmptyType
</Term>
...
<Term def="在编译期查询、变换或关联类型属性，并把结果送入算法、参数传递、存储和约束选择的元函数集合。">
  Type Traits
</Term>
