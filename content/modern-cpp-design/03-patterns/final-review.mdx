...
import { FinalReviewDecisionLab, FinalReviewMechanismMap, FinalReviewFailureDiagram } from "@/components/mdx/modern-cpp-design/final-review";
...
<Term def="把类型身份、对象所有权、可调用代码、内存域和全局生命周期串成一条可验证依赖的系统设计。">cross-component contract chain</Term>
...
<Term def="由类型列表定义产品或元素集合，再驱动 interface、factory 或 visitor 生成的编译期结构契约。">generated type schema</Term>
...
<Term def="分配器、对象 destructor 所在代码和最终释放者共同存活的范围；跨域释放必须由显式 owner/deleter 连接。">allocation domain</Term>
...
<Term def="在 erased callable 存活与调用期间阻止其实现代码或 receiver 所属模块被卸载的持有凭证。">callable module lease</Term>
...
<Term def="在 object destruction 前先阻止新请求、排空进行中调用、注销 entries，再按依赖顺序释放模块的生命周期阶段。">explicit shutdown phase</Term>
...
<Term def="资源 handle、deleter、backend/device 与 module lease 共同移动和销毁，禁止跨 family 或卸载后释放。">resource-owner bundle</Term>
...
<Term def="先由 runtime ID 选择模块或产品 creator，再由一个 Abstract Factory 限制整组相容产品的两级创建结构。">two-level creation architecture</Term>
...
<Term def="在稳定 element schema 上通过 Accept/Visit 两次分派选择一元 operation，并将遍历策略与分派分开。">node Visitor boundary</Term>
...
<Term def="按两个对象的动态类型 pair 查找 interaction handler，并显式定义 symmetry、unknown pair 与 cast safety。">interaction dispatcher</Term>
...
<Term def="按类型、资源、可调用代码、全局生命周期与内存域记录 owner、证明、失败和测试的审计表。">contract ledger</Term>
