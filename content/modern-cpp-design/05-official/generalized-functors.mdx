...
<Term def="把请求及其接收者、参数或撤销状态封装成可存储和延迟执行对象的行为模式。">
  Command
</Term>
...
<Term def="可通过函数调用语法或 invoke 规则执行的函数、成员函数指针、函数对象或 lambda。">
  callable entity
</Term>
...
<Term def="以固定调用签名拥有一个擦除实现对象、可复制并暴露统一 operator() 的值语义包装器。">
  generalized Functor
</Term>
...
<Term def="Functor 的公共调用操作，把固定签名参数送入擦除接口，并把返回值按 contract 传回调用者。">
  forwarding operator
</Term>
...
<Term def="保存具体 callable 并实现擦除接口 invoke/clone 的派生模板，把静态类型适配到统一 Functor。">
  FunctorHandler
</Term>
...
<Term def="通过 erased implementation 的 virtual clone 在不知道 concrete callable type 时复制目标，从而维持 wrapper 值语义。">
  virtual clone
</Term>
...
<Term def="在 Functor 公共签名与 concrete callable 签名之间进行受约束的参数和返回类型适配。">
  callable conversion
</Term>
...
<Term def="由接收对象与 member function pointer 共同组成、通过 invoke 规则执行的 callable 适配。">
...
<Term def="预先保存 callable 的部分实参，生成参数更少的新 callable 的适配操作。">
  binding
</Term>
...
<Term def="把多个 callable 的执行顺序与错误/返回策略封装成一个新 callable 的组合操作。">
  request chaining
</Term>
...
<Term def="统一 callable 边界产生的额外函数层、间接分派、参数传递与失去跨边界内联机会。">
  forwarding cost
</Term>
...
<Term def="为擦除 handler 动态取得存储的成本，包括分配延迟、失败、局部性与复制时重新分配。">
...
<Term def="命令执行时保存可恢复状态或逆操作，使历史栈能够撤销并重新执行请求的协议。">
  reversible command
</Term>
