...
<Term def="以对象生命周期管理动态资源并提供受控 pointer-like access 的 RAII handle。">
  smart pointer
</Term>
...
<Term def="定义 handle 表示、访问类型、空值与最终销毁操作的 SmartPtr 策略维度。">
  StoragePolicy
</Term>
...
<Term def="在统一 public API 中按顺序编排 storage、ownership、checking 与 conversion 协议的操作集合。">
...
<Term def="规定复制、移动、共享、释放与最后 owner 判定如何改变资源身份的策略。">
  OwnershipPolicy
</Term>
...
<Term def="在 wrapper 地址与底层 handle 输出地址之间容易混淆、可能破坏现有 ownership 的 API 边界。">
  address-of boundary
</Term>
...
<Term def="无需显式调用即可把 owning wrapper 变成 non-owning raw pointer 的转换，便利但弱化 lifetime 边界。">
...
<Term def="明确智能指针相等究竟比较 pointee address、resource identity 还是 ownership identity 的关系。">
  pointer equality
</Term>
...
<Term def="为 associative containers 定义稳定 strict weak order，并明确按地址还是按 ownership 排序。">
...
<Term def="在解引用或构造边界验证 handle precondition，并选择断言、异常、终止或无检查的策略。">
  CheckingPolicy
</Term>
...
<Term def="pointee constness 与 handle constness 两个独立限定：前者限制对象访问，后者限制 wrapper 重绑定。">
...
<Term def="拥有一组同类型动态元素并要求 delete-array、索引与可选长度契约的智能资源形式。">
...
<Term def="分别约束 control state、handle object 与 pointee state 的并发访问，而不是笼统宣称智能指针线程安全。">
...
<Term def="把四个 Policy 的 state 与操作按统一 exception/lifetime contract 编排成完整 SmartPtr 类型。">
