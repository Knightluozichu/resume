...
<Term def="根据运行时 identifier 选择已注册 creator 并返回抽象 product owner 的组件。">
  Object Factory
</Term>
...
<Term def="把无法作为运行时值直接存储的 concrete class，转换为可存储 creator callable 或 prototype object 的表示步骤。">
...
<Term def="保存 identifier 到 creator 的关联表，并提供受控 register/unregister/create 操作的注册式实现。">
  creator registry
</Term>
...
<Term def="在 factory 所定义范围内稳定指代某类 creator/product 的可比较键。">
  Type Identifier
</Term>
...
<Term def="把 product、key、creator signature 与失败/容器策略参数化，使同一注册算法服务不同领域。">
...
<Term def="围绕 registration 冲突、unknown lookup、异常、线程、重入、注销和诊断的边界规则。">
  Factory minutiae
</Term>
...
<Term def="按 prototype 或 source dynamic type 查找 copier/clone operation，并产生同类新对象的工厂。">
  CloneFactory
</Term>
...
<Term def="把 creator、registry lifetime、类型清单、返回所有权和分配策略分别交给前述通用组件并保持边界清晰。">
  Factory composition
</Term>
