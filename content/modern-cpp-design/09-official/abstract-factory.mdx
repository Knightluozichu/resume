...
import { AbstractFactoryDecisionLab, AbstractFactoryMechanismMap, AbstractFactoryFailureDiagram } from "@/components/mdx/modern-cpp-design/abstract-factory";
...
<AbstractFactoryMechanismMap />

<AbstractFactoryDecisionLab />

<AbstractFactoryFailureDiagram />
...
<Term def="提供一组相关或相容抽象产品的创建接口，使 client 不依赖 concrete classes 且保持产品族一致。">Abstract Factory</Term>
...
<Term def="由抽象产品类型序列生成、为每种产品提供统一创建协议的可扩展 factory interface。">generic Abstract Factory interface</Term>
...
<Term def="为某一个抽象产品 T 定义 virtual creation slot、并作为散射层次一个 base 的接口单元。">AbstractFactoryUnit</Term>
...
<Term def="把 concrete product list 按位置映射到 abstract product slots，并生成每个 virtual creator 实现的类层次。">ConcreteFactory</Term>
...
<Term def="让同一 ProductList 同时生成并列 abstract slots 与顺序 concrete overrides，避免平行声明漂移的结构。">family class generation</Term>
...
<Term def="为每种抽象产品保存一个 concrete prototype，并通过 clone 创建新产品的 Abstract Factory 实现。">Prototype Abstract Factory</Term>
...
<Term def="将完整 prototype family 视为不可变快照并一次发布，避免并发 client 观察到半更新产品族。">family snapshot</Term>
...
<Term def="产品列表、具体列表、CreatorUnit、返回 owner 与 family invariant 共同构成的 Abstract Factory 完整契约。">Abstract Factory contract</Term>
