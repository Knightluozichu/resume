...
<Term def="根据多个参数的运行时类型组合选择最具体操作的多重动态分派机制。">
  multimethod
</Term>
...
<Term def="当操作语义由多个独立多态对象的具体类型共同决定、且单分派归属会产生耦合时的使用条件。">
  multimethod need
</Term>
...
<Term def="通过嵌套 dynamic type tests 穷举两个参数具体类型组合并直接调用对应函数的方法。">
...
<Term def="以类型列表生成嵌套 dynamic type tests，消除手写 pair 分支但保留穷举查找性质的 dispatcher。">
...
<Term def="当交换两个参数不改变操作语义时，把两个有序类型 pair 规范为一个注册项并正确调整实参次序。">
  symmetric dispatch
</Term>
...
<Term def="以有序动态类型 pair 为键、在关联容器中对数查找已注册 callback 的双分派引擎。">
...
<Term def="保存 function callbacks、处理 type-pair lookup 与可选 symmetry normalization 的运行时 dispatcher。">
  FnDispatcher
</Term>
...
<Term def="把每个动态类型 pair 映射到可保存状态且类型擦除的 callable，而不只支持 free function pointer。">
  Functor multimethod
</Term>
...
<Term def="在 erased Base references 转回 registered concrete references 时，选择运行时验证或依赖外部 type-key invariant 的边界。">
...
<Term def="把动态类型映射为紧凑索引并直接访问二维 handler table，以常数时间选择 pair operation 的引擎。">
...
<Term def="分别封装通用对数查找与快速常数查找的数据结构 Policy，供同一 multimethod Host 选择。">
...
<Term def="根据类型集合开放性、扩展方向与性能选择 Visitor、variant visit、registry 或 dense type-ID table 的演进视角。">
