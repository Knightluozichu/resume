...
section: "计算机图形学与渲染"
order: 2
description: "GLSL基础、着色器阶段、编译与链接"
demo: true
math: false
...
- 能用自己的话解释Shader Fundamentals的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：GLSL基础与着色器阶段之间的关系是什么？

</Objectives>

## 为什么需要Shader Fundamentals

OpenGL Programming Guide: The Official Guide to Learning OpenGL, 9th Edition将Shader Fundamentals作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
GLSL基础、着色器阶段、编译与链接正是这一哲学在特定领域的具体体现。
...
Vulkan 把Shader Fundamentals从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Shader Fundamentals涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Shader Fundamentals相关功能前，必须完成：
...
启用验证层后，Shader Fundamentals相关的常见错误会被捕获：
...
1. Shader Fundamentals是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Shader Fundamentals涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《OpenGL Programming Guide: The Official Guide to Learning OpenGL, 9th Edition》（https://www.informit.com/store/opengl-programming-guide-the-official-guide-to-learning-9780134495552）第 2 章整理重写，
代码示例为教学简化版本。
</Attribution>
