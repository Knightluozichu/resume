...
- 能用自己的话解释Drawing的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：图元类型与顶点指定之间的关系是什么？

</Objectives>

## 为什么需要Drawing

OpenGL Programming Guide: The Official Guide to Learning OpenGL, 9th Edition将Drawing作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
图元类型、顶点指定、绘制命令正是这一哲学在特定领域的具体体现。
...
Vulkan 把Drawing从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Drawing涉及的对象遵循统一的生命周期：
...
<Answer>
练习 1：画出Drawing涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《OpenGL Programming Guide: The Official Guide to Learning OpenGL, 9th Edition》（https://www.informit.com/store/opengl-programming-guide-the-official-guide-to-learning-9780134495552）第 3 章整理重写，
代码示例为教学简化版本。
</Attribution>
