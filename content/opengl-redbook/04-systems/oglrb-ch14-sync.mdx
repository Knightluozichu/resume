...
- 能用自己的话解释Advanced Synchronization的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：Fence与Sync对象之间的关系是什么？

</Objectives>

## 为什么需要Advanced Synchronization

OpenGL Programming Guide: The Official Guide to Learning OpenGL, 9th Edition将Advanced Synchronization作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
Fence、Sync对象、条件渲染正是这一哲学在特定领域的具体体现。
...
Vulkan 把Advanced Synchronization从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Advanced Synchronization涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Advanced Synchronization相关功能前，必须完成：
...
// 录制与Advanced Synchronization相关的命令
// ... 具体的 vkCmd* 调用 ...
...
启用验证层后，Advanced Synchronization相关的常见错误会被捕获：
...
1. Advanced Synchronization是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Advanced Synchronization涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《OpenGL Programming Guide: The Official Guide to Learning OpenGL, 9th Edition》（https://www.informit.com/store/opengl-programming-guide-the-official-guide-to-learning-9780134495552）第 14 章整理重写，
代码示例为教学简化版本。
</Attribution>
