---
title: "Our First OpenGL Program"
type: B
section: "计算机图形学与渲染"
order: 2
description: "窗口创建、上下文初始化、渲染循环、清除屏幕"
demo: true
math: false
...
- 能用自己的话解释Our First OpenGL Program的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：窗口创建与上下文初始化之间的关系是什么？

</Objectives>

## 为什么需要Our First OpenGL Program

OpenGL SuperBible: Comprehensive Tutorial and Reference, 7th Edition将Our First OpenGL Program作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
窗口创建、上下文初始化、渲染循环、清除屏幕正是这一哲学在特定领域的具体体现。
...
Vulkan 把Our First OpenGL Program从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Our First OpenGL Program涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Our First OpenGL Program相关功能前，必须完成：
...
// 录制与Our First OpenGL Program相关的命令
// ... 具体的 vkCmd* 调用 ...
...
启用验证层后，Our First OpenGL Program相关的常见错误会被捕获：
...
1. Our First OpenGL Program是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Our First OpenGL Program涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《OpenGL SuperBible: Comprehensive Tutorial and Reference, 7th Edition》（https://www.informit.com/store/opengl-superbible-comprehensive-tutorial-and-reference-9780134193137）第 2 章整理重写，
代码示例为教学简化版本。
</Attribution>
