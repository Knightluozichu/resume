---
title: "Math for 3D Graphics"
type: B
section: "计算机图形学与渲染"
order: 4
description: "向量、矩阵、四元数、投影变换"
demo: true
math: false
...
- 能用自己的话解释Math for 3D Graphics的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：向量与矩阵之间的关系是什么？

</Objectives>

## 为什么需要Math for 3D Graphics

OpenGL SuperBible: Comprehensive Tutorial and Reference, 7th Edition将Math for 3D Graphics作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
向量、矩阵、四元数、投影变换正是这一哲学在特定领域的具体体现。
...
Vulkan 把Math for 3D Graphics从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Math for 3D Graphics涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Math for 3D Graphics相关功能前，必须完成：
...
启用验证层后，Math for 3D Graphics相关的常见错误会被捕获：
...
1. Math for 3D Graphics是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Math for 3D Graphics涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《OpenGL SuperBible: Comprehensive Tutorial and Reference, 7th Edition》（https://www.informit.com/store/opengl-superbible-comprehensive-tutorial-and-reference-9780134193137）第 4 章整理重写，
代码示例为教学简化版本。
</Attribution>
