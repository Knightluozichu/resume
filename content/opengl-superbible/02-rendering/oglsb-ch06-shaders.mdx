---
title: "Shaders and Programs"
type: B
section: "计算机图形学与渲染"
order: 6
description: "GLSL语法、着色器编译链接、Program Pipeline"
demo: true
math: false
...
- 能用自己的话解释Shaders and Programs的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：GLSL语法与着色器编译链接之间的关系是什么？

</Objectives>

## 为什么需要Shaders and Programs

OpenGL SuperBible: Comprehensive Tutorial and Reference, 7th Edition将Shaders and Programs作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
GLSL语法、着色器编译链接、Program Pipeline正是这一哲学在特定领域的具体体现。
...
Vulkan 把Shaders and Programs从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Shaders and Programs涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Shaders and Programs相关功能前，必须完成：
...
启用验证层后，Shaders and Programs相关的常见错误会被捕获：
...
1. Shaders and Programs是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Shaders and Programs涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《OpenGL SuperBible: Comprehensive Tutorial and Reference, 7th Edition》（https://www.informit.com/store/opengl-superbible-comprehensive-tutorial-and-reference-9780134193137）第 6 章整理重写，
代码示例为教学简化版本。
</Attribution>
