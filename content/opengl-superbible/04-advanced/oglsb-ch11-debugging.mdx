...
- 能用自己的话解释Debugging and Error Handling的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：调试输出与错误码之间的关系是什么？

</Objectives>

## 为什么需要Debugging and Error Handling

OpenGL SuperBible: Comprehensive Tutorial and Reference, 7th Edition将Debugging and Error Handling作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
调试输出、错误码、GPU调试工具正是这一哲学在特定领域的具体体现。
...
Vulkan 把Debugging and Error Handling从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Debugging and Error Handling涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Debugging and Error Handling相关功能前，必须完成：
...
// 录制与Debugging and Error Handling相关的命令
// ... 具体的 vkCmd* 调用 ...
...
启用验证层后，Debugging and Error Handling相关的常见错误会被捕获：
...
1. Debugging and Error Handling是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Debugging and Error Handling涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《OpenGL SuperBible: Comprehensive Tutorial and Reference, 7th Edition》（https://www.informit.com/store/opengl-superbible-comprehensive-tutorial-and-reference-9780134193137）第 11 章整理重写，
代码示例为教学简化版本。
</Attribution>
