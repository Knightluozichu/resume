...
<Term def="在需求、设计、实现、测试和发布各阶段持续管理 latency、throughput、memory、energy 等性能约束。">
...
<Term def="通过减少 work、改变增长级别、改善 locality 或使用硬件并行，使目标 workload 的资源消耗产生可重复下降。">
  optimization is effective
</Term>
...
<Term def="在 correctness test、明确目标、profile/cost evidence 和可回滚设计约束下进行的受控性能改造。">
  it is ok to optimize
</Term>
...
<Term def="单次极小成本乘以巨大执行次数后成为显著总耗时/容量，冷路径大改进则可能因低频而无系统价值的规模原则。">
...
<Term def="从 compiler、algorithm、library、work elimination、data representation、parallelism 和 allocator 中选择最高杠杆组合。">
  optimization strategy portfolio
</Term>

### Use a Better Compiler, Use Your Compiler Better

<Term def="使用成熟 optimizer、正确 target/release options、link-time/profile information，并让 source 保持 defined behavior 和可见语义。">
...
<Term def="通过降低时间/空间增长或利用 input properties 移除数量级 work，而不是只缩小常数。">
  better algorithms
</Term>
...
<Term def="减少 dynamic object 创建、reallocation、deep copy 和 memory traffic，同时保持 ownership/lifetime 明确。">
...
<Term def="通过 precompute/cache/skip/batch/closed form 让 operation 不发生或更少发生，并维护 invalidation correctness。">
  remove computation
</Term>
...
<Term def="把足够大且独立的 work 分配给可用 execution resources，并把 synchronization/communication 控制在收益以内。">
