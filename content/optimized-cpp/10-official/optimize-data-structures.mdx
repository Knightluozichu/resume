...
<Term def="标准规定语义、iterator/invalidation 和复杂度要求的 sequence/associative/container adaptor 家族。">standard library containers</Term>

<Term def="在相同语义、输入与操作混合下替换一个 container/representation，采集时间、allocation、memory 与 correctness 的受控比较。">container experimenting</Term>
...
<Term def="按元素顺序保存值的 vector/deque/list/forward_list 等容器，访问与修改成本由表示和位置决定。">sequence containers</Term>

<Term def="按 key 的 ordering 或 hashing 组织 value/key，支持 exact/range/duplicate 等关联操作的容器。">associative containers</Term>
...
<Term def="container 超出 capacity 后取得更大 storage 并搬移现有 elements 的增长事件，会产生峰值 memory 与引用失效。">reallocation</Term>
...
<Term def="以多个 blocks 和索引结构组成、支持 random access 与两端高效增长的分段序列容器。">std::deque</Term>
...
<Term def="双向 node-based sequence，已知位置插入/删除只重连节点并保持其他节点引用，但每节点有分配与链接。">std::list</Term>

<Term def="单向 node-based sequence，以更少 link 换只有 forward traversal、需要 predecessor 才能插删。">std::forward_list</Term>
...
<Term def="按 strict weak ordering 保存 key/value，支持唯一 key、ordered iteration 与 logarithmic search/update 的关联容器。">std::map</Term>
...
<Term def="以 hash/bucket 和 equality 组织 unique keys 的关联容器，提供 average constant exact lookup 而无 ordered iteration。">std::unordered_map</Term>

<Term def="允许 equivalent duplicate keys 的 hashed associative 容器，可用 equal_range 取得同 key group。">std::unordered_multimap</Term>
...
<Term def="在标准 sequence/associative contract 无法表达领域操作时采用的 flat、heap、bitset、trie、ring、pool 或 spatial 等结构。">other data structures</Term>
