...
<Term def="按 consumer contract 一次完成 open、storage planning、read、partial/error/EOF handling 与 output ownership 的文件读取流程。">
...
<Term def="只表达 I/O 必需 source、destination/owner、policy 和 error 的克制函数接口，使 allocation 与 copy 可见。">
...
<Term def="删除 I/O hotspot 中无语义价值的转发、重复 open/validation/copy 层，同时保留必要边界。">
...
<Term def="用 size hint、reserve/reuse 或 bounded chunk 避免 output 在读取中反复增长并复制旧内容。">
...
<Term def="每次读取更大连续 chunk，以更少调用和检查处理相同 bytes，同时受 memory 与 latency 约束。">
  take bigger bites
</Term>

<Term def="在 source 与 parser 之间复用较大 storage，减少 refill/call 次数；大小必须通过 workload sweep 选择。">
  bigger input buffer
</Term>
...
<Term def="以 delimiter-defined line 为增量单位读取与消费，降低 whole-file ownership，需定义长行、换行和 encoding contract。">
...
<Term def="经同环境 A/B 证明无稳定端到端收益、或 correctness/resource trade-off 不合格的优化假设。">
...
<Term def="数据到达时增量读取或生成时增量写出，不要求一次拥有全部内容的 I/O 模式。">
  streaming I/O
</Term>
