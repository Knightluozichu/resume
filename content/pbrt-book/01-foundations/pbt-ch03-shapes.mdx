...
section: "计算机图形学与渲染"
order: 3
description: "球体、圆柱、圆盘、三角形、曲线等几何体的表示与求交"
demo: true
math: false
...
Physically Based Rendering: From Theory to Implementation, 4th Edition将Shapes作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
球体、圆柱、圆盘、三角形、曲线等几何体的表示与求交正是这一哲学在特定领域的具体体现。
...
Vulkan 把Shapes从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Shapes涉及的对象遵循统一的生命周期：
...
<Answer>
练习 1：画出Shapes涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Physically Based Rendering: From Theory to Implementation, 4th Edition》（https://pbr-book.org/4ed/）第 3 章整理重写，
代码示例为教学简化版本。
</Attribution>
