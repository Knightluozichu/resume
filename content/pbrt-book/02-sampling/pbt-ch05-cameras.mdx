...
section: "计算机图形学与渲染"
order: 5
description: "针孔模型、薄透镜、景深、快门与运动模糊、胶片响应"
demo: true
math: false
...
- 能用自己的话解释Cameras and Film的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：针孔模型与薄透镜之间的关系是什么？

</Objectives>

## 为什么需要Cameras and Film

Physically Based Rendering: From Theory to Implementation, 4th Edition将Cameras and Film作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
针孔模型、薄透镜、景深、快门与运动模糊、胶片响应正是这一哲学在特定领域的具体体现。
...
Vulkan 把Cameras and Film从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Cameras and Film涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Cameras and Film相关功能前，必须完成：
...
启用验证层后，Cameras and Film相关的常见错误会被捕获：
...
1. Cameras and Film是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Cameras and Film涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Physically Based Rendering: From Theory to Implementation, 4th Edition》（https://pbr-book.org/4ed/）第 5 章整理重写，
代码示例为教学简化版本。
</Attribution>
