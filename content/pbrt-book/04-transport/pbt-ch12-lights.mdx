...
section: "计算机图形学与渲染"
order: 12
description: "点光源、面光源、环境光、IBL、光源采样策略"
demo: true
math: false
...
- 能用自己的话解释Light Sources的核心概念与设计动机
...
Physically Based Rendering: From Theory to Implementation, 4th Edition将Light Sources作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
点光源、面光源、环境光、IBL、光源采样策略正是这一哲学在特定领域的具体体现。
...
Vulkan 把Light Sources从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Light Sources涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Light Sources相关功能前，必须完成：
...
启用验证层后，Light Sources相关的常见错误会被捕获：
...
1. Light Sources是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Light Sources涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Physically Based Rendering: From Theory to Implementation, 4th Edition》（https://pbr-book.org/4ed/）第 12 章整理重写，
代码示例为教学简化版本。
</Attribution>
