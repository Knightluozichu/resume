...
section: "计算机图形学与渲染"
order: 15
description: "体积路径追踪、Delta tracking、光谱渲染"
demo: true
math: false
...
- 能用自己的话解释Light Transport II: Volume Rendering的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：体积路径追踪与Delta tracking之间的关系是什么？

</Objectives>

## 为什么需要Light Transport II: Volume Rendering

Physically Based Rendering: From Theory to Implementation, 4th Edition将Light Transport II: Volume Rendering作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
体积路径追踪、Delta tracking、光谱渲染正是这一哲学在特定领域的具体体现。
...
Vulkan 把Light Transport II: Volume Rendering从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Light Transport II: Volume Rendering涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Light Transport II: Volume Rendering相关功能前，必须完成：
...
// 录制与Light Transport II: Volume Rendering相关的命令
// ... 具体的 vkCmd* 调用 ...
...
启用验证层后，Light Transport II: Volume Rendering相关的常见错误会被捕获：
...
1. Light Transport II: Volume Rendering是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Light Transport II: Volume Rendering涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Physically Based Rendering: From Theory to Implementation, 4th Edition》（https://pbr-book.org/4ed/）第 15 章整理重写，
代码示例为教学简化版本。
</Attribution>
