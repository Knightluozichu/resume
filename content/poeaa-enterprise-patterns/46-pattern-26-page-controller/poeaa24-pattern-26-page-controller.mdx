...
import { Poeaa24Pattern26PageControllerDiagram } from "@/components/mdx/poeaa-enterprise-patterns/diagrams/poeaa24-pattern-26-page-controller-diagram";
...
<Callout type="trap" title="把页面控制器写成领域对象">
  现象：控制器里出现库存扣减、价格计算和事务规则。
  <br />
  原因：把“接收请求”误解成“拥有业务决定”。
  <br />
  修法：把决定移到应用服务或领域模型，控制器只传递已验证的输入并选择响应。
</Callout>

<Callout type="trap" title="让模板反向读取请求与数据库">
  现象：视图为了显示一个按钮，自己查询订单并判断权限。
  <br />
  原因：没有先构造稳定的视图模型，导致输出层重新承担输入和数据访问。
  <br />
  修法：控制器调用应用服务生成视图模型，模板只渲染它并保持转义边界。
</Callout>

<Callout type="trap" title="重复出现就立刻建前端控制器">
  现象：两个页面都有少量公共检查，就把所有页面塞进一个巨型入口。
  <br />
  原因：没有区分横切策略与页面特有流程，中央入口逐渐拥有业务逻辑。
  <br />
  修法：只把认证、日志、错误映射等稳定横切职责集中；仍然保持页面控制器的输入契约和可测试边界。
</Callout>
...
<Stepper>
  <Step title="1. 锁定页面边界">
    ...
    <Poeaa24Pattern26PageControllerDiagram interactive={false} />
