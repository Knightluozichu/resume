...
import { Poeaa24Pattern29TransformViewDiagram } from "@/components/mdx/poeaa-enterprise-patterns/diagrams/poeaa24-pattern-29-transform-view-diagram";
...
<Term def="把已准备好的呈现数据按显式转换规则变成 HTML、PDF 或邮件等目标输出的视图实现">
  转换视图
</Term>
不是“把所有渲染代码搬到另一个文件”，而是一条数据到输出的窄边界。它可以使用
XSLT、专门的转换函数或其他声明式规则；具体技术可以替换，但输入必须保持可检查，转换不能偷偷读取请求上下文或修改领域状态。
...
<Poeaa24Pattern29TransformViewDiagram focus="input" interactive={false} />
...
<Poeaa24Pattern29TransformViewDiagram focus="transform" interactive={false} />
...
<Poeaa24Pattern29TransformViewDiagram focus="fault" interactive={false} />
...
<Answer>
  输入已经进入呈现转换边界，`toHtml`
  或它调用的专用编码函数应把文本转义为普通内容；测试应断言输出不包含可执行标签。数据库负责存储，应用服务负责业务授权，模板若再猜测原始输入会让安全责任重复且不可定位。
</Answer>

**问题 2：改 Demo 代码。** 现在还要输出纯文本邮件。怎样复用订单数据，又怎样证明邮件格式没有复制“订单是否可取消”的业务判断？

<Answer>
  让应用服务只构造一次 `OrderViewModel`，新增 `toEmailText(model)`
  作为只读转换器。用同一输入分别断言 HTML
  和邮件正文的字段一致，再用应用服务测试单独覆盖可取消判断；如果邮件转换器没有仓储依赖和状态变更，就证明业务决定没有被复制。
</Answer>
