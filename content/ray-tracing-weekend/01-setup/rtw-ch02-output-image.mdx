...
- 能用自己的话解释Output an Image的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：PPM格式与像素循环之间的关系是什么？

</Objectives>

## 为什么需要Output an Image

Ray Tracing in One Weekend将Output an Image作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
PPM格式、像素循环、颜色输出正是这一哲学在特定领域的具体体现。
...
Vulkan 把Output an Image从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Output an Image涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Output an Image相关功能前，必须完成：
...
启用验证层后，Output an Image相关的常见错误会被捕获：
...
1. Output an Image是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Output an Image涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Ray Tracing in One Weekend》（https://raytracing.github.io/books/RayTracingInOneWeekend.html）第 2 章整理重写，
代码示例为教学简化版本。
</Attribution>
