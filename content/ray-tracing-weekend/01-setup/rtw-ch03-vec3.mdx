...
- 能用自己的话解释The vec3 Class的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：向量运算与颜色表示之间的关系是什么？

</Objectives>

## 为什么需要The vec3 Class

Ray Tracing in One Weekend将The vec3 Class作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
向量运算、颜色表示、工具函数正是这一哲学在特定领域的具体体现。
...
Vulkan 把The vec3 Class从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。The vec3 Class涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用The vec3 Class相关功能前，必须完成：
...
启用验证层后，The vec3 Class相关的常见错误会被捕获：
...
1. The vec3 Class是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出The vec3 Class涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Ray Tracing in One Weekend》（https://raytracing.github.io/books/RayTracingInOneWeekend.html）第 3 章整理重写，
代码示例为教学简化版本。
</Attribution>
