...
- 能用自己的话解释Rays, a Simple Camera, and Background的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：射线定义与视口映射之间的关系是什么？

</Objectives>

## 为什么需要Rays, a Simple Camera, and Background

Ray Tracing in One Weekend将Rays, a Simple Camera, and Background作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
射线定义、视口映射、渐变背景正是这一哲学在特定领域的具体体现。
...
Vulkan 把Rays, a Simple Camera, and Background从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Rays, a Simple Camera, and Background涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Rays, a Simple Camera, and Background相关功能前，必须完成：
...
// 录制与Rays, a Simple Camera, and Background相关的命令
// ... 具体的 vkCmd* 调用 ...
...
启用验证层后，Rays, a Simple Camera, and Background相关的常见错误会被捕获：
...
1. Rays, a Simple Camera, and Background是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Rays, a Simple Camera, and Background涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Ray Tracing in One Weekend》（https://raytracing.github.io/books/RayTracingInOneWeekend.html）第 4 章整理重写，
代码示例为教学简化版本。
</Attribution>
