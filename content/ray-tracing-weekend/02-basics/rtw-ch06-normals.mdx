...
section: "计算机图形学与渲染"
order: 6
description: "法线可视化、Hittable列表、最近命中"
demo: true
math: false
...
- 能用自己的话解释Surface Normals and Multiple Objects的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：法线可视化与Hittable列表之间的关系是什么？

</Objectives>

## 为什么需要Surface Normals and Multiple Objects

Ray Tracing in One Weekend将Surface Normals and Multiple Objects作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
法线可视化、Hittable列表、最近命中正是这一哲学在特定领域的具体体现。
...
Vulkan 把Surface Normals and Multiple Objects从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Surface Normals and Multiple Objects涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Surface Normals and Multiple Objects相关功能前，必须完成：
...
// 录制与Surface Normals and Multiple Objects相关的命令
// ... 具体的 vkCmd* 调用 ...
...
启用验证层后，Surface Normals and Multiple Objects相关的常见错误会被捕获：
...
1. Surface Normals and Multiple Objects是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Surface Normals and Multiple Objects涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Ray Tracing in One Weekend》（https://raytracing.github.io/books/RayTracingInOneWeekend.html）第 6 章整理重写，
代码示例为教学简化版本。
</Attribution>
