...
- 能用自己的话解释Moving Camera Code into Its Own Class的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：相机封装与视场角之间的关系是什么？

</Objectives>

## 为什么需要Moving Camera Code into Its Own Class

Ray Tracing in One Weekend将Moving Camera Code into Its Own Class作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
Vulkan 把Moving Camera Code into Its Own Class从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Moving Camera Code into Its Own Class涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Moving Camera Code into Its Own Class相关功能前，必须完成：
...
// 录制与Moving Camera Code into Its Own Class相关的命令
// ... 具体的 vkCmd* 调用 ...
...
启用验证层后，Moving Camera Code into Its Own Class相关的常见错误会被捕获：
...
1. Moving Camera Code into Its Own Class是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Moving Camera Code into Its Own Class涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Ray Tracing in One Weekend》（https://raytracing.github.io/books/RayTracingInOneWeekend.html）第 7 章整理重写，
代码示例为教学简化版本。
</Attribution>
