...
- 能用自己的话解释Defocus Blur的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：薄透镜模型与光圈之间的关系是什么？

</Objectives>

## 为什么需要Defocus Blur

Ray Tracing in One Weekend将Defocus Blur作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
Vulkan 把Defocus Blur从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Defocus Blur涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Defocus Blur相关功能前，必须完成：
...
启用验证层后，Defocus Blur相关的常见错误会被捕获：
...
1. Defocus Blur是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Defocus Blur涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Ray Tracing in One Weekend》（https://raytracing.github.io/books/RayTracingInOneWeekend.html）第 9 章整理重写，
代码示例为教学简化版本。
</Attribution>
