...
- 能用自己的话解释Where Next的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：扩展方向与后续系列之间的关系是什么？

</Objectives>

## 为什么需要Where Next

Ray Tracing in One Weekend将Where Next作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
扩展方向、后续系列、学习资源正是这一哲学在特定领域的具体体现。
...
Vulkan 把Where Next从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Where Next涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Where Next相关功能前，必须完成：
...
1. Where Next是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Where Next涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Ray Tracing in One Weekend》（https://raytracing.github.io/books/RayTracingInOneWeekend.html）第 11 章整理重写，
代码示例为教学简化版本。
</Attribution>
