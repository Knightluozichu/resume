...
section: "计算机图形学与渲染"
order: 4
description: "齐次坐标、仿射变换、法线变换、场景图"
demo: true
math: false
...
- 能用自己的话解释Transforms的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：齐次坐标与仿射变换之间的关系是什么？

</Objectives>

## 为什么需要Transforms

Real-Time Rendering, Fourth Edition将Transforms作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
齐次坐标、仿射变换、法线变换、场景图正是这一哲学在特定领域的具体体现。
...
Vulkan 把Transforms从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Transforms涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Transforms相关功能前，必须完成：
...
1. Transforms是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Transforms涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Real-Time Rendering, Fourth Edition》（https://www.realtimerendering.com/）第 4 章整理重写，
代码示例为教学简化版本。
</Attribution>
