---
title: "Curves and Curved Surfaces"
type: B
section: "计算机图形学与渲染"
order: 17
description: "Bezier、B-Spline、NURBS、细分曲面"
demo: true
math: false
...
- 能用自己的话解释Curves and Curved Surfaces的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：Bezier与B-Spline之间的关系是什么？

</Objectives>

## 为什么需要Curves and Curved Surfaces

Real-Time Rendering, Fourth Edition将Curves and Curved Surfaces作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
Bezier、B-Spline、NURBS、细分曲面正是这一哲学在特定领域的具体体现。
...
Vulkan 把Curves and Curved Surfaces从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Curves and Curved Surfaces涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Curves and Curved Surfaces相关功能前，必须完成：
...
// 录制与Curves and Curved Surfaces相关的命令
// ... 具体的 vkCmd* 调用 ...
...
启用验证层后，Curves and Curved Surfaces相关的常见错误会被捕获：
...
1. Curves and Curved Surfaces是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Curves and Curved Surfaces涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Real-Time Rendering, Fourth Edition》（https://www.realtimerendering.com/）第 17 章整理重写，
代码示例为教学简化版本。
</Attribution>
