...
import { RdiStructureLab } from "@/components/mdx/redis-design-implementation/v2/rdi-12-event";
...
<Answer>
文件事件驱动网络 I/O（accept/read/write），时间事件执行 serverCron 周期维护，两者在同一循环中调度。
</Answer>

**问题 2：** 为什么事件处理器中禁止阻塞？

<Answer>
单线程模型下任何阻塞都会停住全部客户端处理，表现为全站超时。
</Answer>

**问题 3：** 分析一次 KEYS * 导致的卡顿：事件循环视角发生了什么？（独立实现）

<Answer>
KEYS 在处理器中遍历全库数秒，期间所有就绪事件排队，客户端集体超时，日志出现慢查询。
</Answer>
