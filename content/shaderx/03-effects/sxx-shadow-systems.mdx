...
import { SxxShadowSystemsMapLab, SxxShadowSystemsExperimentLab, SxxShadowSystemsEvidenceLab } from "@/components/mdx/shaderx/diagrams/sxx-shadow-systems";

<Objectives>
- 能解释遮挡判据、光空间分配与偏移策略的生产者到消费者关系
...
先预测：如果只把ShaderX中的DirectX 8/9、早期GLSL或固定功能接口替换成现代API，哪些数据顺序、精度、过滤和同步假设会改变？阴影系统必须把表示、样本分配、偏移和过滤分开验证，柔和边缘不等于正确遮挡。 本页用<Term def="从光源到接收点是否被阻挡的深度或几何测试。">遮挡判据</Term>、<Term def="把有限阴影样本分配到相机可见区域的策略。">光空间分配</Term>、<Term def="基于深度、法线、斜率与接收面控制自阴影误差。">偏移策略</Term>、<Term def="过滤比较结果、深度、矩或可见性分布的区别。">过滤语义</Term>和<Term def="覆盖acne、悬浮、级联边界、动态稳定、漏光和GPU成本。">阴影证书</Term>建立统一验收语言。

<SxxShadowSystemsMapLab />
...
<SxxShadowSystemsEvidenceLab />

## 三步复现实验

<Stepper>
  <Step title="还原原篇问题与假设"><SxxShadowSystemsMapLab /></Step>
  <Step title="建立参考与现代双路径"><SxxShadowSystemsExperimentLab /></Step>
  <Step title="捕获边界、失败和成本"><SxxShadowSystemsEvidenceLab /></Step>
</Stepper>
...
<Callout type="trap" title="范围或身份错误">用更大bias消除acne，却没有测掠射接收面和接触阴影脱离。</Callout>

<Callout type="trap" title="证据或边界缺失">只测静态相机，级联和投影随相机移动时持续抖动。</Callout>

## 练习

<Exercises>
**问题1：如何从遮挡判据建立最小可核查实验？**
...
<Glossary>
  <GlossaryItem term="遮挡判据" definition="从光源到接收点是否被阻挡的深度或几何测试。" />
  <GlossaryItem term="光空间分配" definition="把有限阴影样本分配到相机可见区域的策略。" />
  <GlossaryItem term="偏移策略" definition="基于深度、法线、斜率与接收面控制自阴影误差。" />
  <GlossaryItem term="过滤语义" definition="过滤比较结果、深度、矩或可见性分布的区别。" />
  <GlossaryItem term="阴影证书" definition="覆盖acne、悬浮、级联边界、动态稳定、漏光和GPU成本。" />
</Glossary>
