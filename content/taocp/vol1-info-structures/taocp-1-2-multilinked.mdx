---
title: "Multilinked Structures"
type: B
section: "计算机科学与算法"
order: 6
description: "图、网络、稀疏矩阵"
demo: true
math: false
...
- 能用自己的话解释Multilinked Structures的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：图与网络之间的关系是什么？

</Objectives>

## 为什么需要Multilinked Structures

The Art of Computer Programming, Volumes 1-4B将Multilinked Structures作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
Vulkan 把Multilinked Structures从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Multilinked Structures涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用Multilinked Structures相关功能前，必须完成：
...
// 录制与Multilinked Structures相关的命令
// ... 具体的 vkCmd* 调用 ...
...
启用验证层后，Multilinked Structures相关的常见错误会被捕获：
...
1. Multilinked Structures是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出Multilinked Structures涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《The Art of Computer Programming, Volumes 1-4B》（https://www.informit.com/store/art-of-computer-programming-volumes-1-4b-boxed-set-9780137935109）第 6 章整理重写，
代码示例为教学简化版本。
</Attribution>
