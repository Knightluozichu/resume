...
The Art of Computer Programming, Volumes 1-4B将Trees作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
Vulkan 把Trees从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Trees涉及的对象遵循统一的生命周期：
...
<Answer>
练习 1：画出Trees涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《The Art of Computer Programming, Volumes 1-4B》（https://www.informit.com/store/art-of-computer-programming-volumes-1-4b-boxed-set-9780137935109）第 5 章整理重写，
代码示例为教学简化版本。
</Attribution>
