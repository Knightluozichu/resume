...
- 能用自己的话解释Hashing的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：散列函数与冲突解决之间的关系是什么？

</Objectives>

## 为什么需要Hashing

The Art of Computer Programming, Volumes 1-4B将Hashing作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
散列函数、冲突解决、完美散列正是这一哲学在特定领域的具体体现。
...
Vulkan 把Hashing从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。Hashing涉及的对象遵循统一的生命周期：
...
<Answer>
练习 1：画出Hashing涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《The Art of Computer Programming, Volumes 1-4B》（https://www.informit.com/store/art-of-computer-programming-volumes-1-4b-boxed-set-9780137935109）第 19 章整理重写，
代码示例为教学简化版本。
</Attribution>
