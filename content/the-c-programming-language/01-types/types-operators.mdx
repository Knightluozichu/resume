...
<Term def="执行加、减、乘、除和取余的运算符。运算前会发生整数提升与通常算术转换；结果类型由转换后的公共类型决定。">
  算术运算符
</Term>
...
<Term def="前缀形式先修改对象再产生新值，后缀形式产生旧值并安排对象递增或递减；两者都有副作用，不能与同一完整表达式中的其他未排序访问随意混用。">
  递增与递减
</Term>
...
<Term def="当多个运算符出现在同一表达式中时，决定哪个运算符与操作数绑定更紧的语法规则。它决定分组，不决定兄弟子表达式的实际求值先后。">
  运算符优先级
</Term>
让 `a + b * c` 分组为 `a + (b *
c)`；结合性决定同级运算符如何分组，例如赋值从右结合。二者都不等于
<Term def="抽象执行中各子表达式被计算及其副作用完成的先后约束。若标准没有规定，程序不能依赖编译器某次选择。">
  求值顺序
</Term>
。
