...
import { TypesOperatorsDecisionLab, TypesOperatorsMechanismMap, TypesOperatorsFailureDiagram } from "@/components/mdx/the-c-programming-language/types-operators";
...
<TypesOperatorsMechanismMap />

<TypesOperatorsDecisionLab />

<TypesOperatorsFailureDiagram />
...
在《类型、运算符与表达式》的官方单元 kr2-01 中，变量与算术表达式连接本章第 2 组知识约束。学习时要同时说明它接受什么输入、改变什么状态、在何种边界失效；再以本章示例的编译诊断、固定输入输出或失败用例复核结论，不能只记术语名称。

### for 语句：机制、边界与证据

在《类型、运算符与表达式》的官方单元 kr2-01 中，for 语句连接本章第 3 组知识约束。学习时要同时说明它接受什么输入、改变什么状态、在何种边界失效；再以本章示例的编译诊断、固定输入输出或失败用例复核结论，不能只记术语名称。

<Attribution
  adaptedFrom="The C Programming Language, Second Edition, Chapter 2: Types, Operators, and Expressions"
