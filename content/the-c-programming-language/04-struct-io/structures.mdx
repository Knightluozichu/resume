...
<Term def="由一组有名字、可具有不同类型的成员组成的聚合类型；成员按声明顺序形成一个整体值。">
  结构体
</Term>
...
<Term def="实现为满足成员访问与结构数组步长等要求，在成员之间或对象尾部保留的未命名字节。">
  填充
</Term>
...
<Term def="元素类型为某个结构类型的连续数组；每个元素都有相同 sizeof 步长，并可能含成员间和尾部填充。">
  结构体数组
</Term>
...
<Term def="指向结构对象的指针，可通过箭头运算符访问成员；它只提供位置，不自动携带数组长度、所有权或生命周期。">
  结构体指针
</Term>
...
<Term def="通过指针成员间接指向同一结构类型的结构，常用于链表、树和哈希桶的碰撞链。">
  自引用结构
</Term>
...
<Term def="把键映射到桶，再在桶内处理碰撞的表结构；哈希值只选择候选位置，最终相等仍由键比较确认。">
  哈希表
</Term>
...
<Term def="先定位候选桶，再沿碰撞链比较完整键，返回匹配节点或空指针的检索过程。">
  表查找
</Term>
要求 `key`
...
<Term def="为已有类型声明一个别名；别名与原类型兼容，不会像结构标签那样创建新的独立类型。">
  typedef
</Term>
...
<Term def="所有成员从同一存储位置开始、在足以容纳任一成员并满足其对齐要求的对象中共享表示的类型。">
  联合
</Term>
的大小足以容纳最大成员并满足各成员对齐，但不保证数值恰好等于最大
`sizeof`；实现仍可能加入填充。联合适合表示互斥变体，但存储本身不会记录当前写入哪一项，所以可移植程序通常配合枚举形成
<Term def="用独立判别字段记录联合当前活动成员的结构；每次构造、更新和读取都必须让标签与成员同步。">
  标签联合
</Term>
。
...
<Term def="在结构或联合声明中带冒号和位数的成员；实现把它放入某个分配单元，布局方向、跨单元方式和对齐含实现定义部分。">
  位域
</Term>
