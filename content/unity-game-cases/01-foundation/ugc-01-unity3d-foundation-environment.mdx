...
import {
  Ugc01Unity3dFoundationEnvironmentMapLab,
  Ugc01Unity3dFoundationEnvironmentExperimentLab,
  Ugc01Unity3dFoundationEnvironmentEvidenceLab,
} from "@/components/mdx/unity-game-cases/diagrams/ugc-01-unity3d-foundation-environment";
...
动手前先预测：改变当前步骤的一项输入后，编辑器、目标SDK或运行日志中哪个会最先变化？固定Unity版本、目标平台、输入样本、场景入口和资产提交。

<Stepper>
  <Step title="登记原章案例合同">
    <Ugc01Unity3dFoundationEnvironmentMapLab />

列出本页原目录单元、场景入口、关键对象与胜负条件。为Unity版本、编辑器、目标SDK指定唯一所有者，拒绝缺失引用和重复场景ID。

```csharp
var contract = new CaseContract(
    chapter: "第1章",
    entryScene: "Unity版本",
    authoritativeState: "目标SDK");
...
用同一初始快照执行正常、边界和故障三次回放。每次只改一项输入，记录Unity版本、编辑器、目标SDK、项目导入、场景入口、运行日志的阶段状态和首个失败节点。
