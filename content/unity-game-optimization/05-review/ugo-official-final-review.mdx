...
<Term def="同一场景中 CPU 主线程、渲染线程、GPU、I/O 和同步等待组成的最长执行链。">
  关键路径
</Term>
决定帧时间：
...
<Term def="P50、P95、P99 等分位数描述的慢帧分布，避免平均值掩盖卡顿。">
  长尾帧时间
</Term>
用于验收稳定性：
...
<Term def="画面、物理、交互、资源生命周期和双眼输出在优化前后保持目标行为。">
  正确性护栏
</Term>
约束候选不能靠删功能获胜：
...
<Term def="把有效场景、指标和阈值纳入持续验证，阻止后续版本重新引入性能问题。">
  性能回归门禁
</Term>
最终决定是否交付：
