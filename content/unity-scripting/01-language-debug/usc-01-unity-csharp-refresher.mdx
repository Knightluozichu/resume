...
<Term def="Project 中参与编译但尚未自动成为场景实例的 C# 源文件。">脚本资产</Term>
必须放回本章责任链：指出输入来自哪里、状态由谁拥有、结果由谁消费，以及错误时先观察哪一个信号。

<Term def="脚本类型挂载到具体 GameObject 后产生、拥有独立状态的对象。">组件实例</Term>
必须放回本章责任链：指出输入来自哪里、状态由谁拥有、结果由谁消费，以及错误时先观察哪一个信号。

<Term def="消费者通过基类或接口调用不同具体实现而无需知道其类型细节。">多态</Term>
必须放回本章责任链：指出输入来自哪里、状态由谁拥有、结果由谁消费，以及错误时先观察哪一个信号。

<Term def="用 get/set 控制读写、验证和值变化副作用的成员。">C# 属性</Term>
必须放回本章责任链：指出输入来自哪里、状态由谁拥有、结果由谁消费，以及错误时先观察哪一个信号。

<Term def="按字符串名称查找组件方法的 SendMessage/BroadcastMessage 运行时调用方式。">消息派发</Term>
必须放回本章责任链：指出输入来自哪里、状态由谁拥有、结果由谁消费，以及错误时先观察哪一个信号。
