...
import {
  Usc02DebuggingMapLab,
  Usc02DebuggingExperimentLab,
  Usc02DebuggingEvidenceLab,
} from "@/components/mdx/unity-scripting/diagrams/usc-02-debugging";

<Objectives>

- 能区分编译、运行时、逻辑、视觉和性能错误并选择对应观测工具

- 能实现由日志、Gizmo、Profiler、断点、Watch 与 Call Stack 组成的定位流程

- 能复现并验证包含输入、错误上下文、修复结果和回归样本的诊断记录
...
<Term def="源码无法生成程序集、会阻止进入运行阶段的语法或类型错误。">编译错误</Term>
必须放回本章责任链：指出输入来自哪里、状态由谁拥有、结果由谁消费，以及错误时先观察哪一个信号。

<Term def="程序能够运行但状态转换或结果不符合设计约束的错误。">逻辑错误</Term>
必须放回本章责任链：指出输入来自哪里、状态由谁拥有、结果由谁消费，以及错误时先观察哪一个信号。

<Term def="只有表达式满足时才暂停，用于隔离偶发或特定对象故障。">条件断点</Term>
必须放回本章责任链：指出输入来自哪里、状态由谁拥有、结果由谁消费，以及错误时先观察哪一个信号。

<Term def="显示当前执行点由哪些方法逐层调用而来的上下文。">调用栈</Term>
必须放回本章责任链：指出输入来自哪里、状态由谁拥有、结果由谁消费，以及错误时先观察哪一个信号。

<Term def="用 Gizmo、Debug.DrawLine 或覆盖图显示坐标、范围和路径。">视觉调试</Term>
必须放回本章责任链：指出输入来自哪里、状态由谁拥有、结果由谁消费，以及错误时先观察哪一个信号。
...
为数据状态加结构化日志，为空间关系加 Gizmo，为控制流加条件断点和 Watch，为来源读 Call Stack，为耗时加 Profiler marker。
...
```csharp
using (new Unity.Profiling.ProfilerMarker("Enemy.Decide").Auto()) {
    Decide();
}
```
...
<Callout type="trap">
  **因果陷阱**：大量无上下文 Debug.Log
  会改变时序、淹没关键信号并增加分配，日志数量不等于可观测性。
</Callout>

<Callout type="trap">
  **版本陷阱**：只在 Editor
  里调试会遗漏构建差异、平台权限、优化级别和目标设备性能。
</Callout>
...
**问题 1** 偶发只影响第 200 个敌人的错误适合什么断点？

<Answer>
  使用以对象 ID、索引或状态为条件的条件断点，避免前 199 次无关暂停。
</Answer>

**问题 2** 为什么 Gizmo 适合排查视线错误？
...
<Glossary>
  <GlossaryItem term="编译错误">
    源码无法生成程序集、会阻止进入运行阶段的语法或类型错误。
  </GlossaryItem>
  <GlossaryItem term="逻辑错误">
    程序能够运行但状态转换或结果不符合设计约束的错误。
  </GlossaryItem>
  <GlossaryItem term="条件断点">
    只有表达式满足时才暂停，用于隔离偶发或特定对象故障。
  </GlossaryItem>
  <GlossaryItem term="调用栈">
    显示当前执行点由哪些方法逐层调用而来的上下文。
  </GlossaryItem>
  <GlossaryItem term="视觉调试">
    用 Gizmo、Debug.DrawLine 或覆盖图显示坐标、范围和路径。
  </GlossaryItem>
</Glossary>
