...
<Term def="每帧由 OnGUI 代码重新声明控件并即时处理事件与绘制的旧界面模型。">
  即时模式 GUI
</Term>
不是孤立术语。它要放回本章的数据流中，说明谁产生它、谁修改它、谁消费它，以及配置错误时会出现什么可观察结果。

<Term def="界面元素作为持久对象存在，属性、层级、布局和事件可独立检查的模型。">
  保留模式 UI
</Term>
不是孤立术语。它要放回本章的数据流中，说明谁产生它、谁修改它、谁消费它，以及配置错误时会出现什么可观察结果。

<Term def="在 Transform 基础上加入锚点、轴心、尺寸与矩形偏移的 UI 几何组件。">
  Rect Transform
</Term>
不是孤立术语。它要放回本章的数据流中，说明谁产生它、谁修改它、谁消费它，以及配置错误时会出现什么可观察结果。

<Term def="组织 UI 图元并决定覆盖层、相机或世界空间渲染方式的根节点。">
  Canvas
</Term>
不是孤立术语。它要放回本章的数据流中，说明谁产生它、谁修改它、谁消费它，以及配置错误时会出现什么可观察结果。

<Term def="可序列化并能在 Inspector 连接监听器的 Unity 事件容器。">
  UnityEvent
</Term>
不是孤立术语。它要放回本章的数据流中，说明谁产生它、谁修改它、谁消费它，以及配置错误时会出现什么可观察结果。
