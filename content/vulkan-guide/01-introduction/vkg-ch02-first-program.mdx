...
section: "计算机图形学与渲染"
order: 2
description: "Vulkan安装、Hello World伪代码、初始化握手、交换链、着色器编译与渲染循环"
demo: true
math: false
...
- 能用自己的话解释你的第一个Vulkan伪代码程序的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：Vulkan安装与Hello World伪代码之间的关系是什么？

</Objectives>

## 为什么需要你的第一个Vulkan伪代码程序

Learning Vulkan将你的第一个Vulkan伪代码程序作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
Vulkan安装、Hello World伪代码、初始化握手、交换链、着色器编译与渲染循环正是这一哲学在特定领域的具体体现。
...
Vulkan 把你的第一个Vulkan伪代码程序从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。你的第一个Vulkan伪代码程序涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用你的第一个Vulkan伪代码程序相关功能前，必须完成：
...
启用验证层后，你的第一个Vulkan伪代码程序相关的常见错误会被捕获：
...
1. 你的第一个Vulkan伪代码程序是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出你的第一个Vulkan伪代码程序涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Learning Vulkan》（https://docs.vulkan.org/guide/latest/）第 2 章整理重写，
代码示例为教学简化版本。
</Attribution>
