...
section: "计算机图形学与渲染"
order: 3
description: "LunarG SDK、CMake工程设置、扩展、实例创建、物理设备与逻辑设备、队列族"
demo: true
math: false
...
- 能用自己的话解释连接硬件设备的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：LunarG SDK与CMake工程设置之间的关系是什么？

</Objectives>

## 为什么需要连接硬件设备

Learning Vulkan将连接硬件设备作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
LunarG SDK、CMake工程设置、扩展、实例创建、物理设备与逻辑设备、队列族正是这一哲学在特定领域的具体体现。
...
Vulkan 把连接硬件设备从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。连接硬件设备涉及的对象遵循统一的生命周期：
...
<Answer>
练习 1：画出连接硬件设备涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Learning Vulkan》（https://docs.vulkan.org/guide/latest/）第 3 章整理重写，
代码示例为教学简化版本。
</Attribution>
