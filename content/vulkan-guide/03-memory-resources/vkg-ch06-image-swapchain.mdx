...
section: "计算机图形学与渲染"
order: 6
description: "图像资源、图像视图、图像布局、交换链创建与深度图"
demo: true
math: false
...
- 能用自己的话解释图像资源分配与交换链构建的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：图像资源与图像视图之间的关系是什么？

</Objectives>

## 为什么需要图像资源分配与交换链构建

Learning Vulkan将图像资源分配与交换链构建作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
图像资源、图像视图、图像布局、交换链创建与深度图正是这一哲学在特定领域的具体体现。
...
Vulkan 把图像资源分配与交换链构建从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。图像资源分配与交换链构建涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用图像资源分配与交换链构建相关功能前，必须完成：
...
启用验证层后，图像资源分配与交换链构建相关的常见错误会被捕获：
...
1. 图像资源分配与交换链构建是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出图像资源分配与交换链构建涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Learning Vulkan》（https://docs.vulkan.org/guide/latest/）第 6 章整理重写，
代码示例为教学简化版本。
</Attribution>
