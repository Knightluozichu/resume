...
section: "计算机图形学与渲染"
order: 7
description: "缓冲区资源、渲染通道子通道与附件、帧缓存、录制渲染指令"
demo: true
math: false
...
- 能用自己的话解释缓存资源、渲染通道与帧缓存的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：缓冲区资源与渲染通道子通道与附件之间的关系是什么？

</Objectives>

## 为什么需要缓存资源、渲染通道与帧缓存

Learning Vulkan将缓存资源、渲染通道与帧缓存作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
缓冲区资源、渲染通道子通道与附件、帧缓存、录制渲染指令正是这一哲学在特定领域的具体体现。
...
Vulkan 把缓存资源、渲染通道与帧缓存从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。缓存资源、渲染通道与帧缓存涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用缓存资源、渲染通道与帧缓存相关功能前，必须完成：
...
启用验证层后，缓存资源、渲染通道与帧缓存相关的常见错误会被捕获：
...
1. 缓存资源、渲染通道与帧缓存是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出缓存资源、渲染通道与帧缓存涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Learning Vulkan》（https://docs.vulkan.org/guide/latest/）第 7 章整理重写，
代码示例为教学简化版本。
</Attribution>
