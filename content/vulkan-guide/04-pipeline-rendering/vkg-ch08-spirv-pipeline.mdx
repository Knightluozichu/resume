...
section: "计算机图形学与渲染"
order: 8
description: "SPIR-V概述、glslang编译器、图形流水线组件、流水线布局与状态"
demo: true
math: false
...
- 能用自己的话解释SPIR-V着色器与图形流水线的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：SPIR-V概述与glslang编译器之间的关系是什么？

</Objectives>

## 为什么需要SPIR-V着色器与图形流水线

Learning Vulkan将SPIR-V着色器与图形流水线作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
SPIR-V概述、glslang编译器、图形流水线组件、流水线布局与状态正是这一哲学在特定领域的具体体现。
...
Vulkan 把SPIR-V着色器与图形流水线从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。SPIR-V着色器与图形流水线涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用SPIR-V着色器与图形流水线相关功能前，必须完成：
...
启用验证层后，SPIR-V着色器与图形流水线相关的常见错误会被捕获：
...
1. SPIR-V着色器与图形流水线是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出SPIR-V着色器与图形流水线涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Learning Vulkan》（https://docs.vulkan.org/guide/latest/）第 8 章整理重写，
代码示例为教学简化版本。
</Attribution>
