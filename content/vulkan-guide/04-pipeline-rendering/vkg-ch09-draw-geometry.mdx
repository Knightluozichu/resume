...
section: "计算机图形学与渲染"
order: 9
description: "顶点缓冲区、索引缓冲区、Uniform Buffer、绘制指令录制、渲染循环与同步"
demo: true
math: false
...
- 能用自己的话解释绘制几何体的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：顶点缓冲区与索引缓冲区之间的关系是什么？

</Objectives>

## 为什么需要绘制几何体

Learning Vulkan将绘制几何体作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
顶点缓冲区、索引缓冲区、Uniform Buffer、绘制指令录制、渲染循环与同步正是这一哲学在特定领域的具体体现。
...
Vulkan 把绘制几何体从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。绘制几何体涉及的对象遵循统一的生命周期：
...
<Answer>
练习 1：画出绘制几何体涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Learning Vulkan》（https://docs.vulkan.org/guide/latest/）第 9 章整理重写，
代码示例为教学简化版本。
</Attribution>
