...
section: "计算机图形学与渲染"
order: 10
description: "纹理资源基础、图像上传至设备、采样器对象、着色器中采样纹理"
demo: true
math: false
...
- 能用自己的话解释使用纹理与采样器的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：纹理资源基础与图像上传至设备之间的关系是什么？

</Objectives>

## 为什么需要使用纹理与采样器

Learning Vulkan将使用纹理与采样器作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
纹理资源基础、图像上传至设备、采样器对象、着色器中采样纹理正是这一哲学在特定领域的具体体现。
...
Vulkan 把使用纹理与采样器从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。使用纹理与采样器涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用使用纹理与采样器相关功能前，必须完成：
...
1. 使用纹理与采样器是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出使用纹理与采样器涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Learning Vulkan》（https://docs.vulkan.org/guide/latest/）第 10 章整理重写，
代码示例为教学简化版本。
</Attribution>
