...
section: "计算机图形学与渲染"
order: 11
description: "描述符池与描述符集、更新描述符集、管线布局绑定、Push常量"
demo: true
math: false
...
- 能用自己的话解释描述符集与资源绑定的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：描述符池与描述符集与更新描述符集之间的关系是什么？

</Objectives>

## 为什么需要描述符集与资源绑定

Learning Vulkan将描述符集与资源绑定作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
描述符池与描述符集、更新描述符集、管线布局绑定、Push常量正是这一哲学在特定领域的具体体现。
...
Vulkan 把描述符集与资源绑定从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。描述符集与资源绑定涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用描述符集与资源绑定相关功能前，必须完成：
...
1. 描述符集与资源绑定是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出描述符集与资源绑定涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Learning Vulkan》（https://docs.vulkan.org/guide/latest/）第 11 章整理重写，
代码示例为教学简化版本。
</Attribution>
