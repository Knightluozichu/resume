...
section: "计算机图形学与渲染"
order: 13
description: "计算管线概念、工作组与局部调用、存储缓冲区、调度计算任务"
demo: true
math: false
...
- 能用自己的话解释计算流水线的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：计算管线概念与工作组与局部调用之间的关系是什么？

</Objectives>

## 为什么需要计算流水线

Learning Vulkan将计算流水线作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
计算管线概念、工作组与局部调用、存储缓冲区、调度计算任务正是这一哲学在特定领域的具体体现。
...
Vulkan 把计算流水线从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。计算流水线涉及的对象遵循统一的生命周期：
...
<Answer>
练习 1：画出计算流水线涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Learning Vulkan》（https://docs.vulkan.org/guide/latest/）第 13 章整理重写，
代码示例为教学简化版本。
</Attribution>
