...
section: "计算机图形学与渲染"
order: 14
description: "线程安全规则、多线程录制命令缓冲区、多线程资源创建、多队列使用"
demo: true
math: false
...
- 能用自己的话解释多线程Vulkan应用程序的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：线程安全规则与多线程录制命令缓冲区之间的关系是什么？

</Objectives>

## 为什么需要多线程Vulkan应用程序

Learning Vulkan将多线程Vulkan应用程序作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
线程安全规则、多线程录制命令缓冲区、多线程资源创建、多队列使用正是这一哲学在特定领域的具体体现。
...
Vulkan 把多线程Vulkan应用程序从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。多线程Vulkan应用程序涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用多线程Vulkan应用程序相关功能前，必须完成：
...
启用验证层后，多线程Vulkan应用程序相关的常见错误会被捕获：
...
1. 多线程Vulkan应用程序是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出多线程Vulkan应用程序涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Learning Vulkan》（https://docs.vulkan.org/guide/latest/）第 14 章整理重写，
代码示例为教学简化版本。
</Attribution>
