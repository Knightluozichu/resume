...
section: "计算机图形学与渲染"
order: 15
description: "实例化渲染、多重采样抗锯齿MSAA、动态管线状态、延迟渲染基础"
demo: true
math: false
...
- 能用自己的话解释高级渲染技术简介的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：实例化渲染与多重采样抗锯齿MSAA之间的关系是什么？

</Objectives>

## 为什么需要高级渲染技术简介

Learning Vulkan将高级渲染技术简介作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
实例化渲染、多重采样抗锯齿MSAA、动态管线状态、延迟渲染基础正是这一哲学在特定领域的具体体现。
...
Vulkan 把高级渲染技术简介从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。高级渲染技术简介涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用高级渲染技术简介相关功能前，必须完成：
...
1. 高级渲染技术简介是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出高级渲染技术简介涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Learning Vulkan》（https://docs.vulkan.org/guide/latest/）第 15 章整理重写，
代码示例为教学简化版本。
</Attribution>
