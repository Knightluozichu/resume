...
section: "计算机图形学与渲染"
order: 16
description: "Android平台Vulkan环境、窗口表面、移动端设备特性与性能考量"
demo: true
math: false
...
- 能用自己的话解释移动端Vulkan的核心概念与设计动机
- 能看懂并独立修改相关的 Vulkan API 调用代码
- 能回答：Android平台Vulkan环境与窗口表面之间的关系是什么？

</Objectives>

## 为什么需要移动端Vulkan

Learning Vulkan将移动端Vulkan作为独立章节，是因为它解决了 Vulkan 编程中一个不可绕过的核心问题。
...
Android平台Vulkan环境、窗口表面、移动端设备特性与性能考量正是这一哲学在特定领域的具体体现。
...
Vulkan 把移动端Vulkan从驱动黑盒中抽出来，变成应用可控的显式流程。
这带来三个好处：
...
Vulkan 中所有资源都是不透明句柄（handle）。移动端Vulkan涉及的对象遵循统一的生命周期：
...
在 Vulkan 中使用移动端Vulkan相关功能前，必须完成：
...
1. 移动端Vulkan是 Vulkan 显式控制哲学的具体体现
...
<Answer>
练习 1：画出移动端Vulkan涉及的对象生命周期图，标注每个阶段的 API 调用。
</Answer>
...
<Attribution>
内容基于《Learning Vulkan》（https://docs.vulkan.org/guide/latest/）第 16 章整理重写，
代码示例为教学简化版本。
</Attribution>
