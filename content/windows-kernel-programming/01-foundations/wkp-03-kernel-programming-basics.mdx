...
import { Wkp03IrqlLadderLab } from "@/components/mdx/windows-kernel-programming/v2/wkp-03-irql-ladder-lab";
...
<Glossary>
  ...
  <GlossaryItem term="分页池">
    一种内核内存池，分配的内存可能被系统换出到磁盘。优点是节省物理内存，缺点是访问时可能触发页错误。因此只能在 IRQL <= APC_LEVEL 时安全使用。用 ExAllocatePool2(POOL_FLAG_PAGED, ...) 分配。
  </GlossaryItem>
