...
import { Wkp07IrpJourneyLab } from "@/components/mdx/windows-kernel-programming/v2/wkp-07-irp-journey-lab";
...
<Callout type="warn">
  API 差异点：BUFFERED 的 <code>SystemBuffer</code> 由系统分配并复制，驱动完成后系统自动把输出拷回用户缓冲区；DIRECT 的 <code>MdlAddress</code> 描述用户页面，<code>MmGetSystemAddressForMdlSafe</code> 的返回必须判空（映射可能失败），且锁定的页面必须在 IRQL <= DISPATCH_LEVEL 访问。IOCTL 的传输方法在 CTL_CODE 里编码（第 4 章），读写的传输方法由设备对象 Flags 决定。
</Callout>
