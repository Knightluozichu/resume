---
title: "第2章 监听网络线路"
description: "在集线器、交换机和路由环境中选择端口镜像、分流器或受控ARP方法"
type: A
...
officialUnitId: ppa3-02-tapping-into-wire
---
...
import { Ppa302TappingIntoWireDecisionLab, Ppa302TappingIntoWireMechanismMap, Ppa302TappingIntoWireFailureDiagram } from "@/components/mdx/wireshark-packet-analysis/ppa3-02-tapping-into-wire";
...
<Term def="可复现网络流量的原始捕获文件">混杂模式</Term>、<Term def="定位数据包方向、字段和状态的关键概念">端口镜像</Term>、<Term def="连接原始字节与协议解释的分析要素">网络分流器</Term>、<Term def="用于构造最小反例和对照实验的变量">ARP缓存污染</Term>、<Term def="独立复核时必须能够重新定位的证据入口">捕获点</Term>
...
<Answer>
  另一位学习者无需口头补充即可定位全部10个正式目录节点，复现正常与故障事务，并依据“捕获点确实位于目标通信路径，并记录丢包、方向性和拓扑影响”得到相同结论。
</Answer>
...
<Glossary>
  <GlossaryItem term="混杂模式">
    混杂模式是本页帧级证据链的一部分。掌握标准是能在原始PCAP、协议字段或历史工具中定位，并构造最小失败反例。
  </GlossaryItem>
  <GlossaryItem term="端口镜像">
    端口镜像是本页帧级证据链的一部分。掌握标准是能在原始PCAP、协议字段或历史工具中定位，并构造最小失败反例。
  </GlossaryItem>
  <GlossaryItem term="网络分流器">
    网络分流器是本页帧级证据链的一部分。掌握标准是能在原始PCAP、协议字段或历史工具中定位，并构造最小失败反例。
  </GlossaryItem>
  <GlossaryItem term="ARP缓存污染">
    ARP缓存污染是本页帧级证据链的一部分。掌握标准是能在原始PCAP、协议字段或历史工具中定位，并构造最小失败反例。
  </GlossaryItem>
  <GlossaryItem term="捕获点">
    捕获点是本页帧级证据链的一部分。掌握标准是能在原始PCAP、协议字段或历史工具中定位，并构造最小失败反例。
  </GlossaryItem>
</Glossary>

[← 上一页：第1章 数据包分析技术与网络基础](/learn/wireshark-packet-analysis/02-ppa3-01-packet-analysis-network-basics/ppa3-01-packet-analysis-network-basics) · [下一页：第3章 Wireshark入门 →](/learn/wireshark-packet-analysis/04-ppa3-03-introduction-wireshark/ppa3-03-introduction-wireshark)
