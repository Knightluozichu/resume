---
title: "第7章 网络层协议"
description: "逐字段分析ARP、IPv4、IPv6、ICMP、ICMPv6和traceroute数据包"
type: A
...
officialUnitId: ppa3-07-network-layer-protocols
---
...
import { Ppa307NetworkLayerProtocolsDecisionLab, Ppa307NetworkLayerProtocolsMechanismMap, Ppa307NetworkLayerProtocolsFailureDiagram } from "@/components/mdx/wireshark-packet-analysis/ppa3-07-network-layer-protocols";
...
<Term def="可复现网络流量的原始捕获文件">ARP</Term>、<Term def="定位数据包方向、字段和状态的关键概念">IPv4</Term>、<Term def="连接原始字节与协议解释的分析要素">IPv6</Term>、<Term def="用于构造最小反例和对照实验的变量">ICMP</Term>、<Term def="独立复核时必须能够重新定位的证据入口">traceroute</Term>
...
<Answer>
  另一位学习者无需口头补充即可定位全部15个正式目录节点，复现正常与故障事务，并依据“链路地址、网络地址、协议字段、校验、跳数与请求响应能逐字节对应”得到相同结论。
</Answer>
...
[← 上一页：第6章 用命令行分析数据包](/learn/wireshark-packet-analysis/07-ppa3-06-command-line-analysis/ppa3-06-command-line-analysis) · [下一页：第8章 传输层协议 →](/learn/wireshark-packet-analysis/09-ppa3-08-transport-layer-protocols/ppa3-08-transport-layer-protocols)
