...
function processFile(fp) {
  ...
  function collectReturns(node, returns) {
    ...
    ts.forEachChild(node, c => collectReturns(c, returns));
  }
  function visit(node) {
    if (ts.isFunctionDeclaration(node) && node.name &&
        node.modifiers?.some(m => m.kind === ts.SyntaxKind.ExportKeyword)) {
      const returns = [];
      if (node.body) collectReturns(node.body, returns);
      for (const ret of returns) {
        let expr = ret.expression;
        if (ts.isParenthesizedExpression(expr)) expr = expr.expression;
        if (ts.isJsxElement(expr) && !expr.getText().includes('resetExperiment')) {
