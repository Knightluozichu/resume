 ...
 *
 * 检测逻辑：
 * 1. 读取 quality/original-toc.json 获取每本书的原版章节数
...
const GENERIC_SECTIONS = new Set([
  "00-fundamentals", "01-core", "02-advanced", "03-review",
  "00-foundations", "01-foundations", "02-advanced", "03-advanced",
]);
...
function countSubstantiveChapters(bookDir) {
  const chapters = [];
  const sections = fs.readdirSync(bookDir, { withFileTypes: true })
  ...
  for (const section of sections) {
    ...
    for (const file of files) {
      ...
      // 排除结构页
      if (/learning-map|final-review|official-learning-map|official-final-review/.test(slug)) continue;
...
function gradeBook(platformChapters, originalChapters, isGeneric, shortChapters) {
  if (!originalChapters || originalChapters <= 0) return { grade: "?", coverage: null };
  ...
  if (coverage >= 0.80) return { grade: "B", coverage };
  if (coverage >= 0.50) return { grade: "C", coverage };
...
for (const book of books) {
  ...
  const originalChapters = toc ? toc.originalChapters : null;

  const { grade, coverage } = gradeBook(platformCount, originalChapters, isGeneric, shortChapters);
...
console.log("=== 目录完整性审计 ===");
console.log(`A 级（>= 95%）：${grades.A.length} 本`);
console.log(`B 级（>= 80%）：${grades.B.length} 本`);
console.log(`C 级（< 80%）：${grades.C.length} 本`);
console.log(`D 级（< 50%）：${grades.D.length} 本`);
console.log(`? 级（无原版数据）：${grades["?"].length} 本`);
console.log("");
...
if (grades.D.length > 0) {
  ...
  for (const r of grades.D) {
    ...
    console.log(`  ${r.book}: ${r.platformChapters}/${r.originalChapters} [${flags.join("] [")}]`);
  }
  console.log("");
}
...
if (grades["?"].length > 0 && !singleBook) {
  console.log(`=== ? 级（无原版目录数据，需补充）：${grades["?"].length} 本 ===`);
  // 只列出使用通用模板的（高度疑似不合格）
...
// 退出码：有 C/D 级则返回 1
const failCount = grades.C.length + grades.D.length;
if (failCount > 0) {
  console.log(`\n总计 ${failCount} 本不合格。`);
  process.exit(1);
} else {
