...
function profilesFor(manifest, saved) {
  ...
  return walk(CONTENT_ROOT).map((filePath, order) => {
    ...
    const prior = saved?.find((item) => item.chapterSlug === chapterSlug) ?? extractOriginal(filePath); const core = MODELS[chapterSlug]; if (!core) throw new Error(`缺少章专属模型：${chapterSlug}`);
    const chain = ["冻结JDK与输入", "区分规范和实现", "执行章专属探针", "注入失败并恢复", "保存原始发布证据"];
    const model = { ...core, metric: `${core.studio}复现度`, risk: `${core.axisB.label}失真风险`, task: `${prior.focus}；执行下面探针，保存基线、变体、故障、恢复与复位证据。`, artifact: prior.artifact };
