...
function profilesFor(manifest) {
  ...
  return walk(CONTENT_ROOT).map((filePath, order) => {
    ...
    return {
      ...
      model: {
        ...model,
        metric: `${model.studio}可信度`,
        risk: `${model.axisA.label}误判风险`,
        task: `围绕${parsed.data.title}固定输入与目标配置；只改变${model.axisA.label}或${model.axisB.label}，保存基线、故障、恢复和复位证据。`,
      },
