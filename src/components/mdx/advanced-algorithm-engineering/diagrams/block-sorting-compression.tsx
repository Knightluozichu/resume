...
export function PaeFmBackwardSearchLab() {
  ...
  return (
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="border border-border bg-elevated p-4 sm:p-5">
        <div className="flex gap-2" role="group" aria-label="FM backward search 步骤">{fmSteps.map((item, index) => <button key={item.suffix} type="button" onClick={() => setStep(index)} className={"min-h-10 border px-4 text-sm font-semibold " + (step === index ? "border-accent bg-accent text-background" : "border-border bg-background text-primary")}>suffix {item.suffix}</button>)}</div>
