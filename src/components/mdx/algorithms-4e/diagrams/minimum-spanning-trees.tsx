...
function WeightedGraphCanvas({
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="edge-weighted graph"
    >
        ...
        return (
          <g key={edgeKey(edge)}>
            <line
              ...
              strokeWidth={current ? 6 : active ? 5 : cut ? 4 : 2}
            />
            <text
              x={(start.x + end.x) / 2}
              y={(start.y + end.y) / 2 - 5}
      ...
      <div className="border border-border bg-elevated p-4 sm:p-5">
        ...
        <div className="mt-3 border border-border bg-background p-3 text-xs text-secondary">
          ...
          <span className="font-mono text-primary">
            {incident(edge.v).map(edgeLabel).join(" · ")}
          </span>
        </div>
      </div>
      <figcaption className="mt-2 text-center text-sm text-secondary">
        Immutable Edge storestwo endpoints与real weight；undirected adjacency
        lists在两端引用同一个Edge object。
      </figcaption>
    </figure>
  );
}
function crossingEdges(left: readonly number[]) {
  ...
  return weightedEdges
    .filter((edge) => side.has(edge.v) !== side.has(edge.w))
...
class UnionFind {
  ...
  ids(count: number) {
    const roots = Array.from({ length: count }, (_, vertex) =>
      this.find(vertex),
    );
...
function lazyPrimTrace() {
  ...
  function scan(vertex: number) {
    ...
    for (const edge of incident(vertex))
      if (!marked.has(other(edge, vertex))) pq.push(edge);
  }
  scan(0);
  while (pq.length > 0 && mst.length < points.length - 1) {
    ...
    steps.push({
      edge,
      status: obsolete ? "obsolete" : "accept",
      marked: [...marked],
      mst: [...mst],
...
function eagerPrimTrace() {
  ...
  while (marked.some((value) => !value)) {
    ...
    for (let vertex = 0; vertex < points.length; vertex += 1) {
      if (
        !marked[vertex] &&
        (current === -1 || distTo[vertex] < distTo[current])
      )
        current = vertex;
    }
    ...
    states.push({
      current,
      marked: marked.flatMap((value, vertex) => (value ? [vertex] : [])),
      edgeTo: [...edgeTo],
      distTo: [...distTo],
      mst: edgeTo.filter(
        (edge): edge is WeightedEdge =>
          edge !== null && marked[edge.v] && marked[edge.w],
      ),
    });
  }
  return states;
...
function candidateEdges(mode: CertificateMode) {
  const safe =
    weightedEdges.find((edge) => edgeKey(edge) === "3-4") ?? weightedEdges[0];
  const heavy =
    weightedEdges.find((edge) => edgeKey(edge) === "2-4") ?? weightedEdges[0];
  if (mode === "cycle") return [...mstEdges, heavy];
  if (mode === "disconnected")
    return mstEdges.filter((edge) => edgeKey(edge) !== edgeKey(safe));
  if (mode === "nonoptimal")
    return mstEdges
      .filter((edge) => edgeKey(edge) !== edgeKey(safe))
      .concat(heavy);
  return mstEdges;
}
...
function certify(candidate: WeightedEdge[]) {
  ...
  if (cutOptimal) {
    for (const treeEdge of candidate) {
      ...
      for (const edge of candidate)
        if (edgeKey(edge) !== edgeKey(treeEdge)) cutUf.union(edge.v, edge.w);
      const lighterCrossing = weightedEdges.some(
        (edge) =>
          !cutUf.connected(edge.v, edge.w) &&
          edge.weight < treeEdge.weight - 1e-12,
      );
...
function Certificate({ label, pass }: { label: string; pass: boolean }) {
  return (
    ...
    >
      {label}
      <div className="mt-1 font-mono">{pass ? "PASS" : "FAIL"}</div>
    </div>
  );
}
