...
export function Aad817MemoryOptimizationDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第17章 内存优化：机制与证据"
      prompt="切换《第17章 内存优化》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第17章 内存优化》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Aad817MemoryOptimizationMechanismMap() {
  return (
    <ChapterMechanismMap title="第17章 内存优化：机制路径" stages={STAGES} />
  );
}

export function Aad817MemoryOptimizationFailureDiagram() {
  return (
    <ChapterFailureMatrix title="第17章 内存优化：失效与核验" stages={STAGES} />
  );
}
