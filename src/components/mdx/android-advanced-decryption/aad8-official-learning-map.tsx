...
const STAGES: readonly DecisionStage[] = [
  ...
  },
  {
    label: "原书目录核对清单",
    mechanism:
      "本节把「原书目录核对清单」放回《Android 8.0源码权威学习地图》的输入、状态变化与输出路径中理解。",
    failure:
...
export function Aad8OfficialLearningMapDecisionLab() {
  return (
    <ChapterDecisionLab
      title="Android 8.0源码权威学习地图：机制与证据"
      prompt="切换《Android 8.0源码权威学习地图》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《Android 8.0源码权威学习地图》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Aad8OfficialLearningMapMechanismMap() {
  return (
    <ChapterMechanismMap
      title="Android 8.0源码权威学习地图：机制路径"
      stages={STAGES}
    />
  );
}

export function Aad8OfficialLearningMapFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="Android 8.0源码权威学习地图：失效与核验"
      stages={STAGES}
    />
  );
}
