...
export function Aca1801ComponentFoundationsDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第1章 组件化基础：机制与证据"
      prompt="切换《第1章 组件化基础》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第1章 组件化基础》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Aca1801ComponentFoundationsMechanismMap() {
  return (
    <ChapterMechanismMap title="第1章 组件化基础：机制路径" stages={STAGES} />
  );
}

export function Aca1801ComponentFoundationsFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第1章 组件化基础：失效与核验"
      stages={STAGES}
    />
  );
}
