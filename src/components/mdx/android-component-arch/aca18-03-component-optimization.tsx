...
export function Aca1803ComponentOptimizationDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第3章 组件化优化：机制与证据"
      prompt="切换《第3章 组件化优化》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第3章 组件化优化》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Aca1803ComponentOptimizationMechanismMap() {
  return (
    <ChapterMechanismMap title="第3章 组件化优化：机制路径" stages={STAGES} />
  );
}

export function Aca1803ComponentOptimizationFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第3章 组件化优化：失效与核验"
      stages={STAGES}
    />
  );
}
