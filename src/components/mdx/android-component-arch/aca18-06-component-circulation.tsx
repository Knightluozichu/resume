...
export function Aca1806ComponentCirculationDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第6章 组件化流通：机制与证据"
      prompt="切换《第6章 组件化流通》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第6章 组件化流通》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Aca1806ComponentCirculationMechanismMap() {
  return (
    <ChapterMechanismMap title="第6章 组件化流通：机制路径" stages={STAGES} />
  );
}

export function Aca1806ComponentCirculationFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第6章 组件化流通：失效与核验"
      stages={STAGES}
    />
  );
}
