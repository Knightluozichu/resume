...
export function Aca18OfficialLearningMapDecisionLab() {
  return (
    <ChapterDecisionLab
      title="《Android组件化架构》权威学习地图：机制与证据"
      prompt="切换《《Android组件化架构》权威学习地图》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《《Android组件化架构》权威学习地图》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Aca18OfficialLearningMapMechanismMap() {
  return (
    <ChapterMechanismMap
      title="《Android组件化架构》权威学习地图：机制路径"
      stages={STAGES}
    />
  );
}

export function Aca18OfficialLearningMapFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="《Android组件化架构》权威学习地图：失效与核验"
      stages={STAGES}
    />
  );
}
