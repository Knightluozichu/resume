...
type Props = {
  model: CppGameBuildModel;
  view: "pipeline" | "frame" | "fault";
};

function BuildSlice({ model }: { model: CppGameBuildModel }) {
  ...
  return (
    <section
      ...
      data-visual-kind={`cpp-game-build-slice-${model.unitId}`}
    >
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-cyan-200 px-4 py-3 dark:border-cyan-900 sm:px-5">
        <div className="min-w-0 flex-1">
          ...
          <h3 className="mt-1 font-semibold">{model.title}：可运行切片</h3>
      ...
      <div className="grid min-w-0 gap-4 p-4 sm:p-5 lg:grid-cols-[0.78fr_1.22fr]">
        <div className="min-w-0 space-y-4">
          <div>
            <p className="text-xs font-semibold text-slate-500">选择输入或构建场景</p>
        ...
        <div className="min-w-0 rounded-xl bg-slate-950 p-4 text-slate-100">
          ...
          <p className="mt-2 break-words text-sm text-slate-200">
            <strong className="text-cyan-300">{concept}</strong>：
            {scenario.input}
          </p>
          ...
          <div className="mt-4 rounded-lg border border-slate-700 p-3">
            <p className="text-xs font-semibold text-cyan-300">可验收结果</p>
...
function FrameTrace({ model }: { model: CppGameBuildModel }) {
  ...
  return (
    <section
      ...
      data-visual-kind={`cpp-game-frame-trace-${model.unitId}`}
    >
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-violet-200 px-4 py-3 dark:border-violet-900 sm:px-5">
        <div>
          ...
          <h3 className="mt-1 font-semibold">{model.title}：状态执行轨迹</h3>
        </div>
        <button
          aria-label="重置状态轨迹"
...
function FaultReplay({ model }: { model: CppGameBuildModel }) {
  const [faultInjected, setFaultInjected] = useState(false);
  const [replay, setReplay] = useState(1);
  const events = [
    {
      phase: "固定构建与输入",
      detail: `第 ${replay} 次重放使用同一源码、资源、初始状态和输入序列`,
      pass: true,
    },
    {
      phase: "程序状态",
  ...
  return (
    <section
      ...
      data-visual-kind={`cpp-game-fault-replay-${model.unitId}`}
    >
...
export function CppGameBuildLab({ model, view }: Props) {
  if (view === "pipeline") return <BuildSlice model={model} />;
  if (view === "frame") return <FrameTrace model={model} />;
