...
type LabMode = "system" | "assembly" | "evidence";
...
export function OfficialCsi23BookLab({
  unitId,
  title,
  nodes,
  focuses,
  ...
  const result = useMemo(() => {
    const scenarioPenalty =
      scenario === "normal" ? 0 : scenario === "boundary" ? 16 : 34;
    const efficiency = Math.max(
      42,
      96 - Math.round(load * 0.18) - scenarioPenalty,
    );
    const residual = Math.max(
      0,
      Math.round((load - margin - scenarioPenalty / 2) * 0.12),
    );
    const accepted = scenario !== "fault" && residual <= 6 && margin >= 18;
    return { efficiency, residual, accepted };
  }, [load, margin, scenario, run]);
    ...
    >
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <button
          type="button"
          onClick={reset}
          className="inline-flex size-9 items-center justify-center rounded border border-zinc-300 bg-white hover:bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-950 dark:hover:bg-zinc-800"
          title="重置实验"
          aria-label="重置实验"
        >
      ...
      <div className="grid gap-0 lg:grid-cols-[minmax(0,1.25fr)_minmax(260px,0.75fr)]">
        <div className="border-b border-zinc-200 p-4 lg:border-b-0 lg:border-r dark:border-zinc-800">
          <div className="mb-4 flex items-center gap-2 text-sm font-semibold">
            <span
              aria-hidden="true"
              className="inline-flex size-5 items-center justify-center border border-current text-[10px]"
            >
              {mode === "system" ? "S" : mode === "assembly" ? "A" : "E"}
            </span>
            {mode === "system"
              ? "主路径追踪"
              : mode === "assembly"
          ...
          <div className="grid grid-cols-5 gap-1" style={{ minHeight: 118 }}>
        ...
        <div className="p-4">
          ...
          <dl className="mt-4 grid grid-cols-2 gap-2 text-sm">
            <div className="border border-zinc-200 p-3 dark:border-zinc-800">
              <dt className="text-zinc-500">路径效率</dt>
              <dd className="mt-1 text-xl font-semibold">
                {result.efficiency}%
              </dd>
            </div>
            <div className="border border-zinc-200 p-3 dark:border-zinc-800">
              <dt className="text-zinc-500">残差</dt>
              <dd className="mt-1 text-xl font-semibold">{result.residual}</dd>
            </div>
          </dl>
          <div
            className={
          ...
          >
            ...
            <p className="mt-1">
              首查：{focuses[Math.min(stage, focuses.length - 1)]}；运行 #{run}
            </p>
          </div>
          <button
            type="button"
