...
const PIPELINE_STAGES = [
  { label: "取指", detail: "PC + 指令", tone: COLORS.success },
  { label: "译码", detail: "操作数", tone: COLORS.accent },
  { label: "执行", detail: "功能单元", tone: COLORS.accent },
  { label: "访存", detail: "缓存层级", tone: COLORS.warning },
  { label: "提交", detail: "架构状态", tone: COLORS.success },
] as const;
...
export function Crv18Section0104PipelineDiagram() {
  const cardWidth = 166;
  const gap = 20;
  const startX = 22;
  const cardY = 122;
  ...
  return (
    <figure
      ...
      data-visual-kind="crv18-section-01-04-pipeline"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="CPU 指令从取指、译码、执行、访存到提交的流水线阶段"
...
const EVIDENCE_ROWS = [
  ...
  { label: "缓存", normal: "命中", boundary: "容量边界", fault: "未命中" },
  {
    label: "分支",
    normal: "预测命中",
    boundary: "路径变化",
...
export function Crv18Section0104EvidenceDiagram() {
  const columns = [
  ...
  return (
    <figure
      className="my-6 overflow-hidden rounded-card border border-border bg-elevated p-4"
      data-visual-kind="crv18-section-01-04-pipeline-evidence"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="CPU 指令、缓存、分支和提交在正常边界故障场景下的证据矩阵"
        className="mx-auto block h-auto w-full max-w-[940px]"
      >
        <text
          x="470"
...
const LAB_SCENARIOS = [
  {
    ...
    label: "基线流水线",
    tone: COLORS.success,
    title: "独立指令连续推进，缓存全部命中",
    evidence: "I1 IF→ID→EX→MEM hit→WB; I2 overlaps; cycles = reference",
    decision: "通过：阶段重叠，提交顺序与基线一致",
  },
  {
    id: 2,
    label: "依赖停顿",
    tone: COLORS.warning,
    title: "I2 读取 I1 尚未产生的结果",
    evidence: "I1 EX result pending → I2 ID waits → forward or bubble → commit",
    decision: "边界：记录转发或气泡，不把停顿算成错误结果",
  },
  {
    id: 3,
    label: "缓存故障",
    tone: COLORS.danger,
    title: "数据访问未命中且错误地继续提交",
    evidence: "MEM miss → fill pending → dependent commit before value ready",
    decision: "拒绝：提交越过访存证据，修复等待后重放",
  },
] as const;
...
export function Crv18Section0104Lab() {
  ...
  return (
    <section
      ...
      data-visual-kind="crv18-section-01-04-pipeline-lab"
      aria-label="CPU 流水线与缓存实验"
      aria-labelledby="crv18-section-01-04-lab-title"
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h3
            id="crv18-section-01-04-lab-title"
