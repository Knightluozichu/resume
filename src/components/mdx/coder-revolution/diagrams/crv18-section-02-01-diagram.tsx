...
const PORTABILITY_STAGES = [
  { label: "源码", detail: "Java API" },
  { label: "class", detail: "字节码格式" },
  { label: "验证", detail: "结构与版本" },
  { label: "JVM", detail: "解释/JIT" },
  { label: "平台", detail: "文件与权限" },
] as const;
...
export function Crv18Section0201PortabilityDiagram() {
  const cardWidth = 166;
  const gap = 20;
  const startX = 22;
  const cardY = 122;
  ...
  return (
    <figure
      ...
      data-visual-kind="crv18-section-02-01-portability"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="从 Java 源码、class 文件、验证、JVM 执行到平台服务的跨平台边界"
      ...
      >
        ...
        >
          可移植的是分层契约，平台资源仍需要最后一层适配与权限
        </text>
        {PORTABILITY_STAGES.slice(0, -1).map((stage, index) => {
        ...
        {PORTABILITY_STAGES.map((stage, index) => {
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          同一 class 文件不保证同一文件路径、权限、原生库和性能表现
        </text>
      </svg>
      <figcaption className="mt-2 text-center text-xs text-secondary">
        专属图示：把“跨平台”拆成可验证的类文件、JVM 和平台边界。
      </figcaption>
    </figure>
  );
}
...
const EVIDENCE_ROWS = [
  { label: "版本", normal: "目标匹配", boundary: "旧 JVM", fault: "拒绝加载" },
  {
    label: "类路径",
    normal: "依赖齐全",
    boundary: "缺少模块",
...
export function Crv18Section0201EvidenceDiagram() {
  const columns = [
  ...
  return (
    <figure
      className="my-6 overflow-hidden rounded-card border border-border bg-elevated p-4"
      data-visual-kind="crv18-section-02-01-evidence"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="Java 版本、类路径、执行和平台服务的正常、边界与单一故障证据矩阵"
        className="mx-auto block h-auto w-full max-w-[940px]"
      >
        <text
          x="470"
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          先定位构建、加载、执行或平台层，再选择修复动作
        </text>
      </svg>
      <figcaption className="mt-2 text-center text-xs text-secondary">
        专属图示：用版本、加载、执行和资源证据拆开“不能运行”。
      </figcaption>
    </figure>
  );
}
...
const LAB_SCENARIOS = [
  {
    ...
    label: "共同语义",
    tone: COLORS.success,
    title: "同一 class 在两台 JVM 输出一致",
    evidence: "verify ok → interpret/JIT → pure-memory result matches baseline",
    decision: "accept：语义通过，性能另行分层测量",
  },
  {
    id: 2,
    label: "版本边界",
    tone: COLORS.warning,
    title: "class 文件目标版本高于运行时",
    evidence:
      "compile ok → class version 65 → runtime supports 61 → load rejected",
    decision: "repair：降低目标或升级 JVM",
  },
  {
    id: 3,
    label: "平台故障",
    ...
    title: "语义计算成功但原生库缺失",
    evidence:
      "bytecode verified → computation ok → native dependency unavailable",
    decision: "adapt：保留语义结果并拒绝平台调用",
  },
] as const;

export function Crv18Section0201JvmLab() {
  ...
  return (
    <section
      ...
      data-visual-kind="crv18-section-02-01-jvm-lab"
      aria-label="Java class 文件与 JVM 边界样本实验"
      aria-labelledby="crv18-section-02-01-lab-title"
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h3
            id="crv18-section-02-01-lab-title"
      ...
      <p className="mt-4 text-xs text-secondary" aria-live="polite">
        当前样本：{scenario.label}；保存 class
        版本、JVM、类路径、执行阶段和平台错误。
      </p>
    </section>
  );
}
