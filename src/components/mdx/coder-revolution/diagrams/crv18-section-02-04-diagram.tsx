...
const JDBC_STAGES = [
  { label: "配置", detail: "驱动与 URL" },
  { label: "工厂", detail: "DataSource" },
  { label: "连接", detail: "事务边界" },
  { label: "语句", detail: "参数化 SQL" },
  { label: "结果", detail: "游标与关闭" },
] as const;
...
export function Crv18Section0204JdbcDiagram() {
  const cardWidth = 166;
  const gap = 20;
  const startX = 22;
  const cardY = 122;
  ...
  return (
    <figure
      ...
      data-visual-kind="crv18-section-02-04-jdbc"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="从驱动配置、工厂、数据源、连接、参数化语句到结果集的 JDBC 访问链"
      ...
      >
        ...
        >
          工厂选实现，驱动做翻译，连接管事务，语句与结果管资源
        </text>
        {JDBC_STAGES.slice(0, -1).map((stage, index) => {
        ...
        {JDBC_STAGES.map((stage, index) => {
          ...
          return (
            <g key={stage.label}>
              ...
              >
                {active ? "当前输入边界" : index < 2 ? "选择证据" : "资源证据"}
              </text>
            </g>
          );
        })}
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          统一 API 不代表方言、驱动版本、权限和事务语义完全相同
        </text>
      </svg>
      <figcaption className="mt-2 text-center text-xs text-secondary">
        专属图示：一次 JDBC 调用的每个抽象层都有可验证的责任。
      </figcaption>
    </figure>
  );
}
...
const EVIDENCE_ROWS = [
  ...
  { label: "参数", normal: "类型绑定", boundary: "空/大值", fault: "拼接注入" },
  {
    label: "资源",
    normal: "嵌套关闭",
    boundary: "池接近上限",
...
export function Crv18Section0204EvidenceDiagram() {
  const columns = [
  ...
  return (
    <figure
      className="my-6 overflow-hidden rounded-card border border-border bg-elevated p-4"
      data-visual-kind="crv18-section-02-04-evidence"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="JDBC 驱动、参数、资源和事务的正常、边界与单一故障证据矩阵"
        className="mx-auto block h-auto w-full max-w-[940px]"
      >
        <text
          x="470"
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          先区分配置、接口、驱动和资源层，再决定重试或修复
        </text>
      </svg>
      <figcaption className="mt-2 text-center text-xs text-secondary">
        专属图示：把 JDBC 的统一调用与差异、资源、事务证据放在一张表里。
      </figcaption>
    </figure>
  );
}
...
const LAB_SCENARIOS = [
  {
    ...
    label: "参数化查询",
    tone: COLORS.success,
    title: "输入作为参数绑定，不改变 SQL 结构",
    evidence:
      "prepare(sql) → bind(prefix) → execute → close result/statement/connection",
    decision: "accept：查询与资源合同成立",
  },
  {
    id: 2,
    label: "连接池边界",
    tone: COLORS.warning,
    title: "异常路径漏关连接，池逐渐耗尽",
    evidence: "borrow=20 → return=18 → waiters grow → timeout",
    decision: "repair：补齐关闭并验证借还平衡",
  },
  {
    id: 3,
    label: "事务故障",
    tone: COLORS.danger,
    title: "第二条写入失败，客户端准备重试",
    evidence:
      "autoCommit=false → statement-2 error → rollback → retry key check",
    decision: "rollback/retry：避免部分提交和重复写入",
  },
] as const;

export function Crv18Section0204JdbcLab() {
  ...
  return (
    <section
      ...
      data-visual-kind="crv18-section-02-04-jdbc-lab"
      aria-label="JDBC 参数、资源与事务样本实验"
      aria-labelledby="crv18-section-02-04-lab-title"
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h3
            id="crv18-section-02-04-lab-title"
            className="mt-1 text-lg font-semibold text-primary"
          >
            JDBC 访问边界实验
          </h3>
          <p className="mt-1 max-w-2xl text-sm text-secondary">
            只改变一个输入、资源或事务条件，观察接口调用如何分叉。
          </p>
        </div>
        <button
          type="button"
      ...
      <p className="mt-4 text-xs text-secondary" aria-live="polite">
        当前样本：{scenario.label}；保存驱动、参数、借还、事务和异常记录。
      </p>
    </section>
  );
}
