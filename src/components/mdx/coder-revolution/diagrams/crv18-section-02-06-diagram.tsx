...
const RENDER_STAGES = [
  { label: "请求", detail: "身份与参数" },
  { label: "控制器", detail: "授权与业务" },
  { label: "模型", detail: "只读数据" },
  { label: "模板", detail: "标签与渲染" },
  { label: "响应", detail: "转义与缓存" },
] as const;
...
export function Crv18Section0206RenderDiagram() {
  const cardWidth = 166;
  const gap = 20;
  const startX = 22;
  const cardY = 122;
  ...
  return (
    <figure
      ...
      data-visual-kind="crv18-section-02-06-render"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="从 HTTP 请求、控制器、只读模型、模板渲染到转义响应的服务器端页面链"
      ...
      >
        ...
        >
          每个动态插槽都要声明上下文、转义、缓存和失败行为
        </text>
        {RENDER_STAGES.slice(0, -1).map((stage, index) => {
        ...
        {RENDER_STAGES.map((stage, index) => {
          ...
          return (
            <g key={stage.label}>
              ...
              >
                {active ? "当前表示层" : index < 2 ? "输入证据" : "输出证据"}
              </text>
            </g>
          );
        })}
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          HTML 文本、属性、URL、JavaScript 和 CSS 插槽不能共用一个转义假设
        </text>
      </svg>
...
const EVIDENCE_ROWS = [
  { label: "输入", normal: "参数受控", boundary: "长/空值", fault: "未授权" },
  { label: "模型", normal: "字段最小", boundary: "缺值", fault: "敏感字段" },
  {
    label: "插槽",
    normal: "上下文明确",
    boundary: "属性/URL",
...
export function Crv18Section0206EvidenceDiagram() {
  const columns = [
  ...
  return (
    <figure
      className="my-6 overflow-hidden rounded-card border border-border bg-elevated p-4"
      data-visual-kind="crv18-section-02-06-evidence"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="服务器端页面输入、模型、动态插槽和缓存的正常、边界与单一故障证据矩阵"
        className="mx-auto block h-auto w-full max-w-[940px]"
      >
        <text
          x="470"
...
const LAB_SCENARIOS = [
  {
    ...
    label: "正常渲染",
    tone: COLORS.success,
    title: "控制器提供最小模型，模板生成完整 HTML",
    evidence:
      "request-id → authorized model → escaped text/URL → 200 complete response",
    decision: "accept：响应完整且缓存键可解释",
  },
  {
    id: 2,
    label: "上下文边界",
    tone: COLORS.warning,
    title: "同一用户值从文本插槽进入 URL 属性",
    evidence: "text escape ≠ URL encode → validate scheme → render safe href",
    decision: "repair：按插槽上下文编码与限制",
  },
  {
    id: 3,
    label: "缓存故障",
    tone: COLORS.danger,
    title: "缓存只按路径键导致跨用户串页",
    evidence: "cache key=/profile → user A model → user B receives A fragment",
    decision: "invalidate：加入身份/版本或禁止共享缓存",
  },
] as const;

export function Crv18Section0206JspLab() {
  ...
  return (
    <section
      ...
      data-visual-kind="crv18-section-02-06-jsp-lab"
      aria-label="服务器端页面渲染与缓存样本实验"
      aria-labelledby="crv18-section-02-06-lab-title"
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h3
            id="crv18-section-02-06-lab-title"
