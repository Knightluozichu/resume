...
const QUEUE_STAGES = [
  { label: "生产消息", detail: "业务键 + 版本" },
  { label: "代理持久化", detail: "保存 + 位置" },
  { label: "分派消费", detail: "组 + 尝试次数" },
  { label: "业务落库", detail: "原子副作用" },
  { label: "确认/重试", detail: "ACK 或重放" },
] as const;
...
export function Crv18Section0207QueueDiagram() {
  const cardWidth = 166;
  const gap = 20;
  const startX = 22;
  const cardY = 120;
  ...
  return (
    <figure
      ...
      data-visual-kind="crv18-section-02-07-queue"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="消息从生产、代理持久化、分派消费到业务落库并确认或重试的状态链"
      ...
      >
        ...
        >
          同一个业务键贯穿消息、落库、确认和安全重放
        </text>
        {QUEUE_STAGES.slice(0, -1).map((stage, index) => {
        ...
        {QUEUE_STAGES.map((stage, index) => {
          ...
          return (
            <g key={stage.label}>
              ...
              >
                {active ? "副作用边界" : index < 2 ? "传输证据" : "结果证据"}
              </text>
            </g>
          );
        })}
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          确认丢失不等于业务丢失：先用业务键判断已应用，再安全重放
        </text>
      </svg>
      <figcaption className="mt-2 text-center text-xs text-secondary">
        专属图示：消息代理把同步等待拆成可查询、可恢复的五段轨迹。
      </figcaption>
    </figure>
  );
}
...
const EVIDENCE_ROWS = [
  ...
  { label: "恢复", normal: "ACK", boundary: "有限重试", fault: "幂等重放" },
] as const;

export function Crv18Section0207EvidenceDiagram() {
  const columns = [
  ...
  return (
    <figure
      className="my-6 overflow-hidden rounded-card border border-border bg-elevated p-4"
      data-visual-kind="crv18-section-02-07-evidence"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="消息身份、代理、业务落库和恢复的正常边界故障证据矩阵"
        className="mx-auto block h-auto w-full max-w-[940px]"
      >
        <text
          x="470"
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          先记录最早偏离，再判断 ACK、重试或隔离是否正确
        </text>
      </svg>
      <figcaption className="mt-2 text-center text-xs text-secondary">
        专属图示：同一业务键把传输证据、业务副作用和恢复动作串成闭环。
      </figcaption>
    </figure>
  );
}
...
const LAB_SCENARIOS = [
  {
    ...
    label: "正常投递",
    tone: COLORS.success,
    title: "消息保存、一次落库，随后确认",
    evidence: "m-104 → saved → delivered → applied(order-7) → ACK",
    decision: "accept：一次业务效果，积压回到基线",
  },
  {
    id: 2,
    label: "重复边界",
    ...
    title: "同一业务键被两次取出",
    evidence:
      "m-104 + retry → unique businessKey → second read = already applied",
    decision: "dedupe：第二次只确认，不重复副作用",
  },
  {
    id: 3,
    label: "确认前断线",
    tone: COLORS.danger,
    title: "业务提交后消费者在 ACK 前退出",
    evidence: "applied(order-7) → connection lost → redeliver → idempotent hit",
    decision: "replay：重放得到同一结果，再确认消息",
  },
] as const;

export function Crv18Section0207QueueLab() {
  ...
  return (
    <section
      ...
      data-visual-kind="crv18-section-02-07-queue-lab"
      aria-label="消息队列投递与幂等重放实验"
      aria-labelledby="crv18-section-02-07-lab-title"
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h3
            id="crv18-section-02-07-lab-title"
      ...
      <p className="mt-4 text-xs text-secondary" aria-live="polite">
        当前样本：{scenario.label}；保存消息
        ID、业务键、尝试次数、落库状态和确认结果。
      </p>
    </section>
  );
}
