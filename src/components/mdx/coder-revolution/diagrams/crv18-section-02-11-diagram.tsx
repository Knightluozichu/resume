...
const LOGGING_STAGES = [
  { label: "创建事件", detail: "事实 + trace" },
  { label: "选择 Logger", detail: "层级 + 配置" },
  { label: "级别过滤", detail: "阈值判断" },
  { label: "布局格式", detail: "文本/JSON" },
  { label: "Appender 输出", detail: "交付/降级" },
] as const;
...
export function Crv18Section0211LoggingDiagram() {
  const cardWidth = 166;
  const gap = 20;
  const startX = 22;
  const cardY = 120;
  ...
  return (
    <figure
      ...
      data-visual-kind="crv18-section-02-11-logging"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="日志从创建事件、选择 Logger、级别过滤、布局格式到 Appender 输出的管线"
      ...
      >
        ...
        >
          调用点只提交事件，过滤、布局和目的地各自保留可验证边界
        </text>
        {LOGGING_STAGES.slice(0, -1).map((stage, index) => {
        ...
        {LOGGING_STAGES.map((stage, index) => {
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          被过滤的事件不应执行昂贵构造；Appender 失败也要有可解释降级
        </text>
      </svg>
...
const EVIDENCE_ROWS = [
  ...
  { label: "交付", normal: "写入成功", boundary: "背压", fault: "端点超时" },
] as const;

export function Crv18Section0211EvidenceDiagram() {
  const columns = [
  ...
  return (
    <figure
      className="my-6 overflow-hidden rounded-card border border-border bg-elevated p-4"
      data-visual-kind="crv18-section-02-11-evidence"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="日志事件、过滤、布局和交付在正常边界故障场景下的证据矩阵"
        className="mx-auto block h-auto w-full max-w-[940px]"
      >
        <text
          x="470"
...
const LAB_SCENARIOS = [
  {
    ...
    label: "正常记录",
    tone: COLORS.success,
    title: "INFO 开启，结构化事件交给本地 Appender",
    evidence: "order.created + trace-17 → threshold pass → JSON → file append",
    decision: "accept：字段可检索，业务线程只提交一次事件",
  },
  {
    id: 2,
    label: "级别关闭",
    tone: COLORS.warning,
    title: "DEBUG 关闭，昂贵参数计算不应发生",
    evidence:
      "debug disabled → supplier not evaluated → constructed=0 → no output",
    decision: "protect：过滤提前，成本计数保持为零",
  },
  {
    id: 3,
    label: "Appender 超时",
    tone: COLORS.danger,
    title: "网络目的地超时，走有限等待与本地兜底",
    evidence:
      "JSON event → endpoint timeout → bounded wait → local fallback + alert",
    decision: "degrade：业务不被拖住，trace 仍能定位故障",
  },
] as const;

export function Crv18Section0211LoggingLab() {
  ...
  return (
    <section
      ...
      data-visual-kind="crv18-section-02-11-logging-lab"
      aria-label="日志级别与 Appender 实验"
      aria-labelledby="crv18-section-02-11-lab-title"
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h3
            id="crv18-section-02-11-lab-title"
