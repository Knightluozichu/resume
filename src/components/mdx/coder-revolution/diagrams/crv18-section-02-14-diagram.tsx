...
const SPRING_STAGES = [
  { label: "Bean 定义", detail: "类型 + 作用域" },
  { label: "创建实例", detail: "构造/初始化" },
  { label: "解析依赖", detail: "有向图" },
  { label: "生成代理", detail: "切点 + 通知" },
  { label: "调用业务", detail: "入口可追踪" },
] as const;
...
export function Crv18Section0214SpringDiagram() {
  const cardWidth = 166;
  const gap = 20;
  const startX = 22;
  const cardY = 120;
  ...
  return (
    <figure
      ...
      data-visual-kind="crv18-section-02-14-spring"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="Spring 从 Bean 定义、创建实例、解析依赖、生成代理到调用业务的容器链"
      ...
      >
        ...
        >
          目标对象由容器创建，调用者拿到的可能是承接通知的代理
        </text>
        {SPRING_STAGES.slice(0, -1).map((stage, index) => {
...
const EVIDENCE_ROWS = [
  { label: "定义", normal: "扫描命中", boundary: "多实现", fault: "缺 Bean" },
  {
    label: "实例",
    normal: "身份稳定",
    boundary: "作用域变更",
  ...
  { label: "依赖", normal: "图可排序", boundary: "可选边", fault: "循环边" },
  {
    label: "通知",
    normal: "外部入代理",
    boundary: "代理限制",
...
export function Crv18Section0214EvidenceDiagram() {
  const columns = [
  ...
  return (
    <figure
      className="my-6 overflow-hidden rounded-card border border-border bg-elevated p-4"
      data-visual-kind="crv18-section-02-14-evidence"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="Spring 定义、实例、依赖和通知在正常边界故障场景下的证据矩阵"
        className="mx-auto block h-auto w-full max-w-[940px]"
      >
        <text
          x="470"
...
const LAB_SCENARIOS = [
  {
    ...
    label: "容器装配",
    tone: COLORS.success,
    title: "容器创建 Checkout 并注入 PaymentPort",
    evidence:
      "bean definition → constructor injection → proxy → external submit",
    decision: "accept：依赖身份稳定，外部入口经过容器边界",
  },
  {
    id: 2,
    label: "循环依赖",
    tone: COLORS.warning,
    title: "A 需要 B，B 又需要 A，图无法拓扑排序",
    evidence: "A → B → A → unresolved cycle → split collaborator",
    decision: "repair：拆分端口或改变生命周期，不填充半成品对象",
  },
  {
    id: 3,
    label: "自调用绕过",
    tone: COLORS.danger,
    title: "外部调用有通知，this.work() 直接进入目标",
    evidence: "proxy.entry → advice=1; target.thisCall → advice=0",
    decision: "reject：改为协作者/代理入口，并用计数器验证",
  },
] as const;

export function Crv18Section0214SpringLab() {
  ...
  return (
    <section
      ...
      data-visual-kind="crv18-section-02-14-spring-lab"
      aria-label="Spring Bean 与代理实验"
      aria-labelledby="crv18-section-02-14-lab-title"
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h3
            id="crv18-section-02-14-lab-title"
      ...
      <p
        ...
        aria-live="polite"
      >
        当前样本：{scenario.label}；保存 Bean
        定义、实例身份、依赖边、代理入口和通知计数。
      </p>
    </section>
  );
}
