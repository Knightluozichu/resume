...
const URI_STAGES = [
  { label: "创建URI", detail: "资源标识", tone: COLORS.accent },
  { label: "解析目标", detail: "基准 + 链接", tone: COLORS.success },
  { label: "HTTP请求", detail: "方法 + 消息", tone: COLORS.warning },
  { label: "返回表示", detail: "状态 + 内容", tone: COLORS.accent },
  { label: "跟随链接", detail: "下一资源", tone: COLORS.success },
] as const;
...
export function Crv18Section0301UriDiagram() {
  const cardWidth = 166;
  const gap = 20;
  const startX = 22;
  const cardY = 122;
  ...
  return (
    <figure
      ...
      data-visual-kind="crv18-section-03-01-uri-chain"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="Web 起源从创建 URI、解析目标、HTTP 请求、返回表示到跟随链接"
      ...
      >
        ...
        >
          URI 不是文件路径，链接不是复制文件，HTTP 表示也不是资源本身
        </text>
        {URI_STAGES.slice(0, -1).map((stage, index) => {
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          开放协议让客户端、服务器和资源提供者可以分别实现
        </text>
      </svg>
      <figcaption className="mt-2 text-center text-xs text-secondary">
        专属图示：把 URI 标识、HTTP 交换和超文本关系放入一条可重放路径。
      </figcaption>
    </figure>
  );
}
...
const EVIDENCE_ROWS = [
  { label: "标识", normal: "稳定", boundary: "相对", fault: "内路径" },
  { label: "目标", normal: "可解析", boundary: "重定向", fault: "非法" },
  { label: "表示", normal: "兼容", boundary: "多媒体", fault: "404" },
  { label: "链接", normal: "下一步", boundary: "有限", fault: "循环" },
] as const;

export function Crv18Section0301EvidenceDiagram() {
  const columns = [
  ...
  return (
    <figure
      className="my-6 overflow-hidden rounded-card border border-border bg-elevated p-4"
      data-visual-kind="crv18-section-03-01-uri-evidence"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="Web URI 标识、目标、表示和链接在正常边界故障场景下的证据矩阵"
        className="mx-auto block h-auto w-full max-w-[940px]"
      >
        <text
          x="470"
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          URL 是 URI 的一种定位形式，但资源与服务器文件路径仍然分离
        </text>
      </svg>
      <figcaption className="mt-2 text-center text-xs text-secondary">
        专属图示：分别验收标识、解析、表示和链接跟随的协议边界。
      </figcaption>
    </figure>
  );
}
...
const LAB_SCENARIOS = [
  {
    ...
    label: "绝对 URI",
    tone: COLORS.success,
    title: "绝对 URI 直接解析到允许的资源表示",
    evidence: "base=— → https://example.test/a → GET → 200 text/html → follow",
    decision: "通过：资源标识与表示语义一致，跟随次数可重放",
  },
  {
    id: 2,
    label: "相对链接",
    tone: COLORS.warning,
    title: "相对链接必须结合当前文档基准解析",
    evidence: "base=/docs/index.html + ../api → /api → GET → 200",
    decision: "边界：记录基准和规范化目标，不使用服务器内部路径",
  },
  {
    id: 3,
    label: "非法目标",
    tone: COLORS.danger,
    title: "链接携带非法 scheme 或形成循环，策略先拒绝",
    evidence: "href=file:.../secret or cycle → policy reject → no request",
    decision: "拒绝：保留解析链和拒绝原因，清空历史后重放",
  },
] as const;
...
export function Crv18Section0301Lab() {
  ...
  return (
    <section
      ...
      data-visual-kind="crv18-section-03-01-uri-lab"
      aria-label="URI 解析与超文本实验"
      aria-labelledby="crv18-section-03-01-lab-title"
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h3
            id="crv18-section-03-01-lab-title"
            className="mt-1 text-lg font-semibold text-primary"
          >
            URI 解析与表示实验
          </h3>
          <p className="mt-1 max-w-2xl text-sm text-secondary">
            一次只改变基准、链接或目标策略，观察解析结果和下一次请求。
          </p>
        </div>
        <button
          type="button"
      ...
      <p
        ...
        aria-live="polite"
      >
        当前场景：{scenario.label}；记录基准
        URI、解析目标、请求、状态码、媒体类型、跟随次数和复位。
      </p>
    </section>
  );
}
