...
const OFFICIAL_CONCEPT_LABELS = ["3.9 什么是框架"] as const;

const FRAMEWORK_STAGES = [
  { label: "启动框架", detail: "上下文 + 控制流" },
  { label: "读取配置", detail: "环境 + 对象图" },
  { label: "创建扩展", detail: "注入 + owner" },
  { label: "回调应用", detail: "请求 + 事件" },
  { label: "统一收尾", detail: "释放 + 关闭" },
] as const;
...
export function Crv18Section0309FrameworkDiagram() {
  const cardWidth = 166;
  const gap = 20;
  const startX = 22;
  const cardY = 120;
  ...
  return (
    <figure
      ...
      data-visual-kind="crv18-section-03-09-framework"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label={`3.9 什么是框架的生命周期证据链：${OFFICIAL_CONCEPT_LABELS.join("、")}`}
      ...
      >
        ...
        >
          配置、对象图、回调和资源收尾必须落在同一上下文中
        </text>
        {FRAMEWORK_STAGES.slice(0, -1).map((stage, index) => {
        ...
        {FRAMEWORK_STAGES.map((stage, index) => {
          ...
          return (
            <g key={stage.label}>
              ...
              >
                {active ? "控制反转边界" : index < 2 ? "启动证据" : "收尾证据"}
              </text>
            </g>
          );
        })}
...
export function Crv18Section0309EvidenceDiagram() {
  const columns = [
  ...
  return (
    <figure
      className="my-6 overflow-hidden rounded-card border border-border bg-elevated p-4"
      data-visual-kind="crv18-section-03-09-evidence"
    >
      <svg
        viewBox="0 0 940 390"
        role="img"
        aria-label="3.9 什么是框架的控制流、对象图、回调和收尾在正常边界故障场景下的证据矩阵"
        className="mx-auto block h-auto w-full max-w-[940px]"
      >
        <text
          x="470"
...
const LAB_SCENARIOS = [
  {
    ...
    label: "生命周期闭环",
    tone: COLORS.success,
    title: "框架创建对象并在正常关闭时释放全部资源",
    evidence:
      "start → config=ok → extension=managed → callback=ok → close=resources-zero",
    decision: "accept：控制反转与收尾合同成立",
  },
  {
    id: 2,
    label: "配置边界",
    tone: COLORS.warning,
    title: "缺少必填配置，在创建扩展前明确拒绝",
    evidence:
      "start → config=missing → callback=not-called → close=context-released",
    decision: "reject：错误发生在正确阶段且可诊断",
  },
  {
    id: 3,
    label: "绕过容器",
    tone: COLORS.danger,
    title: "手动构造对象，业务返回但代理与资源 owner 缺失",
    evidence:
      "manual-new → callback=ok → interceptor=missing → close=leak-risk",
    decision: "fail：记录绕过入口并恢复框架创建路径",
  },
] as const;

export function Crv18Section0309FrameworkLab() {
  ...
  return (
    <section
      ...
      data-visual-kind="crv18-section-03-09-framework-lab"
      aria-label="框架生命周期与控制反转实验"
      aria-labelledby="crv18-section-03-09-lab-title"
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h3
            id="crv18-section-03-09-lab-title"
      ...
      <p
        ...
        aria-live="polite"
      >
        当前样本：{scenario.label}；保存上下文 ID、配置版本、对象 ID、回调、资源
        owner、首错和复位轨迹。
      </p>
    </section>
  );
