      ...
      <div className="overflow-hidden border border-border bg-elevated p-4 sm:p-5">
        <svg viewBox="0 0 820 340" role="img" aria-label="数组34512由两个非递减子数组345和12组成，最小值位于第二段起点。" className="mx-auto block h-auto w-full max-w-[820px]">
          ...
          {values.map((value, index) => {
            ...
            return <g key={value + "-" + index}><rect x={x} y="105" width="94" height="72" rx="5" fill={active ? "var(--success)" : "var(--bg)"} fillOpacity={active ? 0.12 : 1} stroke={active ? "var(--success)" : "var(--border)"} strokeWidth={active ? 2 : 1} /><text x={x + 47} y="148" textAnchor="middle" fontSize="20" fontWeight="700" fill="var(--text-primary)">{value}</text><text x={x + 47} y="198" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">index {index}</text></g>;
          })}
          ...
          <text x="410" y="298" textAnchor="middle" fontSize="10.5" fill="var(--text-primary)">二分不依赖数值连续，只依赖两段内部非递减和边界关系。</text>
        </svg>
      </div>
    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden border border-border bg-elevated p-4 sm:p-5">
        <svg viewBox="0 0 820 300" role="img" aria-label="数组10111在左中右位置都是1，无法判断最小值0位于哪一侧。" className="mx-auto block h-auto w-full max-w-[820px]">
          ...
          {[[1,120,"left"],[0,238,""],[1,356,"mid"],[1,474,""],[1,592,"right"]].map(([value,x,label], index) => <g key={index}><rect x={Number(x)} y="100" width="92" height="64" rx="5" fill={value === 0 ? "var(--warning)" : "var(--bg)"} fillOpacity={value === 0 ? 0.12 : 1} stroke={value === 0 ? "var(--warning)" : "var(--border)"} /><text x={Number(x)+46} y="138" textAnchor="middle" fontSize="18" fontWeight="700" fill="var(--text-primary)">{value}</text><text x={Number(x)+46} y="184" textAnchor="middle" fontSize="10" fill="var(--accent)">{label}</text></g>)}
          <path d="M166 214 C285 260 535 260 638 214" fill="none" stroke="var(--warning)" strokeWidth="3" strokeDasharray="6 4" />
          <text x="410" y="262" textAnchor="middle" fontSize="10.5" fontWeight="700" fill="var(--warning)">left=mid=right=1不能排除任何一侧，只能逐项确认0。</text>
        </svg>
      </div>
