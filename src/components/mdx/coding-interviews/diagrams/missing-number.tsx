    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="border border-border bg-elevated p-4 sm:p-5">
          ...
          {values.map((value, index) => {
            ...
            return (
              <div
                key={index}
                className={
                  "relative flex min-h-[92px] flex-col items-center justify-center border p-2 " +
                  (index === 3
              ...
              >
                {index === 3 && (
                  <span className="absolute -top-2 bg-danger px-1 text-[10px] font-semibold text-white">
                    首个错位
                  </span>
                )}
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          <div className="border-l-4 border-success bg-background p-3 text-sm text-secondary">
            下标 0 到 2：缺口之前，值与下标一致
          </div>
          <div className="border-l-4 border-warning bg-background p-3 text-sm text-secondary">
            下标 3 到 4：缺口之后，值整体比下标大 1
          </div>
        </div>
      </div>
