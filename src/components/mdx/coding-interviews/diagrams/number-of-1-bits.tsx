      ...
      <div className="overflow-hidden border border-border bg-elevated p-4 sm:p-5">
        <svg viewBox="0 0 820 330" role="img" aria-label="数值10的低8位为00001010，单比特掩码从右向左检查每一位。" className="mx-auto block h-auto w-full max-w-[820px]">
          ...
          {bits.map((bit, index) => {
            ...
            const active = bit === "1";
            return <g key={index}><rect x={x} y="102" width="62" height="62" rx="4" fill={active ? "var(--success)" : "var(--bg)"} fillOpacity={active ? 0.12 : 1} stroke={active ? "var(--success)" : "var(--border)"} /><text x={x+31} y="141" textAnchor="middle" fontSize="19" fontWeight="700" fill="var(--text-primary)">{bit}</text><text x={x+31} y="188" textAnchor="middle" fontSize="9.5" fill="var(--text-secondary)">bit {7-index}</text></g>;
          })}
          ...
          <text x="620" y="260" textAnchor="middle" fontSize="10.5" fontWeight="700" fill="var(--accent)">mask每轮左移一位</text>
          <rect x="205" y="286" width="410" height="28" rx="5" fill="var(--accent)" fillOpacity="0.06" stroke="var(--accent)" />
          <text x="410" y="305" textAnchor="middle" fontSize="10.5" fill="var(--text-primary)">value &amp; mask 非零，说明该位置为1并计数。</text>
        </svg>
      </div>
    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-x-auto border border-border bg-elevated p-4 sm:p-5">
        <table className="w-full min-w-[680px] border-collapse text-left text-sm">
          ...
          <tbody>{stages.map((row) => <tr key={row[0]} className="border-b border-border last:border-0">{row.map((cell, index) => <td key={cell} className={"p-3 " + (index === 1 ? "font-mono font-semibold text-accent" : "text-secondary")}>{cell}</td>)}</tr>)}</tbody>
        </table>
      </div>
