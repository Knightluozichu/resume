...
export function StringToIntContractDiagram() {
  const phases = [
    ["1", "重置", "g_nStatus = kInvalid", "防止沿用上一次成功状态"],
    ["2", "入口检查", "非空指针且非空串", "否则返回无效零"],
    ["3", "符号检查", "可选一个 + 或 -", "符号后必须还有字符"],
    ["4", "数字核心", "每个字符都必须是 0…9", "非法字符或越界立即失败"],
    ["5", "提交", "指针到达终止符", "只有完整消费才置 kValid"],
  ] as const;

  return (
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="grid gap-2 border border-border bg-elevated p-4 sm:grid-cols-5 sm:p-5">
        {phases.map(([index, title, rule, reason]) => (
          <div key={index} className="border border-border bg-background p-3">
            <div className="flex items-center gap-2">
              <span className="grid size-7 place-items-center border border-accent bg-accent/10 text-xs font-semibold text-accent">{index}</span>
              <strong className="text-sm text-primary">{title}</strong>
            </div>
            <div className="mt-3 font-mono text-xs text-primary">{rule}</div>
            <div className="mt-2 text-xs leading-5 text-secondary">{reason}</div>
          </div>
        ))}
      </div>
