...
export function Cnt804DataPlaneDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第4章 网络层：数据平面：机制与证据"
      prompt="切换《第4章 网络层：数据平面》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第4章 网络层：数据平面》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Cnt804DataPlaneMechanismMap() {
  return (
    <ChapterMechanismMap
      title="第4章 网络层：数据平面：机制路径"
      stages={STAGES}
    />
  );
}

export function Cnt804DataPlaneFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第4章 网络层：数据平面：失效与核验"
      stages={STAGES}
    />
  );
}
