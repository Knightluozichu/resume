...
export function Cra403EventMechanismDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第3章 Android事件机制：机制与证据"
      prompt="切换《第3章 Android事件机制》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第3章 Android事件机制》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Cra403EventMechanismMechanismMap() {
  return (
    <ChapterMechanismMap
      title="第3章 Android事件机制：机制路径"
      stages={STAGES}
    />
  );
}

export function Cra403EventMechanismFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第3章 Android事件机制：失效与核验"
      stages={STAGES}
    />
  );
}
