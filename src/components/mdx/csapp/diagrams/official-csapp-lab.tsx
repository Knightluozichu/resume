...
const modes = {
  normal: "正常",
  boundary: "边界",
  missingContract: "契约缺失",
  race: "次序冲突",
  recovery: "恢复",
} as const;

export function OfficialCsappLab({
  title,
  label,
  color,
  soft,
  ...
  const evidence = useMemo(() => {
    const failed = mode === "missingContract" || mode === "race";
    return {
      states: chain.length * samples,
      divergent: mode === "boundary" ? 1 : failed ? samples : 0,
      cycles: samples * (failed ? 13 : mode === "boundary" ? 8 : 5),
      firstError: failed
        ? modes[mode]
        : mode === "boundary"
          ? "边界已捕获"
  ...
  }, [chain.length, mode, predicted, samples]);
      ...
      <div className="grid lg:grid-cols-[minmax(0,1fr)_300px]">
        ...
        <aside className="border-t border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-900 lg:border-l lg:border-t-0">
          <div className="flex items-center justify-between gap-3">
            <h4 className="text-sm font-semibold">因果证据</h4>
            <span
          ...
          <p className="mt-4 border-t border-zinc-200 pt-3 text-xs leading-5 text-zinc-600 dark:border-zinc-700 dark:text-zinc-300">
            先预测位、地址、机器状态和资源所有权，再运行样本并停在首个因果分叉。
          </p>
        </aside>
      </div>
    </section>
