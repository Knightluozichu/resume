...
export function Dak1420ApkBuildDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第20章 Android应用程序的编译和打包：机制与证据"
      prompt="切换《第20章 Android应用程序的编译和打包》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第20章 Android应用程序的编译和打包》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Dak1420ApkBuildMechanismMap() {
  return (
    <ChapterMechanismMap
      title="第20章 Android应用程序的编译和打包：机制路径"
      stages={STAGES}
    />
  );
}

export function Dak1420ApkBuildFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第20章 Android应用程序的编译和打包：失效与核验"
      stages={STAGES}
    />
  );
}
