...
const nodes = [
  ...
  },
  {
    ...
    stage: "恢复同输入重放",
    mechanism:
      "7.2.2 MediaStore.Images.Media的query函数分析在“从MediaProvider启动进入SQLite、跨进程Cursor、关闭与文件描述符传递”中的责任要追踪数据从入口到持久化或观察者回调的完整闭环。固定URI、文件或账户样本，记录事务、远端资源、通知与关闭结果，再用重复输入验证幂等和泄漏。",
    probe:
  ...
  },
  {
    ...
    stage: "复现正常轨迹",
    mechanism:
      "7.4.1 提取query关键点在“从MediaProvider启动进入SQLite、跨进程Cursor、关闭与文件描述符传递”中的责任要追踪数据从入口到持久化或观察者回调的完整闭环。固定URI、文件或账户样本，记录事务、远端资源、通知与关闭结果，再用重复输入验证幂等和泄漏。",
    probe:
  ...
  },
  {
    ...
    stage: "定位首个分叉",
    mechanism:
      "7.4.3 query关键点分析在“从MediaProvider启动进入SQLite、跨进程Cursor、关闭与文件描述符传递”中的责任要追踪数据从入口到持久化或观察者回调的完整闭环。固定URI、文件或账户样本，记录事务、远端资源、通知与关闭结果，再用重复输入验证幂等和泄漏。",
    probe:
  ...
  },
  {
    ...
    stage: "复现正常轨迹",
    mechanism:
      "7.5.1 客户端close分析在“从MediaProvider启动进入SQLite、跨进程Cursor、关闭与文件描述符传递”中的责任要追踪数据从入口到持久化或观察者回调的完整闭环。固定URI、文件或账户样本，记录事务、远端资源、通知与关闭结果，再用重复输入验证幂等和泄漏。",
    probe:
  ...
  },
  {
    ...
    stage: "注入单一故障",
    mechanism:
      "7.5.2 服务端close分析在“从MediaProvider启动进入SQLite、跨进程Cursor、关闭与文件描述符传递”中的责任要追踪数据从入口到持久化或观察者回调的完整闭环。固定URI、文件或账户样本，记录事务、远端资源、通知与关闭结果，再用重复输入验证幂等和泄漏。",
    probe:
  ...
  },
  {
    ...
    stage: "定位首个分叉",
    mechanism:
      "7.5.3 finalize函数分析在“从MediaProvider启动进入SQLite、跨进程Cursor、关闭与文件描述符传递”中的责任要追踪数据从入口到持久化或观察者回调的完整闭环。固定URI、文件或账户样本，记录事务、远端资源、通知与关闭结果，再用重复输入验证幂等和泄漏。",
    probe:
  ...
  },
  {
    ...
    stage: "复现正常轨迹",
    mechanism:
      "7.6.1 openAssetFileDescriptor之客户端调用分析在“从MediaProvider启动进入SQLite、跨进程Cursor、关闭与文件描述符传递”中的责任要追踪数据从入口到持久化或观察者回调的完整闭环。固定URI、文件或账户样本，记录事务、远端资源、通知与关闭结果，再用重复输入验证幂等和泄漏。",
    probe:
  ...
  },
  {
    ...
    stage: "锁定历史基线",
    mechanism:
      "7.7 本章学习指导在“从MediaProvider启动进入SQLite、跨进程Cursor、关闭与文件描述符传递”中的责任要追踪数据从入口到持久化或观察者回调的完整闭环。固定URI、文件或账户样本，记录事务、远端资源、通知与关闭结果，再用重复输入验证幂等和泄漏。",
    probe:
