...
export type DezeroEvidenceModel = {
  unitId: string;
  title: string;
  question: string;
...
type Props = {
  model: DezeroEvidenceModel;
  view: "graph-contract" | "backward-trace" | "framework-gate";
};
...
function GraphContractLab({ model }: { model: DezeroEvidenceModel }) {
  ...
  return (
    <section
      ...
      data-visual-kind="dezero-graph-contract"
      aria-label={`${model.title}计算图合同`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium text-accent">DeZero 计算图合同</p>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        {[
          ["进入本步的对象状态", activeStage.input],
          ["框架扩展或算子动作", activeStage.operation],
          ["离开本步的图状态", activeStage.output],
          ["身份、shape、梯度与生命周期检查", activeStage.check],
        ].map(([label, detail]) => (
          <div
            key={label}
            className="rounded-card border border-border bg-background p-4"
...
function BackwardTraceLab({ model }: { model: DezeroEvidenceModel }) {
  ...
  return (
    <section
      className="not-prose my-6 rounded-card border border-border bg-elevated p-4 sm:p-6"
      data-visual-kind="dezero-backward-trace"
      aria-label={`${model.title}反向传播轨迹`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <div className="rounded-card border border-border bg-background p-4">
          <p className="text-xs font-medium text-success">框架不变量</p>
...
function FrameworkGateLab({ model }: { model: DezeroEvidenceModel }) {
  ...
  return (
    <section
      className="not-prose my-6 rounded-card border border-border bg-elevated p-4 sm:p-6"
      data-visual-kind="dezero-framework-gate"
      aria-label={`${model.title}框架验收门`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium text-accent">DeZero 框架验收门</p>
          <h3 className="mt-1 text-lg font-semibold text-primary">
            逐项锁定对象、计算图、梯度、资源与复现条件
          </h3>
        </div>
        <ResetButton onReset={reset} />
      </div>
      ...
      >
        {artifactVisible ? "收起框架证据包" : "展开框架证据包"}
      </button>
...
export function DezeroEvidenceLab({ model, view }: Props) {
  if (view === "graph-contract") return <GraphContractLab model={model} />;
  if (view === "backward-trace") return <BackwardTraceLab model={model} />;
  return <FrameworkGateLab model={model} />;
}
