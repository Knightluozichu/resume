...
type Props = {
  model: DlsEvidenceModel;
  view: "tensor-path" | "gradient-trace" | "training-gate";
};
...
function TensorPathLab({ model }: { model: DlsEvidenceModel }) {
  ...
  return (
    <section
      ...
      data-visual-kind="dls-tensor-path"
      aria-label={`${model.title}张量信号路径`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium text-accent">张量信号路径</p>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        {[
          ["输入张量或程序状态", activeStage.input],
          ["本步算子或代码动作", activeStage.operation],
          ["输出张量或训练状态", activeStage.output],
          ["形状、梯度与数值检查", activeStage.check],
        ].map(([label, detail]) => (
          <div
            key={label}
            className="rounded-card border border-border bg-background p-4"
...
function GradientTraceLab({ model }: { model: DlsEvidenceModel }) {
  ...
  return (
    <section
      className="not-prose my-6 rounded-card border border-border bg-elevated p-4 sm:p-6"
      data-visual-kind="dls-gradient-trace"
      aria-label={`${model.title}梯度轨迹`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium text-accent">梯度与训练轨迹</p>
          <h3 className="mt-1 text-lg font-semibold text-primary">
            对照参考运行与单一故障
          </h3>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <div className="rounded-card border border-border bg-background p-4">
          <p className="text-xs font-medium text-success">实现不变量</p>
...
function TrainingGateLab({ model }: { model: DlsEvidenceModel }) {
  ...
  return (
    <section
      className="not-prose my-6 rounded-card border border-border bg-elevated p-4 sm:p-6"
      data-visual-kind="dls-training-gate"
      aria-label={`${model.title}训练验收门`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium text-accent">训练验收门</p>
          <h3 className="mt-1 text-lg font-semibold text-primary">
            逐项锁定数据、实现、梯度与评估条件
          </h3>
        </div>
      ...
      >
        {artifactVisible ? "收起训练证据包" : "展开训练证据包"}
      </button>
...
export function DlsEvidenceLab({ model, view }: Props) {
  if (view === "tensor-path") return <TensorPathLab model={model} />;
  if (view === "gradient-trace") return <GradientTraceLab model={model} />;
  return <TrainingGateLab model={model} />;
}
