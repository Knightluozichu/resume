    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox="0 0 720 400" role="img" aria-label="OpenGL 架构与状态机：对象协作与绑定" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x="140" y="160" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">记录属性配置</text>
          <text x="140" y="176" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">属性缓冲关联 + EBO 绑定</text>
          ...
          <text x="360" y="160" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">顶点属性数据</text>
          <text x="360" y="176" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">位置/法线/UV</text>
          ...
          <text x="580" y="160" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">索引数组</text>
          <text x="580" y="176" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">复用顶点</text>
          ...
          <text x="200" y="108" fontSize="11" fill="var(--accent)">配置时记录关联</text>
          ...
          {["DEPTH_TEST", "BLEND", "CULL_FACE", "useProgram"].map((s, i) => (
            <g key={s}>
              ...
              <text x={145 + i * 150} y="258" textAnchor="middle" fontSize="11" fill="var(--text-primary)">{s}</text>
            </g>
          ))}
          ...
          <text x="360" y="376" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">绘制命令按当前 VAO + VBO + EBO + 程序 + 开关状态跑一遍管线</text>
          <defs>
