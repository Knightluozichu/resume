    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox="0 0 720 400" role="img" aria-label="渲染优化：批处理/实例化/状态排序" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x="160" y="110" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">每命令固定开销 × N</text>

          {/* 箭头 */}
          <line x1="280" y1="92" x2="340" y2="92" stroke="var(--accent)" strokeWidth="1.6" markerEnd="url(#roArrow)" />
          <text x="310" y="84" textAnchor="middle" fontSize="11" fill="var(--accent)">合并</text>
          ...
          <text x="500" y="110" textAnchor="middle" fontSize="11" fill="var(--accent)">批处理合并 / 实例化复制</text>
          ...
          <text x="60" y="184" fontSize="11" fill="var(--text-secondary)">乱序：S1/T1 → S2/T2 → S1/T1 → S2/T1 → S1/T2（频繁切换）</text>
          <text x="60" y="206" fontSize="11" fill="var(--accent)">排序后：S1/T1 → S1/T1 → S1/T2 → S2/T1 → S2/T2（最少切换）</text>
          <text x="60" y="226" fontSize="11" fill="var(--text-secondary)">同 shader 同纹理排在一起，useProgram/bindTexture 次数大减</text>
          ...
          <text x="60" y="304" fontSize="11" fill="var(--accent)">1. 减 draw call：批处理（同材质合并 VBO）· 实例化（同几何副本）</text>
          <text x="60" y="324" fontSize="11" fill="var(--accent)">2. 排状态：按 shader→纹理→其他分组，减少 useProgram/bindTexture 切换</text>
          <text x="60" y="344" fontSize="11" fill="var(--accent)">3. 降工作量：压缩/降分辨率 · 剔除 · LOD；图集主要减少绑定并扩大批次</text>
          <text x="60" y="364" fontSize="11" fill="var(--text-secondary)">目标：让 CPU、GPU、带宽与功耗都落在目标帧预算内</text>
