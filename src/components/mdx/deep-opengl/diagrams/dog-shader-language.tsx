    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox="0 0 720 400" role="img" aria-label="GLSL ES 顶点片元传值与限定符" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x="56" y="164" fontSize="11" fill="var(--text-secondary)">in a_pos / a_uv（attribute）</text>
          <text x="56" y="180" fontSize="11" fill="var(--text-secondary)">uniform u_mvp</text>
          <text x="56" y="196" fontSize="11" fill="var(--accent)">out v_uv / v_color（varying）</text>
          <text x="56" y="216" fontSize="11" fill="var(--text-primary)">gl_Position = u_mvp·pos</text>
          <text x="56" y="236" fontSize="11" fill="var(--text-secondary)">逐顶点执行</text>

          {/* 光栅化插值 */}
          <rect x="300" y="150" width="120" height="90" rx="8" fill="var(--accent)" fillOpacity="0.08" stroke="var(--accent)" strokeWidth="1.2" />
          <text x="360" y="180" textAnchor="middle" fontSize="11" fontWeight="700" fill="var(--accent)">光栅化</text>
          <text x="360" y="198" textAnchor="middle" fontSize="11" fill="var(--text-primary)">按重心坐标</text>
          <text x="360" y="212" textAnchor="middle" fontSize="11" fill="var(--text-primary)">插值 varying</text>
          <text x="360" y="230" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">逐像素</text>
          ...
          <text x="456" y="164" fontSize="11" fill="var(--accent)">in v_uv / v_color（插值后）</text>
          <text x="456" y="180" fontSize="11" fill="var(--text-secondary)">uniform u_tex（sampler2D）</text>
          <text x="456" y="196" fontSize="11" fill="var(--text-secondary)">precision mediump float</text>
          <text x="456" y="216" fontSize="11" fill="var(--text-primary)">out fragColor = tex·color</text>
          <text x="456" y="236" fontSize="11" fill="var(--text-secondary)">逐片元执行</text>
          ...
          <text x="360" y="306" textAnchor="middle" fontSize="11" fill="var(--text-primary)">阶段接口须满足名称、类型、精度与插值契约</text>
          ...
          <text x="360" y="346" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">限定符：uniform（共享常量）· in/out（顶点片元传值）· precision（精度）</text>
          <text x="360" y="364" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">片元 float 无默认精度；ES3 用 in/out 取代 ES2 的 attribute/varying</text>
