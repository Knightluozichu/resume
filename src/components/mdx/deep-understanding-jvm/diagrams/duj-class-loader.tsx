    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        ...
        >
          ...
          <text x="195" y="354" textAnchor="middle" fontSize="10" fill="var(--text-tertiary)">连接 = 验证 + 准备 + 解析</text>
          ...
          <text x="117" y="392" textAnchor="middle" fontSize="10" fill="var(--success)">使用（Using）</text>
          ...
          <text x="272" y="392" textAnchor="middle" fontSize="10" fill="var(--success)">卸载（Unloading）</text>
          ...
          <text x="545" y="100" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">加载 rt.jar / java.lang.* 等</text>
          ...
          <text x="545" y="172" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">加载 ext 目录 / java.ext.dirs</text>
          ...
          <text x="545" y="244" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">加载 classpath / -cp</text>
          ...
          <text x="545" y="316" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">网络/加密/热部署加载</text>
          ...
          <text x="395" y="386" fontSize="10" fill="var(--text-secondary)">1. 先委派父加载器尝试加载</text>
          <text x="395" y="402" fontSize="10" fill="var(--text-secondary)">2. 父加载器无法加载时，子加载器才尝试</text>
          <text x="395" y="418" fontSize="10" fill="var(--text-secondary)">3. 保证核心类的安全性与唯一性</text>
          <text x="395" y="438" fontSize="10" fill="var(--danger)">打破：SPI（JDBC）、OSGi、Tomcat</text>
          <text x="395" y="458" fontSize="10" fill="var(--text-secondary)">线程上下文类加载器（TCCL）</text>
        </svg>
      </div>
