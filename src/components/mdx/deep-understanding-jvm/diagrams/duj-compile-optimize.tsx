    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        ...
        >
          ...
          <text x="110" y="100" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">解释执行</text>
          <text x="110" y="114" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">采集 profile</text>
          ...
          <text x="295" y="100" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">C1 编译 + 优化</text>
          <text x="295" y="114" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">带 profiling</text>
          ...
          <text x="480" y="100" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">C2 编译</text>
          <text x="480" y="114" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">激进优化</text>
          ...
          <text x="647" y="100" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">乐观假设失败</text>
          <text x="647" y="114" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">回退重编译</text>
          ...
          <text x="137" y="200" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">把目标方法代码复制到调用处</text>
          <text x="137" y="214" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">消除方法调用开销</text>
          <text x="137" y="228" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">为其他优化铺路</text>
          ...
          <text x="369" y="200" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">减少循环次数</text>
          <text x="369" y="214" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">减少分支预测失败</text>
          <text x="369" y="228" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">利于向量化</text>
          ...
          <text x="601" y="200" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">逃逸分析证明无竞争 → 消除</text>
          <text x="601" y="214" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">相邻同步块 → 合并粗化</text>
          <text x="601" y="228" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">减少 monitorenter/exit</text>
          ...
          <text x="137" y="314" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">对象仅在方法内使用</text>
          <text x="137" y="330" textAnchor="middle" fontSize="10" fill="var(--danger)">→ 标量替换</text>
          <text x="137" y="346" textAnchor="middle" fontSize="10" fill="var(--danger)">→ 栈上分配</text>
          <text x="137" y="360" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">无需堆分配，无 GC</text>
          ...
          <text x="369" y="314" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">对象被作为参数传递</text>
          <text x="369" y="330" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">或被外部方法引用</text>
          <text x="369" y="346" textAnchor="middle" fontSize="10" fill="var(--danger)">→ 堆分配</text>
          <text x="369" y="360" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">但可做部分优化</text>
          ...
          <text x="601" y="314" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">对象被其他线程访问</text>
          <text x="601" y="330" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">如赋值给静态字段</text>
          <text x="601" y="346" textAnchor="middle" fontSize="10" fill="var(--danger)">→ 堆分配</text>
          <text x="601" y="360" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">需考虑线程安全</text>
          ...
          <text x="370" y="422" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">将未逃逸的聚合对象拆解为成员变量（标量），分散到寄存器/栈帧</text>
          <text x="370" y="438" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">效果：减少堆分配 → 降低 GC 压力；对象无需真正创建</text>
          <text x="370" y="456" textAnchor="middle" fontSize="10" fill="var(--text-tertiary)">-XX:+DoEscapeAnalysis（JDK 8 默认开启）</text>
        </svg>
      </div>
