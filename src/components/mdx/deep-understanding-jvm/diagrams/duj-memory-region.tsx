    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        ...
        >
          ...
          <text x="195" y="132" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">记录当前线程执行字节码行号</text>
          <text x="195" y="146" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">唯一不会 OOM 的区域</text>
          ...
          <text x="195" y="204" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">每个方法调用 = 一个栈帧</text>
          <text x="195" y="218" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">局部变量表 / 操作数栈 / 动态链接 / 返回地址</text>
          <text x="195" y="236" textAnchor="middle" fontSize="10" fill="var(--danger)">异常：StackOverflowError / OOM</text>
          ...
          <text x="195" y="296" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">为 Native 方法服务</text>
          <text x="195" y="310" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">HotSpot 与虚拟机栈合二为一</text>

          <text x="195" y="348" textAnchor="middle" fontSize="10" fill="var(--text-tertiary)">随线程创建而生，随线程消亡而灭</text>
          <text x="195" y="364" textAnchor="middle" fontSize="10" fill="var(--text-tertiary)">无需 GC 干预</text>
          ...
          <text x="545" y="132" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">存放对象实例与数组——GC 主战场</text>
          <text x="545" y="148" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">新生代（Eden + S0 + S1）</text>
          <text x="545" y="162" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">老年代（Old Generation）</text>
          <text x="545" y="180" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">-Xms / -Xmx 控制大小</text>
          <text x="545" y="196" textAnchor="middle" fontSize="10" fill="var(--danger)">异常：OutOfMemoryError: Java heap space</text>
          ...
          <text x="545" y="254" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">类信息 / 常量池 / 静态变量 / JIT 代码</text>
          <text x="545" y="270" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">JDK 7 永久代（PermGen）→ JDK 8 元空间（Metaspace）</text>
          <text x="545" y="286" textAnchor="middle" fontSize="10" fill="var(--danger)">异常：OOM: Metaspace / PermGen space</text>
          ...
          <text x="545" y="346" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">NIO Buffer / 堆外分配</text>
          <text x="545" y="360" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">-XX:MaxDirectMemorySize 控制</text>

          <text x="545" y="396" textAnchor="middle" fontSize="10" fill="var(--text-tertiary)">所有线程共享，需要 GC 回收</text>
          <text x="545" y="412" textAnchor="middle" fontSize="10" fill="var(--text-tertiary)">堆是 GC 核心区域</text>
        </svg>
      </div>
