    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        ...
        >
          ...
          <text x="107" y="90" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">jstat / GC 日志</text>
          <text x="107" y="102" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">JMX / Prometheus</text>
          ...
          <text x="287" y="90" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">jmap dump / MAT</text>
          <text x="287" y="102" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">jstack 线程分析</text>
          ...
          <text x="467" y="90" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">JVM 参数调优</text>
          <text x="467" y="102" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">代码/架构优化</text>
          ...
          <text x="640" y="90" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">压测对比</text>
          <text x="640" y="102" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">指标回归</text>
          ...
          <text x="50" y="190" fontSize="9" fill="var(--text-secondary)">原因：老年代空间不足 / 大对象 / 内存泄漏</text>
          <text x="50" y="206" fontSize="9" fill="var(--text-secondary)">参数：-Xmx / -Xms / -XX:NewRatio</text>
          <text x="50" y="222" fontSize="9" fill="var(--text-secondary)">      -XX:MaxTenuringThreshold / -XX:PretenureSizeThreshold</text>
          <text x="50" y="236" fontSize="9" fill="var(--danger)">工具：jstat -gcutil / jmap -histo / GC 日志</text>
          ...
          <text x="400" y="190" fontSize="9" fill="var(--text-secondary)">原因：堆溢出 / 元空间溢出 / 直接内存溢出</text>
          <text x="400" y="206" fontSize="9" fill="var(--text-secondary)">参数：-XX:+HeapDumpOnOutOfMemoryError</text>
          <text x="400" y="222" fontSize="9" fill="var(--text-secondary)">      -XX:HeapDumpPath / -XX:MaxMetaspaceSize</text>
          <text x="400" y="236" fontSize="9" fill="var(--danger)">工具：MAT 分析 dump / jcmd / Arthas</text>
          ...
          <text x="50" y="294" fontSize="9" fill="var(--text-secondary)">原因：Full GC / CMS Concurrent Mode Failure</text>
          <text x="50" y="310" fontSize="9" fill="var(--text-secondary)">参数：-XX:MaxGCPauseMillis（G1）</text>
          <text x="50" y="326" fontSize="9" fill="var(--text-secondary)">      -XX:G1HeapRegionSize / -XX:InitiatingHeapOccupancyPercent</text>
          <text x="50" y="340" fontSize="9" fill="var(--danger)">策略：CMS → G1 → ZGC 迁移</text>
          ...
          <text x="400" y="294" fontSize="9" fill="var(--text-secondary)">目标：减少 GC 时间占比</text>
          <text x="400" y="310" fontSize="9" fill="var(--text-secondary)">参数：-XX:GCTimeRatio（GC时间占比 1/N）</text>
          <text x="400" y="326" fontSize="9" fill="var(--text-secondary)">      -XX:MaxGCMinorPauseMillis / -XX:+UseParallelGC</text>
          <text x="400" y="340" fontSize="9" fill="var(--danger)">策略：Parallel Scavenge + Parallel Old</text>
          ...
          <text x="82" y="410" textAnchor="middle" fontSize="10" fill="var(--warning)">jps / jstat</text>

          <rect x="145" y="388" width="105" height="36" rx="6" fill="var(--accent)" fillOpacity="0.08" stroke="var(--accent)" strokeWidth="1" />
          <text x="197" y="410" textAnchor="middle" fontSize="10" fill="var(--accent)">jmap / jhat</text>

          <rect x="260" y="388" width="105" height="36" rx="6" fill="var(--danger)" fillOpacity="0.08" stroke="var(--danger)" strokeWidth="1" />
          <text x="312" y="410" textAnchor="middle" fontSize="10" fill="var(--danger)">jstack / jcmd</text>

          <rect x="375" y="388" width="105" height="36" rx="6" fill="var(--success)" fillOpacity="0.08" stroke="var(--success)" strokeWidth="1" />
          <text x="427" y="410" textAnchor="middle" fontSize="10" fill="var(--success)">MAT / JProfiler</text>

          <rect x="490" y="388" width="105" height="36" rx="6" fill="var(--text-primary)" fillOpacity="0.06" stroke="var(--text-primary)" strokeWidth="1" />
          <text x="542" y="410" textAnchor="middle" fontSize="10" fill="var(--text-primary)">Arthas</text>

          <rect x="605" y="388" width="105" height="36" rx="6" fill="var(--text-primary)" fillOpacity="0.06" stroke="var(--text-primary)" strokeWidth="1" />
          <text x="657" y="410" textAnchor="middle" fontSize="10" fill="var(--text-primary)">GCViewer</text>

          <text x={VIEW_W / 2} y="450" textAnchor="middle" fontSize="10" fill="var(--text-tertiary)">原则：先量化再调优；一次只改一个参数；有基线才能对比</text>
          <text x={VIEW_W / 2} y="466" textAnchor="middle" fontSize="10" fill="var(--text-tertiary)">-Xlog:gc*（JDK 9+）/ -XX:+PrintGCDetails（JDK 8）</text>
        </svg>
      </div>
