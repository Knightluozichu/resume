  ...
  return (
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-x-auto overflow-hidden rounded-card border border-border bg-elevated p-4">
        ...
        >
          ...
          <g transform="translate(14, 34)">
            ...
            <path
              d="M 100 182 C 100 240, 145 240, 155 210"
              fill="none"
              stroke={danger}
              strokeWidth={2}
              markerEnd="url(#crRedArr)"
            />
            <path
              d="M 258 182 C 258 240, 213 240, 203 210"
              fill="none"
              stroke={danger}
              strokeWidth={2}
              markerEnd="url(#crRedArr)"
          ...
          <text
            ...
            textAnchor="middle"
          >
            口诀：A 持有 B 用 shared，B 回看 A 用 weak —— 单向 shared + 反向
            weak 是安全公式
          </text>
      ...
      <figcaption className="mt-2 text-center text-xs text-secondary">
        <strong>左侧（循环引用）</strong>：A 和 B 各持对方的
        shared_ptr，离开作用域后引用计数各剩 1，谁都不释放 →{" "}
        <strong>内存泄漏</strong>。<strong>右侧（weak_ptr 解法）</strong>：B
        改用 weak_ptr 观察 A，不增加 A 的引用计数 → 正常析构顺序释放。
      </figcaption>
    </figure>
  );
}
