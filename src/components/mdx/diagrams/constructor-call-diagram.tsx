...
export function ConstructorCallDiagram({
  ...
  const steps = [
    { label: "① 分析类定义", y: 48, desc: "编译器看到类定义，确定各成员类型" },
    {
      label: "② 分配内存",
      y: 108,
      desc: "在栈或堆上分配 sizeof(Sales_data) 字节",
    },
    {
      label: "③ 成员初始化列表",
      y: 168,
      desc: "初始化各数据成员（默认值/指定值）",
    },
    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-4">
        ...
        >
          ...
          <g
            opacity={
              active(1) || active(2) || active(3) || active(4) ? 1 : 0.25
            }
          >
            <text
              x={256}
      ...
      <figcaption className="mt-2 text-center text-xs text-secondary">
        构造函数执行分两阶段：先跑成员初始化列表（各数据成员按声明顺序初始化），再跑构造体。进入{" "}
        {"{}"} 时成员已有确定值。
      </figcaption>
    </figure>
  );
