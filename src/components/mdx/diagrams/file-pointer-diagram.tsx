    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-4">
        ...
        >
          ...
          <text x={110} y={132} textAnchor="middle" fontSize="11" fill={primary} fontFamily="monospace">
            fprintf(fp, ...)
          </text>
          <text x={110} y={150} textAnchor="middle" fontSize="11" fill={primary} fontFamily="monospace">
            fgets(buf, n, fp)
          </text>
          <text x={110} y={168} textAnchor="middle" fontSize="11" fill={primary} fontFamily="monospace">
            fread(ptr, sz, n, fp)
          </text>
          ...
          <text x={110} y={248} textAnchor="middle" fontSize="11" fill={secondary}>
            指向流对象
          </text>

          {/* 箭头 程序 → 缓冲 */}
          <line x1={180} y1={208} x2={248} y2={208} stroke={accent} strokeWidth="2" markerEnd="url(#fpArrow)" />
          <text x={214} y={198} textAnchor="middle" fontSize="11" fill={secondary}>
            读写
          </text>
          ...
          <text x={320} y={148} textAnchor="middle" fontSize="11" fill={secondary}>
            行缓冲 / 全缓冲
          </text>
          <text x={320} y={168} textAnchor="middle" fontSize="11" fill={secondary}>
            当前读写位置
          </text>
          <text x={320} y={188} textAnchor="middle" fontSize="11" fill={secondary}>
            EOF / 错误标志
          </text>
          <text x={320} y={212} textAnchor="middle" fontSize="11" fontFamily="monospace" fill={primary}>
            stdin stdout stderr
          </text>
          <text x={320} y={232} textAnchor="middle" fontSize="11" fill={secondary}>
            也是 FILE*
          </text>
          ...
          <text x={530} y={248} textAnchor="middle" fontSize="11" fill={secondary} fontFamily="monospace">
            scores.dat
          </text>
          <text x={530} y={264} textAnchor="middle" fontSize="11" fill={secondary}>
            持久化字节序列
          </text>
          ...
          <text x={24} y={328} fontSize="11" fill={secondary} fontFamily="monospace">
            stdin → 键盘 · stdout → 屏幕 · stderr → 诊断输出
          </text>
          <text x={24} y={346} fontSize="11" fill={secondary}>
