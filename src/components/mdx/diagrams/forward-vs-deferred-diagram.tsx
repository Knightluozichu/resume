 ...
 *  上排「前向着色」：几何 → 每个已着色片元 × 每盏灯立刻算光照 → 帧缓冲。
 *    高深度复杂度会产生后来被覆盖的额外工作；早期深度测试能减少但不能消除多光源成本。
    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated">
        <svg
          viewBox="0 0 620 320"
          role="img"
          aria-label="前向着色和延迟着色两条渲染管线对比。上排是前向着色：几何阶段之后，每个已执行片段着色器的片元都立即遍历每盏灯；高深度复杂度会产生后来被覆盖的额外光照工作，早期深度测试可减少一部分。下排是延迟着色：先做几何 pass，用 MRT 把每个片元的位置、法线、反照率填进 G-buffer，完全不算光照；再做光照 pass，画一个全屏四边形，只对每个可见像素乘每盏灯算一次光照，写进帧缓冲。它的光照计算次数大约等于屏幕像素数乘以灯数，和场景复杂度、overdraw 解耦。结论：延迟着色把光照从跟随物体的片元着色，变成只对可见像素重建一次，所以能支持大量光源。"
      ...
      <figcaption className="mt-2 text-center text-xs text-secondary">
        ...
        每个已着色片元 × 每盏灯立刻算光照（高深度复杂时有额外工作），光照次数 ~{" "}
        <code>已着色片元 × 灯</code>。
