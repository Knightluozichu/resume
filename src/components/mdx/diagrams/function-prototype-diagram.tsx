...
export function FunctionPrototypeDiagram({
  step = 4,
}: FunctionPrototypeDiagramProps) {
      ...
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-4">
        ...
        >
          ...
          <g opacity={step >= 3 ? 1 : 0.25}>
            <path
              d={`M ${protoX + boxW} ${48 + boxH / 2} L 340 ${48 + boxH / 2} L 340 300 L ${callX + boxW * 0.75} 300 L ${callX + boxW * 0.75} ${48 + boxH}`}
              fill="none"
              stroke={accent}
              strokeWidth="1.5"
              strokeDasharray="6 4"
          ...
          <g opacity={step >= 4 ? 1 : 0.25}>
            <path
              d={`M ${callX + boxW * 0.25} ${48 + boxH} L ${callX + boxW * 0.25} 212 L ${defX + boxW} 212`}
              fill="none"
              stroke={accent}
              strokeWidth="2"
          ...
          <g opacity={highlight(4)}>
            ...
            <text x={56} y={374} fontSize="11" fill={primary}>
              声明让编译器认识 cube；定义提供实现；链接器把 main 里的 cube(2.0)
              跳到定义处；返回后 y 得到 8.0
            </text>
          </g>
