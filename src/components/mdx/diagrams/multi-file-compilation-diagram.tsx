...
export function MultiFileCompilationDiagram({ step = 0 }: Props) {
  ...
  const strokeColor = (s: Step) =>
    step === s || step === 0 ? activeColor(s) : dimColor(s);
  ...
  return (
    <figure className="mdx-figure mx-auto my-6">
        ...
        >
          ...
          {step === 1 && (
            <text
              ...
              fill={textPri}
            >
              ① 编译 main.c：gcc -c main.c -o
              main.o，产出函数的机器码，但函数调用还是&quot;空的&quot;
            </text>
          )}
          {step === 2 && (
            <text
              ...
              fill={textPri}
            >
              ② 编译 helper.c：gcc -c helper.c -o helper.o，同样产出一个独立的
              .o 零件
            </text>
          )}
          {step === 3 && (
            <text
              ...
              fill={textPri}
            >
              ③ 链接：gcc main.o helper.o -o app，拼接两个
              .o，把函数调用对接到正确的地址
            </text>
          )}
        </svg>
      ...
      <figcaption className="mt-2 text-center text-xs text-secondary">
        {step === 0 &&
          "多文件编译流程：每个 .c 各自编译成 .o 目标文件，再由链接器拼接成最终可执行文件。"}
        {step === 1 &&
          "第①步：把 main.c 单独编译成 main.o。此时 main 里对 helper 函数的调用还是「未解决」的符号，链接时才填地址。"}
        {step === 2 &&
          "第②步：把 helper.c 单独编译成 helper.o。它和 main.o 是平等的——都是零件，谁都没比谁高级。"}
        {step === 3 &&
          "第③步：链接器把两个 .o 拼在一起，对号入座：main 调用的函数在 helper.o 里找到对应地址，填上跳转指令。"}
      </figcaption>
    </figure>
  );
}
