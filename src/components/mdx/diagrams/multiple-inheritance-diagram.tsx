 ...
 *
 * Step 1: 普通多重继承——数据重复——Panda 继承自 Bear 和 Endangered，
 *         Bear 和 Endangered 各自继承自 ZooAnimal，
 *         Panda 内部有两份 ZooAnimal 子对象（歧义）
 * Step 2: 虚继承——虚基类共享——虚继承只保留一份 ZooAnimal 子对象
 *
  ...
  return (
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-4">
        ...
        >
          <text x={w / 2} y={24} fontSize={15} fontWeight={700} fill={primary} textAnchor="middle">
            {step === 1
              ? "普通多重继承——Panda 内部有两份 ZooAnimal（数据重复）"
              : "虚继承——Panda 内部只有一份 ZooAnimal（共享唯一子对象）"}
          </text>
            ...
            {step === 1 && (
              <>
                ...
                <text x={200} y={440} fontSize={9} fill={red} textAnchor="middle" fontFamily="monospace">
                  panda.name ← 歧义！用 panda.ZooAnimal::name 消除
                </text>
              </>
            )}
          ...
          <g>
            ...
            {/* Right branch: Endangered (virtual) */}
      ...
      <figcaption className="mt-2 text-center text-xs text-secondary">
        {step === 1
          ? "第一步：普通多重继承下，Bear 和 Endangered 各自包含一份 ZooAnimal 子对象，Panda 从两条路径各继承一份 → 两份 ZooAnimal（浪费内存 + 名字歧义）。"
          : "第二步：虚继承（关键字 virtual）让所有派生类共享唯一的虚基类子对象。最底层派生类（Panda）负责直接构造虚基类。每个类内部通过 vptr 指针间接定位 ZooAnimal——比普通继承稍慢，但消除了歧义和浪费。"}
      </figcaption>
    </figure>
  );
}
