...
export function RandomPipelineDiagram({
  step = 1,
}: RandomPipelineDiagramProps) {
  ...
  return (
    <figure className="mdx-figure mx-auto my-6">
        ...
        >
          ...
          <g transform={`translate(20, ${topY})`}>
            <rect
              ...
              height={boxH}
              rx="10"
              fill={step >= 1 ? `${blue}12` : "transparent"}
              stroke={step >= 1 ? blue : border}
          ...
          <g transform={`translate(300, ${topY})`}>
            <rect
              ...
              height={boxH}
              rx="10"
              fill={step >= 2 ? `${warn}12` : "transparent"}
              stroke={step >= 2 ? warn : border}
              strokeWidth={step >= 2 ? 2 : 1}
            />
            <text
              x={boxW / 2}
          ...
          <g transform={`translate(580, ${topY})`}>
            <rect
              ...
              height={boxH}
              rx="10"
              fill={step >= 3 ? `${good}12` : "transparent"}
              stroke={step >= 3 ? good : border}
              strokeWidth={step >= 3 ? 2 : 1}
            />
            <text
              x={boxW / 2}
          ...
          >
            {step === 1 &&
              "① 引擎：mt19937 等算法将种子转换为高质量伪随机 bits——状态空间巨大、周期极长"}
            {step === 2 &&
              "② 分布：将引擎输出的原始无符号整数映射到目标范围/分布——uniform_int 均匀、normal 正态"}
            {step === 3 &&
              "③ 输出：dist(gen) 每次调用返回一个分布后的随机值——引擎状态推进、序列不重复"}
            {step === 4 &&
              "④ 封装：用 lambda 捕获引擎和分布的引用——rand_int() 就是你的自定义 rand()"}
          </text>
          ...
          <g transform={`translate(${cx - 310}, ${h - 88})`}>
            ...
            <text
              ...
              fontFamily="monospace"
            >
              引擎决定&quot;质量&quot;（是否真随机），分布决定&quot;形状&quot;（均匀/正态/指数...）。两者组合
              → 任意分布的高质量随机数。
            </text>
            <text
              x="310"
              ...
              fontFamily="monospace"
            >
              别再写 rand() % 6 + 1 ——取模引入偏差、范围小、随机性差。用
              &lt;random&gt; 才是正确答案。
            </text>
          </g>
      ...
      <figcaption className="mt-2 text-center text-xs text-secondary">
        {step === 1 &&
          "第一阶段：随机数引擎（如 mt19937）用种子初始化内部状态，每次 gen() 调用输出原始无符号整数。"}
        {step === 2 &&
          "第二阶段：分布对象（如 uniform_int_distribution）接收引擎输出，按指定参数将原始值映射到目标范围。"}
        {step === 3 &&
          "第三阶段：dist(gen) 返回分布后的随机值。可重复调用——引擎状态自动推进，每次结果不同。"}
        {step === 4 &&
          "第四阶段：把引擎+分布封装成 lambda，一行 rand_int() 就得到一个范围内的随机整数——既安全又好用。"}
      </figcaption>
    </figure>
  );
}
