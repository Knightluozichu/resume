  ...
  return (
    <figure className="mdx-figure mx-auto my-6">
        ...
        >
          ...
          <g transform={`translate(100, ${topBlockY})`}>
            <rect
              ...
              fill={step >= 1 ? `${accent}0C` : "transparent"}
            />
            <text
              x="12"
              y="14"
          ...
          {step >= 2 && (
            <>
              ...
              <g transform={`translate(100, ${topBlockY + 108})`}>
                ...
                >
                  {step < 3 &&
                    "尝试匹配：当前位置 0 → (\d+) 匹配 '123' → '-' 匹配 → (\w+) 匹配 'abc' ✓"}
          ...
          {step >= 4 && (
            <g transform={`translate(100, ${topBlockY + 256})`}>
              <text
                ...
                fontFamily="monospace"
              >
                sregex_iterator 继续 → 下一个匹配：&quot;456-def&quot; →
          ...
          >
            {step === 1 &&
              "① 模式：用正则字符串构造 regex 对象——(\d+) 捕获数字、(\w+) 捕获字母数字"}
            {step === 2 &&
              "② 输入：待匹配的字符串。引擎从左到右扫描，寻找第一个匹配位置"}
            {step === 3 &&
              "③ 匹配：引擎逐字符比对——\d+ 匹配 '123'、连字符匹配 '-'、\w+ 匹配 'abc'"}
            {step === 4 &&
              "④ 捕获组：smatch[0] 存完整匹配，[1]/[2] 存各子表达式匹配的内容"}
          </text>
      ...
      <figcaption className="mt-2 text-center text-xs text-secondary">
        {step === 1 &&
          "第一步：regex 对象用模式字符串构造——括号 () 定义捕获组，编译为内部自动机。"}
        {step === 2 &&
          "第二步：待匹配输入。regex_search 或 sregex_iterator 开始从左到右扫描。"}
        {step === 3 &&
          "第三步：匹配阶段——引擎逐字符比对，$0 存完整匹配（123-abc），$1 存第一个捕获组（123），$2 存第二个捕获组（abc）。"}
        {step === 4 &&
          "第四步：smatch 结果——下标 0 返回完整匹配字符串，大于 0 的下标返回对应捕获组。regex_iterator 可遍历所有匹配。"}
      </figcaption>
    </figure>
  );
}
