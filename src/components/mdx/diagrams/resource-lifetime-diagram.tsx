...
export function ResourceLifetimeDiagram() {
  ...
  const stages = [
    ...
    },
    {
      ...
      fn: "operator=(const &)",
      color: accent,
      resource: "先 delete 0x1000 再 new",
      status: "释放旧+分配新",
      statusColor: warn,
    },
    {
  ...
  return (
    <figure className="mdx-figure mx-auto my-6">
        ...
        >
          ...
          {stages.map((s, i) => (
            <g key={`res-${i}`}>
              ...
              {/* 箭头连接 */}
              {i < stages.length - 1 &&
                s.statusColor !== stages[i + 1].statusColor && (
                  <text
                    x={s.x + 90}
                    y={175}
                    fontSize="16"
      ...
      <figcaption className="mt-2 text-center text-xs text-secondary">
        对象生命周期六阶段：默认构造（无资源）→ 分配资源（持有堆数据）→
        拷贝构造（新对象独立副本）→ 拷贝赋值（释放旧资源、分配新副本）→
        移动构造（接管资源、源置空）→
        析构（释放一切）。每一步对应一个特殊成员函数——这正是{" "}
        <strong>五法则</strong> 想要规范的全部行为。
      </figcaption>
    </figure>
  );
}
