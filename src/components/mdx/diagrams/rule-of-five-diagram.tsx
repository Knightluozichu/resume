  ...
  return (
    <figure className="mdx-figure mx-auto my-6">
      ...
      <figcaption className="mt-2 text-center text-xs text-secondary">
        析构函数是资源管理的基础。拷贝控制（左）用 const
        左值引用、不修改源对象。移动控制（右）用右值引用、窃取源对象资源并把源置为可安全析构状态。
        <strong>五法则</strong>
        ：自定义了其中任一个，通常这五个都需要显式定义或显式 = default / =
        delete。
      </figcaption>
    </figure>
  );
