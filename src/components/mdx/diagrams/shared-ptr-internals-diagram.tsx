  ...
  return (
    <figure className="mdx-figure mx-auto my-4">
      <div className="overflow-hidden rounded-card border border-border bg-elevated">
        ...
        >
          ...
          <line
            ...
            strokeDasharray="8 4"
          />
          <text x={50} y={170} fontSize={11} fill={secondary}>
            栈（Stack）
          </text>
          <text x={50} y={290} fontSize={11} fill={secondary}>
            堆（Heap）
          </text>
      ...
      <figcaption className="mt-2 text-center text-xs text-secondary">
        {step === 1 && (
          <>
            <strong>① 创建</strong>：<code>make_shared&lt;T&gt;()</code>{" "}
            一次分配控制块和堆对象在连续内存，引用计数初始为 1。优于先{" "}
            <code>new T</code> 再传 <code>shared_ptr</code> 的两步分配。
          </>
        )}
        {step === 2 && (
          <>
            <strong>② 拷贝</strong>：<code>sp₂ = sp₁</code> 后两个 shared_ptr
            指向同一控制块，引用计数增至
            2。两个指针共享同一堆对象——谁最后离开谁负责析构。
          </>
        )}
        {step === 3 && (
          <>
            <strong>③ 重置</strong>：<code>sp₁.reset()</code> 将 sp₁
            置空并减少引用计数（从 2 变为 1）。只剩 sp₂ 持有对象，等 sp₂
            离开作用域时析构。
          </>
        )}
        {step === 4 && (
          <>
            <strong>④ 析构</strong>：最后一个 shared_ptr 离开作用域时{" "}
            <code>use_count → 0</code>
            ，自动析构堆对象并释放控制块。整个过程无需手动 <code>delete</code>。
          </>
        )}
      </figcaption>
    </figure>
