  ...
  return (
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-x-auto overflow-hidden rounded-card border border-border bg-elevated p-4">
        ...
        >
            ...
            <g transform="translate(12, 306)">
              {[
                ...
                <g key={i} transform={`translate(0, ${i * 22})`}>
                  ...
                  <text x={64} y={12} fontSize={11} fill={secondary}>
                    {desc}
                  </text>
                </g>
              ))}
          ...
          <text
            ...
            textAnchor="middle"
          >
            shared_ptr→共享借用 | unique_ptr→独占传递 | weak_ptr→旁观避循环 ——
            选对指针，告别手动 delete 和内存泄漏
          </text>
      ...
      <figcaption className="mt-2 text-center text-xs text-secondary">
        ...
        ：多个指针共享一个堆对象，引用计数为零时自动释放；
        <strong>unique_ptr</strong>：独享堆对象，禁止拷贝只能移动；
        <strong>weak_ptr</strong>
        ：不增加引用计数的弱引用，专治循环引用造成的内存泄漏。
      </figcaption>
    </figure>
  );
}
