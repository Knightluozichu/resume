    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-4">
        ...
        >
          ...
          <text x="60" y="74" textAnchor="middle" fontSize="11" fill="var(--text-primary)">Mat A</text>
          <rect x="96" y="54" width="56" height="32" rx="4" fill="var(--bg-elevated)" stroke="var(--warning)" strokeWidth="1.5" />
          <text x="124" y="74" textAnchor="middle" fontSize="11" fill="var(--text-primary)">Mat B</text>
          <rect x="160" y="54" width="44" height="32" rx="4" fill="var(--bg-elevated)" stroke="var(--success)" strokeWidth="1.5" />
          <text x="182" y="74" textAnchor="middle" fontSize="11" fill="var(--text-primary)">Mat C</text>

          <text x="116" y="108" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">同 Shader · 不同参数</text>
          ...
          <text x="320" y="72" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">统一 CB 布局</text>
          <text x="320" y="88" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">合并提交批次</text>
          <text x="320" y="110" textAnchor="middle" fontSize="11" fill="var(--success)">1× SetPass</text>
          ...
          <text x="510" y="72" textAnchor="middle" fontSize="11" fill="var(--text-primary)">Draw A → Draw B → Draw C</text>

          <text x="510" y="102" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">同一管线状态 · 3 个 Draw</text>

          {/* Prerequisites */}
          <rect x="16" y="154" width="574" height="56" rx="6" fill="var(--bg)" stroke="var(--border)" strokeWidth="1" />
          <text x="24" y="176" fontSize="11" fontWeight="600" fill="var(--text-primary)">Shader 前提条件：</text>
          <text x="164" y="176" fontSize="11" fontFamily="monospace" fill="var(--accent)">CBUFFER_START(UnityPerMaterial)</text>
          <text x="24" y="196" fontSize="11" fill="var(--text-secondary)">
          ...
          <text x="24" y="246" fontSize="11" fill="var(--danger)">⚠ 中断 SRP Batcher 的操作：① MaterialPropertyBlock ② 不兼容的 Shader ③ 修改 Render State 参数（Blend/ZTest 等）</text>
          <text x="24" y="260" fontSize="11" fill="var(--text-secondary)">验证方法：Frame Debugger → 查看 Draw Call 是否有 SRP Batch 标签</text>
        </svg>
      </div>
