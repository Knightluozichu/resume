...
export function VariadicTemplateDiagram({
  step = 1,
}: VariadicTemplateDiagramProps) {
  ...
  const args = [["hello", "42", "3.14"], ["42", "3.14"], ["3.14"], []];
  ...
  return (
    <figure className="mdx-figure mx-auto my-6">
            ...
            return (
              <g key={i} opacity={show ? 1 : 0.2}>
                ...
                <text
                  ...
                  fontSize="11"
                  fontWeight="700"
                  fill={isActive ? accent : secondary}
                  textAnchor="start"
                >
                  {i === 0 ? "①" : i === 1 ? "②" : i === 2 ? "③" : "④"}
                </text>
      ...
      <figcaption className="mt-2 text-center text-xs text-secondary">
        {step === 1 &&
          "初始状态——参数包包含 {Hello, 42, 3.14} 三个参数，sizeof...(Args) = 3。"}
        {step === 2 &&
          "第一次递归——取出 Hello 作为 first，剩余 {42, 3.14} 作为 rest。对 first 执行操作，对 rest 递归调用。"}
        {step === 3 &&
          "第二次递归——取出 42，剩余 {3.14}。sizeof...(Args) 减少到 1。"}
        {step === 4 &&
          "第三次递归——取出 3.14，剩余 {}（空包）。下一次将触发终止——调用无参重载。"}
        {step === 5 &&
          "递归终止——rest 是空包，编译器匹配到 void print() 重载（base case），递归结束，不再展开。"}
      </figcaption>
    </figure>
  );
}
