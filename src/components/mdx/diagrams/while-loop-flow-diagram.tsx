...
export function WhileLoopFlowDiagram({ step = 4 }: WhileLoopFlowDiagramProps) {
  ...
  // 节点位置
  const startX = 340; // w / 2
  const startY = 40;
  const condY = 80;
  const bodyX = startX + 160; // 500
  const bodyY = 80;
  const updateX = startX + 160; // 500
  const updateY = 210;
  const endX = startX - 160; // 180
  const endY = 210;
        ...
        >
          ...
          <g opacity={isActive(1) ? 1 : 0.3}>
            ...
            <text
              ...
              fontWeight="700"
              fill={
                isActive(1) ? "var(--text-primary)" : "var(--text-secondary)"
              }
          ...
          <g opacity={isActive(2) ? 1 : 0.3}>
            ...
            <text
              ...
              fontWeight="700"
              fill={
                isActive(2) ? "var(--text-primary)" : "var(--text-secondary)"
              }
              fontFamily="system-ui"
            >
              ② 执行循环体
          ...
          <g opacity={isActive(3) ? 1 : 0.3}>
            ...
            <text
              ...
              fontWeight="700"
              fill={
                isActive(3) ? "var(--text-primary)" : "var(--text-secondary)"
              }
              fontFamily="system-ui"
            >
              ③ 更新条件变量
          ...
          <g opacity={isActive(4) ? 1 : 0.3}>
            <path
              d={`M ${updateX} ${updateY + boxH / 2} L ${updateX} 280 L 60 280 L 60 ${condY} L ${startX - diamondW / 2} ${condY}`}
              fill="none"
              stroke={isActive(4) ? "var(--accent)" : "var(--text-secondary)"}
              strokeWidth="1.5"
              markerEnd={
                isActive(4) ? "url(#loop-arrow)" : "url(#loop-arrow-dim)"
              }
            />
            <text
              x={64}
          ...
          <g opacity={isActive(4) ? 1 : 0.3}>
            <path
              d={`M ${startX - diamondW / 2} ${condY} L ${startX - diamondW / 2} 150 L ${endX} 150 L ${endX} ${endY - boxH / 2}`}
              fill="none"
              stroke="var(--text-secondary)"
              strokeWidth="1.5"
            />
          ...
          <text
            ...
            fontFamily="system-ui"
          >
            菱形 = 条件判断 · 矩形 = 执行步骤 · 返回弧线 = 继续循环 · 向左分支 =
            退出
          </text>
        </svg>
      </div>
      <figcaption className="mt-2 text-center text-xs text-secondary">
        while 循环的标准执行流程：先检查条件 → 条件为真执行循环体 → 更新条件变量
        → 回到检查。条件为假时退出循环。
      </figcaption>
    </figure>
  );
