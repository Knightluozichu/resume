    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="四大排序算法对比。插入排序：逐元素插入前缀 O(n²)/O(n)最好。归并排序：分治合并 O(n log n) 稳定。快速排序：基准分区平均 O(n log n) 最坏 O(n²)。堆排序：建堆取顶 O(n log n) 原地。底部复杂度表：插入 O(n²)/O(n²)/O(n) 不稳定；归并 O(n log n) 稳定；快排 O(n log n)/O(n²) 不稳定；堆排 O(n log n) 不稳定。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x="180" y="106" textAnchor="middle" fontSize="11" fill={secondary}>逐元素插入已排序前缀</text>
          ...
          <text x="180" y="190" textAnchor="middle" fontSize="11" fill={secondary}>最好 O(n)，最坏 O(n²)，稳定</text>

          {/* ===== 右上：归并排序 ===== */}
          <text x="540" y="88" textAnchor="middle" fontSize="13" fontWeight="700" fill={success}>归并排序</text>
          <text x="540" y="106" textAnchor="middle" fontSize="11" fill={secondary}>分治：切半 → 排序 → 合并</text>

          {/* 树形分治结构 */}
          <rect x="480" y="118" width="120" height="22" rx="3" fill={success} fillOpacity="0.08" stroke={success} strokeWidth="1" />
          <text x="540" y="133" textAnchor="middle" fontSize="11" fill={success}>[5 2 8 1]</text>

          <rect x="430" y="148" width="56" height="22" rx="3" fill={success} fillOpacity="0.06" stroke={success} strokeWidth="1" />
          <text x="458" y="163" textAnchor="middle" fontSize="11" fill={success}>[5 2]</text>
          <rect x="594" y="148" width="56" height="22" rx="3" fill={success} fillOpacity="0.06" stroke={success} strokeWidth="1" />
          <text x="622" y="163" textAnchor="middle" fontSize="11" fill={success}>[8 1]</text>
          <line x1="500" y1="140" x2="458" y2="148" stroke={border} strokeWidth="1" />
          <line x1="580" y1="140" x2="622" y2="148" stroke={border} strokeWidth="1" />

          <text x="458" y="182" textAnchor="middle" fontSize="11" fill={secondary}>[2 5]</text>
          <text x="622" y="182" textAnchor="middle" fontSize="11" fill={secondary}>[1 8]</text>

          <rect x="480" y="190" width="120" height="0" rx="0" fill="none" />
          <text x="540" y="192" textAnchor="middle" fontSize="11" fill={success} fontWeight="600">→ [1 2 5 8]</text>
          <text x="540" y="210" textAnchor="middle" fontSize="11" fill={secondary}>保证 O(n log n)，稳定，空间 O(n)</text>

          {/* ===== 左下：快速排序 ===== */}
          <text x="180" y="226" textAnchor="middle" fontSize="13" fontWeight="700" fill={warning}>快速排序</text>
          <text x="180" y="244" textAnchor="middle" fontSize="11" fill={secondary}>选基准 → 分区 → 递归</text>
          ...
          <text x="74" y="296" textAnchor="middle" fontSize="11" fill={warning}>{"<3"}</text>
          <text x="106" y="296" textAnchor="middle" fontSize="11" fill={accent}>pivot</text>
          <text x="138" y="296" textAnchor="middle" fontSize="11" fill={danger}>{">3"}</text>
          <text x="180" y="316" textAnchor="middle" fontSize="11" fill={secondary}>平均 O(n log n)，最坏 O(n²)，不稳定</text>

          {/* ===== 右下：堆排序 ===== */}
          <text x="540" y="226" textAnchor="middle" fontSize="13" fontWeight="700" fill={danger}>堆排序</text>
          <text x="540" y="244" textAnchor="middle" fontSize="11" fill={secondary}>建最大堆 → 取顶放末尾</text>

          {/* 堆树结构 */}
          <circle cx="540" cy="260" r="12" fill={danger} fillOpacity="0.1" stroke={danger} strokeWidth="1.5" />
          <text x="540" y="264" textAnchor="middle" fontSize="11" fill={danger}>8</text>
          <circle cx="510" cy="288" r="10" fill={danger} fillOpacity="0.06" stroke={danger} strokeWidth="1" />
          <text x="510" y="291" textAnchor="middle" fontSize="11" fill={danger}>5</text>
          <circle cx="570" cy="288" r="10" fill={danger} fillOpacity="0.06" stroke={danger} strokeWidth="1" />
          <text x="570" y="291" textAnchor="middle" fontSize="11" fill={danger}>3</text>
          <circle cx="494" cy="312" r="9" fill={danger} fillOpacity="0.04" stroke={danger} strokeWidth="1" />
          <text x="494" y="315" textAnchor="middle" fontSize="11" fill={danger}>2</text>
          <circle cx="526" cy="312" r="9" fill={danger} fillOpacity="0.04" stroke={danger} strokeWidth="1" />
          <text x="526" y="315" textAnchor="middle" fontSize="11" fill={danger}>1</text>
          ...
          <text x="540" y="336" textAnchor="middle" fontSize="11" fill={secondary}>O(n log n) 原地，不稳定</text>
          ...
          {[
            ...
            <g key={r.name}>
              <text x="70" y={408 + i * 12} fontSize="11" fill={r.color} fontWeight="600">{r.name}</text>
              <text x="180" y={408 + i * 12} fontSize="11" fill={primary}>{r.best}</text>
              <text x="280" y={408 + i * 12} fontSize="11" fill={primary}>{r.avg}</text>
              <text x="380" y={408 + i * 12} fontSize="11" fill={primary}>{r.worst}</text>
              <text x="480" y={408 + i * 12} fontSize="11" fill={primary}>{r.space}</text>
              <text x="580" y={408 + i * 12} fontSize="11" fill={primary}>{r.stable}</text>
            </g>
          ))}
        </svg>
      </div>
