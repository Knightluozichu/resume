    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        ...
        >
          ...
          <text x="160" y="198" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">Activity 生命周期</text>
          <text x="160" y="212" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">Service 启动/绑定</text>
          <text x="160" y="226" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">Broadcast 动态/静态</text>
          ...
          <text x="370" y="198" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">SharedPreferences</text>
          <text x="370" y="212" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">SQLite / Room</text>
          <text x="370" y="226" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">文件 / ContentProvider</text>
          ...
          <text x="580" y="198" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">布局/RecyclerView</text>
          <text x="580" y="212" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">网络/OkHttp/Retrofit</text>
          <text x="580" y="226" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">Material/Jetpack/MVVM</text>
          ...
          <text x="55" y="318" textAnchor="start" fontSize="11" fill="var(--text-secondary)">列表展示</text>
          <text x="240" y="318" textAnchor="start" fontSize="11" fill="var(--success)">RecyclerView + Adapter</text>
          <text x="480" y="318" textAnchor="start" fontSize="11" fill="var(--text-secondary)">ViewHolder复用，性能优</text>

          <text x="55" y="338" textAnchor="start" fontSize="11" fill="var(--text-secondary)">本地存储</text>
          <text x="240" y="338" textAnchor="start" fontSize="11" fill="var(--success)">Room（Jetpack）</text>
          <text x="480" y="338" textAnchor="start" fontSize="11" fill="var(--text-secondary)">编译期检查+响应式查询</text>

          <text x="55" y="358" textAnchor="start" fontSize="11" fill="var(--text-secondary)">网络请求</text>
          <text x="240" y="358" textAnchor="start" fontSize="11" fill="var(--success)">Retrofit + OkHttp</text>
          <text x="480" y="358" textAnchor="start" fontSize="11" fill="var(--text-secondary)">接口化+拦截器+连接池</text>

          <text x="55" y="378" textAnchor="start" fontSize="11" fill="var(--text-secondary)">后台任务</text>
          <text x="240" y="378" textAnchor="start" fontSize="11" fill="var(--success)">WorkManager</text>
          <text x="480" y="378" textAnchor="start" fontSize="11" fill="var(--text-secondary)">可约束/可延迟/系统调度</text>
          ...
          <text x="370" y="454" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">ListView（性能差） → RecyclerView（ViewHolder复用） → Jetpack ListAdapter/DiffUtil</text>
          <text x="370" y="472" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">SQLite（手写SQL） → LitePal（ORM） → Room（编译期检查+LiveData/Flow）</text>
          <text x="370" y="490" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">Thread+Handler → AsyncTask（废弃） → Coroutines/Flow（协程）</text>
          <text x="370" y="508" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">MVC → MVP → MVVM（ViewModel+LiveData） → MVI（单向数据流）</text>
          <text x="370" y="526" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">Intent → Navigation Component（可视化导航图）</text>
