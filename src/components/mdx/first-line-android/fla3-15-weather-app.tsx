...
export function Fla315WeatherAppDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第15章 进入实战，开发一个天气预报App：机制与证据"
      prompt="切换《第15章 进入实战，开发一个天气预报App》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第15章 进入实战，开发一个天气预报App》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Fla315WeatherAppMechanismMap() {
  return (
    <ChapterMechanismMap
      title="第15章 进入实战，开发一个天气预报App：机制路径"
      stages={STAGES}
    />
  );
}

export function Fla315WeatherAppFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第15章 进入实战，开发一个天气预报App：失效与核验"
      stages={STAGES}
    />
  );
}
