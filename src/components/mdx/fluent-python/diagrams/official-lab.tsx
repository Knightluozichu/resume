...
export interface FluentPythonCase {
  label: string;
  input: string;
  mechanism: string;
  evidence: string;
...
export function FluentPythonOfficialLab({
  title,
  caption,
  cases,
  tone = "cyan",
  ...
  title: string;
  caption: string;
  cases: ReadonlyArray<FluentPythonCase>;
    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      ...
      <div className="border border-border bg-elevated p-4 sm:p-5">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
          {cases.map((entry, index) => (
            <button
              ...
              className={`min-h-11 border px-2 text-xs text-primary ${
                selected === index
                  ? active[tone]
                  : "border-border bg-bg"
              }`}
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-[1fr_auto_1fr_auto_1fr]">
          <Cell label="输入 / 场景" value={item.input} />
          <span className="self-center text-center text-secondary" aria-hidden="true">
            →
          </span>
          <Cell label="Python 机制" value={item.mechanism} />
          <span className="self-center text-center text-secondary" aria-hidden="true">
            →
          </span>
          <Cell label="验收证据" value={item.evidence} />
        </div>
...
function Cell({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-h-24 border border-border bg-bg p-3">
      ...
      <strong className="mt-2 block text-sm leading-6 text-primary">
        {value}
      </strong>
    </div>
  );
