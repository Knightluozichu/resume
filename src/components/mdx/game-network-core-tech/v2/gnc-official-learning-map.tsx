...
const model = {
  ...
  topologies: [
    {
      name: "权威服务器",
      owner:
        "《网络游戏核心技术与实战》367坐标证据学习地图的服务端分片拥有可提交游戏状态，客户端只提交带序列号的输入意图",
      input: "认证主体、客户端构建、会话、输入序号、客户端tick与动作参数",
      validation:
        "服务端验证身份、权限、速率、时序和游戏规则后执行：选择一个正式坐标，沿时代身份、参与者、消息、权威状态、网络条件、可观察结果和恢复门定位依赖",
      fanout:
    ...
    },
    {
      name: "反射或中继拓扑",
      owner:
        "《网络游戏核心技术与实战》367坐标证据学习地图明确区分只转发字节的中继与真正裁决状态的主机或服务",
    ...
    },
    {
      name: "Peer输入交换",
      owner:
        "《网络游戏核心技术与实战》367坐标证据学习地图为房主、确定性模拟或每类资源指定唯一调停者，不能让所有peer都默认权威",
  ...
  traces: [
    {
      name: "《网络游戏核心技术与实战》367坐标证据学习地图参考消息轨迹",
      steps: [
        "《网络游戏核心技术与实战》367坐标证据学习地图基线1：冻结客户端与服务端构建、协议schema、区域、tick、玩家行为和成功条件。",
        "《网络游戏核心技术与实战》367坐标证据学习地图基线2：以已认证主体发送唯一序列输入，权威节点验证后只迁移一次状态。",
        "《网络游戏核心技术与实战》367坐标证据学习地图基线3：保存原始消息、确认、状态版本、抓包、trace、metric和玩家可见结果。",
        "《网络游戏核心技术与实战》367坐标证据学习地图基线4：停止会话并检查连接、队列、房间、对象、事务和指标标签均已回收。",
      ],
      verdict:
        "《网络游戏核心技术与实战》367坐标证据学习地图只有同时满足“367个正式坐标都必须绑定来源边界、时代轨、拓扑、权威状态、单变量故障、原始工件和恢复条件”并交付367坐标覆盖表、九章依赖图、双版本边界、当前标准索引和证据键目录，参考路径才成立。",
    },
    {
      name: "《网络游戏核心技术与实战》367坐标证据学习地图单一故障轨迹",
      steps: [
        "《网络游戏核心技术与实战》367坐标证据学习地图故障1：保持其余身份不变，只注入“把目录当正文、把作者历史样例当当前安全基线，或把2011原版与2014译本混成同一版本”；对照冻结客户端与服务端构建、协议schema、区域、tick、玩家行为和成功条件。",
        "《网络游戏核心技术与实战》367坐标证据学习地图故障2：保持其余身份不变，只注入“把目录当正文、把作者历史样例当当前安全基线，或把2011原版与2014译本混成同一版本”；对照以已认证主体发送唯一序列输入，权威节点验证后只迁移一次状态。",
        "《网络游戏核心技术与实战》367坐标证据学习地图故障3：保持其余身份不变，只注入“把目录当正文、把作者历史样例当当前安全基线，或把2011原版与2014译本混成同一版本”；对照保存原始消息、确认、状态版本、抓包、trace、metric和玩家可见结果。",
        "《网络游戏核心技术与实战》367坐标证据学习地图故障4：保持其余身份不变，只注入“把目录当正文、把作者历史样例当当前安全基线，或把2011原版与2014译本混成同一版本”；对照停止会话并检查连接、队列、房间、对象、事务和指标标签均已回收。",
      ],
      verdict:
        "《网络游戏核心技术与实战》367坐标证据学习地图的首个分岔必须能由“把目录当正文、把作者历史样例当当前安全基线，或把2011原版与2014译本混成同一版本”解释，否则保留竞争性解释。",
    },
    {
      name: "《网络游戏核心技术与实战》367坐标证据学习地图同输入恢复轨迹",
      steps: [
        "《网络游戏核心技术与实战》367坐标证据学习地图恢复1：撤销唯一变量并从干净状态重放；重新核对冻结客户端与服务端构建、协议schema、区域、tick、玩家行为和成功条件。",
        "《网络游戏核心技术与实战》367坐标证据学习地图恢复2：撤销唯一变量并从干净状态重放；重新核对以已认证主体发送唯一序列输入，权威节点验证后只迁移一次状态。",
        "《网络游戏核心技术与实战》367坐标证据学习地图恢复3：撤销唯一变量并从干净状态重放；重新核对保存原始消息、确认、状态版本、抓包、trace、metric和玩家可见结果。",
        "《网络游戏核心技术与实战》367坐标证据学习地图恢复4：撤销唯一变量并从干净状态重放；重新核对停止会话并检查连接、队列、房间、对象、事务和指标标签均已回收。",
      ],
      verdict:
        "《网络游戏核心技术与实战》367坐标证据学习地图必须恢复同一状态版本、消息轨迹和玩家观察，且没有连接、事务或资源残留。",
    },
  ],
  gates: [
    {
      label: "原版、译本与样例许可门",
      detail:
        "《网络游戏核心技术与实战》367坐标证据学习地图分开2011原版、2014译本和作者样例固定提交；只有README明确声明的样例游戏本体源码属于公有领域。",
    },
    {
      label: "参与者、拓扑与权威门",
      detail:
        "《网络游戏核心技术与实战》367坐标证据学习地图列出客户端、服务器、peer、中继、数据库和运营者，并为每类可变状态指定唯一裁决责任。",
    },
    {
      label: "消息schema与幂等门",
      detail:
        "《网络游戏核心技术与实战》367坐标证据学习地图固定传输、TLS、帧边界、schema版本、序列号、确认、幂等键、错误和兼容向量。",
    },
    {
      label: "tick、时延与故障分布门",
      detail:
        "《网络游戏核心技术与实战》367坐标证据学习地图记录tick、区域、分位RTT、抖动、丢包、乱序、重连与“把目录当正文、把作者历史样例当当前安全基线，或把2011原版与2014译本混成同一版本”的唯一注入位置。",
    },
    {
      label: "安全、滥用与持久化门",
      detail:
        "《网络游戏核心技术与实战》367坐标证据学习地图验证主体、权限、速率、服务器规则、事务隔离、唯一约束、审计、撤销和隐私。",
    },
    {
      label: "遥测、恢复与发布门",
      detail:
        "《网络游戏核心技术与实战》367坐标证据学习地图保存原始抓包、trace、metric、日志、玩家结果和同输入恢复，交付367坐标覆盖表、九章依赖图、双版本边界、当前标准索引和证据键目录并报告未知项。",
    },
  ],
} as const satisfies GameNetworkEvidenceModel;

export function GncOfficialLearningMapAuthorityContractLab() {
  return <GameNetworkEvidenceLab model={model} view="authority-contract" />;
}

export function GncOfficialLearningMapTrafficBudgetLab() {
  return <GameNetworkEvidenceLab model={model} view="traffic-budget" />;
}

export function GncOfficialLearningMapTraceGateLab() {
