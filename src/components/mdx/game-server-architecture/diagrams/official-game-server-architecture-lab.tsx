...
export function GameServerArchitectureEvidenceLab({
  title,
  label,
  nodes,
  mode,
  ...
  const evidence = useMemo(() => {
    ...
    const queue = Math.round(
      sessions / 48 + pressure + stage * 3 + (fault ? 40 : 0),
    );
    const latency = Math.round(
      10 + sessions / 72 + pressure * 0.9 + stage * 2 + (fault ? 52 : 0),
    );
    const divergence =
  ...
  }, [fault, nodes, sample, sessions, stage]);
  const pass =
    evidence.queue <= 96 &&
