    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        ...
        >
          ...
          <text x="140" y="92" textAnchor="middle" fontSize="8" fill="var(--danger)">GC（不可预测）</text>
          ...
          <text x="245" y="92" textAnchor="middle" fontSize="8" fill="var(--danger)">GC?</text>
          <rect x="270" y="76" width="50" height="24" fill="var(--success)" fillOpacity="0.2" stroke="var(--success)" strokeWidth="0.6" />

          <text x="185" y="116" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">堆越大 → 停顿越长</text>
          <text x="185" y="130" textAnchor="middle" fontSize="9" fill="var(--danger)">无法保证截止时间</text>
          ...
          <text x="433" y="92" textAnchor="middle" fontSize="7" fill="var(--success)">m</text>
          <text x="453" y="92" textAnchor="middle" fontSize="7" fill="var(--accent)">g</text>

          <text x="555" y="116" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">每个时间片：mutator + 少量GC工作</text>
          <text x="555" y="130" textAnchor="middle" fontSize="9" fill="var(--success)">停顿 ≤ 预算（如1ms）</text>
          ...
          <text x="60" y="275" fontSize="9" fill="var(--text-tertiary)">t0</text>
          <text x="340" y="275" fontSize="9" fill="var(--text-tertiary)">t1</text>
          <text x="680" y="275" fontSize="9" fill="var(--text-tertiary)">deadline</text>

          {/* mutator工作 */}
          <rect x="60" y="225" width="280" height="28" fill="var(--success)" fillOpacity="0.15" stroke="var(--success)" strokeWidth="0.8" />
          <text x="200" y="243" textAnchor="middle" fontSize="9" fill="var(--success)">mutator工作（分配对象）</text>

          {/* GC工作 */}
          <rect x="60" y="262" width="280" height="20" fill="var(--accent)" fillOpacity="0.2" stroke="var(--accent)" strokeWidth="0.8" />
          <text x="200" y="275" textAnchor="middle" fontSize="8" fill="var(--accent)">GC增量工作</text>
          ...
          <text x="510" y="243" textAnchor="middle" fontSize="9" fill="var(--warning)">Slack（空闲余量）</text>

          <text x="60" y="300" fontSize="9" fill="var(--text-secondary)">工作量 W = 分配率 × 时间 → 每个周期必须完成的GC工作量</text>
          <text x="60" y="315" fontSize="9" fill="var(--text-secondary)">Slack = 预算时间 - 必需GC时间 → 正值 = 安全，负值 = 溢出</text>
          ...
          <text x="75" y="408" fontSize="9" fill="var(--text-secondary)">固定周期（如1ms）分配预算：</text>
          <text x="75" y="422" fontSize="9" fill="var(--text-secondary)">{`  - 60% mutator, 40% GC`}</text>
          <text x="75" y="436" fontSize="9" fill="var(--text-tertiary)">IBM Metronome 经典实现</text>
          ...
          <text x="395" y="408" fontSize="9" fill="var(--text-secondary)">动态调整GC工作时机：</text>
          <text x="395" y="422" fontSize="9" fill="var(--text-secondary)">{`  - Slack大 → 少做GC`}</text>
          <text x="395" y="436" fontSize="9" fill="var(--text-tertiary)">{`  - Slack小 → 多做GC`}</text>
        </svg>
      </div>
