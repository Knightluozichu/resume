...
export function GoWebOfficialLab({ title, caption, cases, tone = "cyan", initial = 0 }: {
  title: string;
  caption: string;
  cases: ReadonlyArray<GoWebCase>;
    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="border border-border bg-elevated p-4 sm:p-5">
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-[1fr_auto_1fr_auto_1fr]">
          <Cell label="输入 / 触发" value={item.input} />
          <span className="self-center text-center text-secondary" aria-hidden="true">→</span>
          <Cell label="Go Web边界" value={item.boundary} />
          <span className="self-center text-center text-secondary" aria-hidden="true">→</span>
          <Cell label="输出 / 证据" value={item.output} />
        </div>
        <div className="mt-3 border border-border bg-bg p-3 text-sm leading-6 text-primary">
          <span className="mr-2 text-xs text-secondary">必须保持</span>{item.invariant}
        </div>
      </div>
...
function Cell({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-h-24 border border-border bg-bg p-3">
