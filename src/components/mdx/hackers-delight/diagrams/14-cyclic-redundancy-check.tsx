...
export function HD14ParameterModelLab() {
  ...
  return (
    <Figure caption="A CRC name is not a complete specification; width, polynomial, init, input/output reflection, xorout, check, and residue identify the model.">
      <div className="grid gap-3 sm:grid-cols-3"><label className="flex min-h-11 items-center gap-2 text-sm font-semibold text-primary"><input type="checkbox" checked={reflected} onChange={(event) => setReflected(event.target.checked)} />RefIn / RefOut</label><label className="flex min-h-11 items-center gap-2 text-sm font-semibold text-primary"><input type="checkbox" checked={nonzeroInit} onChange={(event) => setNonzeroInit(event.target.checked)} />Init FFFFFFFF</label><label className="flex min-h-11 items-center gap-2 text-sm font-semibold text-primary"><input type="checkbox" checked={finalXor} onChange={(event) => setFinalXor(event.target.checked)} />XorOut FFFFFFFF</label></div>
...
export function HD14CRCVsCorrectionLab() {
  ...
  return (
    <Figure caption="A CRC syndrome normally detects corruption but does not identify the error location; recovery needs retransmission or a separate correction code.">
      <div className="grid gap-3 sm:grid-cols-3"><label className="flex min-h-11 items-center gap-2 text-sm font-semibold text-primary"><input type="checkbox" checked={detected} onChange={(event) => setDetected(event.target.checked)} />nonzero syndrome</label><label className="flex min-h-11 items-center gap-2 text-sm font-semibold text-primary"><input type="checkbox" checked={locationKnown} onChange={(event) => setLocationKnown(event.target.checked)} />location known</label><label className="flex min-h-11 items-center gap-2 text-sm font-semibold text-primary"><input type="checkbox" checked={retransmission} onChange={(event) => setRetransmission(event.target.checked)} />retransmission available</label></div>
...
export function HD14CRCCertificateLab() {
  ...
  return (
    <Figure caption="A CRC implementation is ready when its complete parameter tuple, canonical check/residue vectors, and bitwise-versus-table cross-check all agree.">
      <div className="grid gap-3 sm:grid-cols-3"><label className="flex min-h-11 items-center gap-2 text-sm font-semibold text-primary"><input type="checkbox" checked={parameters} onChange={(event) => setParameters(event.target.checked)} />parameter tuple</label><label className="flex min-h-11 items-center gap-2 text-sm font-semibold text-primary"><input type="checkbox" checked={checkVector} onChange={(event) => setCheckVector(event.target.checked)} />check and residue</label><label className="flex min-h-11 items-center gap-2 text-sm font-semibold text-primary"><input type="checkbox" checked={crossImplementation} onChange={(event) => setCrossImplementation(event.target.checked)} />bitwise/table match</label></div>
