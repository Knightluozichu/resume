    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        ...
        >
          ...
          <text x="520" y="116" fontSize="11" fill="var(--text-primary)">
            - beverage: Beverage
          </text>
          <line
            x1="520"
            ...
            strokeWidth="1"
          />
          <text x="520" y="138" fontSize="11" fill="var(--text-primary)">
            + cost() {`{`}
          </text>
          <text x="524" y="148" fontSize="11" fill="var(--text-secondary)">
            beverage.cost() + ...
          </text>
          ...
          />
          <polygon
            points="220,349 214,346 214,352"
            fill="var(--text-secondary)"
          />
          <line
            x1="360"
          ...
          />
          <polygon
            points="380,349 374,346 374,352"
            fill="var(--text-secondary)"
          />
      ...
      <figcaption className="mt-2 text-center text-sm text-secondary">
        装饰器和具体组件都实现同一个 Beverage
        接口，装饰器内部持有被装饰对象。调用 cost() 时层层委托：Whip 先问内层
        Mocha 的价格，加上自己的 0.10，Mocha 再问 DarkRoast 的价格加上
        0.20。装饰器可以在不修改原类的情况下动态叠加功能。
      </figcaption>
    </figure>
  );
}

// OFFICIAL_QUALITY_WRAPPER_HeadFirstDesignPatterns_HfdDecorator
const officialQualityProps = {
  title: "装饰器模式",
  stages: ["第3章 装饰器模式", "开闭原则", "对象包装", "透明组合", "Java I/O"],
  sourceLabel: "O'Reilly Head First Design Patterns 2e",
} as const;

export function OfficialHeadFirstDesignPatternsHfdDecoratorMapLab() {
  return <OfficialCourseLab {...officialQualityProps} mode="map" />;
}

export function OfficialHeadFirstDesignPatternsHfdDecoratorExperimentLab() {
  return <OfficialCourseLab {...officialQualityProps} mode="experiment" />;
}

export function OfficialHeadFirstDesignPatternsHfdDecoratorEvidenceLab() {
