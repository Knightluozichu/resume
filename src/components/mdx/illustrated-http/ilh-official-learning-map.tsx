...
export function IlhOfficialLearningMapDecisionLab() {
  return (
    <ChapterDecisionLab
      title="2014年首版权威学习地图：机制与证据"
      prompt="切换《2014年首版权威学习地图》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《2014年首版权威学习地图》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function IlhOfficialLearningMapMechanismMap() {
  return (
    <ChapterMechanismMap
      title="2014年首版权威学习地图：机制路径"
      stages={STAGES}
    />
  );
}

export function IlhOfficialLearningMapFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="2014年首版权威学习地图：失效与核验"
      stages={STAGES}
    />
  );
}
