    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        ...
        >
          ...
          <text x="45" y="90" fontSize="11" fill="var(--text-secondary)">@Override / @Deprecated / @SuppressWarnings</text>
          <text x="45" y="106" fontSize="11" fill="var(--text-secondary)">@FunctionalInterface / @Repeatable</text>
          <text x="45" y="126" fontSize="11" fill="var(--text-secondary)">元注解:</text>
          <text x="45" y="142" fontSize="11" fill="var(--text-secondary)">  @Target: 可标注位置</text>
          <text x="45" y="158" fontSize="11" fill="var(--text-secondary)">  @Retention: 保留期</text>
          <text x="45" y="174" fontSize="11" fill="var(--text-secondary)">    SOURCE / CLASS / RUNTIME</text>
          <text x="45" y="194" fontSize="11" fill="var(--text-secondary)">@Retention(RUNTIME) 可被反射读取</text>
          ...
          <text x="405" y="90" fontSize="11" fill="var(--text-secondary)">Class&lt;?&gt; cls = obj.getClass();</text>
          <text x="405" y="106" fontSize="11" fill="var(--text-secondary)">Field[] fs = cls.getDeclaredFields();</text>
          <text x="405" y="122" fontSize="11" fill="var(--text-secondary)">Method[] ms = cls.getDeclaredMethods();</text>
          <text x="405" y="138" fontSize="11" fill="var(--text-secondary)">Constructor&lt;?&gt;[] cs = cls.getConstructors();</text>
          <text x="405" y="158" fontSize="11" fill="var(--text-secondary)">f.setAccessible(true);  // 突破private</text>
          <text x="405" y="174" fontSize="11" fill="var(--text-secondary)">m.invoke(obj, args);  // 动态调用</text>
          <text x="405" y="194" fontSize="11" fill="var(--text-secondary)">用途: 框架/Spring/JSON序列化</text>
          ...
          <text x="370" y="286" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">加载 classpath 下的类</text>

          <text x="370" y="308" textAnchor="middle" fontSize="14" fill="var(--text-tertiary)">&uarr;</text>
          <text x="400" y="308" fontSize="11" fill="var(--text-tertiary)">委派</text>
          ...
          <text x="370" y="348" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">加载 ext 目录扩展类</text>
          ...
          <text x="370" y="410" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">加载 rt.jar 核心类</text>
          ...
          <text x="370" y="470" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">加载 &rarr; 验证 &rarr; 准备 &rarr; 解析 &rarr; 初始化 &rarr; 使用 &rarr; 卸载</text>
        </svg>
      </div>
