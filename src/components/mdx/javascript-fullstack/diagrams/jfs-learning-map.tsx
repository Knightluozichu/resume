/**
 * <JfsLearningMapDiagram>：JavaScript全栈开发全书学习地图图解。
...
export function JfsLearningMapDiagram() {
  return (
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg
          ...
          role="img"
          aria-label="JavaScript全栈开发全书学习地图图解"
          className="mx-auto block h-auto w-full max-w-[740px]"
        >
          <text x={VIEW_W / 2} y="30" textAnchor="middle" fontSize="16" fontWeight="700" fill="var(--text-primary)">
            JavaScript全栈开发 全书学习地图
          </text>
          ...
          <text x="205" y="104" textAnchor="middle" fontSize="13" fontWeight="600" fill="var(--success)">前端基础</text>
          <text x="205" y="122" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">React 核心概念（组件/Hooks/虚拟DOM）</text>
          <text x="205" y="134" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">状态管理（Redux/Zustand/Context）</text>
          ...
          <text x="535" y="122" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">五阶段全栈进阶总览</text>
          <text x="535" y="134" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">从用户点击到数据落盘</text>
          ...
          <text x="205" y="196" textAnchor="middle" fontSize="13" fontWeight="600" fill="var(--warning)">后端服务</text>
          <text x="205" y="214" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">Node.js 服务器（事件循环/Stream）</text>
          <text x="205" y="226" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">Web 框架（Express/Koa/Fastify）</text>

          <rect x="380" y="174" width="310" height="56" rx="8" fill="var(--warning)" fillOpacity="0.12" stroke="var(--warning)" strokeWidth="1.2" />
          <text x="535" y="196" textAnchor="middle" fontSize="13" fontWeight="600" fill="var(--warning)">后端层目标</text>
          <text x="535" y="214" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">能处理：非阻塞 I/O 接收请求</text>
          <text x="535" y="226" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">能分发：中间件链路由</text>
          ...
          <text x="205" y="306" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">MongoDB 与 Mongoose ODM</text>
          <text x="205" y="318" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">GraphQL API 设计与实现</text>
          ...
          <text x="535" y="306" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">能存取：文档建模与关联查询</text>
          <text x="535" y="318" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">能查询：按需取数防 N+1</text>
          ...
          <text x="205" y="398" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">认证与安全（JWT/OAuth/CORS）</text>
          <text x="205" y="410" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">测试与部署（Jest/Docker/CI-CD）</text>
          ...
          <text x="535" y="398" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">全栈请求生命周期闭环</text>
          <text x="535" y="410" textAnchor="middle" fontSize="10" fill="var(--text-secondary)">全栈架构判断力</text>

          <text x={VIEW_W / 2} y="436" textAnchor="middle" fontSize="11" fill="var(--text-tertiary)">
            关键洞察：从「会写前端」到「懂全链路」的五层进阶
          </text>
        </svg>
      </div>
      <figcaption className="mt-2 text-center text-sm text-secondary">
        JavaScript全栈开发全书学习地图——前端、后端、数据层、工程化、总复习五阶段递进路径
      </figcaption>
    </figure>
  );
}
