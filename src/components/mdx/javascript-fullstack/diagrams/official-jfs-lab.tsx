...
export function JfsBookLab({ title, label, nodes, mode }: Props) {
  ...
  const evidence = useMemo(() => {
    ...
    const pending = Math.round(
      requests / 8 + pressure + stage * 2 + (fault ? 21 : 0),
    );
    const duration = Math.round(
      6 + requests / 3 + pressure * 0.9 + stage * 2 + (fault ? 30 : 0),
    );
    const resources = Math.round(
      requests / 12 + pressure / 4 + (fault ? 9 : 0),
    );
    const divergence =
      sample === "failure" || fault
        ? nodes[Math.min(stage, nodes.length - 1)]
        : "无";
    return { pending, duration, resources, divergence };
  }, [fault, nodes, requests, sample, stage]);
  const pass =
    evidence.pending <= 48 &&
    evidence.duration <= 64 &&
      ...
      <div className="grid lg:grid-cols-[minmax(0,1fr)_300px]">
              ...
              return (
                <li key={node}>
                  <button
                    ...
                    style={{
                      borderColor: active ? color.strong : "#d4d4d8",
                  ...
                  >
                    <span
                      ...
                      style={{ borderColor: color.strong, color: color.ink }}
                    >
                      {index + 1}
                    </span>
        ...
        <aside className="border-t border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-900 lg:border-l lg:border-t-0">
          ...
          <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
            <div className="border border-zinc-200 bg-white p-3 dark:border-zinc-700 dark:bg-zinc-950">
              <dt className="text-xs text-zinc-500">待处理任务</dt>
              <dd className="mt-1 text-lg font-semibold">{evidence.pending}</dd>
            </div>
            <div className="border border-zinc-200 bg-white p-3 dark:border-zinc-700 dark:bg-zinc-950">
              <dt className="text-xs text-zinc-500">耗时预算</dt>
              <dd className="mt-1 text-lg font-semibold">
                {evidence.duration} ms
              </dd>
            </div>
            <div className="border border-zinc-200 bg-white p-3 dark:border-zinc-700 dark:bg-zinc-950">
              <dt className="text-xs text-zinc-500">宿主资源</dt>
              <dd className="mt-1 text-lg font-semibold">
                {evidence.resources}
              </dd>
            </div>
          ...
          <p className="mt-4 text-xs leading-5 text-zinc-600 dark:text-zinc-300">
            比较语言值、浏览器状态、HTTP 流与 Node.js
            资源。恢复样本只有在清除故障并重放同一输入后才可签发。
          </p>
        </aside>
      </div>
    </section>
