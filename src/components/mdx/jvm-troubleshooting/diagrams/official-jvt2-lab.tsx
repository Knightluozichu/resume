...
export function OfficialJvt2Studio({
  unitId,
  unitTitle,
  concepts,
  stages,
  ...
  const result = useMemo(() => {
    ...
    const riskShift = scenario === "fault" ? 30 : scenario === "recovery" ? -15 : 0;
    const signal = clamp(40 + axisA * 14 + axisB * 12 + signalShift);
    const risk = clamp(
      33 +
    ...
    const evidence = clamp(47 + axisB * 14 + captures * 5 - (scenario === "fault" ? 7 : 0));
    return { signal, risk, evidence, accepted: signal >= 58 && risk <= 64 && evidence >= 60 };
    ...
    >
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="min-w-0">
          <p className="text-xs font-semibold text-rose-700 dark:text-rose-300">
            Troubleshooting Java 2e · {model.studio} · {mode === "investigation" ? "假设地图" : mode === "timeline" ? "时间线对照" : "证据与恢复"}
          </p>
      ...
      <div className="grid min-w-0 lg:grid-cols-[minmax(0,1.12fr)_minmax(280px,0.88fr)]">
          ...
          {mode === "investigation" ? (
            <>
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {concepts.map((concept, index) => (
                    <button
                      ...
                      className={`min-h-11 min-w-0 rounded border px-3 py-2 text-left text-xs leading-5 [overflow-wrap:anywhere] ${index === conceptIndex ? "border-rose-600 bg-rose-50 text-rose-950 dark:bg-rose-950 dark:text-rose-50" : "border-zinc-300 bg-white hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900"}`}
                    >
                      {concept}
                    </button>
                  ))}
              ...
              <ol className="mt-4 grid gap-2 sm:grid-cols-5">
                {stages.map((stage, index) => (
                  <li key={stage} className={`min-w-0 border p-2 text-xs [overflow-wrap:anywhere] ${stage === currentStage ? "border-cyan-500 bg-cyan-50 dark:bg-cyan-950" : "border-zinc-200 dark:border-zinc-800"}`}>
            ...
            <div className="space-y-5">
              {[
                ...
                const typedAxis = axis as InvestigationModel["axisA"];
                return (
              ...
              <div className="rounded border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-900">
                <p className="text-xs font-semibold text-zinc-500">本章采集命令或探针</p>
            ...
            <div>
              <p className="text-sm text-zinc-600 dark:text-zinc-300">
                故障窗口用于暴露首个分叉；修复重放必须使用同一负载，最后用重置核对初值。
              </p>
        ...
        <div className="min-w-0 p-4">
          <p className="text-xs font-semibold text-zinc-500">当前调查坐标</p>
          ...
          <div className={`mt-3 border p-3 text-sm ${result.accepted ? "border-emerald-500 bg-emerald-50 text-emerald-950 dark:bg-emerald-950 dark:text-emerald-50" : "border-amber-500 bg-amber-50 text-amber-950 dark:bg-amber-950 dark:text-amber-50"}`}>
            <strong>{result.accepted ? "调查证据可接受" : "继续采证或撤回根因"}</strong>
