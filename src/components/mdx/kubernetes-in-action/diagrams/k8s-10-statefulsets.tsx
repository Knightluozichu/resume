...
const meta = {
  unitTitle: "第10章 StatefulSet ：部署有状态的多副本应用",
  focus: "用StatefulSet提供稳定序号、DNS、专属存储、顺序保证和节点故障处理",
  invariant:
    "每个实例的身份、网络名与PVC跨重建保持稳定，同一成员不会在分区两侧并发运行",
  artifact:
    "StatefulSet与RS对照、Headless Service、PVC映射、伙伴发现和网络分区演练",
  nodes: [
    "10.1 复制有状态 pod",
...
export function K8s10StatefulsetsObjectLab() {
  return <OfficialKubernetesActionLab mode="object" {...meta} />;
}
export function K8s10StatefulsetsReconcileLab() {
  return <OfficialKubernetesActionLab mode="reconcile" {...meta} />;
}
export function K8s10StatefulsetsEvidenceLab() {
