...
type Props = {
  model: LlmEvidenceModel;
  view: "context-contract" | "compute-trace" | "evidence-gate";
};
...
function ContextContract({ model }: { model: LlmEvidenceModel }) {
  ...
  return (
    <section
      ...
      data-visual-kind="llm-context-contract"
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.3fr)]">
        ...
        <article className="rounded-card border border-border bg-background p-4">
          ...
          <dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2">
            ...
            <div>
              <dt className="text-xs text-muted-foreground">裁决</dt>
              <dd className="mt-1 text-foreground">
                {mode === "reference"
                  ? stage?.check
                  : `“${concept}”只注入该反例后，寻找首个不再满足“${stage?.check}”的状态。`}
              </dd>
            </div>
          </dl>
        </article>
...
function ComputeTrace({ model }: { model: LlmEvidenceModel }) {
  ...
  return (
    <section
      className="not-prose my-6 rounded-card border border-border bg-card p-4 shadow-sm sm:p-5"
      data-visual-kind="llm-compute-trace"
      data-unit-id={model.unitId}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
      ...
      <p className="mt-4 rounded-control bg-muted px-3 py-2 text-sm text-foreground">
        当前裁决：
        {mode === "reference"
          ? `计算必须持续满足“${model.invariant}”。`
          : `首个分岔必须能归因到“${model.fault}”，不能同时更换语料、模型、解码和评估集。`}
      </p>
    </section>
  );
}

function EvidenceGate({ model }: { model: LlmEvidenceModel }) {
  ...
  return (
    <section
      className="not-prose my-6 rounded-card border border-border bg-card p-4 shadow-sm sm:p-5"
      data-visual-kind="llm-evidence-gate"
      data-unit-id={model.unitId}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
...
export function LlmEvidenceLab({ model, view }: Props) {
  if (view === "context-contract") return <ContextContract model={model} />;
  if (view === "compute-trace") return <ComputeTrace model={model} />;
