  ...
  return (
    <Frame caption="列向量写在矩阵右边决定代数作用顺序；列主序只描述这 16 个数在线性内存中的存放顺序。">
      ...
      >
        ...
        <text x="306" y="174" fontSize="20" fill={secondary}>
          ×
        </text>
        {["x", "y", "z", "1"].map((value, i) => (
          <g key={value}>
...
export function TransformSequenceDiagram({
  step = 0,
}: {
  step?: 0 | 1 | 2 | 3;
}) {
    ...
    <Frame caption="同一点按 S → R → T 演算：公式写作 T · R · S · v，最靠近 v 的 S 先执行。">
      ...
      >
          ...
          return (
            <g key={stage.title} opacity={active ? 1 : 0.24} data-stage={i}>
              ...
              <circle
                ...
                cy={
                  190 - Number(stage.result.match(/,\s*(-?\d+)/)?.[1] ?? 0) * 18
                }
                r="6"
                fill={stage.color}
              />
        ...
        {[210, 430, 650].map((x) => (
          <path
            key={x}
            d={`M${x} 145 H${x + 26} M${x + 18} 137 L${x + 27} 145 L${x + 18} 153`}
            fill="none"
            stroke={danger}
            strokeWidth="2"
          />
