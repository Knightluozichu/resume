...
type OfficialLinuxKernelDesignLabProps = {
  title: string;
  label: string;
  color: string;
  soft: string;
...
export function OfficialLinuxKernelDesignLab({
  title,
  label,
  color,
  soft,
  ...
  concepts,
  view,
}: OfficialLinuxKernelDesignLabProps) {
  const [mode, setMode] = useState<LabMode>("baseline");
  const [stage, setStage] = useState(0);
  const [load, setLoad] = useState(4);

  const evidence = useMemo(() => {
    const penalties = { baseline: 0, boundary: 12, fault: 37, recovery: 4 };
    const penalty = penalties[mode] + stage * 2 + Math.max(0, load - 8);
    return {
      confidence: Math.max(18, 98 - penalty),
      events: load * (stage + 1),
      lost:
        mode === "fault" ? Math.ceil(load / 3) : mode === "boundary" ? 1 : 0,
      status:
        mode === "fault"
          ? "首错已捕获"
          : mode === "recovery"
        ...
        <aside className="border-t border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-900 lg:border-l lg:border-t-0">
          ...
          <dl className="mt-5 grid gap-4 text-sm">
            ...
            <div className="grid grid-cols-3 gap-2 border-y border-zinc-200 py-3 text-center dark:border-zinc-700">
              ...
              <div>
                <dt className="text-[11px] text-zinc-500">丢失</dt>
                <dd className="font-bold">{evidence.lost}</dd>
              </div>
            </div>
          </dl>
          <p className="mt-4 flex gap-2 text-xs leading-5 text-zinc-600 dark:text-zinc-300">
            ...
            固定版本和负载，在首个计数、事件、栈或延迟分叉处停止；清理后以同一输入重放。
          </p>
        </aside>
      </div>
    </section>
