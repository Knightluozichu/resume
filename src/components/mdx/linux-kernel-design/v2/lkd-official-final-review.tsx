...
const model = {
  ...
  invariant: "单故障首错可归因且撤销后任务、引用、队列、内存和I/O恢复",
  fault: "跨子系统同时注错、混用配置或只保留最终现象",
  artifact: "全链构建清单、跨章对象ID、逐故障trace与回归报告",
  probe: "cross",
  stages: [
    {
      label: "冻结对象身份",
      object:
        "《Linux内核设计与实现（原书第3版）》404坐标全书证据总复习涉及的进程接口、事件并发、时间内存、文件I/O、设备调试与贡献流程",
    ...
    },
    {
      label: "执行参考路径",
      object:
        "《Linux内核设计与实现（原书第3版）》404坐标全书证据总复习的参考对象链",
      control: "只运行预注册基线操作",
      signal:
        "状态转移、tracepoint、计数与“单故障首错可归因且撤销后任务、引用、队列、内存和I/O恢复”",
      rollback: "停止负载并核对无残留对象",
    },
    {
      label: "注入唯一故障",
      object:
        "《Linux内核设计与实现（原书第3版）》404坐标全书证据总复习的故障边界",
      control: "只注入“跨子系统同时注错、混用配置或只保留最终现象”",
    ...
    },
    {
      label: "同输入恢复",
      object:
        "《Linux内核设计与实现（原书第3版）》404坐标全书证据总复习的恢复对象链",
      control: "以相同构建、机器和输入重放",
      signal:
        "重新满足“单故障首错可归因且撤销后任务、引用、队列、内存和I/O恢复”且资源计数回基线",
...
export function LkdOfficialFinalReviewObjectVersionLab() {
  return <KernelDesignEvidenceLab model={model} view="object-version" />;
}

export function LkdOfficialFinalReviewExecutableProbeLab() {
  return <KernelDesignEvidenceLab model={model} view="executable-probe" />;
}

export function LkdOfficialFinalReviewTraceGateLab() {
