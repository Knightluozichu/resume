...
const TYPES: readonly TypeInfo[] = [
  ...
  { name: "boolean", color: warning, desc: "true / false", example: "local ok = true" },
  { name: "number", color: accent, desc: "浮点数（统一）", example: "local n = 3.14" },
  { name: "string", color: success, desc: "不可变字节序列", example: 'local s = "hi"' },
  { name: "function", color: accent, desc: "一等公民", example: "local f = print" },
  { name: "table", color: success, desc: "唯一数据结构", example: "local t = {}" },
      ...
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        ...
        >
          ...
          {TYPES.map((t, i) => {
            ...
            return (
              <g key={t.name}>
                <rect x={x} y={y} width={152} height={80} rx="8" fill={t.color} fillOpacity="0.06" stroke={t.color} strokeWidth="1.2" strokeOpacity="0.4" />
                <text x={x + 76} y={y + 22} textAnchor="middle" fontSize="13" fontWeight="700" fill={t.color}>
                  {t.name}
                </text>
                <text x={x + 76} y={y + 42} textAnchor="middle" fontSize="11" fill={secondary}>
                  {t.desc}
                </text>
                <text x={x + 76} y={y + 62} textAnchor="middle" fontSize="10" fill={primary} fontFamily="monospace">
                  {t.example}
                </text>
              </g>
            );
          ...
          {/* 底部说明 */}
          <line x1={32} y1={284} x2={VIEW_W - 32} y2={284} stroke={border} strokeWidth="1" strokeDasharray="4 3" />
          <text x={VIEW_W / 2} y={306} textAnchor="middle" fontSize="13" fontWeight="700" fill={accent}>
            核心特性
          </text>
          <text x={VIEW_W / 2} y={326} textAnchor="middle" fontSize="11" fill={secondary}>
