    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="函数复合与逆函数图解。左侧展示函数复合 f(g(x))：x→g(x)=2x→f(2x)=(2x)²+1=4x²+1，三个圆圈用箭头串联。右侧上方展示逆函数：f(x)=x²+1 和 f⁻¹(y)=√(y-1) 互为逆。右侧下方展示四种函数族的增长曲线对比：对数、线性、多项式、指数。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x="64" y="248" fontSize="11" fontFamily="monospace" fill={primary}>f(x) = x² + 1</text>
          <text x="64" y="266" fontSize="11" fontFamily="monospace" fill={primary}>f⁻¹(y) = √(y - 1)</text>
          <text x="64" y="280" fontSize="11" fill={secondary}>f⁻¹(f(x)) = x（撤销操作）</text>
          ...
          <text x="64" y="344" fontSize="11" fill={primary}>数学函数 f:X→Y = 编程纯函数 f(x:X)→Y</text>
          <text x="64" y="360" fontSize="11" fill={primary}>复合 f∘g = 函数组合/链式调用　　逆函数 = 反操作/解码</text>
          <text x="64" y="372" fontSize="11" fill={secondary}>理解数学函数有助于设计无副作用的纯函数（函数式编程核心）</text>
        </svg>
      </div>
