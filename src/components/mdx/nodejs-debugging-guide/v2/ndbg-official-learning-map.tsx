...
const model = {
  ...
  stages: [
    {
      label: "冻结症状与输入",
      hypothesis:
        "《Node.js调试指南》152坐标证据学习地图先把“有异常指标或用户故障，但尚无可证伪根因”写成可重复条件，不预设工具结论",
    ...
    },
    {
      label: "采集低扰动基线",
      hypothesis:
        "《Node.js调试指南》152坐标证据学习地图先用指标和运行时身份判断异常属于CPU、内存、队列、错误或依赖",
      capture:
        "152坐标覆盖矩阵、八条诊断链、Node 8/当前迁移表和工件安全分级中的低开销指标、日志和诊断报告",
    ...
    },
    {
      label: "缩小到原始工件",
      hypothesis:
        "《Node.js调试指南》152坐标证据学习地图用剖析、快照、trace或错误事件区分至少两个竞争性解释",
    ...
    },
    {
      label: "单故障与同输入恢复",
      hypothesis:
        "《Node.js调试指南》152坐标证据学习地图只注入“先选熟悉工具再定义问题，或把Node 8工具直接当成当前方案”并用最小修复推翻根因假设",
      capture: "参考、故障、恢复三条时间对齐轨迹与残留资源检查",
      identity: "相同Node/依赖/主机/流量，唯一变量和回滚提交明确",
      falsifier:
        "撤销后仍不满足“152个坐标都必须绑定症状、假设、原始工件、反证、恢复和版本边界”，或其他变量同步变化",
    },
  ],
  cases: [
    {
      name: "《Node.js调试指南》152坐标证据学习地图作者原仓库复现",
      input: "Node 8.9.4、Ubuntu 16.04、固定提交403f5c6",
      historical:
        "保留8个章标题、30个编号主题与114个下级目录的全书学习地图在作者原公开章节中的工具身份、示例目的和时代限制。",
      current: "当前轨道只建立迁移差分，不修改或假装更新作者原文。",
      boundary:
        "作者仓库没有LICENSE文件；公开可读不等于允许复制，本站只做独立表达和短引核对。",
    },
    {
      name: "《Node.js调试指南》152坐标证据学习地图当前内置能力迁移",
    ...
    {
      name: "《Node.js调试指南》152坐标证据学习地图生产安全采集",
  ...
  baselineTrace: [
    "《Node.js调试指南》152坐标证据学习地图基线1：冻结症状与输入，保存用户时间线、固定请求集、并发、数据、启动/预热与成功判据。",
    "《Node.js调试指南》152坐标证据学习地图基线2：采集低扰动基线，保存152坐标覆盖矩阵、八条诊断链、Node 8/当前迁移表和工件安全分级中的低开销指标、日志和诊断报告。",
    "《Node.js调试指南》152坐标证据学习地图基线3：缩小到原始工件，保存带参数和时间窗口的CPU/堆/事件/trace原始文件，不只保存截图。",
    "《Node.js调试指南》152坐标证据学习地图基线4：单故障与同输入恢复，保存参考、故障、恢复三条时间对齐轨迹与残留资源检查。",
  ],
  faultTrace: [
    "《Node.js调试指南》152坐标证据学习地图故障1：冻结症状与输入只改变“先选熟悉工具再定义问题，或把Node 8工具直接当成当前方案”，检查同输入不能稳定重现症状，或基线自身漂移超过故障差分。",
    "《Node.js调试指南》152坐标证据学习地图故障2：采集低扰动基线只改变“先选熟悉工具再定义问题，或把Node 8工具直接当成当前方案”，检查工件来自不同进程、版本、时间窗口或请求集。",
    "《Node.js调试指南》152坐标证据学习地图故障3：缩小到原始工件只改变“先选熟悉工具再定义问题，或把Node 8工具直接当成当前方案”，检查独立工件不支持同一首个分岔，或采集扰动足以解释差异。",
    "《Node.js调试指南》152坐标证据学习地图故障4：单故障与同输入恢复只改变“先选熟悉工具再定义问题，或把Node 8工具直接当成当前方案”，检查撤销后仍不满足“152个坐标都必须绑定症状、假设、原始工件、反证、恢复和版本边界”，或其他变量同步变化。",
  ],
  recoveryTrace: [
    "《Node.js调试指南》152坐标证据学习地图恢复1：冻结症状与输入以同一输入重放，核对Node 8.9.4、V8、Ubuntu 16.04、依赖锁与作者仓库固定提交。",
    "《Node.js调试指南》152坐标证据学习地图恢复2：采集低扰动基线以同一输入重放，核对PID、进程启动时间、构建哈希、主机/容器、时钟和采集配置。",
    "《Node.js调试指南》152坐标证据学习地图恢复3：缩小到原始工件以同一输入重放，核对文件哈希、工具版本、采样率、过滤、符号、isolate与丢失计数。",
    "《Node.js调试指南》152坐标证据学习地图恢复4：单故障与同输入恢复以同一输入重放，核对相同Node/依赖/主机/流量，唯一变量和回滚提交明确。",
  ],
  gates: [
    {
      label: "原作身份与许可门",
      detail:
        "《Node.js调试指南》152坐标证据学习地图固定作者仓库提交、Node 8.9.4和Ubuntu 16.04；无LICENSE意味着只核对和独立重构，不复制原文、代码或图像。",
    },
    {
      label: "工件身份与三角校验门",
      detail:
        "《Node.js调试指南》152坐标证据学习地图保存PID、构建、时间、工具参数与文件哈希，并让指标、剖析/快照和事件至少两类工件互证。",
    },
    {
      label: "采集扰动与数据安全门",
      detail:
        "《Node.js调试指南》152坐标证据学习地图评估CPU、阻塞、额外内存、磁盘、隐私和远程执行风险；超出预算就降级或停止。",
    },
    {
      label: "单变量与回归门",
      detail:
        "《Node.js调试指南》152坐标证据学习地图只注入“先选熟悉工具再定义问题，或把Node 8工具直接当成当前方案”，撤销后同一输入满足“152个坐标都必须绑定症状、假设、原始工件、反证、恢复和版本边界”并交付152坐标覆盖矩阵、八条诊断链、Node 8/当前迁移表和工件安全分级。",
    },
  ],
} as const satisfies NodeDiagnosticsEvidenceModel;

export function NdbgOfficialLearningMapHypothesisLadderLab() {
  return <NodeDiagnosticsEvidenceLab model={model} view="hypothesis-ladder" />;
}

export function NdbgOfficialLearningMapArtifactTriangulationLab() {
  return (
    <NodeDiagnosticsEvidenceLab model={model} view="artifact-triangulation" />
  );
}

export function NdbgOfficialLearningMapMigrationGateLab() {
