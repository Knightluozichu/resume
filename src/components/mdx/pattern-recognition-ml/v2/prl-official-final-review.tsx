...
const model = {
  ...
  stages: [
    ...
    },
    {
      name: "比较模型",
      prior:
        "《模式识别与机器学习》概率建模总复习：构造联合分布、函数或图结构，保持其余概率合同不变",
      operation:
        "记录假设、参数化、归一与条件独立，并持续满足“每个选择都能回到原版坐标、概率假设、计算轨迹、预测证据、失败边界和2006/当前时间标签”",
      posterior: "比较模型产生可计算模型状态",
      check:
        "可计算模型状态、概率质量与数值断言；出现“按单次指标排名模型，隐藏数据泄漏、近似误差、随机性和不确定性语义差异”时停止",
    },
    {
      name: "选择推断",
      prior:
        "《模式识别与机器学习》概率建模总复习：选择精确、近似、优化或采样步骤，保持其余概率合同不变",
      operation:
        "保存初值、顺序、随机性、目标和残差，并持续满足“每个选择都能回到原版坐标、概率假设、计算轨迹、预测证据、失败边界和2006/当前时间标签”",
      posterior: "选择推断产生可重放推断状态",
      check:
        "可重放推断状态、概率质量与数值断言；出现“按单次指标排名模型，隐藏数据泄漏、近似误差、随机性和不确定性语义差异”时停止",
    },
    {
      name: "形成预测",
      prior:
        "《模式识别与机器学习》概率建模总复习：从后验或参数形成任务输出，保持其余概率合同不变",
      operation:
        "同时保留点结果、不确定性和损失语义，并持续满足“每个选择都能回到原版坐标、概率假设、计算轨迹、预测证据、失败边界和2006/当前时间标签”",
      posterior: "形成预测产生可检验预测状态",
      check:
        "可检验预测状态、概率质量与数值断言；出现“按单次指标排名模型，隐藏数据泄漏、近似误差、随机性和不确定性语义差异”时停止",
    },
    {
      name: "压力测试",
      prior:
        "《模式识别与机器学习》概率建模总复习：执行归一、收敛、校准与反例检查，保持其余概率合同不变",
      operation:
        "隔离测试角色并登记适用域和时间边界，并持续满足“每个选择都能回到原版坐标、概率假设、计算轨迹、预测证据、失败边界和2006/当前时间标签”",
      posterior: "压力测试产生独立概率证据包",
      check:
        "独立概率证据包、概率质量与数值断言；出现“按单次指标排名模型，隐藏数据泄漏、近似误差、随机性和不确定性语义差异”时停止",
    },
  ],
  cases: [
    {
    ...
    },
    {
      name: "边界反例",
      observation:
        "为一个混合静态、序列和潜变量的任务提交端到端概率建模评审包。 其余条件不变，只注入“按单次指标排名模型，隐藏数据泄漏、近似误差、随机性和不确定性语义差异”。",
      prediction: "定位第一处概率、条件独立、推断或预测状态偏离，并拒绝结论。",
      boundary: "失败轨迹必须保留；撤销故障后以相同输入重放。",
    },
  ],
  referenceTrace: [
    "为“《模式识别与机器学习》概率建模总复习”冻结观测、数据角色、参数化、初值、顺序、容差和随机种子",
    "执行界定任务、比较模型，保存支持集、假设、分布或图结构",
    "推进选择推断、形成预测，记录推断目标、更新、残差与预测不确定性",
    "在压力测试交付266层覆盖账本、模型候选矩阵、数据协议、推断预算、诊断轨迹、预测检查、反例、选择理由和复现环境。",
  ],
  faultTrace: [
    "“《模式识别与机器学习》概率建模总复习”复用相同观测、数据角色、参数化、初值、顺序、容差和种子",
    "只改变一个条件：按单次指标排名模型，隐藏数据泄漏、近似误差、随机性和不确定性语义差异",
    "沿“界定任务 → 比较模型 → 选择推断 → 形成预测 → 压力测试”寻找最早的概率或数值分叉",
    "撤销故障重放；只有“每个选择都能回到原版坐标、概率假设、计算轨迹、预测证据、失败边界和2006/当前时间标签”恢复才接受修正",
  ],
  invariant:
  ...
  gates: [
    {
      label: "观测与数据角色",
      detail:
        "“《模式识别与机器学习》概率建模总复习”的变量、支持集、采样/切分、允许读取的信息和版本可追溯。",
    },
    {
      label: "模型与概率语义",
      detail:
        "“《模式识别与机器学习》概率建模总复习”的结构、参数化、先验、似然、条件独立和归一约定已冻结。",
    },
    {
      label: "推断与数值诊断",
      detail:
        "“《模式识别与机器学习》概率建模总复习”的初值、顺序、随机性、目标、更新、容差、残差和近似误差可重放。",
    },
    {
      label: "预测与外部边界",
      detail:
        "“《模式识别与机器学习》概率建模总复习”归档不确定性、损失、校准、独立测试、反例、适用域和时间标签。",
    },
  ],
} as const satisfies PrmlEvidenceModel;

export function PrlOfficialFinalReviewProbabilisticModelLab() {
  return <PrmlEvidenceLab model={model} view="probabilistic-model" />;
}

export function PrlOfficialFinalReviewInferenceTraceLab() {
  return <PrmlEvidenceLab model={model} view="inference-trace" />;
}

export function PrlOfficialFinalReviewPredictiveCheckLab() {
