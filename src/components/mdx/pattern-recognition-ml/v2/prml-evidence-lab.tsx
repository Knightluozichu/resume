...
type Props = {
  model: PrmlEvidenceModel;
  view: "probabilistic-model" | "inference-trace" | "predictive-check";
};
...
function ProbabilisticModelLab({ model }: { model: PrmlEvidenceModel }) {
  ...
  return (
    <section
      ...
      data-visual-kind="prml-probabilistic-model"
      aria-label={`${model.title}概率模型状态`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium text-accent">概率模型状态</p>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        {[
          ["进入本步的状态", activeStage.prior],
          ["条件化或变换", activeStage.operation],
          ["离开本步的状态", activeStage.posterior],
          ["概率与数值检查", activeStage.check],
        ].map(([label, detail]) => (
          <div
            key={label}
            className="rounded-card border border-border bg-background p-4"
...
function InferenceTraceLab({ model }: { model: PrmlEvidenceModel }) {
  ...
  return (
    <section
      className="not-prose my-6 rounded-card border border-border bg-elevated p-4 sm:p-6"
      data-visual-kind="prml-inference-trace"
      aria-label={`${model.title}推断轨迹`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium text-accent">推断轨迹</p>
          <h3 className="mt-1 text-lg font-semibold text-primary">
            对照参考运行与单一故障
          </h3>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <div className="rounded-card border border-border bg-background p-4">
          <p className="text-xs font-medium text-success">推断不变量</p>
...
function PredictiveCheckLab({ model }: { model: PrmlEvidenceModel }) {
  ...
  return (
    <section
      className="not-prose my-6 rounded-card border border-border bg-elevated p-4 sm:p-6"
      data-visual-kind="prml-predictive-check"
      aria-label={`${model.title}预测检验门`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium text-accent">预测检验门</p>
          <h3 className="mt-1 text-lg font-semibold text-primary">
            逐项锁定模型、推断与外部检验条件
          </h3>
        </div>
      ...
      >
        {artifactVisible ? "收起概率证据包" : "展开概率证据包"}
      </button>
...
export function PrmlEvidenceLab({ model, view }: Props) {
  if (view === "probabilistic-model")
    return <ProbabilisticModelLab model={model} />;
  if (view === "inference-trace") return <InferenceTraceLab model={model} />;
  return <PredictiveCheckLab model={model} />;
}
