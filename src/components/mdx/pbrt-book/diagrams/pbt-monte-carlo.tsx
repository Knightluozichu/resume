...
export function PbtMonteCarloDiagram() {
  return (
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox="0 0 720 400" role="img" aria-label="蒙特卡洛积分三种技术：均匀采样基本估计量、重要性采样降低方差、俄罗斯轮盘赌控制递归深度。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <g>
            ...
            <text x={52} y={92} fontSize="13" fontWeight="700" fill={accent}>{`基本估计量`}</text>
            <rect x={158} y={92} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`均匀采样`}</text>
            <rect x={310} y={92} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`1/N 求和`}</text>
            ...
            <text x={532} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`无偏但高方差`}</text>
          ...
          <g>
            ...
            <text x={52} y={169} fontSize="13" fontWeight="700" fill={success}>{`重要性采样`}</text>
            <rect x={158} y={169} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`p(x) 正比 f(x)`}</text>
            <rect x={310} y={169} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`高贡献多采样`}</text>
            ...
            <text x={532} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`方差大幅降低`}</text>
          ...
          <g>
            ...
            <text x={52} y={246} fontSize="13" fontWeight="700" fill={warning}>{`俄罗斯轮盘赌`}</text>
            <rect x={158} y={246} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`概率 q 继续`}</text>
            <rect x={310} y={246} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`贡献除以 q`}</text>
            ...
            <text x={532} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`无偏截断`}</text>
          ...
          <g>
            ...
            <text x={228} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`方差 正比 1/N`}</text>
            <rect x={310} y={323} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`二倍采样半噪声`}</text>
            ...
            <text x={532} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`维度无关`}</text>
            <line x1={450} y1={340} x2={462} y2={340} stroke={border} strokeWidth="1" strokeDasharray="3 2" />
          </g>
          <defs>
            <marker id="pbtMonteCarlo-arrow" markerWidth="8" markerHeight="8" refX="4" refY="3" orient="auto">
      ...
      <figcaption className="mt-2 text-center text-sm text-secondary">
        蒙特卡洛积分：基本估计量保证无偏，重要性采样降低方差，俄罗斯轮盘赌控制深度。
      </figcaption>
    </figure>
  );
}
