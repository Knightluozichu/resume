/**
 * <PbtSystemArchitectureDiagram>: PBRT 系统架构与并行
 *
 * 场景->BVH->积分器->瓦片并行->帧缓冲
  ...
  return (
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox="0 0 720 400" role="img" aria-label="PBRT系统架构：场景数据通过BVH加速结构、积分器渲染、瓦片化并行输出到帧缓冲。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <g>
            ...
            <text x={52} y={92} fontSize="13" fontWeight="700" fill={accent}>{`数据层`}</text>
            <rect x={158} y={92} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`Scene 场景`}</text>
            <rect x={310} y={92} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`Primitive 图元`}</text>
            ...
            <text x={532} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`Material 材质`}</text>
          ...
          <g>
            ...
            <text x={52} y={169} fontSize="13" fontWeight="700" fill={success}>{`加速结构`}</text>
            <rect x={158} y={169} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`BVH 树`}</text>
            <rect x={310} y={169} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`AABB 包围盒`}</text>
            ...
            <text x={532} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`O(logN) 查询`}</text>
          ...
          <g>
            ...
            <text x={52} y={246} fontSize="13" fontWeight="700" fill={warning}>{`渲染核心`}</text>
            <rect x={158} y={246} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`Integrator 积分器`}</text>
            <rect x={310} y={246} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`Sampler 采样器`}</text>
            ...
            <text x={532} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`Camera 相机`}</text>
          ...
          <g>
            ...
            <text x={52} y={323} fontSize="13" fontWeight="700" fill={danger}>{`并行输出`}</text>
            <rect x={158} y={323} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`瓦片化`}</text>
            <rect x={310} y={323} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`线程池`}</text>
            ...
            <text x={532} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`Film 帧缓冲`}</text>
            <line x1={450} y1={340} x2={462} y2={340} stroke={border} strokeWidth="1" strokeDasharray="3 2" />
          </g>
          <defs>
            <marker id="pbtSystemArchitecture-arrow" markerWidth="8" markerHeight="8" refX="4" refY="3" orient="auto">
      ...
      <figcaption className="mt-2 text-center text-sm text-secondary">
        PBRT 架构：BVH加速求交，积分器求解光传输，瓦片化并行渲染，采样器保证质量。
      </figcaption>
    </figure>
  );
}
