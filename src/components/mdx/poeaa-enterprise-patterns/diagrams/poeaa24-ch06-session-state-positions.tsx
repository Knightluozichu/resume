...
export function Poeaa24Ch06SessionStatePositionsDiagram() {
  return (
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg
          ...
          role="img"
          aria-label="会话状态三种存储位置对比。覆盖第6章 会话状态、6.1 无状态的价值、6.2 会话状态、6.3 存储会话状态的方法。左：客户端会话，数据存在浏览器，服务器无状态易扩展，但数据量受限且安全性差。中：服务器会话，数据存在应用服务器内存，读写快但服务器故障会丢失状态。右：数据库会话，数据持久化到数据库表，任意节点可恢复但每次读写都走数据库。"
        ...
        >
          ...
          <line
            x1={60}
            y1={PANEL_Y + PANEL_H + 44}
            x2={660}
            y2={PANEL_Y + PANEL_H + 44}
            stroke={T.secondary}
            strokeWidth="1.2"
            markerEnd="url(#ch06-axis)"
          />
          <text
            x={60}
