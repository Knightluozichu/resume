...
export function Poeaa24Ch11BehaviorCollaboration() {
  return (
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg
          ...
          role="img"
          aria-label="对象-关系行为模式协作图。Unit of Work 是最外层容器，跟踪所有新建、修改、删除的对象；Identity Map 在 UoW 内部作为缓存，确保同一 ID 只有一个实例；Lazy Load 挂在对象的关联属性上，首次访问时才触发查询。三者协作完成一次事务中的对象生命周期管理。"
          className="mx-auto block h-auto w-full max-w-[720px]"
        >
          <DiagramTitle x={VIEW_W / 2} y={36} text="对象-关系行为模式：三者如何协作" />
          ...
          {/* 连接线：Order → Lazy Load */}
          <defs>
            <marker id="ch11-arr" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto">
              <path d="M0 0 L6 3.5 L0 7 z" fill="#E5B567" />
            </marker>
          </defs>
          <line x1={244} y1={208} x2={504} y2={200} stroke="#E5B567" strokeWidth="1" strokeDasharray="4 3" markerEnd="url(#ch11-arr)" />

          {/* 连接线：Lazy Load → Identity Map (查缓存) */}
          <line x1={504} y1={262} x2={244} y2={280} stroke="#3FB97F" strokeWidth="1" strokeDasharray="4 3" markerEnd="url(#ch11-arr)" />

          {/* 底部说明 */}
          <text x={48} y={384} fontSize="11" fill={T.primary}>
            协作流程：UoW 管理事务边界 → Identity Map 保证唯一实例 → Lazy Load 延迟加载关联
          </text>

          <DiagramCaption x={VIEW_W / 2} y={VIEW_H - 12} text="三个模式解决三个问题：何时写（UoW）、是否重复（IM）、何时读（LL）" />
        </svg>
      </div>
      <figcaption className="mt-2 text-center text-sm text-secondary">
        Unit of Work 跟踪变更并决定何时写回；Identity Map 确保同一 ID 只有一个内存实例；
        Lazy Load 把关联加载推迟到首次访问。三者协作管理一次事务中的完整对象生命周期。
      </figcaption>
    </figure>
  );
}
