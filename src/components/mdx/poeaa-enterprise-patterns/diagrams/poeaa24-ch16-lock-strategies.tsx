/**
 * <Poeaa24Ch16LockStrategies>：离线并发锁策略对比图（POEAA 第16章概览）。
 *
...
export function Poeaa24Ch16LockStrategies() {
  const startX = 48;
  const startY = 80;
  const rowH = 84;
  const boxW = 624;
    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg
          ...
          role="img"
          aria-label="离线并发锁策略对比图。四种策略按行排列：乐观锁在提交时检查版本号，冲突时重试；悲观锁在读取时立即加锁，持有期间阻塞他人；粗粒度锁一次锁住整个聚合根；隐式锁由框架自动管理。每种策略标注时序和代价。"
          ...
          {STRATEGIES.map((s, i) => {
            ...
            return (
              <g key={s.name}>
                ...
                <line x1={startX + 520} y1={y + 44} x2={startX + 520} y2={y + 64} stroke={T.border} strokeWidth="0.8" />
              </g>
            );
          })}

          <DiagramCaption x={VIEW_W / 2} y={VIEW_H - 12} text="冲突少 → 乐观锁；冲突多 → 悲观锁；聚合边界清晰 → 粗粒度锁" />
        </svg>
      </div>
