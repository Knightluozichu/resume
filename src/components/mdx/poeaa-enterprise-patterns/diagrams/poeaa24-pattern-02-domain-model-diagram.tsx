/**
 * <Poeaa24Pattern02DomainModelDiagram>：领域模型结构图。
 * 展示对象网络 + 多态的核心结构。Server Component。
 */
import { T, DiagramTitle, DiagramCaption } from "../poeaa-svg-primitives";
const VIEW_W = 720; const VIEW_H = 380;
export function Poeaa24Pattern02DomainModelDiagram() {
  return (
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="领域模型结构图。多个领域对象（Order、Customer、Product）通过关联形成对象网络，每个对象封装自己的业务规则和行为。多态让不同子类有不同实现。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={60} y={118} fontSize="11" fontFamily="monospace" fill={T.primary}>- items: OrderItem[]</text>
          <text x={60} y={136} fontSize="11" fontFamily="monospace" fill={T.primary}>- customer: Customer</text>
          <text x={60} y={158} fontSize="11" fontFamily="monospace" fill="#3FB97F">+ calculateTotal()</text>
          {/* Customer */}
          ...
          <text x={292} y={118} fontSize="11" fontFamily="monospace" fill={T.primary}>- creditLimit: Money</text>
          <text x={292} y={136} fontSize="11" fontFamily="monospace" fill={T.primary}>- orders: Order[]</text>
          <text x={292} y={158} fontSize="11" fontFamily="monospace" fill="#3FB97F">+ checkCredit()</text>
          {/* Product */}
          ...
          <text x={524} y={118} fontSize="11" fontFamily="monospace" fill={T.primary}>- price: Money</text>
          <text x={524} y={136} fontSize="11" fontFamily="monospace" fill={T.primary}>- discount: Discount</text>
          <text x={524} y={158} fontSize="11" fontFamily="monospace" fill="#3FB97F">+ getDiscountedPrice()</text>
          {/* 关联线 */}
          <line x1={228} y1={122} x2={280} y2={122} stroke={T.secondary} strokeWidth="1" />
          <text x={254} y={114} textAnchor="middle" fontSize="11" fill={T.secondary}>1..*</text>
          ...
          <text x={134} y={268} textAnchor="middle" fontSize="11" fill={T.primary}>PercentDiscount</text>
          <text x={134} y={284} textAnchor="middle" fontSize="11" fill={T.secondary}>price * (1 - rate)</text>
          <rect x={220} y={248} width={140} height={44} rx="6" fill={T.accent} fillOpacity="0.08" stroke={T.accent} strokeWidth="1" />
          <text x={290} y={268} textAnchor="middle" fontSize="11" fill={T.primary}>AbsoluteDiscount</text>
          <text x={290} y={284} textAnchor="middle" fontSize="11" fill={T.secondary}>price - amount</text>
          <rect x={376} y={248} width={140} height={44} rx="6" fill={T.accent} fillOpacity="0.08" stroke={T.accent} strokeWidth="1" />
          <text x={446} y={268} textAnchor="middle" fontSize="11" fill={T.primary}>BulkDiscount</text>
          <text x={446} y={284} textAnchor="middle" fontSize="11" fill={T.secondary}>{'qty > N → 打折'}</text>
...
export const Poeaa24Pattern02DomainModel = Poeaa24Pattern02DomainModelDiagram;
