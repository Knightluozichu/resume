    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="标识映射结构图。展示 IdentityMap 如何保证同一 ID 只加载一次对象，后续请求直接返回缓存实例，避免重复对象和状态矛盾。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={120} y={110} textAnchor="middle" fontSize="11" fontFamily="monospace" fill="#3FB97F">load(id=42)</text>
          <text x={200} y={110} fontSize="11" fill={T.secondary}>→ 缓存未命中</text>
          {/* 第二次请求 */}
          <rect x={48} y={140} width={144} height={36} rx="6" fill="#3FB97F" fillOpacity="0.08" stroke="#3FB97F" strokeWidth="1" />
          <text x={120} y={162} textAnchor="middle" fontSize="11" fontFamily="monospace" fill="#3FB97F">load(id=42)</text>
          <text x={200} y={162} fontSize="11" fill="#E5B567">→ 缓存命中 ✓</text>
          {/* 第三次请求 */}
          <rect x={48} y={192} width={144} height={36} rx="6" fill="#3FB97F" fillOpacity="0.08" stroke="#3FB97F" strokeWidth="1" />
          <text x={120} y={214} textAnchor="middle" fontSize="11" fontFamily="monospace" fill="#3FB97F">load(id=99)</text>
          <text x={200} y={214} fontSize="11" fill={T.secondary}>→ 缓存未命中</text>
          {/* 中间：IdentityMap */}
          ...
          <text x={336} y={120} fontSize="11" fontFamily="monospace" fill={T.primary}>Map&lt;id, object&gt;</text>
          <text x={336} y={144} fontSize="11" fontFamily="monospace" fill="#E5B567">42 → Order@0x1</text>
          <text x={336} y={168} fontSize="11" fontFamily="monospace" fill="#E5B567">99 → Customer@0x2</text>
          <line x1={320} y1={180} x2={500} y2={180} stroke={T.accent} strokeWidth="0.6" strokeOpacity="0.4" />
          <text x={336} y={200} fontSize="11" fontFamily="monospace" fill="#3FB97F">get(id): object</text>
          <text x={336} y={218} fontSize="11" fontFamily="monospace" fill="#3FB97F">put(id, obj): void</text>
          <text x={336} y={236} fontSize="11" fontFamily="monospace" fill="#3FB97F">remove(id): void</text>
          {/* 右侧：保证 */}
          ...
          <text x={556} y={120} fontSize="11" fill="#3FB97F">✓ 同 ID 同实例</text>
          <text x={556} y={144} fontSize="11" fill="#3FB97F">✓ 无重复对象</text>
          <text x={556} y={168} fontSize="11" fill="#3FB97F">✓ 修改可见</text>
          <text x={556} y={196} fontSize="11" fill={T.danger}>✗ 需手动清理</text>
          <text x={556} y={220} fontSize="11" fill={T.danger}>✗ 内存占用</text>
          {/* 底部：作用域 */}
