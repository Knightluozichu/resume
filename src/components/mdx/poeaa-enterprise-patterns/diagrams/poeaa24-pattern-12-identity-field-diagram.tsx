    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="标识字段映射图。对象的 id 字段对应表的主键列，用于在内存中唯一标识对象并与数据库行建立对应关系。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={64} y={112} fontSize="11" fontFamily="monospace" fill="#E5B567">id: 42  ← 标识字段</text>
          <text x={64} y={132} fontSize="11" fontFamily="monospace" fill={T.primary}>customer: Customer</text>
          <text x={64} y={152} fontSize="11" fontFamily="monospace" fill={T.primary}>amount: 199.00</text>
          <text x={64} y={172} fontSize="11" fontFamily="monospace" fill={T.primary}>status: "pending"</text>
          {/* 箭头 */}
          <line x1={288} y1={112} x2={432} y2={112} stroke="#E5B567" strokeWidth="1.5" strokeDasharray="6 3" />
          <text x={360} y={104} textAnchor="middle" fontSize="11" fill="#E5B567">映射</text>
          {/* 表侧 */}
          ...
          <text x={448} y={112} fontSize="11" fontFamily="monospace" fill="#E5B567">id: 42  ← PK</text>
          <text x={448} y={132} fontSize="11" fontFamily="monospace" fill={T.primary}>customer_id: 7</text>
          <text x={448} y={152} fontSize="11" fontFamily="monospace" fill={T.primary}>amount: 199.00</text>
          <text x={448} y={172} fontSize="11" fontFamily="monospace" fill={T.primary}>status: 'pending'</text>
          {/* 底部说明 */}
