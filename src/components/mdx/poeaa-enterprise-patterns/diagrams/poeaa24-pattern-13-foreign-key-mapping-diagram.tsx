    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="外键映射图。对象间的引用关系（Order 引用 Customer）映射为表间的外键列（orders.customer_id → customers.id）。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={64} y={108} fontSize="10" fontFamily="monospace" fill={T.primary}>id: 42</text>
          <text x={64} y={126} fontSize="10" fontFamily="monospace" fill="#E5B567">customer: Customer</text>
          <text x={64} y={144} fontSize="10" fontFamily="monospace" fill={T.primary}>amount: 199</text>
          ...
          <text x={64} y={228} fontSize="10" fontFamily="monospace" fill={T.primary}>id: 7</text>
          <text x={64} y={246} fontSize="10" fontFamily="monospace" fill={T.primary}>name: "Alice"</text>
          {/* 对象引用箭头 */}
          <line x1={138} y1={164} x2={138} y2={184} stroke="#E5B567" strokeWidth="1.2" />
          <text x={148} y={178} fontSize="9" fill="#E5B567">引用</text>
          {/* 中间映射箭头 */}
          <line x1={260} y1={126} x2={432} y2={108} stroke="#E5B567" strokeWidth="1.5" strokeDasharray="6 3" />
          <text x={346} y={108} textAnchor="middle" fontSize="10" fill="#E5B567">对象引用 → FK 列</text>
          {/* 表侧 */}
          ...
          <text x={448} y={108} fontSize="10" fontFamily="monospace" fill={T.primary}>id: 42</text>
          <text x={448} y={126} fontSize="10" fontFamily="monospace" fill="#E5B567">customer_id: 7  ← FK</text>
          <text x={448} y={144} fontSize="10" fontFamily="monospace" fill={T.primary}>amount: 199</text>
          ...
          <text x={448} y={228} fontSize="10" fontFamily="monospace" fill={T.primary}>id: 7  ← PK</text>
          <text x={448} y={246} fontSize="10" fontFamily="monospace" fill={T.primary}>name: 'Alice'</text>
          {/* FK 关系箭头 */}
          <line x1={552} y1={164} x2={552} y2={184} stroke="#E5B567" strokeWidth="1.2" />
          <text x={562} y={178} fontSize="9" fill="#E5B567">FK→PK</text>
