    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="从属映射图。从属对象（OrderItem）没有独立身份，其生命周期完全依附于所有者（Order），由所有者的 Mapper 统一负责加载和保存。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={64} y={132} fontSize="10" fontFamily="monospace" fill="#E5B567">items: OrderItem[]</text>
          <text x={64} y={152} fontSize="10" fontFamily="monospace" fill={T.primary}>total: 597.00</text>
          {/* 从属对象 */}
          ...
          <text x={64} y={248} fontSize="10" fontFamily="monospace" fill={T.primary}>product: "Widget"</text>
          <text x={64} y={266} fontSize="10" fontFamily="monospace" fill={T.primary}>qty: 3, price: 199</text>
          {/* 拥有关系 */}
          <line x1={148} y1={184} x2={148} y2={204} stroke="#3FB97F" strokeWidth="1.5" />
          <text x={160} y={198} fontSize="9" fill="#3FB97F">拥有 ◆</text>
          {/* Mapper */}
          ...
          <text x={336} y={148} fontSize="9" fontFamily="monospace" fill="#3FB97F">find(id) → Order+Items</text>
          <text x={336} y={166} fontSize="9" fontFamily="monospace" fill="#3FB97F">insert(order+items)</text>
          <text x={336} y={184} fontSize="9" fontFamily="monospace" fill="#3FB97F">delete(order+items)</text>
          {/* 箭头 */}
          ...
          <text x={546} y={108} fontSize="10" fontFamily="monospace" fill={T.primary}>id | total</text>
          ...
          <text x={546} y={188} fontSize="10" fontFamily="monospace" fill={T.primary}>order_id: 42 (FK)</text>
          <text x={546} y={206} fontSize="10" fontFamily="monospace" fill={T.primary}>product | qty | price</text>
          {/* 箭头到表 */}
