    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="序列化 LOB 映射图。对象图（含嵌套和集合）被序列化为一个大字段（BLOB/CLOB）存入单行，读取时反序列化恢复完整对象图。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={64} y={112} fontSize="10" fontFamily="monospace" fill={T.primary}>id: 42</text>
          <text x={64} y={132} fontSize="10" fontFamily="monospace" fill={T.primary}>items: [</text>
          <text x={80} y={150} fontSize="10" fontFamily="monospace" fill={T.secondary}>Item("A", 2),</text>
          <text x={80} y={168} fontSize="10" fontFamily="monospace" fill={T.secondary}>Item("B", 1)</text>
          <text x={64} y={186} fontSize="10" fontFamily="monospace" fill={T.primary}>]</text>
          <text x={64} y={206} fontSize="10" fontFamily="monospace" fill={T.primary}>coupon: Coupon(10%)</text>
          {/* 序列化箭头 */}
          <line x1={268} y1={144} x2={380} y2={144} stroke={T.accent} strokeWidth="1.5" />
          <text x={324} y={134} textAnchor="middle" fontSize="10" fill={T.accent}>serialize()</text>
          <line x1={380} y1={174} x2={268} y2={174} stroke="#3FB97F" strokeWidth="1.5" strokeDasharray="4 2" />
          <text x={324} y={192} textAnchor="middle" fontSize="10" fill="#3FB97F">deserialize()</text>
          {/* 表侧 */}
          ...
          <text x={396} y={112} fontSize="10" fontFamily="monospace" fill={T.primary}>id: 42</text>
          ...
          <text x={408} y={142} fontSize="10" fontFamily="monospace" fill={T.accent}>data: BLOB/CLOB</text>
          <text x={408} y={162} fontSize="9" fontFamily="monospace" fill={T.secondary}>{'{"id":42,"items":[{"sku":"A",'}</text>
          <text x={408} y={178} fontSize="9" fontFamily="monospace" fill={T.secondary}>{'"qty":2},...],"coupon":{...}}'}</text>
          <text x={408} y={196} fontSize="9" fill={T.secondary}>整个对象图压缩为一个字段</text>
          {/* 底部说明 */}
