    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="单表继承映射图。整个类继承树映射到一张表，用 type 鉴别器列区分子类，子类特有字段在同一表中以可空列存在。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={290} y={102} textAnchor="middle" fontSize="11" fill="#E5B567">全部→一表</text>
          {/* 表 */}
          ...
          <text x={356} y={112} fontSize="11" fontFamily="monospace" fill={T.primary}>id | name | type</text>
          <text x={356} y={132} fontSize="11" fontFamily="monospace" fill="#E5B567">engineer_skill (nullable)</text>
          <text x={356} y={152} fontSize="11" fontFamily="monospace" fill={T.accent}>manager_budget (nullable)</text>
          <line x1={340} y1={164} x2={680} y2={164} stroke="#E5B567" strokeWidth="0.6" strokeOpacity="0.4" />
          <text x={356} y={184} fontSize="11" fontFamily="monospace" fill={T.secondary}>1 | Alice | Engineer | Go | NULL</text>
          <text x={356} y={202} fontSize="11" fontFamily="monospace" fill={T.secondary}>2 | Bob   | Manager  | NULL | 50k</text>
          <text x={356} y={220} fontSize="11" fontFamily="monospace" fill={T.secondary}>3 | Carol | Engineer | Rust | NULL</text>
          {/* 底部说明 */}
