    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="类表继承映射图。每个类（含抽象父类）对应一张表，子类表通过外键引用父类表主键，查询需要 JOIN。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={280} y={102} textAnchor="middle" fontSize="11" fill="#E5B567">每类→一表</text>
          {/* 表：employees */}
          ...
          <text x={336} y={108} fontSize="11" fontFamily="monospace" fill={T.primary}>id (PK) | name</text>
          <text x={336} y={126} fontSize="11" fontFamily="monospace" fill={T.secondary}>公共字段</text>
          {/* 表：engineers */}
          ...
          <text x={336} y={208} fontSize="11" fontFamily="monospace" fill={T.primary}>emp_id (FK) | skill</text>
          <text x={336} y={224} fontSize="11" fill={T.secondary}>FK → employees.id</text>
          {/* 表：managers */}
          ...
          <text x={336} y={298} fontSize="11" fontFamily="monospace" fill={T.primary}>emp_id (FK) | budget</text>
          <text x={336} y={314} fontSize="11" fill={T.secondary}>FK → employees.id</text>
          {/* 右侧说明 */}
          ...
          <text x={556} y={112} fontSize="11" fill="#3FB97F">✓ 无 NULL 列</text>
          <text x={556} y={134} fontSize="11" fill="#3FB97F">✓ 表结构清晰</text>
          <text x={556} y={160} fontSize="11" fill={T.danger}>✗ 查询需 JOIN</text>
          <text x={556} y={182} fontSize="11" fill={T.danger}>✗ 多态查询复杂</text>
