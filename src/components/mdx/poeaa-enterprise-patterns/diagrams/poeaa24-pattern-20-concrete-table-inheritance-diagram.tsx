    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="具体表继承映射图。只有具体子类有表，每张表包含父类所有字段（冗余），无外键无 JOIN，但多态查询需 UNION。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={280} y={122} textAnchor="middle" fontSize="9" fill="#E5B567">仅具体类</text>
          {/* 表：engineers（含父字段） */}
          ...
          <text x={336} y={108} fontSize="10" fontFamily="monospace" fill={T.secondary}>id | name</text>
          <text x={336} y={126} fontSize="9" fill={T.secondary}>↑ 父类字段冗余</text>
          <text x={336} y={148} fontSize="10" fontFamily="monospace" fill="#E5B567">skill</text>
          {/* 表：managers（含父字段） */}
          ...
          <text x={336} y={228} fontSize="10" fontFamily="monospace" fill={T.secondary}>id | name</text>
          <text x={336} y={246} fontSize="9" fill={T.secondary}>↑ 父类字段冗余</text>
          <text x={336} y={268} fontSize="10" fontFamily="monospace" fill={T.accent}>budget</text>
          {/* 右侧说明 */}
          ...
          <text x={556} y={112} fontSize="10" fill="#3FB97F">✓ 无 JOIN</text>
          <text x={556} y={134} fontSize="10" fill="#3FB97F">✓ 无 NULL 列</text>
          <text x={556} y={160} fontSize="10" fill={T.danger}>✗ 父字段冗余</text>
          <text x={556} y={182} fontSize="10" fill={T.danger}>✗ 多态需 UNION</text>
          <text x={556} y={204} fontSize="10" fill={T.danger}>✗ 父类改字段波及所有表</text>
