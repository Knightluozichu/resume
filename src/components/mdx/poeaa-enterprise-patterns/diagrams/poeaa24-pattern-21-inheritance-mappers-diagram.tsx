    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="继承映射器结构图。父类 Mapper 负责公共字段和类型分发，子类 Mapper 继承父类并处理特有字段，形成与领域继承树平行的 Mapper 继承树。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={130} y={100} textAnchor="middle" fontSize="10" fill="#3FB97F">Employee</text>
          ...
          <text x={100} y={150} textAnchor="middle" fontSize="9" fill="#E5B567">Engineer</text>
          <rect x={158} y={130} width={84} height={32} rx="6" fill={T.accent} fillOpacity="0.08" stroke={T.accent} strokeWidth="1" />
          <text x={200} y={150} textAnchor="middle" fontSize="9" fill={T.accent}>Manager</text>
          {/* 镜像箭头 */}
          <line x1={260} y1={110} x2={340} y2={110} stroke="#E5B567" strokeWidth="1.5" strokeDasharray="6 3" />
          <text x={300} y={102} textAnchor="middle" fontSize="10" fill="#E5B567">镜像</text>
          {/* Mapper 树 */}
          ...
          <text x={470} y={98} textAnchor="middle" fontSize="10" fontWeight="700" fill={T.accent}>EmployeeMapper</text>
          <text x={400} y={118} fontSize="9" fontFamily="monospace" fill="#3FB97F">find(id) → 按 type 分发</text>
          ...
          <text x={430} y={170} textAnchor="middle" fontSize="9" fontWeight="600" fill="#E5B567">EngineerMapper</text>
          <text x={380} y={188} fontSize="8" fontFamily="monospace" fill={T.secondary}>loadSkill()</text>
          <rect x={508} y={150} width={124} height={48} rx="6" fill={T.accent} fillOpacity="0.08" stroke={T.accent} strokeWidth="1" />
          <text x={570} y={170} textAnchor="middle" fontSize="9" fontWeight="600" fill={T.accent}>ManagerMapper</text>
          <text x={520} y={188} fontSize="8" fontFamily="monospace" fill={T.secondary}>loadBudget()</text>
          {/* 底部说明 */}
          ...
          <text x={64} y={274} fontSize="10" fill={T.secondary}>1. 父 Mapper.find(id) 读取 type 列 → 2. 实例化正确子类对象</text>
          <text x={64} y={292} fontSize="10" fill={T.secondary}>3. 委托子 Mapper 加载特有字段 → 4. 返回完整多态对象</text>
