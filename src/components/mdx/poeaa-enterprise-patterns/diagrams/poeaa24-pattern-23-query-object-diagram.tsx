    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="查询对象结构图。将 SQL 查询条件封装为对象，支持组合、复用和数据库无关性，由 QueryObject 翻译为目标 SQL。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={64} y={110} fontSize="9" fontFamily="monospace" fill={T.primary}>Query q = new Query()</text>
          <text x={64} y={128} fontSize="9" fontFamily="monospace" fill={T.primary}>  .where("status","paid")</text>
          <text x={64} y={146} fontSize="9" fontFamily="monospace" fill={T.primary}>{'  .and("amount", ">", 100)'}</text>
          <text x={64} y={164} fontSize="9" fontFamily="monospace" fill={T.primary}>  .orderBy("date", DESC)</text>
          {/* 箭头 */}
          <line x1={248} y1={124} x2={310} y2={124} stroke={T.accent} strokeWidth="1.5" />
          <text x={279} y={116} textAnchor="middle" fontSize="9" fill={T.accent}>构建</text>
          {/* QueryObject */}
          ...
          <text x={326} y={112} fontSize="9" fontFamily="monospace" fill={T.primary}>criteria: []</text>
          <text x={326} y={130} fontSize="9" fontFamily="monospace" fill={T.primary}>order: []</text>
          <text x={326} y={152} fontSize="9" fontFamily="monospace" fill="#3FB97F">toSQL(dialect)</text>
          <text x={326} y={170} fontSize="9" fontFamily="monospace" fill="#3FB97F">execute(session)</text>
          {/* 箭头到 SQL */}
          <line x1={480} y1={124} x2={540} y2={124} stroke="#E5B567" strokeWidth="1.5" />
          <text x={510} y={116} textAnchor="middle" fontSize="9" fill="#E5B567">翻译</text>
          {/* 生成的 SQL */}
          ...
          <text x={556} y={110} fontSize="9" fontFamily="monospace" fill={T.primary}>SELECT * FROM orders</text>
          <text x={556} y={128} fontSize="9" fontFamily="monospace" fill={T.primary}>WHERE status='paid'</text>
          <text x={556} y={146} fontSize="9" fontFamily="monospace" fill={T.primary}>  AND amount &gt; 100</text>
          <text x={556} y={164} fontSize="9" fontFamily="monospace" fill={T.primary}>ORDER BY date DESC</text>
          {/* 底部说明 */}
          ...
          <text x={64} y={262} fontSize="10" fill={T.secondary}>• 查询可组合、可复用、可序列化  • 业务代码不依赖 SQL 方言</text>
          <text x={64} y={282} fontSize="10" fill={T.secondary}>• 可针对元数据验证字段名  • 支持动态构建复杂查询</text>
