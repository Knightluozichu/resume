    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="仓储结构图。Repository 在领域层和数据源之间建立集合式接口，领域代码像操作内存集合一样存取聚合，底层由 Mapper/DataStore 实现。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={64} y={110} fontSize="11" fontFamily="monospace" fill={T.primary}>repo.findById(42)</text>
          <text x={64} y={128} fontSize="11" fontFamily="monospace" fill={T.primary}>repo.save(order)</text>
          <text x={64} y={146} fontSize="11" fontFamily="monospace" fill={T.primary}>repo.findBy(spec)</text>
          {/* 箭头 */}
          ...
          <text x={306} y={112} fontSize="11" fontFamily="monospace" fill="#3FB97F">findById(id): Order</text>
          <text x={306} y={130} fontSize="11" fontFamily="monospace" fill="#3FB97F">save(order): void</text>
          <text x={306} y={148} fontSize="11" fontFamily="monospace" fill="#3FB97F">findBy(spec): Order[]</text>
          {/* 箭头到实现 */}
          <line x1={470} y1={114} x2={530} y2={114} stroke="#E5B567" strokeWidth="1.5" strokeDasharray="5 3" />
          <text x={500} y={106} textAnchor="middle" fontSize="11" fill="#E5B567">实现</text>
          {/* 数据源 */}
          ...
          <text x={546} y={110} fontSize="11" fontFamily="monospace" fill={T.primary}>SQL / ORM</text>
          <text x={546} y={128} fontSize="11" fontFamily="monospace" fill={T.primary}>IdentityMap</text>
          <text x={546} y={146} fontSize="11" fontFamily="monospace" fill={T.primary}>UnitOfWork</text>
          {/* 底部说明 */}
          ...
          <text x={64} y={244} fontSize="11" fill={T.secondary}>• 对领域层暴露集合式接口（add/remove/find），隐藏持久化细节</text>
          <text x={64} y={264} fontSize="11" fill={T.secondary}>• 内部协调 DataMapper + IdentityMap + UnitOfWork 完成实际 I/O</text>
          <text x={64} y={284} fontSize="11" fill={T.secondary}>• 一个聚合根对应一个 Repository，粒度 = 聚合边界</text>
