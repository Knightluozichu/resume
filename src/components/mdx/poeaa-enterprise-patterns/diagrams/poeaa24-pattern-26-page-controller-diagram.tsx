    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="页面控制器请求流图。每个页面/URL 对应一个 Controller 类，处理该页面的输入、调用 Model、选择 View 渲染。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={108} y={94} textAnchor="middle" fontSize="11" fontFamily="monospace" fill={T.primary}>GET /order/42</text>
          {/* 箭头 */}
          ...
          <text x={236} y={112} fontSize="11" fontFamily="monospace" fill="#3FB97F">handleGet(id)</text>
          <text x={236} y={130} fontSize="11" fontFamily="monospace" fill="#3FB97F">handlePost(form)</text>
          <text x={236} y={148} fontSize="11" fill={T.secondary}>解析参数 → 调 Model → 选 View</text>
          {/* 箭头到 Model */}
          <line x1={310} y1={164} x2={310} y2={196} stroke="#3FB97F" strokeWidth="1.2" />
          <text x={320} y={184} fontSize="11" fill="#3FB97F">调用</text>
          {/* Model */}
          <rect x={220} y={196} width={180} height={44} rx="6" fill="#3FB97F" fillOpacity="0.06" stroke="#3FB97F" strokeWidth="1" />
          <text x={310} y={222} textAnchor="middle" fontSize="11" fill="#3FB97F">Order.find(42)</text>
          {/* 箭头到 View */}
          <line x1={400} y1={114} x2={480} y2={114} stroke="#E5B567" strokeWidth="1.2" />
          <text x={440} y={106} textAnchor="middle" fontSize="11" fill="#E5B567">选择</text>
          {/* View */}
          <rect x={480} y={84} width={180} height={60} rx="8" fill="#E5B567" fillOpacity="0.06" stroke="#E5B567" strokeWidth="1.2" />
          <text x={570} y={108} textAnchor="middle" fontSize="11" fontWeight="600" fill="#E5B567">orderDetail.jsp</text>
          <text x={570} y={128} textAnchor="middle" fontSize="11" fill={T.secondary}>渲染 HTML 响应</text>
          {/* 底部说明 */}
