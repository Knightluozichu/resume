    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="前端控制器请求流图。所有请求经过单一入口 Front Controller，由它分发到对应的 Command/Handler 处理，统一处理安全、日志等横切关注点。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={98} y={102} textAnchor="middle" fontSize="11" fontFamily="monospace" fill={T.primary}>所有请求</text>
          ...
          <text x={216} y={112} fontSize="11" fill={T.secondary}>① 认证/日志</text>
          <text x={216} y={130} fontSize="11" fill={T.secondary}>② 解析 URL</text>
          <text x={216} y={148} fontSize="11" fill="#3FB97F">③ 分发到 Command</text>
          <text x={216} y={166} fontSize="11" fill={T.secondary}>④ 选择 View</text>
          {/* 分发箭头 */}
          ...
          <text x={565} y={86} textAnchor="middle" fontSize="11" fill="#3FB97F">ViewOrderCommand</text>
          <rect x={460} y={108} width={210} height={36} rx="6" fill="#3FB97F" fillOpacity="0.06" stroke="#3FB97F" strokeWidth="1" />
          <text x={565} y={130} textAnchor="middle" fontSize="11" fill="#3FB97F">EditOrderCommand</text>
          <rect x={460} y={152} width={210} height={36} rx="6" fill="#3FB97F" fillOpacity="0.06" stroke="#3FB97F" strokeWidth="1" />
          <text x={565} y={174} textAnchor="middle" fontSize="11" fill="#3FB97F">DeleteOrderCommand</text>
          {/* 底部说明 */}
