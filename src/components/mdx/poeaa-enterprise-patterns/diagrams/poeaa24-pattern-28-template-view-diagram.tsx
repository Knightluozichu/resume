    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="模板视图结构图。HTML 模板中嵌入动态标记，渲染时由引擎将 Model 数据填入标记位置，生成最终 HTML。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={64} y={112} fontSize="11" fontFamily="monospace" fill={T.primary}>&lt;h1&gt;{"{{order.id}}"}&lt;/h1&gt;</text>
          <text x={64} y={130} fontSize="11" fontFamily="monospace" fill={T.primary}>&lt;p&gt;{"{{order.total}}"}&lt;/p&gt;</text>
          <text x={64} y={148} fontSize="11" fontFamily="monospace" fill="#3FB97F">{"{{#each items}}"}</text>
          <text x={64} y={166} fontSize="11" fontFamily="monospace" fill={T.primary}>  &lt;li&gt;{"{{name}}"}&lt;/li&gt;</text>
          <text x={64} y={184} fontSize="11" fontFamily="monospace" fill="#3FB97F">{"{{/each}}"}</text>
          {/* 箭头 */}
          <line x1={308} y1={134} x2={380} y2={134} stroke={T.accent} strokeWidth="1.5" />
          <text x={344} y={126} textAnchor="middle" fontSize="11" fill={T.accent}>渲染引擎</text>
          {/* 输出 */}
          ...
          <text x={396} y={112} fontSize="11" fontFamily="monospace" fill={T.primary}>&lt;h1&gt;42&lt;/h1&gt;</text>
          <text x={396} y={130} fontSize="11" fontFamily="monospace" fill={T.primary}>&lt;p&gt;¥597.00&lt;/p&gt;</text>
          <text x={396} y={148} fontSize="11" fontFamily="monospace" fill={T.primary}>&lt;li&gt;Widget&lt;/li&gt;</text>
          <text x={396} y={166} fontSize="11" fontFamily="monospace" fill={T.primary}>&lt;li&gt;Gadget&lt;/li&gt;</text>
          <text x={396} y={184} fontSize="11" fill={T.secondary}>标记已替换为实际数据</text>
