    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="两步视图结构图。第一步将 Model 数据渲染为内容 HTML（无布局），第二步将内容嵌入全局布局模板（导航、页脚），实现内容与布局分离。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={64} y={112} fontSize="11" fontFamily="monospace" fill={T.primary}>orderDetail.html</text>
          <text x={64} y={130} fontSize="11" fill={T.secondary}>只渲染页面主体内容</text>
          <text x={64} y={148} fontSize="11" fill={T.secondary}>无导航、无页脚</text>
          {/* 箭头 */}
          <line x1={248} y1={114} x2={310} y2={114} stroke={T.accent} strokeWidth="1.5" />
          <text x={279} y={106} textAnchor="middle" fontSize="11" fill={T.accent}>嵌入</text>
          {/* Step 2 */}
          ...
          <text x={326} y={112} fontSize="11" fontFamily="monospace" fill={T.primary}>layout.html</text>
          <text x={326} y={130} fontSize="11" fill={T.secondary}>导航 + {"{{content}}"} + 页脚</text>
          <text x={326} y={148} fontSize="11" fill={T.secondary}>全站统一外观</text>
          {/* 箭头到输出 */}
          ...
          <text x={620} y={94} textAnchor="middle" fontSize="11" fontWeight="600" fill="#E5B567">完整页面</text>
          <text x={576} y={114} fontSize="11" fill={T.secondary}>┌ 导航 ┐</text>
          <text x={576} y={130} fontSize="11" fill="#3FB97F">│ 内容 │</text>
          <text x={576} y={146} fontSize="11" fill={T.secondary}>└ 页脚 ┘</text>
          {/* 底部说明 */}
