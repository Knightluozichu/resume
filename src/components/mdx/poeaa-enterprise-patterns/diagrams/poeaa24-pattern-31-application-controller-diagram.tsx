    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="应用控制器结构图。Application Controller 管理多步流程（向导），根据当前步骤和领域状态决定下一步显示哪个 View、执行哪个 Command。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={256} y={112} fontSize="11" fontFamily="monospace" fill="#3FB97F">getCommand(event)</text>
          <text x={256} y={130} fontSize="11" fontFamily="monospace" fill="#3FB97F">getView(state)</text>
          <text x={256} y={152} fontSize="11" fill={T.secondary}>根据领域状态决定下一步</text>
          <text x={256} y={170} fontSize="11" fill={T.secondary}>管理向导/多步流程</text>
          {/* 左侧：输入事件 */}
          <rect x={48} y={80} width={140} height={36} rx="6" fill={T.primary} fillOpacity="0.05" stroke={T.border} strokeWidth="1" />
          <text x={118} y={102} textAnchor="middle" fontSize="11" fill={T.primary}>用户事件/步骤</text>
          ...
          <text x={614} y={84} textAnchor="middle" fontSize="11" fill="#E5B567">Step1: 选商品</text>
          <rect x={540} y={108} width={148} height={32} rx="6" fill="#E5B567" fillOpacity="0.06" stroke="#E5B567" strokeWidth="1" />
          <text x={614} y={128} textAnchor="middle" fontSize="11" fill="#E5B567">Step2: 填地址</text>
          <rect x={540} y={152} width={148} height={32} rx="6" fill="#E5B567" fillOpacity="0.06" stroke="#E5B567" strokeWidth="1" />
          <text x={614} y={172} textAnchor="middle" fontSize="11" fill="#E5B567">Step3: 确认支付</text>
          {/* 底部说明 */}
