    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="远程外观结构图。Remote Facade 在细粒度领域对象之上提供粗粒度接口，将多次远程调用合并为一次用例级操作，减少网络往返。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={64} y={116} fontSize="9" fill={T.secondary}>一次调用完成用例</text>
          <text x={64} y={134} fontSize="9" fill={T.secondary}>无需了解内部结构</text>
          {/* 网络边界 */}
          <line x1={240} y1={64} x2={240} y2={260} stroke={T.accent} strokeWidth="1.5" strokeDasharray="6 4" />
          <text x={240} y={278} textAnchor="middle" fontSize="9" fill={T.accent}>网络边界</text>
          {/* 单次粗粒度调用 */}
          <line x1={208} y1={112} x2={290} y2={112} stroke={T.accent} strokeWidth="2" />
          <text x={249} y={104} textAnchor="middle" fontSize="9" fontWeight="600" fill={T.accent}>1 次调用</text>
          {/* Remote Facade */}
          ...
          <text x={306} y={118} fontSize="9" fontFamily="monospace" fill="#3FB97F">getOrderDetails(id)</text>
          <text x={306} y={136} fontSize="9" fontFamily="monospace" fill="#3FB97F">placeOrder(dto)</text>
          <text x={306} y={158} fontSize="9" fill={T.secondary}>粗粒度 · 无领域逻辑</text>
          {/* 内部细粒度对象 */}
          ...
          <text x={605} y={84} textAnchor="middle" fontSize="9" fill={T.primary}>OrderService</text>
          <rect x={530} y={106} width={150} height={32} rx="6" fill={T.primary} fillOpacity="0.05" stroke={T.border} strokeWidth="1" />
          <text x={605} y={126} textAnchor="middle" fontSize="9" fill={T.primary}>InventoryService</text>
          <rect x={530} y={148} width={150} height={32} rx="6" fill={T.primary} fillOpacity="0.05" stroke={T.border} strokeWidth="1" />
          <text x={605} y={168} textAnchor="middle" fontSize="9" fill={T.primary}>PaymentService</text>
          <text x={605} y={200} textAnchor="middle" fontSize="9" fill={T.secondary}>多次本地调用（无网络开销）</text>
          {/* 底部对比 */}
