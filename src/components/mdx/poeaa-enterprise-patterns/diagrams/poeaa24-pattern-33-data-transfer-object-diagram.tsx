    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="数据传输对象结构图。DTO 是可序列化的数据载体，将多个字段打包为一次传输，避免领域对象直接跨越进程边界。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={64} y={108} fontSize="11" fontFamily="monospace" fill={T.primary}>Order</text>
          <text x={64} y={126} fontSize="11" fontFamily="monospace" fill={T.primary}>  ├ Customer</text>
          <text x={64} y={144} fontSize="11" fontFamily="monospace" fill={T.primary}>  ├ LineItem[]</text>
          <text x={64} y={162} fontSize="11" fontFamily="monospace" fill={T.primary}>  └ Payment</text>
          {/* 箭头：组装 */}
          <line x1={228} y1={100} x2={290} y2={100} stroke={T.accent} strokeWidth="1.5" />
          <text x={259} y={92} textAnchor="middle" fontSize="11" fill={T.accent}>组装</text>
          {/* 中间：DTO */}
          ...
          <text x={306} y={110} fontSize="11" fontFamily="monospace" fill={T.primary}>orderId: 42</text>
          <text x={306} y={128} fontSize="11" fontFamily="monospace" fill={T.primary}>customer: "张三"</text>
          <text x={306} y={146} fontSize="11" fontFamily="monospace" fill={T.primary}>total: 597.00</text>
          <text x={306} y={164} fontSize="11" fontFamily="monospace" fill={T.primary}>items: [...]</text>
          {/* 箭头：传输 */}
          <line x1={450} y1={100} x2={510} y2={100} stroke="#E5B567" strokeWidth="1.5" />
          <text x={480} y={92} textAnchor="middle" fontSize="11" fill="#E5B567">序列化</text>
          {/* 右侧：客户端 */}
          ...
          <text x={526} y={110} fontSize="11" fill={T.secondary}>只拿到纯数据</text>
          <text x={526} y={128} fontSize="11" fill={T.secondary}>无行为、无关联</text>
          <text x={526} y={146} fontSize="11" fill={T.secondary}>一次传输全部字段</text>
          <text x={526} y={164} fontSize="11" fill={T.secondary}>可缓存、可版本化</text>
          {/* 底部说明 */}
