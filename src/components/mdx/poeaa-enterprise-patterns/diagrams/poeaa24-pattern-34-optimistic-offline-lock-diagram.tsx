    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="乐观离线锁时序图。读取时记录版本号，提交时比较版本：版本一致则提交成功并递增版本，版本不一致则拒绝提交者并提供重试路径。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={370} y={90} textAnchor="middle" fontSize="11" fill="#3FB97F">读取记录（version = 1）</text>
          {/* Step 2: B 读取 version=1 */}
          <line x1={400} y1={120} x2={600} y2={120} stroke="#E5B567" strokeWidth="1.2" />
          <text x={500} y={114} textAnchor="middle" fontSize="11" fill="#E5B567">读取（version = 1）</text>
          {/* Step 3: B 先提交成功 */}
          <line x1={400} y1={150} x2={600} y2={150} stroke="#E5B567" strokeWidth="1.5" />
          <text x={500} y={144} textAnchor="middle" fontSize="11" fill="#E5B567">提交：version 1→2 ✓</text>
          {/* Step 4: A 提交被拒绝 */}
          <line x1={140} y1={180} x2={600} y2={180} stroke="#E5634D" strokeWidth="1.5" />
          <text x={370} y={174} textAnchor="middle" fontSize="11" fontWeight="600" fill="#E5634D">提交：期望 version=1，实际=2 ✗ 拒绝</text>
          {/* Step 5: A 重试 */}
          <line x1={140} y1={210} x2={600} y2={210} stroke="#3FB97F" strokeWidth="1.2" strokeDasharray="4 3" />
          <text x={370} y={204} textAnchor="middle" fontSize="11" fill="#3FB97F">重新读取（version = 2）→ 合并修改 → 重试提交</text>
          {/* 底部说明 */}
