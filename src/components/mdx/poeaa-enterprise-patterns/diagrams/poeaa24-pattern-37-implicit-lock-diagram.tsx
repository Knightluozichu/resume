    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="隐含锁结构图。锁的获取隐藏在框架层（如 Unit of Work 或 Data Mapper），业务代码无需显式加锁，所有修改入口自动获得锁保护。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={64} y={108} fontSize="11" fontFamily="monospace" fill={T.primary}>order.setTotal(597)</text>
          <text x={64} y={126} fontSize="11" fontFamily="monospace" fill={T.primary}>uow.commit()</text>
          <text x={64} y={144} fontSize="11" fill={T.secondary}>无显式 lock() 调用</text>
          {/* 箭头 */}
          <line x1={328} y1={109} x2={390} y2={109} stroke={T.accent} strokeWidth="1.5" />
          <text x={359} y={100} textAnchor="middle" fontSize="11" fill={T.accent}>委托</text>
          {/* 框架层 */}
          ...
          <text x={406} y={110} fontSize="11" fontFamily="monospace" fill={T.primary}>commit() → 自动获取锁</text>
          <text x={406} y={128} fontSize="11" fontFamily="monospace" fill={T.primary}>写入前 → 检查版本</text>
          <text x={406} y={146} fontSize="11" fill={T.secondary}>所有修改入口隐式加锁</text>
          {/* 底部说明 */}
