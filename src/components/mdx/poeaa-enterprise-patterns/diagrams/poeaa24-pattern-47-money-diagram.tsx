    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="Money 值对象结构图。Money 封装金额和币种，运算时强制检查币种一致性，避免浮点精度丢失和跨币种混算。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={266} y={110} fontSize="11" fontFamily="monospace" fill={T.primary}>amount: BigDecimal</text>
          <text x={266} y={128} fontSize="11" fontFamily="monospace" fill={T.primary}>currency: CNY</text>
          <text x={266} y={146} fontSize="11" fontFamily="monospace" fill="#3FB97F">add(m) / multiply(n)</text>
          {/* 左侧：正确用法 */}
          <rect x={48} y={64} width={170} height={100} rx="8" fill="#3FB97F" fillOpacity="0.06" stroke="#3FB97F" strokeWidth="1.2" />
          <text x={133} y={86} textAnchor="middle" fontSize="11" fontWeight="600" fill="#3FB97F">✓ 受保护的运算</text>
          <text x={64} y={110} fontSize="11" fontFamily="monospace" fill={T.primary}>¥100 + ¥50 = ¥150</text>
          <text x={64} y={130} fontSize="11" fill={T.secondary}>币种一致才允许相加</text>
          <text x={64} y={148} fontSize="11" fill={T.secondary}>精度由 BigDecimal 保证</text>
          {/* 右侧：错误用法 */}
          <rect x={500} y={64} width={180} height={100} rx="8" fill="#E5634D" fillOpacity="0.06" stroke="#E5634D" strokeWidth="1.2" />
          <text x={590} y={86} textAnchor="middle" fontSize="11" fontWeight="600" fill="#E5634D">✗ 裸数字的问题</text>
          <text x={516} y={110} fontSize="11" fontFamily="monospace" fill={T.primary}>100 + 50 = ?（美元?）</text>
          <text x={516} y={130} fontSize="11" fill={T.secondary}>0.1 + 0.2 ≠ 0.3</text>
          <text x={516} y={148} fontSize="11" fill={T.secondary}>币种混算无警告</text>
          {/* 底部说明 */}
