...
export function Poeaa24Pattern48SpecialCase() {
  return (
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="Special Case 结构图。用子类封装特殊情况的行为（如 NullCustomer），调用者无需到处写 null 检查，多态分派自动处理边界。" className="mx-auto block h-auto w-full max-w-[720px]">
          <DiagramTitle x={VIEW_W / 2} y={36} text="Special Case：用子类封装特殊情况" />
          {/* 基类 */}
          ...
          <text x={64} y={266} fontSize="11" fill={T.secondary}>• 多态分派自动处理边界，特殊情况的行为集中在一处</text>
          <DiagramCaption x={VIEW_W / 2} y={VIEW_H - 8} text="特例子类封装边界行为，调用者无需到处写 null 检查" />
        </svg>
      </div>
