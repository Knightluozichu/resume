/**
 * <Poeaa24Pattern49Plugin>：插件结构图。Server Component。
 */
import { T, DiagramTitle, DiagramCaption } from "../poeaa-svg-primitives";
const VIEW_W = 720; const VIEW_H = 300;
export function Poeaa24Pattern49Plugin() {
  return (
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="Plugin 结构图。通过配置文件或注册机制在运行时选择实现，核心代码只依赖接口，新增实现无需修改核心。" className="mx-auto block h-auto w-full max-w-[720px]">
          <DiagramTitle x={VIEW_W / 2} y={36} text="Plugin：运行时装配，核心不感知具体实现" />
          {/* 核心代码 */}
          ...
          <text x={64} y={246} fontSize="11" fill={T.secondary}>• 运行时通过配置选择实现  • 新增插件不改核心代码  • 开闭原则的落地手段</text>
          <DiagramCaption x={VIEW_W / 2} y={VIEW_H - 12} text="核心只依赖接口，具体实现通过配置在运行时装配" />
        </svg>
      </div>
