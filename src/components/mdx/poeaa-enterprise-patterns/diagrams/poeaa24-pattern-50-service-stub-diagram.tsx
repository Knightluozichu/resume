    ...
    <figure className="mdx-figure mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="Service Stub 结构图。用轻量替身代替外部服务（如支付网关），测试时返回固定结果，消除对外部系统的依赖。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <text x={64} y={110} fontSize="9" fontFamily="monospace" fill={T.primary}>payment.charge(¥597)</text>
          <text x={64} y={128} fontSize="9" fill={T.secondary}>不关心谁在响应</text>
          {/* 接口 */}
          ...
          <text x={600} y={76} textAnchor="middle" fontSize="9" fill="#E5B567">StubPayment（测试）</text>
          <rect x={520} y={120} width={160} height={32} rx="6" fill={T.primary} fillOpacity="0.05" stroke={T.border} strokeWidth="1" />
          <text x={600} y={140} textAnchor="middle" fontSize="9" fill={T.primary}>StripePayment（生产）</text>
          <text x={600} y={170} textAnchor="middle" fontSize="9" fill="#E5B567">固定返回 success</text>
          {/* 底部说明 */}
