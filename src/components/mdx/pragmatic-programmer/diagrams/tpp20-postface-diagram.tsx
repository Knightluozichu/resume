...
const STAGES = [
  { label: "影响", detail: "谁会受影响" },
  { label: "后果", detail: "损失与可逆" },
  { label: "缓解", detail: "降低暴露" },
  { label: "边界", detail: "何时停止" },
  { label: "责任", detail: "决定与复查" },
] as const;
...
export function Tpp20PostfaceLoopDiagram() {
  const cardWidth = 172;
  const gap = 22;
  const startX = 22;
  const cardY = 126;
  ...
  return (
    <figure
      ...
      data-visual-kind="postface-loop"
    >
      <svg
        viewBox="0 0 960 390"
        role="img"
        aria-label="从影响识别、失败后果、缓解措施、不可接受条件到责任机制的回路"
      ...
      >
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          无法降低不可逆伤害时，暂停或拒绝是责任机制的一部分
        </text>
      </svg>
      <figcaption className="mt-2 text-center text-xs text-secondary">
        专属图示：不伤害原则从受影响者出发，以可执行边界和责任复查收口。
      </figcaption>
    </figure>
  );
}
...
const EVIDENCE_ROWS = [
  ...
  { label: "后果", normal: "可逆可测", boundary: "不可逆", fault: "暴露扩大" },
  { label: "决定", normal: "继续有据", boundary: "先升级", fault: "无人负责" },
  {
    label: "恢复",
    normal: "团队接管",
    boundary: "拒绝扩大",
...
export function Tpp20PostfaceEvidenceDiagram() {
  const columns = [
  ...
  return (
    <figure
      className="my-6 overflow-hidden rounded-card border border-border bg-elevated p-4"
      data-visual-kind="postface-evidence"
    >
      <svg
        viewBox="0 0 960 390"
        role="img"
        aria-label="不伤害责任正常、边界和单一故障样本的证据矩阵"
        className="mx-auto block h-auto w-full max-w-[960px]"
      >
        <text
          x="480"
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          保存受影响者、后果、缓解、停止决定和责任人，原则才能执行
        </text>
      </svg>
      <figcaption className="mt-2 text-center text-xs text-secondary">
        专属图示：跋的责任证据覆盖受影响者、伤害、边界和复查。
      </figcaption>
    </figure>
  );
}
...
const LAB_SCENARIOS = [
  {
    ...
    tone: COLORS.success,
    evidence:
      "受影响者明确，后果可逆，缓解措施已验证，责任人和回退都可以被团队接管。",
    firstChange: "责任节点：决定有证据支撑，可以继续并安排复查。",
    recovery: "保留影响评估和复查时间，用边界样本确认缓解仍然有效。",
  },
  {
    label: "先升级",
    tone: COLORS.warning,
    ...
    firstChange: "边界节点：证据不足，必须暂停并转给有权限的复核者。",
    recovery: "缩小范围，补受影响者、后果和缓解证据；复核前不扩大暴露。",
  },
  {
    label: "必须拒绝",
    tone: COLORS.danger,
    evidence: "请求要求绕过同意或制造不可逆伤害，且没有能消除风险的替代方案。",
    firstChange: "责任节点：继续执行会直接放大伤害，拒绝和记录成为必要动作。",
    recovery: "保存拒绝理由和升级渠道，提出合法安全的替代目标并保护受影响者。",
  },
] as const;

export function Tpp20PostfaceLab() {
  ...
  return (
    <section
      ...
      data-visual-kind="postface-lab"
      aria-label="不伤害原则实验"
      aria-labelledby="postface-lab-title"
    >
