...
const STAGES = [
  { label: "观察", detail: "记录成功条件" },
  { label: "列假设", detail: "找出隐藏前提" },
  { label: "验证因果", detail: "只改变一项" },
  { label: "显式设计", detail: "写入边界合同" },
  { label: "回归", detail: "保存最小反例" },
] as const;
...
export function Tpp20Topic38ProgrammingByCoincidenceLoopDiagram() {
  const cardWidth = 172;
  const gap = 22;
  const startX = 22;
  const cardY = 126;
  ...
  return (
    <figure
      ...
      data-visual-kind="coincidence-loop"
    >
      <svg
        viewBox="0 0 960 390"
        role="img"
        aria-label="巧合式编程从观察、列假设、验证因果、显式设计到回归的因果回路"
      ...
      >
        ...
        {STAGES.map((stage, index) => {
          ...
          return (
            <g key={stage.label}>
              ...
              >
                {active ? "当前验证入口" : index < 2 ? "已留证据" : "等待证据"}
              </text>
            </g>
          );
        })}
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          最终结果不同还不够：先找到第一个改变的观察点，再决定如何修复
        </text>
      </svg>
...
const EVIDENCE_ROWS = [
  ...
  { label: "首差", normal: "无", boundary: "边界节点", fault: "因果验证" },
  { label: "恢复", normal: "保存版本", boundary: "补约束", fault: "重放反例" },
] as const;

export function Tpp20Topic38ProgrammingByCoincidenceEvidenceDiagram() {
  const columns = [
  ...
  return (
    <figure
      className="my-6 overflow-hidden rounded-card border border-border bg-elevated p-4"
      data-visual-kind="coincidence-evidence"
    >
      <svg
        viewBox="0 0 960 390"
        role="img"
        aria-label="巧合式编程正常、边界和故障样本的证据矩阵"
        className="mx-auto block h-auto w-full max-w-[960px]"
      >
        <text
          x="480"
...
const LAB_SCENARIOS = [
  {
    label: "契约完整",
    tone: COLORS.success,
    evidence: "查询显式声明排序键，输入格式与时区已验证。",
    firstChange: "无首个差异：反复运行可重建同一结果。",
    recovery: "保存规则、依赖版本和输出摘要。",
  },
  {
    label: "边界反例",
    tone: COLORS.warning,
    evidence: "重复键与空集合进入接口，默认值不再替调用者决定。",
    firstChange: "边界节点：接口返回明确拒绝或空结果。",
    recovery: "把输入规则和拒绝原因写入契约测试。",
  },
  {
    label: "顺序巧合",
    tone: COLORS.danger,
    evidence: "数据源返回顺序改变，调用者仍把第一条记录当成当前值。",
    firstChange: "因果验证：观察结果先于显式设计发生变化。",
    recovery: "保存最小反例，显式排序后重放回归。",
  },
] as const;

export function Tpp20Topic38ProgrammingByCoincidenceLab() {
  ...
  return (
    <section
      ...
      data-visual-kind="coincidence-lab"
      aria-label="巧合式编程实验"
      aria-labelledby="coincidence-lab-title"
    >
