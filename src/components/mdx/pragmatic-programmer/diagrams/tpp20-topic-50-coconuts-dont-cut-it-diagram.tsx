...
const STAGES = [
  { label: "问题", detail: "本地困难" },
  { label: "语境", detail: "真实约束" },
  { label: "机制", detail: "因果假设" },
  { label: "试点", detail: "小范围验证" },
  { label: "决定", detail: "留改或弃" },
] as const;
...
export function Tpp20Topic50CoconutsDontCutItLoopDiagram() {
  const cardWidth = 172;
  const gap = 22;
  const startX = 22;
  const cardY = 126;
  ...
  return (
    <figure
      ...
      data-visual-kind="coconuts-dont-cut-it-loop"
    >
      <svg
        viewBox="0 0 960 390"
        role="img"
        aria-label="从本地问题、组织语境、机制、试点到保留淘汰的做法选择回路"
...
export function Tpp20Topic50CoconutsDontCutItEvidenceDiagram() {
  const columns = [
  ...
  return (
    <figure
      className="my-6 overflow-hidden rounded-card border border-border bg-elevated p-4"
      data-visual-kind="coconuts-dont-cut-it-evidence"
    >
      <svg
        viewBox="0 0 960 390"
        role="img"
        aria-label="外部做法试点正常、边界和单一故障样本的证据矩阵"
        className="mx-auto block h-auto w-full max-w-[960px]"
      >
        <text
          x="480"
        ...
        <text
          ...
          fill={COLORS.primary}
        >
          保存基线、用户反馈、成本、边界和回退，才能知道椰子是否真的有用
        </text>
      </svg>
      <figcaption className="mt-2 text-center text-xs text-secondary">
...
const LAB_SCENARIOS = [
  {
    ...
    tone: COLORS.success,
    evidence:
      "新流程缩短了用户等待，测试和运维成本没有越过阈值，失败时仍可回到原路径。",
    firstChange:
      "试点节点：代表性输入验证了机制，用户结果和成本信号都支持继续观察。",
    recovery: "保留小范围采用，扩大前重跑边界样本并记录新的适用条件。",
  },
  {
    label: "照搬失败",
    tone: COLORS.warning,
    ...
    firstChange: "机制节点：可见动作被复制，却没有连接本地瓶颈和用户结果。",
    recovery: "回到基线，重新定义问题和机制，只保留能减少验证等待的最小做法。",
  },
  {
    label: "边界越过",
    tone: COLORS.danger,
    evidence:
      "试点内部指标变好，却增加了运维成本并让关键用户任务在高峰期失败。",
    firstChange: "决定节点：成本和用户边界不满足，试点必须暂停或回退。",
    recovery: "保存高峰输入和首差，恢复基线，修改机制假设后再设计下一次试点。",
  },
] as const;

export function Tpp20Topic50CoconutsDontCutItLab() {
  ...
  return (
    <section
      ...
      data-visual-kind="coconuts-dont-cut-it-lab"
      aria-label="外部做法试点实验"
      aria-labelledby="coconuts-dont-cut-it-lab-title"
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h3
            id="coconuts-dont-cut-it-lab-title"
