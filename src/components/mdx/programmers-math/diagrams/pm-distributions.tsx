/**
 * <PmDistributionsDiagram>：概率分布概念图（pm-distributions 章）。Server Component。
 */

const VIEW_W = 720;
const VIEW_H = 400;

export function PmDistributionsDiagram() {
  return (
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="三大概率分布对比" className="mx-auto block h-auto w-full max-w-[720px]">
          <text x={VIEW_W / 2} y="36" textAnchor="middle" fontSize="16" fontWeight="700" fill="var(--text-primary)">三大概率分布对比</text>
          <text x={VIEW_W / 2} y="58" textAnchor="middle" fontSize="12" fill="var(--text-secondary)">二项（离散计数）→ 正态（连续测量）→ 泊松（稀有事件）</text>
          <g>
          ...
          <g>
            ...
            <text x="408" y="125" fontSize="13" fontWeight="700" fill="var(--text-primary)">正态分布</text>
            <text x="396" y="148" fontSize="11" fill="var(--text-secondary)">钟形对称曲线</text>
            <text x="396" y="166" fontSize="12" fontWeight="600" fill="var(--success)">μ,σ; 68-95-99.7</text>
          </g>
          <g>
          ...
          <g>
            ...
            <text x="408" y="235" fontSize="13" fontWeight="700" fill="var(--text-primary)">中心极限定理</text>
            <text x="396" y="258" fontSize="11" fill="var(--text-secondary)">和→正态</text>
            <text x="396" y="276" fontSize="12" fontWeight="600" fill="var(--danger)">正态分布之王</text>
          </g>
          ...
          <text x={VIEW_W / 2} y="365" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">二项（离散计数）→ 正态（连续测量）→ 泊松（稀有事件）</text>
        </svg>
      </div>
      <figcaption className="mt-2 text-center text-sm text-secondary">二项（离散计数）→ 正态（连续测量）→ 泊松（稀有事件）</figcaption>
    </figure>
  );
}
