  ...
  return (
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        ...
        >
          ...
          <rect
            ...
            strokeWidth="1"
          />
          <text x={372} y={244} fontSize="11" fill={primary}>
            {":hello.object_id == :hello.object_id  # true"}
          </text>
          <text x={372} y={262} fontSize="11" fill={primary}>
            {'"hello".object_id == "hello".object_id  # false'}
          </text>

          {/* 底部说明 */}
          <text x={360} y={300} fontSize="11" fill={secondary}>
            Symbol 是内部以整数存储的不可变名字
          </text>
          <text x={360} y={318} fontSize="11" fill={secondary}>
            做 Hash 键时 :name 比 &quot;name&quot; 更高效
          </text>
          <text x={360} y={336} fontSize="11" fill={secondary}>
            冻结字符串（freeze）可模拟 Symbol 行为
          </text>
      ...
      <figcaption className="mt-2 text-center text-sm text-secondary">
        Ruby 字符串可变且支持插值，Symbol 不可变且同名唯一——做哈希键优先用
        Symbol。
      </figcaption>
    </figure>
  );
