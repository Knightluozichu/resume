...
const model = {
  ...
  stages: [
    {
      ...
      evidence:
        "保存Ruby基础教程（第5版）· 学习地图的初值、参数、编码或资源位置。",
    },
    {
      label: "执行第2章 便利的对象",
      ...
      evidence:
        "定位“按几个宽泛主题压缩课程，跳过方法、异常、编码和工具整合的中间合同”出现时的第一处不同状态。",
    },
    {
      label: "验收第23章 检索邮政编码",
      input: "恢复基线，再以第23章 检索邮政编码覆盖正常、错误和重复执行。",
      state:
        "最终状态必须重新满足：每个官方章节都有唯一页面、前置依赖、故障回退点和掌握证据。",
      evidence:
        "交付Ruby基础教程（第5版）· 学习地图的输入样本、接收者与方法、关键状态前后值、正常与失败输出、异常或退出状态，以及复位后的再次运行记录。",
    },
  ],
  normalTrace: [
    "固定第1章 Ruby初探的输入和接收者",
  ...
  failureTrace: [
    "保持Ruby基础教程（第5版）· 学习地图的输入与初值不变",
    "仅注入故障：按几个宽泛主题压缩课程，跳过方法、异常、编码和工具整合的中间合同",
    "记录首个对象、控制或边界分岔",
    "拒绝把最终现象误当成根因",
  ],
  ...
  artifact:
    "Ruby基础教程（第5版）· 学习地图的输入样本、接收者与方法、关键状态前后值、正常与失败输出、异常或退出状态，以及复位后的再次运行记录。",
} satisfies RubyEvidenceModel;

export function RubRubLearningMapObjectModelLab() {
  return <RubyEvidenceLab model={model} view="object-model" />;
}

export function RubRubLearningMapControlTraceLab() {
  return <RubyEvidenceLab model={model} view="control-trace" />;
}

export function RubRubLearningMapBoundaryProbeLab() {
