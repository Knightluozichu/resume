"use client";

import { useState } from "react";

export interface RustWayCase {
  label: string;
  input: string;
  mechanism: string;
  result: string;
  invariant: string;
}

const tones = {
  cyan: "border-cyan-500 bg-cyan-500/15",
  emerald: "border-emerald-500 bg-emerald-500/15",
  amber: "border-amber-500 bg-amber-500/15",
  rose: "border-rose-500 bg-rose-500/15",
  violet: "border-violet-500 bg-violet-500/15",
} as const;

export function RustWayOfficialLab({
  title,
  caption,
  cases,
  tone = "cyan",
}: {
  title: string;
  caption: string;
  cases: ReadonlyArray<RustWayCase>;
  tone?: keyof typeof tones;
}) {
  const [selected, setSelected] = useState(0);
  function resetExperiment() {
    setSelected(0);
  }

  const item = cases[selected];

  return (
    <figure className="mdx-figure not-prose mx-auto my-6">
      <span style={{position:'absolute',top:6,right:6,zIndex:20}}><button type="button" onClick={resetExperiment} title="重置实验" aria-label="重置实验" className="inline-flex size-11 shrink-0 items-center justify-center rounded border border-zinc-300 bg-white text-lg hover:bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-950 dark:hover:bg-zinc-800"><span aria-hidden="true">↺</span></button></span>
      <div className="border border-border bg-elevated p-4 sm:p-5">
        <div className="flex items-center justify-between gap-3 border-b border-border pb-3">
          <strong className="text-sm text-primary">{title}</strong>
          <span className="text-xs text-secondary">{selected + 1} / {cases.length}</span>
        </div>
        <div className={`mt-3 grid gap-2 ${cases.length > 3 ? "grid-cols-2 sm:grid-cols-4" : "grid-cols-3"}`}>
          {cases.map((entry, index) => (
            <button
              key={entry.label}
              type="button"
              onClick={() => setSelected(index)}
              aria-pressed={selected === index}
              className={`min-h-11 border px-2 text-xs text-primary ${selected === index ? tones[tone] : "border-border bg-bg"}`}
            >
              {entry.label}
            </button>
          ))}
        </div>
        <div className="mt-4 grid items-stretch gap-2 md:grid-cols-[1fr_auto_1fr_auto_1fr]">
          <FlowCell label="输入 / 状态" value={item.input} />
          <span className="self-center text-center text-secondary" aria-hidden="true">→</span>
          <FlowCell label="规则 / 机制" value={item.mechanism} />
          <span className="self-center text-center text-secondary" aria-hidden="true">→</span>
          <FlowCell label="输出 / 证据" value={item.result} />
        </div>
        <div className="mt-3 border border-border bg-bg p-3 text-sm leading-6 text-primary">
          <span className="mr-2 text-xs text-secondary">必须保持</span>
          {item.invariant}
        </div>
      </div>
      <figcaption className="mt-2 text-center text-sm text-secondary">{caption}</figcaption>
    </figure>
  );
}

function FlowCell({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-h-24 border border-border bg-bg p-3">
      <span className="text-xs text-secondary">{label}</span>
      <strong className="mt-2 block text-sm leading-6 text-primary">{value}</strong>
    </div>
  );
}
