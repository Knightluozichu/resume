...
export interface ShaderXSnapshot {
  ...
  return <figure className="mdx-figure not-prose mx-auto my-6">
    <div className="overflow-hidden rounded-card border border-border bg-elevated">
      <header className="flex min-h-16 flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3">
        ...
        <span className="border border-emerald-500/35 bg-emerald-500/10 px-2 py-1 text-xs font-semibold text-emerald-700 dark:text-emerald-300">329 article evidence</span>
      </header>
    ...
    <figcaption className="mt-2 text-center text-sm text-secondary">点击阶段，对照原篇身份、不变量、现代实现、捕获与失败重放。</figcaption>
  </figure>;
}
