...
const SNAPSHOTS=[
  {
    "label": "身份",
    "stage": "原篇身份与问题",
    "action": "固定8册中的卷号、篇名和31篇唯一归属",
    "evidence": "authorized PDF / official TOC + article id",
    "warning": "只在一张宣传图上匹配颜色，没有扫描粗糙度、视角、光源大小和曝光。"
  },
  {
    "label": "模型",
    "stage": "反射模型",
  ...
  },
  {
    ...
    "warning": "混用切线空间与世界空间法线，靠归一化掩盖坐标错误。"
  }
] as const satisfies ReadonlyArray<ShaderXSnapshot>;

export function SxxMaterialSurfaceMapLab(){return <ShaderXSeriesLab title="材质、BRDF 与风格化表面：31篇复现" mode="map" snapshots={SNAPSHOTS} initial={0}/>;}
export function SxxMaterialSurfaceExperimentLab(){return <ShaderXSeriesLab title="材质、BRDF 与风格化表面：31篇复现" mode="experiment" snapshots={SNAPSHOTS} initial={2}/>;}
export function SxxMaterialSurfaceEvidenceLab(){return <ShaderXSeriesLab title="材质、BRDF 与风格化表面：31篇复现" mode="evidence" snapshots={SNAPSHOTS} initial={4}/>;}
