...
type LabMode = "topology" | "protocol" | "evidence";
...
export function OfficialAes23BookLab({
  unitId,
  title,
  nodes,
  focuses,
  ...
  const [syncMargin, setSyncMargin] = useState(36);
  ...
  const result = useMemo(() => {
    ...
    const utilization = Math.min(
      100,
      Math.round(load + stage * 4 + penalty * 0.45),
    );
    const latency = Math.round(
      4 + utilization * 0.19 + Math.max(0, 28 - syncMargin) * 0.24,
    );
    const coverage = Math.max(0, 98 - penalty - stage * 2);
    const accepted =
      scenario !== "fault" &&
      utilization <= 72 &&
      latency <= 24 &&
      coverage >= 68;
    return { accepted, coverage, latency, trace: run + 1, utilization };
  }, [load, run, scenario, stage, syncMargin]);
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            <label className="text-sm">
              ...
              <input
                ...
                max="80"
                value={syncMargin}
                onChange={(event) => setSyncMargin(Number(event.target.value))}
              />
            </label>
          </div>
        </div>
        ...
        <div className="p-4">
          ...
          <dl className="mt-4 grid grid-cols-3 gap-2 text-sm">
            <div className="border border-zinc-200 p-2 dark:border-zinc-800">
              <dt className="text-zinc-500">利用率</dt>
              <dd className="mt-1 text-lg font-semibold">
                {result.utilization}%
              </dd>
            </div>
            <div className="border border-zinc-200 p-2 dark:border-zinc-800">
              <dt className="text-zinc-500">时延</dt>
              <dd className="mt-1 text-lg font-semibold">
                {result.latency} ms
              </dd>
            </div>
