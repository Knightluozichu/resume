    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        ...
        >
          ...
          <text x="195" y="92" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">日志、事务、安全代码散布在每个方法</text>
          <text x="195" y="108" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">修改日志 = 改每个业务类</text>
          ...
          <text x="545" y="92" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">日志/事务/安全抽成切面，声明式织入</text>
          <text x="545" y="108" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">业务类只写业务逻辑</text>
          ...
          <text x="195" y="210" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">@Aspect 标注的类</text>
          <text x="195" y="228" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">封装横切关注点（如日志切面）</text>
          <text x="195" y="250" textAnchor="middle" fontSize="11" fontWeight="600" fill="var(--warning)">切点 Pointcut</text>
          <text x="195" y="266" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">@Pointcut 定义「在哪里」</text>
          <text x="195" y="282" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">execution(* x.Service.*(..))</text>
          <text x="195" y="304" textAnchor="middle" fontSize="11" fontWeight="600" fill="var(--danger)">通知 Advice</text>
          <text x="195" y="318" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">@Before/@After/@Around 定义「做什么」</text>
          ...
          <text x="545" y="210" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">把切面应用到目标对象</text>
          <text x="545" y="228" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">Spring 用运行时代理（JDK/CGLIB）</text>
          <text x="545" y="250" textAnchor="middle" fontSize="11" fontWeight="600" fill="var(--warning)">目标对象 Target</text>
          <text x="545" y="266" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">被代理的业务Bean</text>
          <text x="545" y="288" textAnchor="middle" fontSize="11" fontWeight="600" fill="var(--danger)">连接点 JoinPoint</text>
          <text x="545" y="304" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">方法执行的那一个时刻</text>
          <text x="545" y="318" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">Spring仅支持方法级连接点</text>
          ...
          <text x="90" y="406" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">方法前执行</text>
          ...
          <text x="235" y="406" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">环绕（前后都执行）</text>
          ...
          <text x="380" y="406" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">业务逻辑执行</text>
          ...
          <text x="525" y="406" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">正常返回后</text>
          ...
          <text x="660" y="406" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">抛异常后</text>
          ...
          <text x="370" y="480" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">无论正常返回还是抛异常都执行——类似 finally，常用于资源清理</text>
          <text x="370" y="492" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">@Around 最强大：可决定是否执行目标、修改参数、修改返回值、吞掉异常</text>
        </svg>
      </div>
