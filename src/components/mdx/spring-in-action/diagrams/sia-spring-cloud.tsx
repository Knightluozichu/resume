    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        ...
        >
          ...
          <text x="370" y="146" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">路由 / 鉴权 / 限流 / 日志</text>
          ...
          <text x="110" y="220" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">Eureka / Nacos</text>
          <text x="110" y="236" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">服务注册与发现</text>
          <text x="110" y="252" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">@EnableEurekaServer</text>
          ...
          <text x="292" y="232" textAnchor="middle" fontSize="10" fontWeight="600" fill="var(--success)">订单服务</text>
          <text x="292" y="248" textAnchor="middle" fontSize="8" fill="var(--text-secondary)">order-service</text>

          <rect x="380" y="212" width="135" height="46" rx="6" fill="var(--success)" fillOpacity="0.10" stroke="var(--success)" strokeWidth="1" />
          <text x="447" y="232" textAnchor="middle" fontSize="10" fontWeight="600" fill="var(--success)">用户服务</text>
          <text x="447" y="248" textAnchor="middle" fontSize="8" fill="var(--text-secondary)">user-service</text>

          <rect x="225" y="270" width="135" height="46" rx="6" fill="var(--success)" fillOpacity="0.10" stroke="var(--success)" strokeWidth="1" />
          <text x="292" y="290" textAnchor="middle" fontSize="10" fontWeight="600" fill="var(--success)">商品服务</text>
          <text x="292" y="306" textAnchor="middle" fontSize="8" fill="var(--text-secondary)">product-service</text>

          <rect x="380" y="270" width="135" height="46" rx="6" fill="var(--success)" fillOpacity="0.10" stroke="var(--success)" strokeWidth="1" />
          <text x="447" y="290" textAnchor="middle" fontSize="10" fontWeight="600" fill="var(--success)">支付服务</text>
          <text x="447" y="306" textAnchor="middle" fontSize="8" fill="var(--text-secondary)">payment-service</text>

          <text x="370" y="340" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">服务间调用：OpenFeign（声明式HTTP）</text>
          <text x="370" y="356" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">负载均衡：Spring Cloud LoadBalancer</text>
          <text x="370" y="372" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">熔断降级：Resilience4j / Circuit Breaker</text>
          ...
          <text x="630" y="220" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">Spring Cloud Config</text>
          <text x="630" y="236" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">/ Nacos Config</text>
          <text x="630" y="252" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">集中配置 + 动态刷新</text>
          ...
          <text x="630" y="330" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">Sleuth + Zipkin</text>
          <text x="630" y="346" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">Micrometer</text>
          <text x="630" y="362" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">分布式链路追踪</text>
          ...
          <text x="130" y="461" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">正常放行请求</text>
          ...
          <text x="350" y="461" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">快速失败/降级</text>
          ...
          <text x="570" y="461" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">放行探测请求</text>
          ...
          <text x="370" y="495" textAnchor="middle" fontSize="9" fill="var(--text-secondary)">保护下游服务：故障时快速失败而非级联雪崩，恢复后自动试探</text>
        </svg>
      </div>
