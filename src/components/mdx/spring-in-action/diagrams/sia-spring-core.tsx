    ...
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        ...
        >
          ...
          <text x="195" y="92" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">对象自己 new 依赖</text>
          <text x="195" y="108" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">A a = new A(); B b = new B();</text>
          <text x="195" y="122" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">改依赖 = 改源码，无法替换</text>
          ...
          <text x="545" y="92" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">容器注入依赖</text>
          <text x="545" y="108" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">@Autowired B b;  // 容器负责</text>
          <text x="545" y="122" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">改依赖 = 改配置，不动源码</text>
          ...
          <text x="370" y="236" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">读取配置/注解，创建Bean实例（反射）</text>
          ...
          <text x="370" y="290" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">@Autowired 注入依赖，@Value 注入配置值</text>
          ...
          <text x="370" y="344" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">@PostConstruct → BeanPostProcessor → 就绪</text>

          <text x="370" y="376" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">销毁：@PreDestroy → 容器关闭时回调</text>
          ...
          <text x="145" y="482" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">不可变、可测试、循环依赖早暴露</text>
          ...
          <text x="370" y="482" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">可选依赖、可重新注入</text>
          ...
          <text x="595" y="482" textAnchor="middle" fontSize="11" fill="var(--text-secondary)">简洁但不推荐（难测试）</text>
        </svg>
      </div>
