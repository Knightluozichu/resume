...
export function TcpEuclidTraceLab() {
  ...
  return (
    <Figure caption="Euclid's algorithm repeatedly replaces (a,b) by (b,a mod b); the second component strictly decreases until the remainder is zero.">
      <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold text-primary">a = {a}<input className="mt-2 h-11 w-full accent-current" type="range" min="1" max="999" value={a} onChange={(event) => setA(Number(event.target.value))} /></label><label className="text-sm font-semibold text-primary">b = {b}<input className="mt-2 h-11 w-full accent-current" type="range" min="1" max="999" value={b} onChange={(event) => setB(Number(event.target.value))} /></label></div>
...
export function TcpInductionLab() {
  ...
  return (
    <Figure caption="Induction separates the base case from a reusable step: assuming the formula at n−1, adding n produces the formula at n.">
      <label className="text-sm font-semibold text-primary">n = {n}<input className="mt-2 h-11 w-full accent-current" type="range" min="1" max="32" value={n} onChange={(event) => setN(Number(event.target.value))} /></label>
...
export function TcpSummationCostLab() {
  ...
  return (
    <Figure caption="A triangular nested loop executes 1+2+…+n inner operations; the bar area makes the quadratic total visible without hiding the exact count.">
      <label className="text-sm font-semibold text-primary">outer iterations n = {n}<input className="mt-2 h-11 w-full accent-current" type="range" min="2" max="32" value={n} onChange={(event) => setN(Number(event.target.value))} /></label>
...
export function TcpAsymptoticLab() {
  ...
  return (
    <Figure caption="Constants can dominate small inputs, while growth rate determines the eventual crossover; both exact cost and asymptotic class belong in the analysis.">
      <label className="text-sm font-semibold text-primary">n = {n}<input className="mt-2 h-11 w-full accent-current" type="range" min="1" max="128" value={n} onChange={(event) => setN(Number(event.target.value))} /></label>
...
export function TcpMixWordLab() {
  ...
  return (
    <Figure caption="A canonical MIX word has a sign and five bytes; viewing the same magnitude as base-64 bytes exposes the representation used by field operations.">
      <label className="text-sm font-semibold text-primary">signed value = {value}<input className="mt-2 h-11 w-full accent-current" type="range" min="-2000000" max="2000000" value={value} onChange={(event) => setValue(Number(event.target.value))} /></label>
...
export function TcpSubroutineLab() {
  ...
  return (
    <Figure caption="A subroutine call must preserve a return point and agreed state; nested calls make the control-link discipline visible as a stack of frames.">
      <label className="text-sm font-semibold text-primary">nested calls = {calls}<input className="mt-2 h-11 w-full accent-current" type="range" min="1" max="6" value={calls} onChange={(event) => setCalls(Number(event.target.value))} /></label>
...
export function TcpInterpreterLab() {
  ...
  return (
    <Figure caption="An interpretive routine fetches and dispatches an encoded operation; the explicit value stack separates guest-program state from host control.">
      <label className="text-sm font-semibold text-primary">executed instructions = {step}<input className="mt-2 h-11 w-full accent-current" type="range" min="0" max={program.length} value={step} onChange={(event) => setStep(Number(event.target.value))} /></label>
...
export function TcpIoBufferLab() {
  ...
  return (
    <Figure caption="Input/output cost is paid per transfer as well as per record; buffering trades memory for fewer device operations and changes the dominant term.">
      <label className="text-sm font-semibold text-primary">records per block = {block}<input className="mt-2 h-11 w-full accent-current" type="range" min="1" max="32" value={block} onChange={(event) => setBlock(Number(event.target.value))} /></label>
...
export function TcpCostModelLab() {
  ...
  return (
    <Figure caption="A weighted operation model keeps counts and assumptions separate: changing memory cost alters the estimate without changing the algorithm trace.">
      <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold text-primary">memory weight = {memoryCost}<input className="mt-2 h-11 w-full accent-current" type="range" min="1" max="10" value={memoryCost} onChange={(event) => setMemoryCost(Number(event.target.value))} /></label><label className="text-sm font-semibold text-primary">branches = {branches}<input className="mt-2 h-11 w-full accent-current" type="range" min="0" max="30" value={branches} onChange={(event) => setBranches(Number(event.target.value))} /></label></div>
