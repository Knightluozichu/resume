...
const config = {
  title: "第3章 链路层",
  focus: "比较以太网、交换、802.11、PPP、环回、MTU、隧道和链路层攻击",
  invariant: "每一帧都能定位介质、源目的链路地址、封装、MTU和下一跳交付边界",
  failure: "把本机抓到的帧外推为整条路径，会遗漏交换、无线重传、隧道和MTU变化",
  stages: [
...
export function Tip203LinkLayerProtocolLab() {
  return (
...
export function Tip203LinkLayerStateLab() {
  return (
    <Tip2ProtocolWorkbench
      {...config}
      stages={[...config.stages]}
...
export function Tip203LinkLayerEvidenceLab() {
  return (
    <Tip2ProtocolWorkbench
      {...config}
      stages={[...config.stages]}
