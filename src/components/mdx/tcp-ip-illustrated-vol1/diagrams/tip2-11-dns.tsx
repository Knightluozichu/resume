...
const config = {
  title: "第11章 名称解析和域名系统",
  focus: "重建DNS命名空间、区域、缓存、消息、资源记录、更新、传送、DNS64与mDNS",
  invariant: "名称结论绑定查询名、类型、类、事务、权威链、TTL、缓存与传输协议",
  failure: "只看最终地址会遗漏CNAME链、缓存层、负缓存、分离DNS和UDP/TCP回退",
  stages: [
...
export function Tip211DnsProtocolLab() {
  return (
...
export function Tip211DnsStateLab() {
  return (
    <Tip2ProtocolWorkbench
      {...config}
      stages={[...config.stages]}
...
export function Tip211DnsEvidenceLab() {
  return (
    <Tip2ProtocolWorkbench
      {...config}
      stages={[...config.stages]}
