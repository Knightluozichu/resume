...
export function UnityNetworkEvidenceLab({ title, label, nodes, mode }: Props) {
  const [stage, setStage] = useState(0);
  const [sample, setSample] = useState<Sample>("baseline");
  const [messages, setMessages] = useState(240);
  ...
  const evidence = useMemo(() => {
    const pressure =
      sample === "baseline" ? 0 : sample === "boundary" ? 24 : 57;
    const queue = Math.round(
      messages / 60 + pressure + stage * 4 + (fault ? 36 : 0),
    );
    const latency = Math.round(
      10 + messages / 80 + pressure * 0.8 + stage * 3 + (fault ? 45 : 0),
    );
    const divergence =
      sample === "failure" || fault
        ? nodes[Math.min(stage, nodes.length - 1)]
        : "无";
    return { queue, latency, divergence };
  }, [fault, messages, nodes, sample, stage]);
  const pass =
    evidence.queue <= 96 &&
      ...
      <div className="grid lg:grid-cols-[minmax(0,1fr)_290px]">
        <div className="p-4">
          ...
          <div
            ...
            aria-label={`阶段完成度 ${Math.round(((stage + 1) / nodes.length) * 100)}%`}
          >
            <div
        ...
        <aside className="border-t border-zinc-200 bg-zinc-50 p-4 text-sm lg:border-l lg:border-t-0 dark:border-zinc-700 dark:bg-zinc-900">
          ...
          <dl className="mt-3 grid grid-cols-2 gap-x-3 gap-y-2">
            ...
            <dt className="text-zinc-500">消息队列</dt>
            <dd className="text-right font-mono">{evidence.queue}/96</dd>
            <dt className="text-zinc-500">P99 延迟</dt>
          ...
          <div className="mt-4 border-t border-zinc-300 pt-3 dark:border-zinc-700">
            ...
            >
              {pass ? "阶段门通过：保存双端轨迹" : "阶段门拒绝：检查序号与版本"}
            </p>
          </div>
        </aside>
      </div>
