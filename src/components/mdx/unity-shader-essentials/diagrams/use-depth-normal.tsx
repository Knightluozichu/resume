/**
 * <UseDepthNormalDiagram>: 深度与法线纹理
 *
 * 深度Z值 + 法线方向 -> 屏幕空间效果数据基础
...
export function UseDepthNormalDiagram() {
  return (
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox="0 0 720 400" role="img" aria-label="深度纹理存储Z值，法线纹理存储法线方向，合并用于SSAO/SSR/雾/边缘检测等屏幕空间效果。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <g>
            ...
            <text x={52} y={92} fontSize="13" fontWeight="700" fill={accent}>深度纹理</text>
            <rect x={158} y={92} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>裁剪空间Z</text>
            <rect x={310} y={92} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>需线性化</text>
            ...
            <text x={532} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>雾/景深/描边</text>
          ...
          <g>
            ...
            <text x={52} y={169} fontSize="13" fontWeight="700" fill={success}>法线纹理</text>
            <rect x={158} y={169} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>法线方向</text>
            <rect x={310} y={169} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>RGB编码</text>
            ...
            <text x={532} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>SSAO/边缘检测</text>
          ...
          <g>
            ...
            <text x={52} y={246} fontSize="13" fontWeight="700" fill={warning}>位置重建</text>
            <rect x={158} y={246} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>逆VP矩阵</text>
            <rect x={310} y={246} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>NDC坐标</text>
            ...
            <text x={532} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>世界空间位置</text>
          ...
          <g>
            ...
            <text x={52} y={323} fontSize="13" fontWeight="700" fill={danger}>应用效果</text>
            <rect x={158} y={323} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>屏幕空间雾</text>
            <rect x={310} y={323} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>SSR反射</text>
            ...
            <text x={532} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>SSAO环境光</text>
            <line x1={450} y1={340} x2={462} y2={340} stroke={border} strokeWidth="1" strokeDasharray="3 2" />
          </g>
          <defs>
            <marker id="useDepthNormal-arrow" markerWidth="8" markerHeight="8" refX="4" refY="3" orient="auto">
      ...
      <figcaption className="mt-2 text-center text-sm text-secondary">
        深度法线纹理：编码3D场景信息到2D图像，是屏幕空间后处理效果的数据基础。
      </figcaption>
    </figure>
  );
}
