/**
 * <UseShaderBasicsDiagram>: Unity 渲染管线流程
 *
 * 顶点着色器 -> 光栅化 -> 片元着色器 -> 输出合并
...
export function UseShaderBasicsDiagram() {
  return (
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox="0 0 720 400" role="img" aria-label="Unity渲染管线：顶点着色器做MVP变换，光栅化生成片元，片元着色器计算颜色，输出合并写帧缓冲。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <g>
            ...
            <text x={52} y={92} fontSize="13" fontWeight="700" fill={accent}>顶点阶段</text>
            <rect x={158} y={92} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>MVP变换</text>
            <rect x={310} y={92} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>属性传递</text>
            ...
            <text x={532} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>模型到裁剪</text>
          ...
          <g>
            ...
            <text x={52} y={169} fontSize="13" fontWeight="700" fill={success}>光栅化</text>
            <rect x={158} y={169} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>三角形扫描</text>
            <rect x={310} y={169} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>片元生成</text>
            ...
            <text x={532} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>属性插值</text>
          ...
          <g>
            ...
            <text x={52} y={246} fontSize="13" fontWeight="700" fill={warning}>片元阶段</text>
            <rect x={158} y={246} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>纹理采样</text>
            <rect x={310} y={246} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>光照计算</text>
            ...
            <text x={532} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>颜色输出</text>
          ...
          <g>
            ...
            <text x={52} y={323} fontSize="13" fontWeight="700" fill={danger}>输出合并</text>
            <rect x={158} y={323} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>深度测试</text>
            <rect x={310} y={323} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>混合Blending</text>
            ...
            <text x={532} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>帧缓冲</text>
            <line x1={450} y1={340} x2={462} y2={340} stroke={border} strokeWidth="1" strokeDasharray="3 2" />
          </g>
          <defs>
            <marker id="useShaderBasics-arrow" markerWidth="8" markerHeight="8" refX="4" refY="3" orient="auto">
      ...
      <figcaption className="mt-2 text-center text-sm text-secondary">
        渲染管线：顶点变换->光栅化->片元着色->输出合并，Shader控制顶点和片元两个可编程阶段。
      </figcaption>
    </figure>
  );
}
