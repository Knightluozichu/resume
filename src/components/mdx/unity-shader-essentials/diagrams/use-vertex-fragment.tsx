/**
 * <UseVertexFragmentDiagram>: 顶点/片元着色器数据流
 *
 * appdata -> 顶点着色器 -> v2f -> 光栅化插值 -> 片元着色器
...
export function UseVertexFragmentDiagram() {
  return (
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox="0 0 720 400" role="img" aria-label="顶点片元着色器数据流：appdata输入，顶点着色器处理，v2f传递，光栅化插值，片元着色器计算颜色。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <g>
            ...
            <text x={52} y={92} fontSize="13" fontWeight="700" fill={accent}>输入appdata</text>
            <rect x={158} y={92} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>POSITION</text>
            <rect x={310} y={92} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>TEXCOORD0</text>
            ...
            <text x={532} y={112} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>NORMAL</text>
          ...
          <g>
            ...
            <text x={52} y={169} fontSize="13" fontWeight="700" fill={success}>顶点着色器</text>
            <rect x={158} y={169} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>MVP变换</text>
            <rect x={310} y={169} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>世界法线</text>
            ...
            <text x={532} y={190} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>输出v2f</text>
          ...
          <g>
            ...
            <text x={52} y={246} fontSize="13" fontWeight="700" fill={warning}>光栅化插值</text>
            <rect x={158} y={246} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>UV插值</text>
            <rect x={310} y={246} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>法线插值</text>
            ...
            <text x={532} y={266} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>透视校正</text>
          ...
          <g>
            ...
            <text x={52} y={323} fontSize="13" fontWeight="700" fill={danger}>片元着色器</text>
            <rect x={158} y={323} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>纹理采样</text>
            <rect x={310} y={323} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>光照计算</text>
            ...
            <text x={532} y={344} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>输出颜色</text>
            <line x1={450} y1={340} x2={462} y2={340} stroke={border} strokeWidth="1" strokeDasharray="3 2" />
          </g>
          <defs>
            <marker id="useVertexFragment-arrow" markerWidth="8" markerHeight="8" refX="4" refY="3" orient="auto">
      ...
      <figcaption className="mt-2 text-center text-sm text-secondary">
        顶点片元数据流：appdata->vert->v2f->光栅化->frag，语义绑定确保数据正确路由。
      </figcaption>
    </figure>
  );
}
