/**
 * <UidLearningMapDiagram>: Unity UI 设计全书学习地图
 *
 * UI系统认知 -> UGUI核心 -> 布局与交互 -> 进阶与优化
...
export function UidLearningMapDiagram() {
  return (
    <figure className="mdx-figure not-prose mx-auto my-6">
      <div className="overflow-hidden rounded-card border border-border bg-elevated p-5">
        <svg viewBox="0 0 720 400" role="img" aria-label="Unity UI 设计全书学习地图。四个板块：UI系统认知、UGUI核心、布局与交互、进阶与优化。" className="mx-auto block h-auto w-full max-w-[720px]">
          ...
          <g>
            ...
            <text x={52} y={98} fontSize="13" fontWeight="700" fill={accent}>{`UI系统认知`}</text>
            ...
            <text x={380} y={118} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`UI系统架构`}</text>
            ...
            <text x={532} y={118} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`Canvas体系`}</text>
          ...
          <g>
            ...
            <text x={52} y={175} fontSize="13" fontWeight="700" fill={success}>{`UGUI核心`}</text>
            <rect x={158} y={175} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={195} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`UGUI基础`}</text>
            <rect x={310} y={175} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={195} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`布局系统`}</text>
            ...
            <text x={532} y={195} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`交互组件`}</text>
          ...
          <g>
            ...
            <text x={228} y={272} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`UI动画`}</text>
            <rect x={310} y={252} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={272} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`UI Toolkit`}</text>
            ...
            <text x={532} y={272} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`响应式设计`}</text>
          ...
          <g>
            ...
            <text x={52} y={329} fontSize="13" fontWeight="700" fill={danger}>{`优化与总结`}</text>
            <rect x={158} y={329} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={228} y={349} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`性能优化`}</text>
            <rect x={310} y={329} width={140} height={33} rx="6" fill={elevated} stroke={border} strokeWidth="1" />
            <text x={380} y={349} textAnchor="middle" fontSize="11" fontWeight="600" fill={primary}>{`合批策略`}</text>
      ...
      <figcaption className="mt-2 text-center text-sm text-secondary">
        Unity UI 设计四大板块：系统认知→UGUI核心→进阶技术→优化总结，从基础到实战。
      </figcaption>
    </figure>
  );
}
