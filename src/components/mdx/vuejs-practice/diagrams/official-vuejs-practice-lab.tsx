...
export function VueJsPracticeEvidenceLab({ title, label, nodes, mode }: Props) {
  ...
  const evidence = useMemo(() => {
    ...
    const pending = Math.round(
      events / 8 + pressure + stage * 2 + (fault ? 26 : 0),
    );
    const duration = Math.round(
      6 + events / 4 + pressure * 0.8 + stage * 2 + (fault ? 35 : 0),
    );
    const retained = Math.round(events / 12 + pressure / 4 + (fault ? 12 : 0));
    const divergence =
        ...
        <aside className="border-t border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-900 lg:border-l lg:border-t-0">
          ...
          <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
            <div className="border border-zinc-200 bg-white p-3 dark:border-zinc-700 dark:bg-zinc-950">
              <dt className="text-xs text-zinc-500">待处理事件</dt>
            ...
            <div className="border border-zinc-200 bg-white p-3 dark:border-zinc-700 dark:bg-zinc-950">
              <dt className="text-xs text-zinc-500">残留资源</dt>
              <dd className="mt-1 text-lg font-semibold">
                {evidence.retained}
              </dd>
            </div>
          ...
          <p className="mt-4 text-xs leading-5 text-zinc-600 dark:text-zinc-300">
            切换样本、阶段和压力，比较最终界面相同但事件、状态或资源轨迹不同的情况。恢复样本只有在清除故障后才可签发。
          </p>
        </aside>
      </div>
    </section>
