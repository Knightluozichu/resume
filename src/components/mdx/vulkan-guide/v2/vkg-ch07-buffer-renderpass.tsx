...
const model = {
  focus:
    "把buffer/image需求匹配到内存类型、绑定、传输与所有权；用attachment layout与load/store合同控制渲染内容生命周期",
  formula:
    "typeBits\\land properties\\ne 0 ; Content_{after}=Store(Render(Load(Content_{before})))",
  invariant:
    "缓存资源、渲染通道与帧缓存的输入、状态、输出与恢复结果可用同一证据包重放",
  fault:
    "只按属性标志挑内存却忽略memoryTypeBits，或跨队列使用未转移所有权；需要旧颜色却使用DONT_CARE，或后续采样前未转换layout与访问范围",
  evidence:
    "requirements、memory type、绑定偏移、copy范围、layout和owner family、初末layout、load/store、clear值、附件范围和后续读者",
  sourceLabel: "Khronos Vulkan Guide",
} satisfies VulkanExperimentModel;
const props = { unitTitle, nodes, model };

export function VkgCh07BufferRenderpassExecutionLab() {
  return <OfficialVulkanExecutionLab mode="execution" {...props} />;
}

export function VkgCh07BufferRenderpassHazardLab() {
  return <OfficialVulkanExecutionLab mode="hazard" {...props} />;
}

export function VkgCh07BufferRenderpassEvidenceLab() {
