...
export function OfficialWindowsJourneyLab({
  title,
  label,
  color,
  soft,
  ...
  const evidence = useMemo(() => {
    const failed = mode === "missingAsset" || mode === "stateLeak";
    return {
      objects: load * (stage + 2),
      drawCalls: load * (failed ? 3 : 2),
      divergences: mode === "boundary" ? 1 : failed ? Math.max(1, load - 2) : 0,
      firstError: failed
        ? modes[mode]
        : mode === "boundary"
          ? "边界已捕获"
        ...
        <div className="p-4">
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            {[
              ...
              ["绘制调用", evidence.drawCalls],
              ["状态偏离", evidence.divergences],
            ].map(([name, value]) => (
              <div
                key={name}
