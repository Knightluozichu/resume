...
const model = {
  unitId: "learningMap",
  title: "《Windows内核编程》185坐标与19项版本差异证据学习地图",
  question:
  ...
  artifact: "185坐标矩阵、19项差异表、11章对象路由、版本迁移与驱动安全门",
  probe: "cross",
  stages: [
    {
      label: "冻结平台与对象",
      object:
        "《Windows内核编程》185坐标与19项版本差异证据学习地图涉及的中文版11章185坐标、作者一版204坐标公开目录差异、授权样章和当前平台轨",
    ...
    },
    {
      label: "执行参考路径",
      object:
        "《Windows内核编程》185坐标与19项版本差异证据学习地图的参考调用链",
      control: "只运行预注册基线请求或回调",
      signal:
        "状态、NTSTATUS、调用栈、ETW/WinDbg信号与“185坐标各覆盖一次，19项差异单列，作者一版/二版/目标平台陈述互不倒写”",
      rollback: "停止负载并核对无残留请求、回调与引用",
    },
    {
      label: "注入唯一故障",
      object:
        "《Windows内核编程》185坐标与19项版本差异证据学习地图的单故障边界",
      control:
        "只注入“把作者一版204坐标冒充中文版185坐标、把18页样章扩大成整书或用Windows 11代替完整build”",
    ...
    },
    {
      label: "同输入恢复",
      object:
        "《Windows内核编程》185坐标与19项版本差异证据学习地图的反向清理链",
      control: "以相同build、驱动包、VM和输入重放",
      signal:
        "重新满足“185坐标各覆盖一次，19项差异单列，作者一版/二版/目标平台陈述互不倒写”且对象与引用计数回到基线",
...
export function WkpOfficialLearningMapVersionContextLab() {
  return <WindowsKernelEvidenceLab model={model} view="version-context" />;
}

export function WkpOfficialLearningMapExecutableProbeLab() {
  return <WindowsKernelEvidenceLab model={model} view="executable-probe" />;
}

export function WkpOfficialLearningMapSafetyGateLab() {
