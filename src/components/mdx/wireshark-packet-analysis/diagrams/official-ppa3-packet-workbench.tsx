...
type Mode = "packet" | "diagnosis" | "evidence";
...
export function Ppa3PacketWorkbench({
  title,
  focus,
  invariant,
  failure,
    ...
    <section className="my-6 overflow-hidden rounded-md border border-zinc-300 bg-white text-zinc-950 shadow-sm dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100">
      ...
      <div className="grid gap-0 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="border-b border-zinc-200 p-4 lg:border-b-0 lg:border-r dark:border-zinc-800">
          ...
          <div className="mt-3 grid grid-cols-3 gap-1 font-mono text-xs">
            {bytes.map((byte, index) => (
              <div
                key={index}
                className={`border p-2 text-center ${fault && index === 3 ? "border-rose-500 bg-rose-100 text-rose-900 dark:bg-rose-950 dark:text-rose-100" : "border-zinc-300 bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-900"}`}
              >
        ...
        <div className="p-4">
          ...
          >
            <p className="font-semibold">{fault ? "反例已出现" : "正常基线"}</p>
