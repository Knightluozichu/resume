...
const STAGES: readonly DecisionStage[] = [
  {
    ...
    mechanism:
      "先写预测：默认配置、自动名称解析和错误接口会制造不存在的协议或时序结论。然后固定接口、捕获点、时钟、snaplen、名称解析和配置方案，保存未经修改的原始PCAP。任何结论必须指向具体帧号、字段、字节偏移和相邻事务。",
    failure:
...
export function Ppa303IntroductionWiresharkDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第3章 Wireshark入门：机制与证据"
      prompt="切换《第3章 Wireshark入门》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第3章 Wireshark入门》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Ppa303IntroductionWiresharkMechanismMap() {
  return (
    <ChapterMechanismMap
      title="第3章 Wireshark入门：机制路径"
      stages={STAGES}
    />
  );
}

export function Ppa303IntroductionWiresharkFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第3章 Wireshark入门：失效与核验"
      stages={STAGES}
    />
  );
}
