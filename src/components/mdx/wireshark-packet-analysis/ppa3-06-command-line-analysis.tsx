...
const STAGES: readonly DecisionStage[] = [
  {
    ...
    mechanism:
      "先写预测：不区分捕获过滤与显示过滤，或忽略shell引用，会得到空文件或错误样本。然后固定接口、捕获点、时钟、snaplen、名称解析和配置方案，保存未经修改的原始PCAP。任何结论必须指向具体帧号、字段、字节偏移和相邻事务。",
    failure:
...
export function Ppa306CommandLineAnalysisDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第6章 用命令行分析数据包：机制与证据"
      prompt="切换《第6章 用命令行分析数据包》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第6章 用命令行分析数据包》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Ppa306CommandLineAnalysisMechanismMap() {
  return (
    <ChapterMechanismMap
      title="第6章 用命令行分析数据包：机制路径"
      stages={STAGES}
    />
  );
}

export function Ppa306CommandLineAnalysisFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第6章 用命令行分析数据包：失效与核验"
      stages={STAGES}
    />
  );
}
