...
export function Ppa307NetworkLayerProtocolsDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第7章 网络层协议：机制与证据"
      prompt="切换《第7章 网络层协议》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第7章 网络层协议》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Ppa307NetworkLayerProtocolsMechanismMap() {
  return (
    <ChapterMechanismMap title="第7章 网络层协议：机制路径" stages={STAGES} />
  );
}

export function Ppa307NetworkLayerProtocolsFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第7章 网络层协议：失效与核验"
      stages={STAGES}
    />
  );
}
