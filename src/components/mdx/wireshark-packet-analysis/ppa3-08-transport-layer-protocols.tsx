...
const STAGES: readonly DecisionStage[] = [
  {
    ...
    mechanism:
      "先写预测：只看SYN或RST单帧无法区分正常拒绝、异常中断与中间设备注入。然后固定接口、捕获点、时钟、snaplen、名称解析和配置方案，保存未经修改的原始PCAP。任何结论必须指向具体帧号、字段、字节偏移和相邻事务。",
    failure:
...
export function Ppa308TransportLayerProtocolsDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第8章 传输层协议：机制与证据"
      prompt="切换《第8章 传输层协议》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第8章 传输层协议》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Ppa308TransportLayerProtocolsMechanismMap() {
  return (
    <ChapterMechanismMap title="第8章 传输层协议：机制路径" stages={STAGES} />
  );
}

export function Ppa308TransportLayerProtocolsFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第8章 传输层协议：失效与核验"
      stages={STAGES}
    />
  );
}
