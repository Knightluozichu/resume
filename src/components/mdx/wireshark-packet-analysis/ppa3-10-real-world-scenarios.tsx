...
const STAGES: readonly DecisionStage[] = [
  {
    ...
    mechanism:
      "先写预测：直接套过滤器或盯最后一个错误包，会忽略真正发生在更早阶段的异常。然后固定接口、捕获点、时钟、snaplen、名称解析和配置方案，保存未经修改的原始PCAP。任何结论必须指向具体帧号、字段、字节偏移和相邻事务。",
    failure:
...
export function Ppa310RealWorldScenariosDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第10章 基础的现实世界场景：机制与证据"
      prompt="切换《第10章 基础的现实世界场景》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第10章 基础的现实世界场景》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Ppa310RealWorldScenariosMechanismMap() {
  return (
    <ChapterMechanismMap
      title="第10章 基础的现实世界场景：机制路径"
      stages={STAGES}
    />
  );
}

export function Ppa310RealWorldScenariosFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第10章 基础的现实世界场景：失效与核验"
      stages={STAGES}
    />
  );
}
