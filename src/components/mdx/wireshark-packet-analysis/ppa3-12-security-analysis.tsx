...
export function Ppa312SecurityAnalysisDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第12章 安全领域的数据包分析：机制与证据"
      prompt="切换《第12章 安全领域的数据包分析》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第12章 安全领域的数据包分析》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Ppa312SecurityAnalysisMechanismMap() {
  return (
    <ChapterMechanismMap
      title="第12章 安全领域的数据包分析：机制路径"
      stages={STAGES}
    />
  );
}

export function Ppa312SecurityAnalysisFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第12章 安全领域的数据包分析：失效与核验"
      stages={STAGES}
    />
  );
}
