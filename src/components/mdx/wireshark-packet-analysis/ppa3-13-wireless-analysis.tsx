...
const STAGES: readonly DecisionStage[] = [
  {
    ...
    mechanism:
      "先写预测：单信道抓包和驱动限制会制造缺帧，不能把未捕获误判为未发送。然后固定接口、捕获点、时钟、snaplen、名称解析和配置方案，保存未经修改的原始PCAP。任何结论必须指向具体帧号、字段、字节偏移和相邻事务。",
    failure:
...
export function Ppa313WirelessAnalysisDecisionLab() {
  return (
    <ChapterDecisionLab
      title="第13章 无线网络数据包分析：机制与证据"
      prompt="切换《第13章 无线网络数据包分析》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《第13章 无线网络数据包分析》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Ppa313WirelessAnalysisMechanismMap() {
  return (
    <ChapterMechanismMap
      title="第13章 无线网络数据包分析：机制路径"
      stages={STAGES}
    />
  );
}

export function Ppa313WirelessAnalysisFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="第13章 无线网络数据包分析：失效与核验"
      stages={STAGES}
    />
  );
}
