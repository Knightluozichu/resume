...
export function Ppa3AppendixBDecisionLab() {
  return (
    <ChapterDecisionLab
      title="附录B 分析数据包结构：机制与证据"
      prompt="切换《附录B 分析数据包结构》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《附录B 分析数据包结构》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Ppa3AppendixBMechanismMap() {
  return (
    <ChapterMechanismMap
      title="附录B 分析数据包结构：机制路径"
      stages={STAGES}
    />
  );
}

export function Ppa3AppendixBFailureDiagram() {
  return (
    <ChapterFailureMatrix
      title="附录B 分析数据包结构：失效与核验"
      stages={STAGES}
    />
  );
}
