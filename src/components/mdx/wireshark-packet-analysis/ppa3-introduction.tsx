...
export function Ppa3IntroductionDecisionLab() {
  return (
    <ChapterDecisionLab
      title="导言：机制与证据"
      prompt="切换《导言》的三个关键教学阶段，先解释机制，再用运行与失败证据验证结论。"
      stages={STAGES}
      conclusion="学完《导言》后，应能从输入和前置条件推导状态变化，并用可重复的构建、运行或边界测试证明结果。"
    />
  );
}

export function Ppa3IntroductionMechanismMap() {
  return <ChapterMechanismMap title="导言：机制路径" stages={STAGES} />;
}

export function Ppa3IntroductionFailureDiagram() {
  return <ChapterFailureMatrix title="导言：失效与核验" stages={STAGES} />;
}
