...
export const cppCopyControlQuestions: ReviewQuestion[] = [
  ...
  },
  {
    ...
    answer:
      "标准库容器（如 `vector`）在扩容时优先使用移动操作——因为它快。但 `vector` 必须保证**异常安全**：如果移动可能抛异常，扩容中途失败会导致部分元素已被移走、部分还在——容器处于不一致状态。所以 `vector` 只会用标了 `noexcept` 的移动操作——没标的宁可走更慢的拷贝（前提是该类型仍可拷贝；若拷贝已被 `= delete`，`vector` 只能退回使用那个可能抛异常的移动）。移动操作通常不分配内存、不调用可能抛异常的函数——所以它们天然就该标 `noexcept`，不标是性能损失。",
