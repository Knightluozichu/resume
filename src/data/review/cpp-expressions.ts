...
export const cppExpressionsQuestions: ReviewQuestion[] = [
  ...
  // ── L4 综合：陷阱 / 全流程 ──
  {
    ...
    answer:
      "三个全都是**未定义行为（UB）**——原因都是「在一个表达式里多次修改同一个变量但没有序列点分隔」。C++ 标准不规定子表达式的求值顺序，不同编译器可能产生不同结果。核心安全原则：**在一个完整表达式里，每个变量最多被修改一次；如果被修改了，就不要在同一表达式里再读它**。把复杂表达式拆成多行简单赋值，清晰永远比聪明重要。",
