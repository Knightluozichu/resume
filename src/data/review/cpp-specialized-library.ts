...
export const cppSpecializedLibraryQuestions: ReviewQuestion[] = [
  ...
  {
    id: "csl-1",
    chapter: "cpp-specialized-library",
    level: 1,
    question: `什么是 \`tuple\`？它和 \`pair\` 有什么区别？`,
    answer: `\`tuple\` 是 C++11 引入的**固定大小的异构容器**——可以存放任意数量、任意类型的元素。\`pair\` 只能存放恰好两个元素（\`.first\` / \`.second\`），而 \`tuple\` 可以存放零个到任意多个。访问方式不同：\`pair\` 用 \`.first/.second\` 成员，\`tuple\` 用 \`get<N>(t)\` 模板函数——\`N\` 必须是编译期常量，写错索引编译期就报错。\`make_tuple\` 可自动推导类型创建 tuple，\`tie\` 可以解包 tuple 到已有变量。`,
  ...
  },
  {
    ...
    answer: `\`bitset<N>\` 是**固定大小的二进制位集合**——把每一位当作独立对象操作。\`N\` 必须是**编译期常量**（constexpr），因为 bitset 是类模板，\`N\` 是非类型模板参数。五种常用操作：① \`set(pos)\`——指定位置设为 1；② \`reset(pos)\`——指定位清零；③ \`flip(pos)\`——翻转指定位置（0↔1）；④ \`test(pos)\`——检查指定位是 0 还是 1（不修改）；⑤ \`count()\`——统计值为 1 的总位数。此外还有 \`to_string()\` 转成字符串、\`to_ulong()\` / \`to_ullong()\` 转成整数。`,
  ...
  },
  {
    ...
    answer: `C++ \`<regex>\` 库的核心组件：① \`regex\`——表示编译后的正则表达式模式，构造时传入模式字符串；② \`smatch\`——存储匹配结果的类（\`match_results<string::const_iterator>\` 的别名），\`smatch[0]\` 是完整匹配、\`smatch[1]\` 起是各捕获组；③ \`regex_search\`——在字符串中搜索第一个匹配；④ \`regex_match\`——检查整个字符串是否完全匹配模式；⑤ \`sregex_iterator\`——迭代遍历字符串中所有匹配项。还有 \`regex_replace\` 用于正则替换。使用 R\"(...)\" 原始字符串字面量（raw string literal）可以避免转义反斜杠。`,
  ...
  },
  {
    ...
    answer: `\`make_tuple\`——根据实参类型自动推导创建 tuple：\`auto t = make_tuple(42, \"hello\", 3.14);\` 产生 \`tuple<int, const char*, double>\`。\`tie\`——把现有变量的引用绑到 tuple 元素上，用于解包：\`int a; string b; tie(a, b) = make_tuple(1, \"x\");\` → a=1, b=\"x\"。可以用 \`ignore\` 占位符跳过不需要的元素。C++17 **结构化绑定**——\`auto [x, y, z] = t;\` 是 tie 的语法糖，直接声明新变量并批量赋值，更简洁。场景对比——需要复用已有变量用 tie，直接声明新变量用结构化绑定。`,
  ...
  },
  {
    ...
    answer: `**捕获组**是正则模式中用括号 \`()\` 包围的子表达式。匹配成功时，括号内匹配到的子串会被单独提取出来——方便你提取「电话号码里的区号」「日期里的年份」等子部分。\`smatch[0]\` 返回整个正则表达式匹配到的完整字符串（全文）。\`smatch[1]\` 返回第一个左括号对应的捕获组匹配内容，\`smatch[2]\` 是第二个，以此类推。例如正则 \`(\\d+)-(\\w+)\` 匹配 \`\"123-abc\"\` 时——\`smatch[0]=\"123-abc\"\`（完整匹配），\`smatch[1]=\"123\"\`（第一个捕获组）、\`smatch[2]=\"abc\"\`（第二个捕获组）。`,
  ...
  // ── L2 理解 ──
  {
    ...
    answer: `tuple 元素在内存中是**连续存储**的——索引 0、1、2 依次排布，各元素紧挨着存放在同一块内存中（实现通常是递归继承）。\`get<N>(t)\` 返回的是**引用**（取决于 t 的 const 限定）：非 const tuple 返回 \`T&\`，const tuple 返回 \`const T&\`。返回引用的原因是：① 允许修改元素——\`get<0>(t) = 100;\` 直接改 tuple 内部；② 避免大对象拷贝。索引 N 必须是**编译期常量**——这样编译器在编译期就能确定返回类型和内存偏移量 → 零运行时开销。写成 \`get<3>(t)\`（超出范围）编译器直接报错。`,
  ...
  },
  {
    ...
    answer: `\`bitset<8> b(42);\`——用整数 42 初始化 8 位 bitset。42 的二进制表示是 \`00101010\`（0×32 + 0×16 + 1×8 + 0×4 + 1×2 + 0×1 = 42），高位补 0 填满 8 位。\`b.to_string()\` → \`\"00101010\"\`——把 bitset 转为二进制字符串，高位在前。\`b.to_ulong()\` → \`42\`——转回无符号长整型，即原始数值。\`b.count()\` → \`3\`——三位为 1（第 1、3、5 位，对应 2¹+2³+2⁵=2+8+32=42）。to_string 和 to_ulong 在 bitset 值超过目标类型范围时会抛 \`overflow_error\` 异常。`,
  ...
  },
  {
    ...
    answer: `\`regex_search\`——在字符串中**搜索第一个匹配的子串**。只要字符串中有某一部分满足模式就成功。场景：从一大段日志里找到所有 IP 地址。\`regex_match\`——检查**整个字符串是否完全匹配**模式。字符串必须从头到尾都满足模式才成功。场景：验证用户输入是不是合法邮箱地址。代码对照——\`regex_search(\"abc123def\", regex(\"\\\\d+\"))\` → true（找到 123）；\`regex_match(\"abc123def\", regex(\"\\\\d+\"))\` → false（\"abc123def\" 不全是数字）。regex_match 更严格、regex_search 更灵活——选哪个取决于你需要「找到包含的」还是「确认整体合规」。`,
  ...
  },
  {
    ...
    answer: `\`sregex_iterator it(str.begin(), str.end(), re)\` 从字符串开头开始搜索——每次递增 \`++it\` 时，引擎从上一个匹配之后的位置继续搜索下一个匹配。当找不到更多匹配时，迭代器变成**尾后迭代器**（即默认构造的 \`sregex_iterator{}\`，不是 \`end()\` 函数返回值）。\`for (auto it = sregex_iterator(...); it != sregex_iterator{}; ++it)\` 是标准遍历模式。需要一个独立的结束哨兵是因为正则匹配的结束位置不是简单的 \`str.end()\`——匹配可能在字符串中间结束，需要引擎内部判断「没有下一个匹配了「。这和不带参数的默认构造函数返回的就是这个结束迭代器。`,
  ...
  },
  {
    ...
    answer: `分布对象可以像函数一样调用是因为它重载了 \`operator()\`——所有分布类都是**函数对象（functor）**。\`dist(engine)\` 等价于 \`dist.operator()(engine)\`。每次调用 \`dist(engine)\` 时——引擎的内部状态会推进，生成一个新的原始随机数，分布对象把它映射到目标范围/分布上。所以**每次调用通常返回不同值**——但并不保证相邻两次一定不同（概率极小但不是零）。如果需要保证每次不同，可以用 \`shuffle\` 配合 \`random_device\`。分布对象的参数（如范围 a,b）在构造后可以通过成员函数修改——不需要重新创建对象。`,
  ...
  },
  {
    ...
    answer: `\`N\` 必须是编译期常量——因为 tuple 的每个元素类型不同（如 \`tuple<int, double, string>\`），\`get<0>(t)\` 返回 \`int&\`、\`get<1>(t)\` 返回 \`double&\`。如果 \`N\` 是运行时变量，编译器无法在编译期确定**返回类型**——因为 \`i\` 不同时返回类型就变了。C++ 是静态类型语言——每个表达式的类型必须在编译期确定。写成 \`get<i>(t)\`（i 是变量）→ 编译错误，报 'the value of i is not usable in a constant expression'。解决方案：把所有元素类型统一（全用某种公共基类或 variant），或改变设计——如果确实需要运行时索引，tuple 不是合适的工具，考虑用 \`vector<variant<...>>\`。`,
  ...
  },
  {
    ...
    chapter: "cpp-specialized-library",
    level: 3,
    question: `写一个 \`extract_emails\` 函数——接受一个字符串，用正则提取所有邮箱地址（形如 \`user@domain.com\`），返回 \`vector<string>\`。要求用 \`sregex_iterator\` 遍历所有匹配。`,
    answer:
  ...
  },
  {
    ...
    chapter: "cpp-specialized-library",
    level: 3,
    question: `写出一个 lambda 封装 \`[a,b]\` 范围均匀随机整数的 \`rand_int\` 函数对象，然后写一个骰子类 \`Dice\` 用这个模式每次调用返回 1~6 的随机值。`,
    answer:
  ...
  },
  {
    ...
    chapter: "cpp-specialized-library",
    level: 3,
    question: `写一个函数 \`parse_date\`——用正则从 \`\"2024-05-17\"\` 中提取年、月、日三个捕获组并返回 \`tuple<int,int,int>\`。模式：\`(\\d{4})-(\\d{2})-(\\d{2})\`。`,
    answer:
  ...
  // ── L4 综合 ──
  {
    ...
    chapter: "cpp-specialized-library",
    level: 4,
    question: `设计一个 \`CSVRow\` 类——用 \`tuple\` 存储一行 CSV 数据的各个字段（类型各不相同），支持：① 用模板构造函数接收任意数量的参数并打包成 tuple；② 提供 \`field<N>()\` 返回第 N 个字段的引用；③ 实现 \`operator==\`（逐字段比较）。说明你的设计中 \`N\` 超出范围时如何处理。`,
    answer:
  ...
  },
  {
    ...
    chapter: "cpp-specialized-library",
    level: 4,
    question: `写一个正则函数 \`replace_phone_format\`——把文本中所有形如 \`13800138000\` 的 11 位手机号替换为 \`138****8000\`（隐藏中间四位）。要求：① 用 \`regex_replace\`；② 用 \`$&\` 或捕获组引用匹配内容。`,
    answer:
  ...
  },
  {
    ...
    chapter: "cpp-specialized-library",
    level: 4,
    question: `设计一个 \`SeededRandom\` 类——用指定种子初始化 \`mt19937\` 引擎，提供 \`uniform(a,b)\`、\`normal(mean,stddev)\`、\`bernoulli(p)\` 三个方法，每个返回对应分布的随机值。回答：为什么引擎是成员而分布是局部变量？`,
    answer:
  ...
  },
  {
    ...
    chapter: "cpp-specialized-library",
    level: 4,
    question: `综合实战——设计一个 \`LogParser\` 类，构造函数接受日志文件路径，用正则从每行 \`[2024-05-17 14:30:22] ERROR: connection timeout (retry 3/5)\` 中提取时间、级别、消息、重试次数，存入 \`tuple<string,string,string,int>\` 的 vector。提供 \`get_all()\` 返回全部记录。`,
    answer:
