...
export const cppStatementsQuestions: ReviewQuestion[] = [
  ...
  // ── L2 理解：为什么 / 机制 ──
  {
    ...
    answer:
      "因为 `switch` 的实现依赖「跳转表」——编译器把每个 case 值编译成偏移量，运行时直接跳过去，O(1) 效率。整数离散精确，适合做跳转表索引。浮点数有精度问题（`3.0` 和 `2.999...` 算同一个值吗？），字符串长度不固定、比较代价大，都不适合。",
  ...
  },
  {
    ...
    answer:
      "`unsigned int` 永远不会小于 0。`i--` 到 i=0 后再减，`0u - 1` 绕回约 43 亿的巨大正数，`i >= 0` 永远 true。修正一：用 `int` 代替 `unsigned int`。修正二：条件改成 `i > 0` 并在循环里最后做 i--，i=1 输出后变为 0 退出。",
